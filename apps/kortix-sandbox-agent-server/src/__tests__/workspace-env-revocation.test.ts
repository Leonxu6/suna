 ...
 * with (opencode.ts:932 / :1203 both build it via mergeWorkspaceEnv(process.env, …)).
 *
 ...
 * passes through BASH_ENV. If mergeWorkspaceEnv only deletes the names still
 * granted, a revoked value survives from the daemon's process.env and is
 * re-injected on the next restart.
 */
describe('mergeWorkspaceEnv — revocation', () => {
  test('a revoked BOOT secret is cleared, not carried over from process.env', () => {
    ...
    const store = createWorkspaceEnvStore(bootEnv)
    ...
    const merged = mergeWorkspaceEnv(bootEnv, store)
  ...
  test('a secret ADDED mid-session and then revoked is also cleared', () => {
    ...
    const merged = mergeWorkspaceEnv(liveEnv, store)
  ...
  test('a rotated secret takes the new value, not the boot one', () => {
    ...
    expect(mergeWorkspaceEnv(bootEnv, store).API_KEY).toBe('new')
  })
