...
type WorkspaceEnvUpdate = {
  changed: boolean
  revision: string
  names: string[]
}

export type WorkspaceEnvStore = {
  snapshot(): WorkspaceEnvSnapshot
  apply(input: { revision: string; env: Record<string, unknown>; names?: unknown }): WorkspaceEnvUpdate
}
...
export function createWorkspaceEnvStore(
  initialEnv: NodeJS.ProcessEnv = process.env,
): WorkspaceEnvStore {
  const initialRevision = (
    initialEnv.KORTIX_WORKSPACE_SECRETS_REVISION ??
    initialEnv.KORTIX_PROJECT_SECRETS_REVISION
  )?.trim()
  let revision: string | null = initialRevision || null
  let names = (
    initialEnv.KORTIX_WORKSPACE_SECRET_NAMES ??
    initialEnv.KORTIX_PROJECT_SECRET_NAMES ??
    ''
  )
    .split(',')
