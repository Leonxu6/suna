...
// ─────────────────────────────────────────────────────────────────────────────
// How the CLI talks git to a workspace — ONE resolver, shared by every command
...
// `kortix ship` to a managed workspace failed with "Managed git push token export
...
/** Which credential a git operation against this workspace should present. */
export type WorkspaceGitCredentialMode =
...
export interface WorkspaceGitTarget {
  repoUrl: string;
  credentialMode: WorkspaceGitCredentialMode;
}
...
export function workspaceIsManaged(workspace: WorkspaceGitRef): boolean {
  const git = workspace.metadata?.git as { managed?: boolean } | undefined;
  return git?.managed === true;
}

/**
 * Resolve the URL + credential kind for any git operation on a workspace.
 *
 * The Kortix git proxy is the UNIVERSAL client-facing origin, so it wins for
 * EVERY workspace that advertises one — managed repos included. We authenticate
 ...
 */
export function resolveWorkspaceGitTarget(workspace: WorkspaceGitRef): WorkspaceGitTarget {
  ...
  if (workspaceIsManaged(workspace)) {
    return { repoUrl: workspace.repo_url, credentialMode: 'managed-git-token' };
  }
  return { repoUrl: workspace.repo_url, credentialMode: 'none' };
}
 ...
 */
export function configureWorkspaceGitAuth(
  repoRoot: string,
  repoUrl: string,
