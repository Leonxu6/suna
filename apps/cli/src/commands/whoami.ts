...
export async function performWhoami(opts: PerformWhoamiOptions): Promise<number> {
  ...
  if (flags.tokenOnly) {
    ...
    if (ctx?.workspace_id) process.stdout.write(`  ${C.dim}workspace   ${C.reset}${ctx.workspace_id}\n`);
  ...
  if (me.token_context?.workspace_id || me.token_context?.session_id || me.token_context?.agent) {
    const ctx = me.token_context;
    const tokenKind = ctx.session_id ? 'session token' : ctx.workspace_id ? 'workspace token' : ctx.auth_type || 'token';
