    ...
    if (install) {
      if (json) {
        ...
        printInstall(
          ctx,
          install,
          status.ok(`Connected to ${install.providerWorkspaceName ?? install.providerWorkspaceId}`),
        );
      }
      return 0;
    }
...
function printInstall(ctx: WorkspaceCtx, install: SlackInstallation, headline: string): void {
  const name = install.providerWorkspaceName ?? install.providerWorkspaceId;
  const webhookUrl = `${ctx.client.apiBase.replace(/\/$/, '')}/v1/webhooks/slack/${ctx.workspaceId}`;
  process.stdout.write(
    `${headline}  ${C.bold}${name}${C.reset}\n` +
      `         team       ${C.dim}${install.providerWorkspaceId}${C.reset}\n` +
...
async function channelsManifest(
  ...
  const requestUrl = `${baseUrl}/v1/webhooks/slack/${ctx.workspaceId}`;
