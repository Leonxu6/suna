...
type ResourceType = 'account' | 'workspace' | 'sandbox' | 'trigger' | 'channel' | 'member' | 'group';
...
Examples:
  ...
  kortix roles create support_agent --name "Support Agent" \\
    --scope workspace --actions workspace.read,workspace.session.start,workspace.trigger.fire
  ...
  try {
    switch (sub) {
      ...
      case 'create': {
        ...
        const resourceType = (f.scope ?? 'workspace') as ResourceType;
      ...
      case 'assignments':
      case 'policies': {
        const qs = f.workspace ? `?scopeType=workspace&scopeId=${encodeURIComponent(f.workspace)}` : '';
      ...
      case 'assign': {
        ...
        const role = findRole(roles, ref);
        if (!role) return notFound(`role "${ref}"`);
        // Resolve scope: --workspace pins a workspace scope; otherwise --scope (default account).
        const scope = f.workspace
          ? { scopeType: 'workspace', scopeId: f.workspace as string | null }
      ...
      case 'import': {
        ...
        for (const r of doc.roles ?? []) {
          ...
          await ctx.client.post(`${base}/roles`, {
            ...
            name: r.name ?? r.key,
            ...(r.description ? { description: r.description } : {}),
            resourceType: r.resource_type ?? 'workspace',
