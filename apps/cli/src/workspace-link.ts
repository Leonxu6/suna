...
export interface WorkspaceLink {
  workspace_id: string;
  account_id: string;
  /** Named host (from ~/.config/kortix/config.json) this workspace lives on. */
  host?: string;
...
/** Is the cwd plausibly a Kortix workspace? We require either an existing
...
export function isKortixWorkspace(cwd = process.cwd()): boolean {
  return (
...
export function loadLink(cwd = process.cwd()): WorkspaceLink | null {
  const path = linkFilePath(cwd);
  if (!existsSync(path)) return null;
  try {
    const parsed = JSON.parse(readFileSync(path, 'utf8')) as Partial<WorkspaceLink> & {
      project_id?: unknown;
    };
    const workspaceId =
...
export function saveLink(link: WorkspaceLink, cwd = process.cwd()): void {
  const path = linkFilePath(cwd);
...
/**
 * Resolve which workspace a CLI command should operate on, in order:
 *   1. --workspace / workspaceArg
 *   2. KORTIX_WORKSPACE_ID env (platform-injected inside a sandbox)
 *   3. .kortix/link.json in cwd (per-repo binding)
 *   4. the active host's global default workspace (`kortix workspaces use`)
