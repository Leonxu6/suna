...
export function useUpdateExperimentalFeature(workspaceId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({
      feature,
    ...
    }) => updateExperimentalFeature(workspaceId, feature, enabled),
    onSuccess: (updated) => {
      queryClient.setQueryData(workspaceKeys.workspace(workspaceId), updated);
 ...
 *  workspace may not have a live sandbox running when you're just configuring
