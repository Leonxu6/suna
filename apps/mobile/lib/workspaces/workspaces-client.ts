 ...
 * endpoints: GET /accounts, GET /workspaces?account_id=, etc.). It's kept as a
 * single file so every existing mobile import path
 * (`@/lib/workspaces/workspaces-client`) keeps working unchanged — see the SDK
 ...
 *  the SDK's `workspaces-client` doesn't cover (kept local — `unwrap` itself is
 ...
 * (`DELETE /executor/workspaces/:id/connectors/:slug/credential`), implemented
...
export async function disconnectConnector(workspaceId: string, slug: string) {
  return unwrapLocal(
    await backendApi.delete<{ ok: boolean }>(
      `/executor/workspaces/${encodeURIComponent(workspaceId)}/connectors/${encodeURIComponent(slug)}/credential`,
    ),
  );
}
...
export async function pipedreamConnect(
  ...
  return unwrapLocal(
    await backendApi.post<{ token?: string; app?: string; connectUrl?: string }>(
      `/executor/workspaces/${encodeURIComponent(workspaceId)}/connectors/${encodeURIComponent(slug)}/connect`,
      {
...
// ── IAM V2: workspace ⇄ group attachments (workspace-scoped) ─────────────────────
 ...
 *  (its `access.ts` only has workspace ⇄ group grants, not the account's group
