/**
 * Workspaces screen — post-login landing, ported from web's /workspaces page.
 *
 * Repo-first model: lists workspaces for the current account (GET /accounts +
 * GET /workspaces?account_id=). A compact header (account switcher + New + the
...
import { AccountSwitcherSheet } from '@/components/workspaces/AccountSwitcherSheet';
import { NewWorkspaceSheet } from '@/components/workspaces/NewWorkspaceSheet';
import { AccountMenuSheet } from '@/components/workspaces/AccountMenuSheet';
import { useAuthContext } from '@/contexts';
import { useAccounts, useArchiveWorkspace, useWorkspaces } from '@/lib/workspaces/hooks';
...
export default function WorkspacesScreen() {
  ...
  const workspacesQuery = useWorkspaces(activeAccountId);

  const filtered: KortixWorkspace[] = React.useMemo(() => {
  ...
  const loading = accountsQuery.isLoading || workspacesQuery.isLoading;
  const showEmpty = !!activeAccountId && !loading && !workspacesQuery.isError && total === 0;
  const showNoResults =
    !!activeAccountId && !loading && !workspacesQuery.isError && total > 0 && filtered.length === 0;
  ...
  const openWorkspace = React.useCallback(
    (p: KortixWorkspace) => router.push(`/workspaces/${p.workspace_id}`),
    [router],
  );
      ...
      Alert.alert('Archive workspace', `Archive "${p.name}"?`, [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              haptics.medium();
              await archive.mutateAsync(p.workspace_id);
              haptics.success();
  ...
  const onCardLongPress = React.useCallback(
    (p: KortixWorkspace) => {
      const canManage = p.effective_workspace_role === 'editor' || !p.effective_workspace_role;
      const buttons: any[] = [{ text: 'Open', onPress: () => openWorkspace(p) }];
  ...
  const onRefresh = React.useCallback(async () => {
    ...
    try {
      ...
      if (activeAccountId) tasks.push(workspacesQuery.refetch());
      await Promise.all(tasks);
    } finally {
      setRefreshing(false);
    }
  }, [accountsQuery, workspacesQuery, activeAccountId]);
    ...
    <View style={{ flex: 1, backgroundColor: isDark ? '#0D0D0D' : '#FFFFFF' }}>
      ...
      >
        ...
        <View className="mt-5">
          ...
          {/* Error */}
          {workspacesQuery.isError && !loading && (
            <View
            ...
            >
              ...
              <Text className="mt-1 text-center font-roobert text-[13px] text-muted-foreground">
                {(workspacesQuery.error as Error)?.message ?? 'Check your connection and try again.'}
              </Text>
              <Pressable
                onPress={() => onRefresh()}
          ...
          {filtered.length > 0 && (
            <View>
              {filtered.map((workspace, i) => {
                const chalk = chalkColors(workspace.name);
                return (
                <Pressable
                  key={workspace.workspace_id}
                  onPress={() => openWorkspace(workspace)}
                  onLongPress={() => onCardLongPress(workspace)}
                  delayLongPress={300}
