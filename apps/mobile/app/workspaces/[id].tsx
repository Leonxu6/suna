...
import { AccountMenuSheet } from '@/components/workspaces/AccountMenuSheet';
...
import type { WorkspaceSession, WorkspaceSessionStatus } from '@/lib/workspaces/workspaces-client';
...
// Quick-start suggestions for the workspace home composer. Mirrors the web's
...
// ─── Workspace session list item (repo-first model) ────────────────────────
// The Workspace detail drawer lists the Workspace's sessions (GET /workspaces/:id/
...
const WORKSPACE_SESSION_STATUS_LABELS: Record<WorkspaceSessionStatus, string> = {
  queued: 'Queued',
  branching: 'Branching',
...
function SessionStatusDot({ status }: { status: WorkspaceSessionStatus }) {
  const { colorScheme } = useColorScheme();
  const isDark = colorScheme === 'dark';
...
function WorkspaceSessionListItem({
  item,
  isActive,
  onPress,
}: {
  ...
  isActive: boolean;
  onPress: (s: WorkspaceSession) => void;
}) {
 ...
 * session connects rather than passing `initial_prompt` to createWorkspaceSession
...
export default function WorkspaceSessionScreen() {
  // Cloned 1:1 from app/home.tsx — same three-pane layout (left drawer · session
  // middle · right drawer). workspaceId is the repo-first workspace we'll scope the
  // new API onto.
  const { id: workspaceId } = useLocalSearchParams<{ id: string }>();
  const insets = useSafeAreaInsets();

  // Tabs are remembered PER WORKSPACE: switch the tab store onto this workspace's
  // scope before the first paint (the store snapshots the outgoing workspace and
  // hydrates this one's saved tabs — empty for a never-visited workspace).
  useLayoutEffect(() => {
  ...
  // Workspace view (web model): never gate on a global sandbox. Sandboxes are
  ...
  // This is what the left drawer lists for the Workspace detail screen.
  const { data: workspaceSessions = [], isLoading: workspaceSessionsLoading } =
    useWorkspaceSessions(workspaceId);
  const [activeWorkspaceSessionId, setActiveWorkspaceSessionId] = useState<string | null>(null);
  // A Workspace session that's provisioning — the middle pane shows a connecting
  // state and the Workspace-session poll opens it once its sandbox is ready.
  const [connectingWorkspaceSessionId, setConnectingWorkspaceSessionId] = useState<string | null>(null);
  ...
  const createWorkspaceSession = useCreateWorkspaceSession(workspaceId);
  ...
  const connectingStatusLabel = useMemo(() => {
    const ps = workspaceSessions.find((s) => s.session_id === connectingWorkspaceSessionId);
    return `${(ps && WORKSPACE_SESSION_STATUS_LABELS[ps.status]) || 'Provisioning'}…`;
  }, [workspaceSessions, connectingWorkspaceSessionId]);

  // Only touch a sandbox once a session is actually open (its sandbox is switched
  // in via connectToWorkspaceSession). On the dashboard there is no authorized
  ...
  const handleNewSession = useCallback(async () => {
    ...
    try {
      haptics.tap();
      setDrawerOpen(false);
      // Repo-first new session (web parity): create a blank Workspace session and
      ...
      const session = await createWorkspaceSession.mutateAsync({});
      setActiveWorkspaceSessionId(session.session_id);
      navigateToSession(null);
      setConnectError(null);
      erroredSessionRef.current = null;
      setConnectingWorkspaceSessionId(session.session_id);
    } catch (err: any) {
  ...
  }, [workspaceId, createWorkspaceSession, navigateToSession, showUpgradeForError]);
  ...
  const connectToWorkspaceSession = useCallback(
    (ps: WorkspaceSession) => {
      ...
      setConnectingWorkspaceSessionId(null);
      setConnectError(null);
      erroredSessionRef.current = null;
      setActiveWorkspaceSessionId(ps.session_id);
  ...
  // Keeps connectingWorkspaceSessionId set so the connecting branch renders the
  ...
  // Bring a Workspace session online and open it. POST /start is the only open
  ...
  const ensureAndOpen = useCallback(
    async (sessionId: string) => {
      if (!workspaceId || ensuringRef.current === sessionId) return;
      ...
      try {
        ...
        while (Date.now() - startedAt < MAX_WAIT_MS) {
          ...
          const start = await startWorkspaceSession(workspaceId, sessionId);
    ...
    [workspaceId, connectToWorkspaceSession, failConnect, showUpgradeForError]
  );

  // Open a Workspace session from the drawer. Always enter the connecting state —
  ...
  const handleOpenWorkspaceSession = useCallback(
    (ps: WorkspaceSession) => {
      haptics.tap();
      setActiveWorkspaceSessionId(ps.session_id);
      setDrawerOpen(false);
      ...
      setConnectError(null);
      erroredSessionRef.current = null;
      setConnectingWorkspaceSessionId(ps.session_id);
    },
    [navigateToSession]
  );

  // Open a session by raw id (e.g. Fix-with-agent returns a new session). Same
  // connecting flow as handleOpenWorkspaceSession, minus the WorkspaceSession shell.
  const handleOpenSessionById = useCallback(
    (sessionId: string) => {
      ...
      setConnectError(null);
      erroredSessionRef.current = null;
      setConnectingWorkspaceSessionId(sessionId);
    },
    [navigateToSession]
  );
  ...
  const handleConfigureSession = useCallback(
    async (prompt: string) => {
      ...
      try {
        haptics.tap();
        const session = await createWorkspaceSession.mutateAsync({ initial_prompt: prompt });
        setActiveWorkspaceSessionId(session.session_id);
        navigateToSession(null);
        setConnectError(null);
        erroredSessionRef.current = null;
  ...
  // restartWorkspaceSession). Tears down + re-provisions the sandbox, clears the
  // error/guard, and re-drives the connect loop.
  const handleRestartSession = useCallback(async () => {
    const sid = connectingWorkspaceSessionId;
    if (!sid || restartingSession) return;
    haptics.tap();
    setRestartingSession(true);
    try {
      await restartWorkspaceSession(workspaceId, sid);
      erroredSessionRef.current = null;
  ...
  }, [connectingWorkspaceSessionId, restartingSession, workspaceId, ensureAndOpen]);
  ...
  // connectingWorkspaceSessionId so the auto-connect effect doesn't race the
  // restart; it's cleared once re-provision resolves so ensureAndOpen runs.
  // The active tab's Workspace-session row. The tab store's activeSessionId is
  // the OPENCODE root id (connectToWorkspaceSession navigates with
  // ps.opencode_session_id), so resolve back to the Kortix row through the pin
  // — every /workspaces/:id/sessions/:sid API call needs the Kortix UUID. The
  ...
  const handleOpenChangeRequest = useCallback(async () => {
    const ps = activeWorkspaceSession;
  ...
  }, [activeWorkspaceSession, sandboxUrl, activeSessionId]);

  const handleRestartActiveSession = useCallback(() => {
    // Kortix id — restartWorkspaceSession, the connecting screen, and
    // ensureAndOpen all operate in the Kortix id space, not the OpenCode one.
    const sid = activeWorkspaceSession?.session_id;
    if (!sid || restartingSession) return;
    Alert.alert(
      'Restart Session',
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            ensuringRef.current = null;
            setConnectError(null);
            setConnectingWorkspaceSessionId(sid);
            try {
              await restartWorkspaceSession(workspaceId, sid);
              erroredSessionRef.current = null;
              void ensureAndOpen(sid);
  ...
  }, [activeWorkspaceSession, restartingSession, workspaceId, ensureAndOpen]);

  // Delete the active session (web parity: deleteWorkspaceSession — destroys the
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            haptics.tap();
            try {
              await deleteWorkspaceSession(workspaceId, ps.session_id);
  ...
  }, [activeWorkspaceSession, workspaceId, closeTab, navigateToSession, queryClient]);
  ...
  useEffect(() => {
    if (!connectingWorkspaceSessionId) return;
    if (erroredSessionRef.current === connectingWorkspaceSessionId) return;
    void ensureAndOpen(connectingWorkspaceSessionId);
  }, [connectingWorkspaceSessionId, ensureAndOpen]);
  ...
  const handleDashboardSend = useCallback(
    async (text: string, options: PromptOptions, mentions?: TrackedMention[]) => {
      if (!workspaceId || isDashboardSending) return;
      if (!text.trim()) return;
      ...
      try {
        const session = await createWorkspaceSession.mutateAsync({ initial_prompt: finalText });
        setActiveWorkspaceSessionId(session.session_id);
            ...
            <ScrollView
              ...
              <AnimatedCollapsible expanded={sessionsExpanded}>
                {workspaceSessions.length === 0 ? (
                  ...
                  workspaceSessions.map((ps) => (
                    <WorkspaceSessionListItem
                      key={ps.session_id}
                      item={ps}
                      isActive={ps.session_id === activeWorkspaceSessionId}
                      onPress={handleOpenWorkspaceSession}
                    />
                  ))
                )}
              </AnimatedCollapsible>
  ...
  const renderRightDrawerContent = useCallback(
    () => <RightDrawerContent onClose={handleRightDrawerClose} workspaceId={workspaceId} />,
            ...
            <>
              ...
              activeSessionId && !showTabsOverview ? (
                <SessionPage
                  ...
                  isRightDrawerOpen={rightDrawerOpen}
                />
              ) : /* Connecting — a Workspace session is provisioning */
              connectingWorkspaceSessionId && !showTabsOverview ? (
              ...
              showTabsOverview ? (
                ...
                <KeyboardAvoidingView
                  ...
                  <Animated.View
                    ...
                    <Avatar variant="custom" size={64} fallbackText={workspaceName} />
                    <Text
          ...
          {!showTabsOverview && (
            <View>
              <BottomBar
                ...
                onOpenChangeRequest={activeWorkspaceSession ? handleOpenChangeRequest : undefined}
                onViewChanges={() => {
                  if (activeSessionId) {
                ...
                onRenameSession={
                  activeWorkspaceSession ? () => renameSessionSheetRef.current?.present() : undefined
                }
                onShareSession={
                  activeWorkspaceSession && activeWorkspaceSession.can_manage_sharing !== false
                ...
                onDeleteSession={activeWorkspaceSession ? handleDeleteActiveSession : undefined}
                customMenuItems={
