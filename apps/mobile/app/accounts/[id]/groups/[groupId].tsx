...
export default function GroupDetailScreen() {
  ...
  const grantsQuery = useQuery({ queryKey: ['group-grants', accountId, groupId], queryFn: () => listGroupWorkspaceGrants(accountId, groupId), staleTime: 20_000 });
  ...
  const confirmDetach = (workspaceId: string, workspaceName: string) => Alert.alert('Detach from workspace', `Members lose their inherited access to "${workspaceName}" (unless granted another way).`, [
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 6, paddingBottom: insets.bottom + 48 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={{ marginTop: 6 }}>
            {grantsQuery.isLoading ? (
              ...
              <View key={g.workspace_id} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 11, borderTopWidth: i === 0 ? 0 : 1, borderTopColor: c.border }}>
                <View style={{ flex: 1, minWidth: 0 }}>
                  <Text style={{ fontSize: 13.5, fontFamily: 'Roobert-Medium', color: c.fg }} numberOfLines={1}>{g.workspace_name}</Text>
                ...
                <TouchableOpacity onPress={() => { haptics.tap(); confirmDetach(g.workspace_id, g.workspace_name); }} hitSlop={8} style={{ width: 32, height: 32, borderRadius: 9999, alignItems: 'center', justifyContent: 'center' }}><X size={15} color="#ef4444" /></TouchableOpacity>
              </View>
            ))}
          </View>
