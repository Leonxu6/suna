/**
 * NewWorkspaceSheet — create a workspace, ported from web's WorkspaceCreateModal.
 *
...
interface NewWorkspaceSheetProps {
  ...
  onClose: () => void;
  onCreated: (workspace: KortixWorkspace) => void;
}

export function NewWorkspaceSheet({ open, accountId, onClose, onCreated }: NewWorkspaceSheetProps) {
  ...
  const handleCreateManaged = useCallback(async () => {
    ...
    if (cleaned.length > WORKSPACE_NAME_MAX_LENGTH) {
      return toast.error(`Workspace name must be ${WORKSPACE_NAME_MAX_LENGTH} characters or fewer`);
    }
    try {
      haptics.medium();
      const workspace = await provision.mutateAsync({
        account_id: accountId,
        name: cleaned,
        starter_template: starterTemplateForManagedWorkspace(),
      });
      haptics.success();
      ...
      >
        ...
        {mode === 'managed' ? (
          <>
            ...
            {/* Every workspace ships with the full Kortix starter skill kit. */}
