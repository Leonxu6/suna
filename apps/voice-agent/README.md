...
`POST /v1/workspaces/:workspaceId/sessions/:sessionId/mcp/voice` — a JSON-RPC 2.0
...
`call_id`/`workspace_id`/`session_id` are never in the tool arguments — the MCP
...
Room metadata contains `workspace_id` and `session_id`. Private dispatch metadata
