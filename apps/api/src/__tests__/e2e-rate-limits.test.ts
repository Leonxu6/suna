...
mock.module('../config', () => ({
  config: {
    ...
    KORTIX_WORKSPACE_WEBHOOK_REQS_PER_MIN: 1,
...
describe('audited rate limits', () => {
  ...
  test('limits workspace webhooks by workspace and client IP without database audit work', async () => {
    const app = new Hono();
    app.use('/v1/webhooks/workspaces/:workspaceId/:slug', createWorkspaceWebhookRateLimitMiddleware());
    app.post('/v1/webhooks/workspaces/:workspaceId/:slug', (c) => c.json({ ok: true }));

    const headers = { 'X-Forwarded-For': '203.0.113.40' };
    const first = await app.request('/v1/webhooks/workspaces/workspace-1/hook', {
      method: 'POST',
      headers,
    });
    expect(first.status).toBe(200);

    const second = await app.request('/v1/webhooks/workspaces/workspace-1/hook', {
      method: 'POST',
      headers,
    });
    expect(second.status).toBe(429);
    ...
    const otherWorkspace = await app.request('/v1/webhooks/workspaces/workspace-2/hook', {
      method: 'POST',
      headers,
    });
