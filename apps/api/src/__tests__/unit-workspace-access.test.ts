...
describe('isUuid workspace-id guard', () => {
  test.each([
...
describe('workspace access roles', () => {
  ...
  test.each([
    ...
    (accountRole, workspaceRole, expected) => {
      expect(effectiveWorkspaceRole(accountRole, workspaceRole)).toBe(expected);
    },
  );
  ...
  test.each([
    ['read', 'workspace.read'],
    ['session', 'workspace.session.start'],
    ['write', 'workspace.write'],
    ['manage', 'workspace.write'],
