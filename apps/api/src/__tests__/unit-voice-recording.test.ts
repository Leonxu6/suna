...
describe('promptVoiceAgent records what the room was given', () => {
  ...
  test('an unresolvable workspace degrades to "delivered but unrecorded", never to a throw', async () => {
    dbResults = [[]];
...
describe('settleAsk — the arriving answer is what clears the flag', () => {
  test('it writes a settle row naming the outcome, in the shape every tool row uses', async () => {
    dbResults = [[{ workspaceId: 'ws-9' }]]; // workspaceIdForSession
  ...
  test('an unresolvable workspace degrades quietly rather than throwing into the watcher', async () => {
    dbResults = [[]];
