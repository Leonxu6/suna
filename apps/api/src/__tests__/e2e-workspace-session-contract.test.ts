...
const WORKSPACE_RUNTIME_PAT = 'kortix_pat_workspace_runtime';
const WORKSPACE_SANDBOX_TOKEN = 'kortix_sb_workspace_runtime';
const WORKSPACE_SA_TOKEN = 'kortix_sa_backend_wrapper';
const WORKSPACE_AGENT_PAT = 'kortix_pat_agent_session';
const WORKSPACE_EXECUTOR_PAT = 'kortix_pat_executor_sandbox';
...
process.env.API_KEY_SECRET = 'test-workspace-secret-key-material-32-bytes';
...
let sessionRow: typeof workspaceSessions.$inferSelect | null;
let sessionSandboxRows: Array<typeof sessionSandboxes.$inferSelect>;
let secretRows: Array<typeof workspaceSecrets.$inferSelect>;
let runtimeContextRows: Array<typeof workspaceSessionRuntimeContexts.$inferSelect>;
let secretValues: Map<string, string>;
let gitConnectionRows: Array<typeof workspaceGitConnections.$inferSelect>;
let gitCredentialRows: Array<typeof workspaceGitCredentials.$inferSelect>;
...
const workspaceRow: typeof workspaces.$inferSelect = {
  workspaceId: WORKSPACE_ID,
  accountId: ACCOUNT_ID,
  sandboxProviderGeneration: 0,
  name: 'Contract Workspace',
  repoUrl: `https://github.com/${TEST_GITHUB_OWNER}/contract-workspace.git`,
  defaultBranch: 'main',
  ...
  metadata: {
    github: {
      auth_source: 'pat',
      full_name: `${TEST_GITHUB_OWNER}/contract-workspace`,
    },
  },
  lastOpenedAt: null,
...
function resetState() {
  ...
  workspaceRow.repoUrl = `https://github.com/${TEST_GITHUB_OWNER}/contract-workspace.git`;
  workspaceRow.defaultBranch = 'main';
  workspaceRow.metadata = {
    github: {
      auth_source: 'pat',
      full_name: `${TEST_GITHUB_OWNER}/contract-workspace`,
    },
...
mock.module('../middleware/auth', () => ({
  ...
  supabaseAuth: async (c: any, next: any) => {
    ...
    if (c.req.header('Authorization') === `Bearer ${WORKSPACE_AGENT_PAT}`) {
    ...
    if (c.req.header('Authorization') === `Bearer ${WORKSPACE_EXECUTOR_PAT}`) {
...
mock.module('../workspaces/git', () => ({
  ...
  // straight from git — no manifest ⇒ null ⇒ the v1-shaped workspaces this suite
...
mock.module('../workspaces/github', () => ({
  parseGitHubRepoUrl: (repoUrl: string) => ({
    ...
    repo:
      repoUrl
        .split('/')
        .pop()
        ?.replace(/\.git$/, '') ?? 'contract-workspace',
  }),
  ...
  getRepo: async () => ({
    id: 7,
    name: 'contract-workspace',
    full_name: 'kortix-org/contract-workspace',
    private: true,
    html_url: 'https://github.com/kortix-org/contract-workspace',
    clone_url: 'https://github.com/kortix-org/contract-workspace.git',
    ssh_url: 'git@github.com:kortix-org/contract-workspace.git',
    default_branch: 'main',
    description: null,
  }),
...
mock.module('../shared/db', () => ({
  ...
  db: {
      ...
      from: (table: unknown) => ({
        where: () => ({
          then: (resolve: (value: unknown[]) => unknown, reject?: (reason: unknown) => unknown) => {
            Promise.resolve(table === workspaceSecrets ? secretRows : []).then(resolve, reject);
          },
          orderBy: async () => {
            if (table === workspaceSecrets) return secretRows;
            if (table === workspaceSessions) return sessionRow ? [sessionRow] : [];
            return [];
          },
          limit: async () => {
            if (fields && Object.keys(fields).includes('activeCount'))
              return [{ activeCount: activeSessionCount }];
            if (table === workspaceSecrets) {
              return secretRows.filter((row) => row.name === 'KORTIX_GIT_AUTH_TOKEN').slice(0, 1);
            }
            if (table === workspaceSessionRuntimeContexts) return runtimeContextRows.slice(0, 1);
            if (table === workspaceGitConnections) return gitConnectionRows.slice(0, 1);
            if (table === workspaceGitCredentials) return gitCredentialRows.slice(0, 1);
            if (table === sessionSandboxes) return sessionSandboxRows.slice(0, 1);
            if (table === workspaces) return [workspaceRow];
    ...
    insert: (table: unknown) => ({
      values: (values: any) => ({
        returning: async () => {
          ...
          if (table === workspaceGitCredentials) {
            ...
            const row = {
              ...
              updatedAt: values.updatedAt ?? now,
            } as typeof workspaceGitCredentials.$inferSelect;
            if (existingIndex >= 0) gitCredentialRows[existingIndex] = row;
            else gitCredentialRows.push(row);
            return [row];
          }
          if (table === workspaceSessionRuntimeContexts) {
            const row: typeof workspaceSessionRuntimeContexts.$inferSelect = {
          ...
          if (table !== workspaceSessions) return [];
          sessionRow = {
            sessionId: values.sessionId,
            accountId: values.accountId,
            workspaceId: values.workspaceId,
        ...
        onConflictDoUpdate: ({
          ...
          set: Partial<typeof workspaceSecrets.$inferInsert>;
        }) => {
    ...
    delete: (table: unknown) => ({
      where: async () => {
        if (table === workspaceSecrets) secretRows = [];
        if (table === workspaceSessionRuntimeContexts) runtimeContextRows = [];
    ...
    update: (table: unknown) => ({
      set: (
        updates: Partial<typeof workspaceSessions.$inferSelect> &
...
describe('workspace session API contract', () => {
  afterAll(() => {
    mock.restore();
  ...
  test('sandbox token acquires, renews, and releases the execution lease route', async () => {
    ...
    const request = (action: 'acquire' | 'renew' | 'release', token: string) =>
      app.request(`/v1/workspaces/${WORKSPACE_ID}/execution-lease`, {
        method: 'POST',
        headers: {
    ...
    const acquire = await request('acquire', WORKSPACE_SANDBOX_TOKEN);
    ...
    const renew = await request('renew', WORKSPACE_SANDBOX_TOKEN);
    ...
    const release = await request('release', WORKSPACE_SANDBOX_TOKEN);
    expect(release.status).toBe(200);
    expect(await release.json()).toEqual({ ok: true });

    const forbidden = await request('renew', WORKSPACE_RUNTIME_PAT);
  ...
  test('GET workspace session inventory rejects callers without workspace.session.read', async () => {
    deniedIamAction = 'workspace.session.read';
    const app = createApp();

    const response = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`);

    expect(response.status).toBe(403);
    expect(assertedIamActions).toContain('workspace.session.read');
  });
  ...
  test('upserts and lists workspace secrets without exposing secret values', async () => {
    const app = createApp();

    const writeRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets`, {
      method: 'POST',
    ...
    const listRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets`);
    ...
    const deleteRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets/openai_api_key`, {
        method: 'DELETE',
    });
  ...
  test('stores provider-neutral git credentials outside runtime workspace secrets', async () => {
    workspaceRow.repoUrl = 'https://gitlab.com/acme/private-workspace.git';
    workspaceRow.metadata = {
      git: { provider: 'gitlab', auth: { method: 'none' } },
    };
    const app = createApp();

    const before = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets`);
    ...
    const writeRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/git-credential`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'gitlab-workspace-token' }),
    });
    expect(writeRes.status).toBe(200);
    const written = await writeRes.json();
    expect(written).toMatchObject({
      ...
      git_connection: {
        provider: 'gitlab',
        repo_url: 'https://gitlab.com/acme/private-workspace.git',
        auth_method: 'workspace_credential',
        status: 'connected',
      },
    });
    expect(written.value).toBeUndefined();
    ...
    const deleteRes = await app.request(
      `/v1/workspaces/${WORKSPACE_ID}/secrets/KORTIX_GIT_AUTH_TOKEN`,
      {
        method: 'DELETE',
      },
    );
    ...
    const createRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const cloneRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/git/clone-credential`, {
  ...
  test('derives session origin from the caller token; only a backend-origin caller may set origin_ref', async () => {
    const app = createApp();
    const forbidden = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const whitespace = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const bodySpoof = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        provider: 'daytona',
    ...
    const userRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ provider: 'daytona', base_ref: 'main' }),
    });
    ...
    const backendRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const patRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const sandboxRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_SANDBOX_TOKEN}`,
      },
      body: JSON.stringify({ provider: 'daytona', base_ref: 'main', origin_ref: 'tenant-42' }),
    });
    ...
    const agentRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_AGENT_PAT}`,
      },
      body: JSON.stringify({ provider: 'daytona', base_ref: 'main', origin_ref: 'tenant-42' }),
    });
    ...
    const executorRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_EXECUTOR_PAT}`,
      },
      body: JSON.stringify({ provider: 'daytona', base_ref: 'main', origin_ref: 'tenant-42' }),
    });
  ...
  test('secrets allowlist is backend-only, existence-checked, and persisted', async () => {
    ...
    const seed = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const unknown = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_RUNTIME_PAT}`,
    ...
    const ok = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_RUNTIME_PAT}`,
    ...
    // An empty allowlist (inject ZERO workspace secrets) is a valid backend request.
    const zero = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_RUNTIME_PAT}`,
  ...
  test('rejects an allowlist whose identifiers collide on one env key (409, not a bricked session)', async () => {
    ...
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_RUNTIME_PAT}`,
  ...
  test('KaaB: backend overrides (model, origin_ref, secrets, agent) each apply at boot', async () => {
    ...
    for (const [name, value] of [
      ...
      const w = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    const res2 = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${WORKSPACE_RUNTIME_PAT}`,
  ...
  test('resolves legacy git auth secret server-side without injecting it into sandbox env', async () => {
    workspaceRow.repoUrl = 'https://git.example.test/legacy-private-workspace';
    workspaceRow.metadata = {};
    secretRows = [
      {
    ...
    expect(cloneRes.status).toBe(200);
    expect(await cloneRes.json()).toMatchObject({
      repo_url: 'https://git.example.test/legacy-private-workspace',
      source: 'workspace_credential',
      auth: {
        username: 'x-access-token',
  ...
  test('rejects reserved platform secret names', async () => {
    const app = createApp();
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: 'KORTIX_TOKEN',
  ...
  test('rejects server-managed and unknown PATCH fields', async () => {
    ...
    for (const { body, message } of forbiddenBodies) {
      const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  test('returns deterministic read errors for invalid or missing sessions and pending sandboxes', async () => {
    ...
    const listSessions = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`);
    ...
    const inventory = await app.request(
      `/v1/workspaces/${WORKSPACE_ID}/sessions?scope=workspace`,
    );
    ...
    const readSession = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}`);
    ...
    const invalidSession = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/not-a-uuid`);
    ...
    const invalidSandbox = await app.request(
      `/v1/workspaces/${WORKSPACE_ID}/sessions/not-a-uuid/start`,
      { method: 'POST' },
    );
    ...
    const pendingSandbox = await app.request(
      `/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}/start`,
      { method: 'POST' },
    );
    ...
    sessionRow = null;
    const missingSession = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}`);
  ...
  test('dashboard start leaves fresh no-external-id provisioning rows alone', async () => {
    ...
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}/start`, {
      method: 'POST',
    });
  ...
  test('dashboard start restores a provider-removed sandbox in place without provisioning', async () => {
    ...
    const ready = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}/start`, {
      method: 'POST',
    });
  ...
  test('concurrent dashboard starts issue exactly one same-id provider recovery', async () => {
    ...
    const first = app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}/start`, {
      method: 'POST',
    });
    await flushUntil(() => providerRecoveryCalls === 1);
    const second = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}/start`, {
      method: 'POST',
    });
  ...
  test('explicit deletion winning during recovery prevents status and billing resurrection', async () => {
    ...
    const request = app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}/start`, {
      method: 'POST',
    });
    await flushUntil(() => providerRecoveryCalls === 1);
    sessionRow = {
  ...
  test('restart of a provider-removed sandbox refuses replacement and preserves identity', async () => {
    ...
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}/restart`, {
      method: 'POST',
    });
    expect(res.status).toBe(409);
    expect(await res.json()).toMatchObject({
  ...
  // ---------------------------------------------------------------------------
  // End-to-end: stored workspace secrets must land in the sandbox env at session
  ...
  // ---------------------------------------------------------------------------
  test('e2e: stored workspace secrets are injected as plaintext env vars at session create', async () => {
    const app = createApp();
    ...
    const shadowRes = await app.request(`/v1/workspaces/${WORKSPACE_ID}/secrets`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('inherits the workspace environment branch and preserves the session/sandbox invariant', async () => {
    workspaceRow.defaultBranch = 'dev';
    const app = createApp();
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('stops a session without deleting its preserved branch row', async () => {
    ...
    sessionRow = null;
    const missing = await app.request(`/v1/workspaces/${WORKSPACE_ID}/sessions/${SESSION_ID}`, {
        method: 'DELETE',
    });
