...
// Capture the body createWorkspaceSession is called with.
...
mock.module('../iam', () => ({
  ...
  unscopedResourceIds: async (_workspaceId: string, _resourceType: string, resourceIds: readonly string[]) => [...resourceIds],
}));
...
// Brand-new thread, claim won → createWorkspaceSession runs exactly once.
...
test('channel agent + model override flow into the session body', async () => {
  selection = { workspaceId: 'proj-1', agentName: 'reviewer', opencodeModel: 'anthropic/claude-opus-4-8' };
  newThreadFifo();
...
test('no overrides → agent "default" and NO opencode_model key', async () => {
  selection = { workspaceId: 'proj-1', agentName: null, opencodeModel: null };
  newThreadFifo();
  await spawnAgentTurn('proj-1', envelope, event);
...
// agent_name, so sessions.ts's `body.agent_name ?? workspaceDefaultAgent`
// fallback never fired — a workspace's configured default_agent was silently
// ignored by every Slack session, diverging from what the settings page
// showed as "Workspace default (agentX)". No channel override should now
// resolve to the workspace's configured default, not the bare sentinel.
test('no channel override, but workspace has a configured default_agent → that agent is used', async () => {
  selection = { workspaceId: 'proj-1', agentName: null, opencodeModel: null };
  dbResults = [
    [{ ...workspace, metadata: { default_agent: 'shipper' } }], // spawnAgentTurn workspace lookup
    [], // chat_threads lookup → brand-new thread
    [{ ...workspace, metadata: { default_agent: 'shipper' } }], // createOrJoinThreadSession workspace lookup
    [{ eventId: 'claim' }], // claimThreadCreate → WON
    [], // re-check chat_threads → none
    [], // chat_threads insert
  ];
...
// the workspace's live agents so the user re-points the channel in one click.
test('deleted channel agent (AGENT_NOT_DECLARED) → in-thread agent picker, not the generic error', async () => {
  selection = { workspaceId: 'proj-1', agentName: 'ghost', opencodeModel: null };
  scopedAgents = [
  ...
  setSlackSessionLifecycleForTest({
    ...
    createSession: async () => ({
      status: 'failed',
      retryable: false,
      error: { status: 400, body: { error: 'Agent "ghost" is not declared in this workspace', code: 'AGENT_NOT_DECLARED' } },
    }),
    resolveWorkspaceAutomationActor: async () => 'user-1',
  });
  newThreadFifo();
