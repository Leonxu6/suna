...
// The workspace routes were decomposed out of the old monolithic workspaces/index.ts
// into workspaces/routes/*.ts + workspaces/lib/*.ts. Scan the whole workspaces/ tree so
...
describe('kortix-projects authorization safety', () => {
  test('session inventory requires workspace.session.read before querying sessions', () => {
    ...
    const sessionQuery = route.indexOf('.from(workspaceSessions)');
