...
import { isSweepStale, mapWithConcurrency, withTimeout } from '../workspaces/lib/triggers';
...
describe('isSweepStale (scheduler stall detection)', () => {
  ...
  test('IN-FLIGHT (fresh leader): first sweep started recently, not yet completed → NOT stale', () => {
    ...
    // workspace manifests). It must not read as stale the instant it starts just
