 ...
 * token whose grant lacks `workspace.cr.merge` is rejected with 403 by the actual
...
beforeAll(async () => {
  ...
  const r = rows[0];
  if (r) ctx = { workspaceId: r.workspace_id, accountId: r.account_id, userId: r.user_id };
});
...
function mergeReq(secret: string) {
  ...
  return app.request(`/v1/workspaces/${ctx!.workspaceId}/change-requests/${crypto.randomUUID()}/merge`, {
    method: 'POST',
...
describe('HTTP enforcement — CR merge gate via the real route', () => {
  test('agent granted cr.open but NOT cr.merge → 403 at the route', async () => {
    if (!ctx) { console.warn('[http] no owner+workspace in local DB — skipping'); return; }
    const secret = await mintToken({ agent: 'release-bot', kortixCli: ['workspace.cr.open'], connectors: [] });
    const res = await mergeReq(secret);
    expect(res.status).toBe(403);
    const body = await res.json().catch(() => ({}));
    expect(JSON.stringify(body)).toContain('workspace.cr.merge');
  });

  test('agent granted cr.merge → passes the scope gate (404 CR-not-found, not 403)', async () => {
    if (!ctx) return;
    const secret = await mintToken({ agent: 'deployer', kortixCli: ['workspace.cr.merge'], connectors: [] });
    const res = await mergeReq(secret);
