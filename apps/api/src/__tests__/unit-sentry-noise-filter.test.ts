...
describe('Sentry ignoreErrors noise filter (BS c672fb5e)', () => {
  ...
  // loads + a user `GET /v1/workspaces` contending for the 20-session pool),
...
describe('index.ts DB-error handler pool-exhaustion guard (BS 721b7efe)', () => {
  ...
  test('the DB-error handler guards captureException with isSentryIgnoredError', () => {
    ...
    expect(indexSrc).toContain('function handleAppError(err: Error, c: any)');
    expect(indexSrc).toContain('app.onError(handleAppError)');
    expect(indexSrc).toMatch(
      /const isPoolExhaustion = isSentryIgnoredError\(\s*databaseError\.causeName \?\? databaseError\.outerName,\s*databaseMessage,\s*\)/,
    );
