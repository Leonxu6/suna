...
const workspace = {
  ...
  repoUrl: 'https://kortix.code.storage/managed-workspace.git',
  defaultBranch: 'main',
  ...
  metadata: {
    git: {
      ...
      managed: true,
      auth: { method: 'managed' },
      upstream_url: 'https://kortix.code.storage/managed-workspace.git',
      repo_id: 'repo-1',
      name: 'managed-workspace',
    },
  },
...
describe('deleteManagedWorkspaceRepo', () => {
  test('deletes a managed repository through its configured backend', async () => {
    ...
    const deleted = await deleteManagedWorkspaceRepo(workspace, {
  ...
  test('leaves user-connected repositories untouched', async () => {
    ...
    const byo = {
      ...
      metadata: {
        git: {
          ...(workspace.metadata.git as Record<string, unknown>),
          managed: false,
