...
mock.module('../workspaces/git', () => ({
  ...
  deleteRemoteSessionBranch: async (_workspace: unknown, branchName: string) => {
...
describe('workspace maintenance', () => {
  ...
  test('deletes expired session branches and records branch GC metadata', async () => {
    branchCandidates = [
      {
        ...
        repoUrl: 'https://github.com/kortix-ai/workspace.git',
        defaultBranch: 'main',
    ...
    const sessionUpdate = updateCalls.find((call) => call.table === workspaceSessions);
