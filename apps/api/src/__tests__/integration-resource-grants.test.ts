...
beforeAll(async () => {
  ...
  await db.execute(
    sql`create unique index if not exists uq_iam_resource_grants on kortix.iam_resource_grants (workspace_id, resource_type, resource_id, principal_type, principal_id)`,
  );
  ...
  if (rows[0]) ctx = { workspaceId: rows[0].workspace_id, accountId: rows[0].account_id };
});
