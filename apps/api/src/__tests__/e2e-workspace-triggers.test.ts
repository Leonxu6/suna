...
const workspaceRow: typeof workspaces.$inferSelect = {
  ...
  repoUrl: 'https://github.com/kortix-ai/trigger-workspace.git',
  defaultBranch: 'main',
...
mock.module('../workspaces/github', () => ({
  parseGitHubRepoUrl: (repoUrl: string) => ({
    owner: 'kortix-org',
    repo: repoUrl.split('/').pop()?.replace(/\.git$/, '') ?? 'trigger-workspace',
  }),
...
const triggerDbMock: any = {
      ...
      from: (table: unknown) => ({
        where: () => {
          ...
          result.limit = async () => {
            ...
            // fallback below.
            if (table === workspaceTriggerRuntime) {
          ...
          (result as any).then = (resolve: (rows: any[]) => unknown) => {
            ...
            } else if (table === workspaceSecrets) resolve(secretRows);
      ...
      values: (values: any) => ({
        returning: async () => {
          ...
          if (table === workspaceSessions) {
            const row: typeof workspaceSessions.$inferSelect = {
    ...
    delete: (table: unknown) => ({
      where: async () => {
        if (table === workspaceTriggerRuntime) runtimeRows = [];
...
describe('git-backed triggers — CRUD', () => {
  ...
  test('POST /triggers commits a new cron trigger into kortix.yaml and returns the listing', async () => {
    const app = createApp();
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/triggers`, {
      method: 'POST',
  ...
  test('POST /triggers commits a webhook trigger and exposes the URL on listing', async () => {
    ...
    expect(body.triggers[0].webhook_url).toContain(`/v1/webhooks/workspaces/${WORKSPACE_ID}/slack-hook`);
  });
  ...
  test('PATCH /triggers/:slug rewrites the manifest entry with the merged spec', async () => {
    ...
    const app = createApp();
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/triggers/one`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('PATCH /triggers/:slug with only `model` persists it to the manifest', async () => {
    ...
    const listing = await app.request(`/v1/workspaces/${WORKSPACE_ID}/triggers`);
  ...
  test('PATCH /triggers/:slug returns 404 when the slug is not in the manifest', async () => {
    const app = createApp();
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/triggers/ghost`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
describe('git-backed triggers — runtime fire paths', () => {
  ...
  test('manual fire spawns a session with the rendered prompt', async () => {
    ...
    const app = createApp();
    const res = await app.request(`/v1/workspaces/${WORKSPACE_ID}/triggers/daily/fire`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{}',
    });
  ...
  test('manual fire without overrides resolves the workspace model and selected agent before provisioning', async () => {
  ...
  test('webhook fires verify the HMAC signature and reject impostors', async () => {
    ...
    const missing = await app.request(`/v1/webhooks/workspaces/${WORKSPACE_ID}/hook`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: rawBody,
    });
    ...
    const wrong = await app.request(`/v1/webhooks/workspaces/${WORKSPACE_ID}/hook`, {
      method: 'POST',
      headers: {
    ...
    const repeated = await app.request(`/v1/webhooks/workspaces/${WORKSPACE_ID}/hook`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  test('webhook fires with a valid HMAC spawn a session', async () => {
    ...
    const duplicate = await app.request(`/v1/webhooks/workspaces/${WORKSPACE_ID}/hook`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Kortix-Signature': sign(rawBody, 'shhh'),
  ...
  test('webhook accepts a valid static token (no HMAC) and rejects a wrong one', async () => {
    ...
    const viaHeader = await app.request(`/v1/webhooks/workspaces/${WORKSPACE_ID}/hook`, {
      method: 'POST',
    ...
    const viaBearer = await app.request(`/v1/webhooks/workspaces/${WORKSPACE_ID}/hook`, {
      method: 'POST',
