 ...
 * fully ISOLATED account + workspace seeded here and torn down after:
 *   - deny-by-default (a plain member with no grant is denied)
 *   - built-in workspace roles (member ⊂ editor ⊂ manager)
 *   - account owner/admin are implicit Managers on every workspace
 *   - group → workspace role (the SCIM/SSO bulk-access channel), incl. max-rank
 *   - DB custom role → policy binding → enforcement, scoped to one workspace
...
const allow = async (userId: string, action: string, target: { type: 'workspace'; id: string }) =>
...
async function grantWorkspace(userId: string, role: 'member' | 'editor' | 'manager') {
  await db.insert(workspaceMembers).values({ accountId: ACCOUNT, workspaceId: WORKSPACE, userId, workspaceRole: role });
}
...
describe('authorizeV2 — deny-by-default + built-in workspace roles', () => {
  test('a plain account member with NO workspace grant is denied everything on the workspace', async () => {
  ...
  test("workspace role 'member' can read + run sessions but not write or manage", async () => {
    const u = await seedMember('member');
  ...
  test('a grant on THIS workspace does not leak to another workspace', async () => {
    const u = await seedMember('member');
    await grantWorkspace(u, 'editor');
    expect(await allow(u, WORKSPACE_ACTIONS.WORKSPACE_WRITE, proj(WORKSPACE))).toBe(true);
...
describe('authorizeV2 — account owner/admin are implicit Managers', () => {
  test('account owner manages any workspace with no explicit grant', async () => {
...
describe('authorizeV2 — group → workspace role (the SCIM/SSO bulk channel)', () => {
  test('membership in a group granted a workspace role confers that role', async () => {
    const u = await seedMember('member');
    const g = await seedGroup(`eng-${uid().slice(0, 6)}`);
    await db.insert(accountGroupMembers).values({ groupId: g, userId: u });
    await db.insert(workspaceGroupGrants).values({ workspaceId: WORKSPACE, groupId: g, accountId: ACCOUNT, role: 'editor' });
    expect(await allow(u, WORKSPACE_ACTIONS.WORKSPACE_WRITE, proj(WORKSPACE))).toBe(true);
    expect(await allow(u, WORKSPACE_ACTIONS.WORKSPACE_MEMBERS_MANAGE, proj(WORKSPACE))).toBe(false);
  });
  ...
  test('two group grants → the HIGHER role wins (max-rank)', async () => {
    ...
    await db.insert(workspaceGroupGrants).values([
...
describe('authorizeV2 — DB custom role → policy binding (allow-only union)', () => {
  test('a workspace-scoped custom role grants an extra action on ONE workspace only', async () => {
    const u = await seedMember('member');
    ...
    // Custom role that grants trigger creation, bound to this member for THIS workspace.
    const roleId = uid();
    await db.insert(iamRoles).values({ roleId, accountId: ACCOUNT, key: `scheduler-${uid().slice(0, 6)}`, name: 'Scheduler', scopeType: 'workspace' });
    ...
    await db.insert(iamPolicies).values({
      accountId: ACCOUNT, principalType: 'member', principalId: u, roleId, scopeType: 'workspace', scopeId: WORKSPACE,
    });
...
describe('authorizeV2 — revoke immediacy (cache invalidation)', () => {
  test('removing a workspace grant + invalidating denies on the very next check', async () => {
    const u = await seedMember('member');
    await grantWorkspace(u, 'editor');
    ...
    await db
      ...
      .where(and(eq(workspaceMembers.workspaceId, WORKSPACE), eq(workspaceMembers.userId, u)));
