...
function createApp() {
  ...
  app.post('/v1/workspaces/:workspaceId/execution-lease', (c) =>
    c.json({
  ...
  app.post('/v1/workspaces/:workspaceId/not-sandbox-enabled', (c) => c.json({ ok: true }));
  return app;
}
...
describe('sandbox execution-lease authentication', () => {
  test('accepts a valid sandbox token and sets its scoped identity', async () => {
    const response = await createApp().request(
      '/v1/workspaces/workspace-1/execution-lease',
      {
        method: 'POST',
  ...
  test('rejects the same sandbox token on an unrelated workspace route', async () => {
    const response = await createApp().request(
      '/v1/workspaces/workspace-1/not-sandbox-enabled',
      {
        method: 'POST',
        headers: { Authorization: 'Bearer kortix_sb_execution_lease' },
      },
