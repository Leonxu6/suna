 ...
 * workspace row — synthetic ids are enough.
 */
 ...
 *     token; the token names the call. No call, session or workspace id appears
...
describe('GET /public/voice-join/:token/transcript', () => {
  ...
  test('reads ONLY the call its own link was minted for', async () => {
    ...
    // Same workspace, different call — a visitor holding one call's link must
  ...
  test("never echoes the link's workspace id back to an anonymous caller", async () => {
    const id = callId();
    const workspaceId = crypto.randomUUID();
    const { token } = await mintJoinLink({ callId: id, workspaceId });
