/**
 * Unit test for the workspace-limit POLICY: `maxWorkspacesForAccount` — the
 * plan→max-workspace-count mapping. Free (and the placeholder `none`) →
 ...
 * The HTTP enforcement of this number lives in `e2e-workspace-limit.test.ts`.
 */
...
describe('maxWorkspacesForAccount — plan → workspace cap', () => {
  beforeEach(() => {
  ...
  test('enterprise → uncapped (unlimited workspaces)', async () => {
