 ...
 *   - account-scoped vs workspace-scoped SA policies apply where they should
 */
import { describe, expect, test, beforeAll, afterAll } from 'bun:test';
import { eq } from 'drizzle-orm';
import { accounts, iamPolicies, iamRoleActions, iamRoles, workspaces, serviceAccounts } from '@kortix/db';
...
async function bindRole(principalId: string, scopeType: 'account' | 'workspace', scopeId: string | null, actions: string[]) {
  const roleId = uid();
...
const can = async (saId: string, action: string, target: { type: 'workspace'; id: string } | undefined) =>
...
describe('service-account authorization (standing identity)', () => {
  test('activated SA is allowed EXACTLY its policy actions, scoped to the policy scope', async () => {
    ...
    await bindRole(sa, 'workspace', WORKSPACE, [WORKSPACE_ACTIONS.WORKSPACE_TRIGGER_CREATE]);
  ...
  test('an account-scoped SA policy grants an account action (and reaches every workspace)', async () => {
    const sa = await seedSA();
    ...
    // An account-scoped policy also confers its workspace actions on EVERY workspace.
