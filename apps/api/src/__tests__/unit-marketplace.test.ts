...
describe('marketplace catalog', () => {
  ...
  test('surfaces Marketing Department as a full cloneable workspace with agents and schedules', async () => {
    const workspaces = await listCatalogItems({ type: 'workspace', source: 'kortix' });
    const marketing = workspaces.find((i) => i.id === 'kortix-projects:marketing-department');
