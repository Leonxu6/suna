 ...
 * workspaces clone`, the sandbox daemon all reach a repo through it), so its
 ...
 * two accounts (personal + team) could create a workspace through the API and
 * then never push to it — while POST /git-token, which hands out a STRONGER
 * credential, accepted them via the per-workspace capability. These pin the
...
mock.module('../shared/db', () => ({
  db: {
    select: () => ({
      from: () => ({
        where: () => ({ limit: async () => (workspaceRow ? [workspaceRow] : []) }),
      }),
    }),
  },
  hasDatabase: true,
...
beforeEach(() => {
  workspaceRow = { workspaceId: WORKSPACE_ID, accountId: OWNER_ACCOUNT, status: 'active' };
...
describe('authorizeGitProxy — CLI PAT', () => {
  ...
  test('read and write map to different capabilities', async () => {
    ...
    expect(authorizeCalls[0]?.action).toBe('workspace.gitops.read');
  });

  test('a workspace-scoped PAT for a different workspace is rejected before any capability check', async () => {
    patResult = {
      isValid: true,
      accountId: OWNER_ACCOUNT,
      userId: 'user-1',
  ...
  test('an archived workspace is 404 regardless of the token', async () => {
    workspaceRow = { workspaceId: WORKSPACE_ID, accountId: OWNER_ACCOUNT, status: 'archived' };
