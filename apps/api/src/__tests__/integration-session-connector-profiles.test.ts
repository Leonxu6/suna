...
describe('session connector profile isolation', () => {
  ...
  test('saveAgentMailInstall does not delete another workspace chat_installs row for the same inbox (pentest 2026-07-27)', async () => {
    ...
    // scoped to the calling workspace, so both rows coexist (unique index allows
    // multiple workspaces per inbox) and resolveWorkspaceForAgentMailInbox keeps
    ...
    const owners = await db
      ...
      .where(
        and(
          ...
          eq(chatInstalls.providerWorkspaceId, sharedInbox),
        ),
      );
