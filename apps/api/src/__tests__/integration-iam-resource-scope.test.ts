 ...
 * on top of the workspace-role verdict:
 *   - an UNSCOPED agent (no grants) stays workspace-wide (no lockout)
...
// workspace member — isolating the per-RESOURCE fold as the thing under test.
const onAgent = (agent: string) => ({ type: 'workspace' as const, id: WORKSPACE, resource: { type: 'agent' as const, id: agent } });
...
describe('per-resource scoping (iam_resource_grants)', () => {
  test('scoping one agent restricts ONLY that agent; unscoped agents stay open', async () => {
    ...
    await db.insert(workspaceMembers).values({ accountId: ACCOUNT, workspaceId: WORKSPACE, userId: alice, workspaceRole: 'editor' });
    const bob = await seedMember('member');
    await db.insert(workspaceMembers).values({ accountId: ACCOUNT, workspaceId: WORKSPACE, userId: bob, workspaceRole: 'editor' });
    ...
    // grant) stays workspace-wide for both.
    expect(await canUse(alice, SCOPED_AGENT)).toBe(true);
  ...
  test('granting the scoped agent to a member lets them in immediately', async () => {
    ...
    await db.insert(workspaceMembers).values({ accountId: ACCOUNT, workspaceId: WORKSPACE, userId: carol, workspaceRole: 'editor' });
  ...
  test('account owner bypasses per-resource scoping (implicit Manager)', async () => {
    ...
    // No workspace grant, no resource grant — still allowed on the scoped agent.
