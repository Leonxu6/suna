 ...
 * "fill this in" link (a workspace secret, or a Pipedream Quick Connect).
 *
 ...
 *     an AEAD envelope encrypted with the WORKSPACE's key (the same per-workspace
 *     HKDF key used for workspace secrets), so a token from one workspace can't be
 ...
 *     in workspaces/routes/r3.ts, which seals its whole state into one encrypted
 *     `flow_id` for the same reasons.
 *   • VALUE-ONLY by construction: the field NAMES are fixed at mint time, so a
 *     leaked token can only SET the named keys in that one workspace before it
...
export function mintSetupLink(
  ...
  const token = TOKEN_PREFIX + Buffer.from(`${workspaceId}.${envelope}`, 'utf8').toString('base64url');
...
export function resolveSetupLink(token: string | undefined | null): ResolvedSetupLink {
  ...
  try {
    ...
    // Wrong workspace key, tampered ciphertext, or garbage → indistinguishable
    // from "never existed". Don't leak which.
    return { ok: false, status: 404, error: 'Invalid or unknown link' };
  }

  if (payload.pid !== workspaceId) return { ok: false, status: 404, error: 'Invalid or unknown link' };
