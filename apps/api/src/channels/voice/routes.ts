 ...
 *
 * Auth here is completely different from every other route on `workspacesApp`:
 ...
 *
 * `voiceMcpRoutes` is mounted standalone BEFORE `workspacesApp` (see
 * index.ts's comment) specifically so this route skips `workspacesApp`'s
