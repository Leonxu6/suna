...
import { listWorkspacesForProviderWorkspace, loadTeamsInstall } from '../install-store';
...
teamsIdentityApp.openapi(
  ...
  async (c: Context) => {
    ...
    const workspaceIds = await listWorkspacesForProviderWorkspace('teams', payload.tenantId);
