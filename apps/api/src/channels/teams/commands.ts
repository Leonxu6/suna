...
async function buildStatusCard(
  ...
  const workspaceName = workspaces.find((p) => p.workspaceId === workspaceId)?.name ?? workspaceId;
...
async function buildWorkspacesCard(tenantId: string, currentWorkspaceId: string) {
  ...
  const options: SelectOption[] = workspaces.slice(0, 8).map((p) => ({
    label: p.name,
    current: p.workspaceId === currentWorkspaceId,
    data: { workspaceId: p.workspaceId },
  }));
  return buildSelectCard({
    emoji: '📁',
    title: 'Connected workspaces',
    subtitle: 'Pick which workspace this conversation runs.',
    verb: 'teams_pick_workspace',
    options,
  });
}
...
async function switchWorkspace(tenantId: string, conversationId: string, arg: string) {
  ...
  const match = q
    ? workspaces.find((p) => p.name.toLowerCase() === q || p.workspaceId === arg.trim())
    : null;
