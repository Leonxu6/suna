...
import { accountMembers, chatChannelBindings, chatInstalls, chatThreads, workspaceSessions, workspaces } from '@kortix/db';
...
async function slashWorkspaceScopedInfo(ctx: SlashCtx): Promise<SlashResponse> {
  ...
  const [workspace] = workspaceId
    ? await db
        .select({ name: workspaces.name, workspaceId: workspaces.workspaceId })
        .from(workspaces)
...
async function slashWorkspaces(ctx: { teamId: string; channelId: string }): Promise<SlashResponse> {
  const rows = await listWorkspaceWorkspaces(ctx.teamId);
  if (rows.length === 0) {
    return {
      response_type: 'ephemeral',
      blocks: [
        {
          ...
          accessory: {
            ...
            action_id: 'workspaces_empty_dashboard',
          },
        },
      ],
    };
          ...
          actions: [
            {
              ...
              action_id: `workspaces_open_${p.workspaceId}`,
            },
            ...(!isBound ? [{
              type: 'button',
              text: { type: 'plain_text', text: 'Switch to this', emoji: true },
              action_id: `switch_workspace_${p.workspaceId}`,
...
async function slashUnbind(ctx: { teamId: string; channelId: string }): Promise<SlashResponse> {
  ...
  await db
    ...
    .where(and(
      ...
      eq(chatChannelBindings.providerWorkspaceId, ctx.teamId),
...
async function slashSessions(ctx: { teamId: string; channelId: string }): Promise<SlashResponse> {
  const rows = await db
    ...
    .from(chatThreads)
    .where(
      and(eq(chatThreads.platform, 'slack'), eq(chatThreads.providerWorkspaceId, ctx.teamId)),
    )
  ...
  const workspaceRows = await db
    .select({ workspaceId: workspaces.workspaceId, name: workspaces.name, repoUrl: workspaces.repoUrl })
    .from(workspaces)
    .where(inArray(workspaces.workspaceId, workspaceIds));
  const workspaceById = new Map(workspaceRows.map((p) => [p.workspaceId, p]));
  const dashboardBase = (config.FRONTEND_URL || 'https://kortix.com').replace(/\/$/, '');
  return {
    response_type: 'ephemeral',
    blocks: [
      ...
      ...rows.flatMap((r) => {
        const p = workspaceById.get(r.workspaceId);
        const workspaceName = p?.name ?? 'workspace';
...
async function slashPanel(ctx: SlashCtx): Promise<SlashResponse> {
  ...
  const [p] = await db
    .select({ workspaceId: workspaces.workspaceId, name: workspaces.name, repoUrl: workspaces.repoUrl, metadata: workspaces.metadata })
    .from(workspaces)
    .where(eq(workspaces.workspaceId, currentId))
    .limit(1);
  if (!p) {
    return {
      response_type: 'ephemeral',
...
async function slashPolicy(ctx: SlashCtx, arg: string): Promise<SlashResponse> {
      ...
      blocks: [
        ...
        },
        {
          ...
          elements: [
            {
              type: 'mrkdwn',
              text: `Default is \`workspace_open\`: linked workspace members can join Slack-started sessions. Use \`owner_approval\` for private threads with owner approval, or \`owner_only\` to block everyone else.`,
            },
          ],
        },
      ],
  ...
  if (next !== requested) {
    return {
      response_type: 'ephemeral',
      text: `Unknown policy \`${requested}\`. Use \`owner_approval\`, \`owner_only\`, or \`workspace_open\`.`,
    };
  }
...
async function slashSession(ctx: SlashCtx): Promise<SlashResponse> {
  ...
  const [s] = await db
    ...
    .where(and(
      eq(workspaceSessions.workspaceId, selection.workspaceId),
      sql`${workspaceSessions.metadata}->'slack'->>'channel' = ${ctx.channelId}`,
    ))
  ...
  try {
    ...
    if (identity) {
      const [proj] = await db
        ...
        .limit(1);
      allowedNames = proj
        ? await filterAccessibleWorkspaceResources(identity.userId, proj.accountId, input.workspaceId, 'agent', names)
...
export async function listWorkspaceWorkspaces(teamId: string): Promise<Array<{ workspaceId: string; name: string; repoUrl: string }>> {
  const installs = await db
    .select({ workspaceId: chatInstalls.workspaceId })
    .from(chatInstalls)
    .where(
      and(eq(chatInstalls.platform, 'slack'), eq(chatInstalls.providerWorkspaceId, teamId)),
    );
