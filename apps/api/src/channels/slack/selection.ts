...
import { chatChannelBindings, workspaces } from '@kortix/db';
...
/** The channel's bound workspace + its agent/model overrides, or null if unbound. */
...
/** Update the channel binding's model (null clears → workspace/platform default). */
...
export interface WorkspaceAgentGovernance {
   ...
   * reject unknown names when this is true — a legacy (undeclared) workspace
 ...
 * declarative `kortix.yaml` `agents:` for adopted workspaces, OpenCode markdown
 * discovery for legacy workspaces. Touches git, so callers must use the async
...
export async function listChannelBindingsForWorkspace(workspaceId: string): Promise<ChannelBindingRow[]> {
  const rows = await db
    .select({
      ...
      providerWorkspaceId: chatChannelBindings.providerWorkspaceId,
  ...
  // `workspace_id` is nullable at the column level (unbound rows can exist
