...
import { chatEventDedup, chatThreads, workspaces } from '@kortix/db';
...
export async function createOrJoinThreadSession(input: {
  ...
  const result = await slackSessionLifecycle.createSession({
    ...
    postCreate: teamId && threadId
      ? [{ type: 'bind_chat_thread', platform: 'slack', providerWorkspaceId: teamId, threadId }]
      : undefined,
    visibility: conversationPolicy === 'workspace_open' ? 'workspace' : 'restricted',
    metadata: {
      source: 'slack',
      slack: {
        team_id: teamId,
