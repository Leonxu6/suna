...
export async function bindChatThread(input: {
  ...
  await db
    ...
    .onConflictDoNothing({
      target: [chatThreads.platform, chatThreads.providerWorkspaceId, chatThreads.threadId],
    });
}

/**
 * Resolve the Slack workspace/team id for a channel from its workspace binding
 * (`chat_channel_bindings`). Returns null when the channel isn't bound to the
 * workspace yet.
 */
export async function resolveProviderWorkspaceIdForChannel(
  workspaceId: string,
  channelId: string,
): Promise<string | null> {
  const [row] = await db
    .select({ providerWorkspaceId: chatChannelBindings.providerWorkspaceId })
    ...
    .limit(1);
  return row?.providerWorkspaceId ?? null;
}
