    ...
    const [row] = await db
      ...
      .where(
        and(
          ...
          eq(chatChannelBindings.providerWorkspaceId, teamId),
...
export async function ensureWorkspaceChannelBinding(
  ...
  await db
    ...
    .values({ platform: 'slack', providerWorkspaceId: teamId, channelId, workspaceId, pickerTs: null })
    .onConflictDoUpdate({
      target: [
...
export async function resolveOauthWorkspace(
  ...
  return { kind: 'ambiguous', workspaceIds: installs.map((i) => i.workspaceId) };
}
...
async function postWorkspacePicker(opts: {
  ...
  const claimed = await db
    .insert(chatChannelBindings)
    .values({ platform: 'slack', providerWorkspaceId: teamId, channelId, workspaceId: null })
    .onConflictDoNothing({
      target: [
        chatChannelBindings.platform,
        chatChannelBindings.providerWorkspaceId,
...
export async function handleAssistantThreadStarted(
  ...
  if (resolution.kind === 'ambiguous') {
    ...
    if (installs.length > 0) {
      await postWorkspacePicker({ teamId, workspaceIds: installs.map((i) => i.workspaceId), channelId, threadTs, isDm: true });
    }
  }
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(chatThreads.providerWorkspaceId, teamId),
...
export async function maybePostChannelIntro(teamId: string, event: SlackEvent): Promise<void> {
  ...
  const [install] = await db
    .select({ workspaceId: chatInstalls.workspaceId })
    .from(chatInstalls)
    .where(and(eq(chatInstalls.platform, 'slack'), eq(chatInstalls.providerWorkspaceId, teamId)))
    .limit(1);
  if (!install) return;
...
export async function spawnAgentTurn(
  ...
  if (teamId && threadId) {
    const [existing] = await db
      ...
      .from(chatThreads)
      .innerJoin(workspaceSessions, eq(workspaceSessions.sessionId, chatThreads.sessionId))
      .where(
        and(
          eq(chatThreads.platform, 'slack'),
          eq(chatThreads.providerWorkspaceId, teamId),
    ...
    if (existing) {
      ...
      // outcome === 'no-session': the durable workspaceSessions row itself is gone
