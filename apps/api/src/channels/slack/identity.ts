...
import { accountMembers, chatUserIdentities, workspaceAccessRequests, workspaces } from '@kortix/db';
  ...
  const [link] = await db
    ...
    .where(
      and(
        ...
        eq(chatUserIdentities.providerWorkspaceId, teamId),
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [
        ...
        chatUserIdentities.providerWorkspaceId,
...
// reusing the same workspaceAccessRequests table the web "Request access" flow uses.
...
export async function createSlackAccessRequest(input: {
  ...
  const [existing] = await db
    .select({ requestId: workspaceAccessRequests.requestId })
    .from(workspaceAccessRequests)
    .where(
      and(
        eq(workspaceAccessRequests.workspaceId, input.workspaceId),
        eq(workspaceAccessRequests.requesterUserId, identity.userId),
  ...
  await db.insert(workspaceAccessRequests).values({
