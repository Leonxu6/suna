...
import { chatChannelBindings, chatInstalls, chatThreads, workspaces } from '@kortix/db';
...
async function handleReviewAction(
  ...
  // Resolve the thread → its workspace so we can scope the item + the actor.
  const [thread] = await db
    .select({ workspaceId: chatThreads.workspaceId })
    .from(chatThreads)
    .where(and(
...
async function handleSwitchWorkspace(payload: SlackInteractionPayload, rawValue: string): Promise<void> {
  ...
  const [install] = await db
    ...
    .where(and(
      ...
      eq(chatInstalls.providerWorkspaceId, teamId),
  ...
  await respondViaUrl(payload.response_url, {
    response_type: 'ephemeral',
    replace_original: true,
    text: `Switched this channel to *${p?.name ?? 'workspace'}*.`,
  });
}
...
async function handleConfigOpen(
  payload: SlackInteractionPayload,
  actionId: 'cfg_open_models' | 'cfg_open_agents' | 'cfg_open_workspaces',
): Promise<void> {
  const teamId = payload.team?.id ?? '';
  const channelId = payload.channel?.id ?? '';
...
export async function handleBlockAction(payload: SlackInteractionPayload): Promise<void> {
  ...
  if (
    ...
    action.action_id === 'cfg_open_workspaces'
  ) {
  ...
  if (action.action_id.startsWith('switch_workspace_')) {
  ...
  if (!action.action_id.startsWith('pick_workspace') && !action.action_id.startsWith('switch_workspace_')) {
    await handleAgentClick(payload, action);
    return;
  }

  if (!action.action_id.startsWith('pick_workspace')) return;
  ...
  if (token && pickerTs) {
    ...
    await updateMessage(
      ...
      channelId,
      pickerTs,
      `✓ Linked ${target} to *${proj?.name ?? 'workspace'}*.`,
    );
  }
