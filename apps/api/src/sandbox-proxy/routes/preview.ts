...
import { observeRuntimeSessionTitleStream } from '../../workspaces/acp-session-title-stream';
import {
...
type PreviewTitleCaptureDependencies = {
  scheduleAfterPrompt: typeof scheduleTitleCaptureAfterPrompt;
  captureAfterRuntimeEvent: typeof captureTitleAfterRuntimeEvent;
};

const defaultTitleCaptureDependencies: PreviewTitleCaptureDependencies = {
  scheduleAfterPrompt: scheduleTitleCaptureAfterPrompt,
  captureAfterRuntimeEvent: captureTitleAfterRuntimeEvent,
};
let titleCaptureDependencies = defaultTitleCaptureDependencies;

/** Replace title-capture callbacks for proxy tests without process-global module mocks. */
export function setPreviewTitleCaptureDependenciesForTest(
  dependencies: PreviewTitleCaptureDependencies | null,
): void {
  titleCaptureDependencies = dependencies ?? defaultTitleCaptureDependencies;
}
 ...
 * that env must not reach OpenCode. See workspaces/lib/secret-grant.ts.
 */
...
export async function forwardToSandbox(
  ...
  const promptDelivery =
    shouldSyncWorkspaceEnvBeforeProxy(port, method, remainingPath) || acpPromptSession !== null;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (shouldSyncWorkspaceEnvBeforeProxy(port, method, remainingPath)) {
        ...
        titleCaptureDependencies.scheduleAfterPrompt({
          sessionId: record.sessionId,
          workspaceId: record.workspaceId,
        ...
        try {
          await syncSandboxEnvForPrompt({
            ...
            // workspaces/lib/secret-grant.ts.
            requestedAgent,
          });
        } catch (err) {
      ...
      const responseBody =
        upstream.body && titleStream
          ? observeRuntimeSessionTitleStream(upstream.body, {
              ...titleStream,
              onTitle: async (event) => {
                await titleCaptureDependencies.captureAfterRuntimeEvent({
                  sessionId: record.sessionId,
                  workspaceId: record.workspaceId,
