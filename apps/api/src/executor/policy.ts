...
export interface EffectiveResolveResult {
  ...
  source: 'workspace' | 'connection' | 'connector' | 'risk_default' | 'allow_all';
}
 ...
 *   4. `defaultMode = risk` → action from risk class.
 *   5. `defaultMode = allow_all` → always_run.
 *
...
export function resolveEffectiveAction(input: EffectiveResolveInput): EffectiveResolveResult {
  ...
  // connector default — but still loses to a workspace rule above, which keeps the
