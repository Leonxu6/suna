 ...
 * (LiveKit config lives in this service's env, not a per-workspace install), so
 ...
 *
 * These actions live on the connector rather than in a workspace's OpenCode
 * config. Connectors are materialized server-side for every workspace.
 * Existing workspaces receive them when this ships. Starter configuration only
 * reaches workspaces created after a change.
 *
