...
import { listWorkspaceSecretsSnapshot } from '../../workspaces/secrets';
...
// `workspace.metadata.default_agent` wasn't populated even though the workspace's
 ...
 *  from one workspace (e.g. account/workspace scope) can also change what a
 *  DIFFERENT workspace's principals resolve. */
 ...
 */
async function connectedByokFallback(workspaceId: string | undefined): Promise<string | null> {
  if (!workspaceId) return null;
  try {
    const snapshot = await listWorkspaceSecretsSnapshot(workspaceId);
...
export async function resolveDefaultModelForPrincipal(
  ...
  const kept = await degradeUnservableDefault(
    ...
    () => connectedByokFallback(principal.workspaceId),
  );
  return kept ?? undefined;
}

/**
 * Whether a concrete wire model can actually be served for an account+workspace —
 ...
 * servable for the account+workspace; an unservable pin (e.g. a BYOK model whose
