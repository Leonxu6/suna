...
beforeEach(() => {
  ...
  spyOn(secretsModule, 'listWorkspaceSecretsSnapshot').mockImplementation(async () => ({
    env: {},
...
describe('resolveEffectiveModel — the /model-defaults GET + picker resolution path', () => {
  test('nothing configured → platform default, no resolution/secrets calls at all', async () => {
    ...
    expect(secretsModule.listWorkspaceSecretsSnapshot).not.toHaveBeenCalled();
  });
...
// workspace_sessions.agent_name whenever workspace.metadata.default_agent wasn't
// populated at the time (common — e.g. a brand-new workspace whose kortix.yaml
