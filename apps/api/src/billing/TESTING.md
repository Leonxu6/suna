...
| #   | Scenario                                | Action                                                                                                          | Expected                                                                                                                       | Verify                                                                                                                                                                                                |
| --- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1  | Start sandbox → metering row opens      | User opens a workspace session                                                                                    | `sandbox_compute_sessions` row with `state='active'`, captured spec, `started_at=now()`, `cost_usd=0`                          | `SELECT id, state, cpu_cores, memory_gb, started_at, cost_usd FROM kortix.sandbox_compute_sessions WHERE sandbox_id='<sandboxId>';`                                                                   |
...
| C7  | Cron tick partial-bills long-running    | Wait for `runWorkspaceMaintenance` (every 5 min) with a session running ≥ 1h                                       | `tickRunningComputeCharges` settles a partial window — `cost_usd` increases, `last_billed_at` advances, row stays open         | `SELECT cost_usd, last_billed_at FROM kortix.sandbox_compute_sessions WHERE id='<id>';` before/after the tick                                                                                         |
...
| C9  | Multiple concurrent sandboxes           | Spin up 3 sandboxes in different workspaces under same account                                                     | All 3 have separate `sandbox_compute_sessions` rows; each debits independently on stop                                         | `SELECT sandbox_id, account_id, state FROM kortix.sandbox_compute_sessions WHERE account_id='<id>' AND ended_at IS NULL;`                                                                             |
...
| C11 | Per-member attribution                  | Member B starts a sandbox in account A's workspace                                                                 | Row's `actor_user_id` = member B's user_id, `account_id` = A's account_id                                                      | `SELECT actor_user_id FROM kortix.sandbox_compute_sessions WHERE sandbox_id='<id>';`                                                                                                                  |
...
| H7 | Workspace ACL respected                                                  | Member without `workspace_members` row can't see owner's workspaces | Owner's workspaces hidden from member's UI                                       | UI check   |
...
3. ✅ As B, open the workspace → see workspace → start a sandbox → chat for 5 min
