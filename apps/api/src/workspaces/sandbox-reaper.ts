...
import { accounts, workspaceSessions, sessionSandboxes } from '@kortix/db';
...
/**
 * Reconcile workspace_sessions stuck in an ACTIVE status that have no genuinely-
 * running box behind them. THE leak that wedged Slack ("I'm queued behind other
 * workspace work") and 429'd new sessions: a session counts against the account's
  ...
  const candidates = await db
    ...
    .where(
      and(
        ...
        or(
          sql`not exists (select 1 from ${sessionSandboxes} sb where sb.session_id = ${workspaceSessions.sessionId} and sb.status = 'active')`,
          sql`(${workspaceSessions.metadata}->>'deletedAt') is not null`,
        ),
        sql`not exists (select 1 from ${chatTurnStreams} t where t.session_id = ${workspaceSessions.sessionId} and t.finalized = false)`,
        sql`not exists (select 1 from ${usageEvents} u where u.session_id = ${workspaceSessions.sessionId} and u.created_at > ${cutoff.toISOString()})`,
      ),
    )
  ...
  for (const c of candidates) {
    try {
      ...
      const updated = await db
        ...
        .set({ status: 'stopped', updatedAt: now })
        .where(and(
          eq(workspaceSessions.sessionId, c.sessionId),
          inArray(workspaceSessions.status, [...ACTIVE_SESSION_STATUSES]),
        ))
 ...
 * scheduler's 60s tick (startWorkspaceTriggerScheduler). A command still `queued`
...
export async function reconcileSandboxStoppedByExternalId(externalId: string, now = new Date()): Promise<boolean> {
  ...
  await db
    ...
    .where(eq(sessionSandboxes.sandboxId, row.sandboxId));
  await db.update(workspaceSessions).set({ status: 'stopped', updatedAt: now }).where(eq(workspaceSessions.sessionId, row.sessionId));
