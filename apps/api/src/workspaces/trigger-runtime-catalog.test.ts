...
describe('reconcileWorkspaceTriggerRuntime', () => {
  test('upserts every readable trigger and removes only proven stale rows', async () => {
    ...
    const store: TriggerRuntimeCatalogStore = {
      ...
      upsert: async (_workspaceId, spec) => {
        upserted.push({ slug: spec.slug, sessionId: spec.pinnedSessionId });
      },
      remove: async (_workspaceId, slug) => {
        removed.push(slug);
      },
    };

    const result = await reconcileWorkspaceTriggerRuntimeWithStore(
      'workspace-1',
  ...
  test('removes all rows when a readable manifest declares no triggers', async () => {
    ...
    await expect(reconcileWorkspaceTriggerRuntimeWithStore('workspace-1', [], store)).resolves.toEqual({
      upserted: 0,
      removed: 1,
    });
