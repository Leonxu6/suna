...
// 'workspace_credential' = provider-neutral git credential stored outside
...
type GitHubAuthSource = 'app_installation' | 'pat' | 'managed' | 'workspace_credential';
...
function requestToken(auth?: Pick<GitHubAuthContext, 'token'>) {
  ...
  throw new Error('GitHub auth is not configured for this request — a GitHub App installation token or a workspace credential is required');
}
...
export async function createInstallationToken(
   ...
   * within the installation's owner). Used for managed repos so a workspace's
