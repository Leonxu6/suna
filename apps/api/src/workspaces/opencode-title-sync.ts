...
//      busy workspace can hold dozens of active sandboxes; an unbounded `Promise.all`
...
function normalizeSnapshot(session: OpenCodeSessionLike): OpenCodeSessionSnapshot | null {
  ...
  return {
    ...
    workspace_id: stringOrNull(session.workspaceID ?? session.workspace_id ?? session.workspaceId),
