...
async function cleanup() {
  const db = testDb();
  await db.delete(workspaces).where(eq(workspaces.workspaceId, PROJECT_ID));
...
async function seed(nextFireAt: Date, metadata: Record<string, unknown> = {}) {
  ...
  await db.insert(workspaceTriggerRuntime).values({
    workspaceId: PROJECT_ID,
    slug: SLUG,
    triggerType: 'cron',
    enabled: true,
...
describeWithDb('durable exact trigger scheduler — real PostgreSQL', () => {
  ...
  test('concurrent scheduler pods claim one slot, coalesce backlog, and advance atomically', async () => {
    ...
    const executions = await db
      ...
      .where(
        and(
          eq(workspaceTriggerExecutions.workspaceId, PROJECT_ID),
          eq(workspaceTriggerExecutions.slug, SLUG),
        ),
      );
    ...
    const [runtime] = await db
      ...
      .where(
        and(eq(workspaceTriggerRuntime.workspaceId, PROJECT_ID), eq(workspaceTriggerRuntime.slug, SLUG)),
      );
  ...
  test('execution leases are CAS-safe, retryable, reclaimable, and never stay stuck on final crash', async () => {
    ...
    await db
      ...
      .where(eq(workspaceTriggerExecutions.executionId, first.executionId));
    ...
    await db
      ...
      .where(eq(workspaceTriggerExecutions.executionId, reclaimed.executionId));
  ...
  test('paused workspaces remain due but are not claimed or advanced', async () => {
    ...
    const executions = await db
      .select()
      .from(workspaceTriggerExecutions)
      .where(eq(workspaceTriggerExecutions.workspaceId, PROJECT_ID));
