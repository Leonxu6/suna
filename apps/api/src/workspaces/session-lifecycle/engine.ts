import { executorExecutions, workspaceSessions, workspaces, serviceAccounts } from '@kortix/db';
...
import { createWorkspaceSession } from '../lib/sessions';
import { openSession } from '../routes/shared';
import { resolveWorkspaceAutomationActor } from './actor';
...
export async function createSession(
  ...
  const backpressure =
    queuePolicy === 'never'
      ? null
      : await sessionBackpressureState(command.workspace.accountId, command.workspace.workspaceId);
  const shouldQueue =
  ...
  if (claimed.existing) {
    ...
    // OWN create_session for this account+workspace must never return the foreign
    ...
    const crossAccount = crossAccountIdempotencyResult(
      {
        ...
        workspaceId: claimed.row.workspaceId,
        commandType: claimed.row.commandType,
      },
      { accountId: command.workspace.accountId, workspaceId: command.workspace.workspaceId },
    );
    ...
    if (existingResult.sessionId) {
      const [row] = await db
        ...
        .where(eq(workspaceSessions.sessionId, existingResult.sessionId))
        .limit(1);
      if (row) {
    ...
    async () => {
      const [fresh] = await db
        .select({
          ...
          sandboxProvider: workspaceSessions.sandboxProvider,
          ...
          accountId: workspaceSessions.accountId,
          metadata: workspaceSessions.metadata,
        })
        .from(workspaceSessions)
        .where(eq(workspaceSessions.sessionId, command.sessionId))
        .limit(1);
...
export async function continueSession(
  ...
  const [session] = await db
    .select({
      accountId: workspaceSessions.accountId,
      workspaceId: workspaceSessions.workspaceId,
      status: workspaceSessions.status,
      metadata: workspaceSessions.metadata,
    })
    .from(workspaceSessions)
  ...
  const userId = command.userId ?? (await resolveWorkspaceAutomationActor(session.accountId));
  if (!userId) {
  ...
  const [workspace] = await db
    ...
    .limit(1);
  if (!workspace) return 'no-session';
  ...
  const loaded = { row: workspace, userId };
...
async function executeQueuedCreate(
  ...
  if (!workspace) {
    return {
      ...
      commandId: row.commandId,
      retryable: false,
      error: { status: 404, body: { error: 'Workspace not found' } },
    };
  }
  const userId = row.actorUserId ?? (await resolveWorkspaceAutomationActor(workspace.accountId));
  if (!userId) {
    return {
      status: 'failed',
      commandId: row.commandId,
      ...
      ? await db
          ...
          .where(
            and(
              ...
              eq(serviceAccounts.accountId, workspace.accountId),
            ),
          )
          .limit(1)
      : [];
    ...
    for (const action of input.actions) {
      if (action.type === 'bind_chat_thread') {
        await bindChatThread({
          ...
          providerWorkspaceId: action.providerWorkspaceId,
