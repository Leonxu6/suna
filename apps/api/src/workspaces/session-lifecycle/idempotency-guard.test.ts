...
const own = { accountId: 'acct-1', workspaceId: 'proj-1', commandType: 'create_session' };
const caller = { accountId: 'acct-1', workspaceId: 'proj-1' };

describe('crossAccountIdempotencyResult', () => {
  test("caller's own create_session (same account+workspace) → no conflict (normal dedupe)", () => {
  ...
  test('same account, DIFFERENT workspace → 409 (no cross-workspace session disclosure)', () => {
    const r = crossAccountIdempotencyResult({ ...own, workspaceId: 'proj-other' }, caller);
    expect(r?.error?.status).toBe(409);
    expect((r?.error?.body as { code?: string })?.code).toBe('IDEMPOTENCY_KEY_CONFLICT');
  });

  test('same account+workspace but a non-create (continue_session) command → 409', () => {
