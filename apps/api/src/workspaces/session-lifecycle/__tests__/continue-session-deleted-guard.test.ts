...
// workspace_sessions.status = 'stopped' — the SAME status a normal hibernate
      ...
      from: (table: unknown) => ({
        where: () => ({
          limit: async () => {
            ...
            if (table === workspaces) return [{ workspaceId: WORKSPACE_ID, accountId: ACCOUNT_ID }];
            return [];
          },
        }),
      }),
...
mock.module('../../lib/sessions', () => ({
  createWorkspaceSession: async () => {
    throw new Error('createWorkspaceSession: not expected in this test');
  },
}));
...
mock.module('../actor', () => ({
  resolveWorkspaceAutomationActor: async () => 'automation-user-1',
}));
...
describe('continueSession — deleted-mid-flight guard', () => {
  test('a stopped session with metadata.deletedAt is never revived', async () => {
    ...
    // No workspace_sessions revival update was attempted — the guard trips
  ...
  test('regression guard: a normal stopped session (no deletedAt) still takes the revival path', async () => {
    ...
    expect(updateCalls[0].table).toBe(workspaceSessions);
