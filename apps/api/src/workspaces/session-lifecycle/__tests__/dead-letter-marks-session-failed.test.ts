...
import { workspaceSessions, sessionLifecycleCommands } from '@kortix/db';
  ...
  db: {
    update: (table: unknown) => ({
      set: (updates: Record<string, unknown>) => ({
        // Awaitable (the workspaceSessions park) AND chainable to `.returning()`
...
describe('markCommandFailed — dead-letter is loud and parks the session', () => {
  test('continue_session exhausting retries ships an error and marks the session failed', async () => {
    ...
    const sessionUpdates = updateCalls.filter((u) => u.table === workspaceSessions);
  ...
  test('non-retryable failure dead-letters on the first attempt', async () => {
    ...
    expect(errorLogs).toHaveLength(1);
    expect(updateCalls.filter((u) => u.table === workspaceSessions)).toHaveLength(1);
  });
  ...
  test('a retryable failure below the attempt cap only requeues — no error, no park', async () => {
    ...
    expect(updateCalls.filter((u) => u.table === workspaceSessions)).toHaveLength(0);
  ...
  test('a create_session dead-letter ships the error but never touches a session row', async () => {
    ...
    await markCommandFailed('cmd-1', 'Workspace not found', { retryable: false, attempts: 1 });
