 ...
 * account's or a DIFFERENT workspace's row (a predictable channel key like
 * `telegram:<workspaceId>:<update_id>`, or two callers both using a low-entropy key
 ...
 * branch, prompt text, channel binding) or serialize a foreign workspace's session
 ...
 * for the SAME account AND workspace → fall through to the normal idempotent dedupe
...
export function crossAccountIdempotencyResult(
  existing: { accountId: string; workspaceId: string; commandType: string },
  caller: { accountId: string; workspaceId: string },
