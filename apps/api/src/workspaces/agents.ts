 ...
 * prompt/model/mode/tools/permission/skill-perms). Once a workspace declares
 ...
 *      CLI/API (workspace-scoped iam actions: deploy, open CRs, triggers, …).
 ...
 *       kortix_cli: ["workspace.trigger.create", "workspace.cr.open"]   # Kortix CLI/API powers
 *
 * Parser mirrors `workspaces/connectors.ts`: never throws on a bad entry, collects
 * them in `errors` so the UI can render them next to the good ones.
 */
import { createHash } from 'node:crypto';
import type { ParsedManifest } from './triggers';
import { WORKSPACE_ACTIONS, VALID_ACTIONS } from '../iam/actions';
...
/**
 * The non-binding agent sentinel. `workspace_sessions.agent_name` defaults to this
...
/**
 * The actions an agent's `kortix_cli` may grant — the workspace-scoped surface,
 * including the manager-tier workspace leaves (`workspace.delete`,
 * `workspace.members.manage`, `workspace.gateway.keys.manage`) — these are still
 * reachable via a workspace's `manager` role, so an agent can be granted them
 * too. CR actions live in WORKSPACE_ACTIONS. The channel.* resource actions
 ...
 * workspace-scoped (`account_tokens.workspace_id`); the IAM v2 engine
 * (`iam/engine-v2.ts` `computeTokenScope`) refuses ANY account-scope action
 * for a workspace-bound token BEFORE this grant is even loaded. This set is a
 ...
 */
export const GRANTABLE_KORTIX_CLI: ReadonlySet<string> = new Set(Object.values(WORKSPACE_ACTIONS));
...
/** `"all"` = every grantable action / every workspace connector (capped at the user). */
export type GrantSet = string[] | 'all';

export interface AgentSpec {
  /** Agent name — unique per workspace. Matches workspaceSessions.agentName + the `.md` filename. */
  name: string;
  /** e.g. `kortix.yaml#agents.<name>` (or the workspace's actual manifest filename) for UI / error reporting. */
  path: string;
   ...
   *  `'all'` = every secret in the workspace (default when the `env` key is
   ...
   * for "Default" — resolve workspace → account → platform (`auto`). A
...
export interface LoadedAgents {
   ...
   * sentinel resolve to a concrete declared agent's grant for a v2 workspace,
  ...
  raw.forEach((entry, index) => {
    ...
    if (seen.has(result.spec.name)) {
      errors.push({
        ...
        error: `Duplicate agent name "${result.spec.name}" — names must be unique within a workspace`,
      });
      return;
    }
...
/**
 * Read + parse a workspace's manifest, then extract `[[agents]]`. Never throws.
 *
 * A workspace with NO manifest committed yet (a blank managed-git workspace
 ...
 * workspace's agent-config PUT (write path) would succeed against the
 ...
 * even though the workspace "should" already resolve to the synthesized
...
export async function loadWorkspaceAgents(workspace: GitBackedWorkspace): Promise<LoadedAgents> {
  ...
  try {
    ...
    return {
      ...
      errors: [{
        name: '(manifest)',
        path: workspace.manifestPath || MANIFEST_FILENAME,
        error: (err as Error).message || 'Failed to read manifest',
      }],
      defaultAgent: null,
    };
  }
  if (!manifest) manifest = synthesizeBlankManifest({ manifestPath: workspace.manifestPath });
 ...
 *     Every existing workspace keeps working exactly as today.
 *   - Agent IS listed → its declared overlay (connectors + kortix_cli).
 *   - Workspace adopted `[[agents]]` but the agent is NOT listed → default-DENY
...
export async function resolveAgentGrant(
  ...
  return grantFromLoadedAgents(agentName, await loadWorkspaceAgents(workspace));
}
...
export function grantFromLoadedAgents(agentName: string, loaded: LoadedAgents): AgentGrant | null {
  // No [[agents]] section parsed and no errors → workspace hasn't adopted
  ...
  // still capped at the launching user's role; identical to a workspace that
  ...
  // pure v2 addition — a v1 workspace (defaultAgent always null) falls straight
  ...
  if (agentName === DEFAULT_AGENT_SENTINEL) {
    ...
    // A workspace locks down its default by setting `default_agent` to a
  ...
  // everything, including secrets/env (an unlisted agent receives no workspace
  // secrets).
...
/**
 * Is this workspace subject to MANDATORY DECLARED AGENTS enforcement?
 ...
 *   the platform-wide flag OR `workspace.metadata.require_declared_agents === true`.
 * New workspaces stamp the metadata flag at creation (see POST /workspaces/provision);
 * pre-existing workspaces stay non-subject (and therefore behave exactly as before)
 * until the platform flag flips or they're explicitly migrated.
 */
export function workspaceRequiresDeclaredAgents(
  workspaceMetadata: unknown,
  platformFlag: boolean,
): boolean {
  if (platformFlag) return true;
  if (!workspaceMetadata || typeof workspaceMetadata !== 'object') return false;
  return (workspaceMetadata as Record<string, unknown>).require_declared_agents === true;
}

/** A session/trigger was rejected outright because the workspace requires
...
/**
 * Resolve the per-agent grant when the workspace MAY be subject to mandatory
 ...
 * unchanged behavior), so a non-subject workspace is provably unaffected.
 *
 * When subject:
 ...
 *     ungoverned workspace, and never silently default-denied-to-running either.
 *   - the `default` sentinel must resolve to the workspace's declared
 *     `default_agent`; a workspace with no `default_agent` configured, or one
 ...
 *     `opts.workspaceDefaultAgent` (the DB `workspace.metadata.default_agent`
 ...
 *     fallback, so a v2 workspace that never separately configured the DB-side
...
export function resolveGovernedAgentGrant(
  agentName: string,
  loaded: LoadedAgents,
  opts: { subject: boolean; workspaceDefaultAgent: string | null },
  ...
  if (agentName === DEFAULT_AGENT_SENTINEL) {
    const declaredDefault = opts.workspaceDefaultAgent ?? loaded.defaultAgent;
    if (!declaredDefault) {
      return {
        ok: false,
        code: 'AGENT_NOT_DECLARED',
        error:
          'This workspace requires declared agents but has no default_agent configured — ' +
          'set one in the workspace settings or kortix.yaml before starting a session.',
      };
    }
    ...
    if (!spec) {
      return {
        ok: false,
        code: 'AGENT_NOT_DECLARED',
        error: `This workspace's default agent "${declaredDefault}" is not declared (or is disabled) in \`agents\` — the "default" sentinel cannot resolve.`,
      };
    }
    return {
      ok: true,
  ...
  if (!spec) {
    return {
      ok: false,
      code: 'AGENT_NOT_DECLARED',
      error: `Agent "${agentName}" is not declared in this workspace's \`agents\` manifest — this workspace requires every session/trigger to name a declared agent.`,
    };
  }
...
function parseAgentEntry(entry: unknown, index: number, filename: string = MANIFEST_FILENAME): ParseOk | ParseErr {
  ...
  return {
    ...
    spec: {
      ...
      kortixCli: canonicalizeKortixGrantSet(kortixParsed.value),
      env: envParsed.value,
      file,
      model,
      sandbox: null,
 ...
 * workspace-action set here (not just at `kortix validate` time), so a manifest
...
function canonicalizeKortixGrantSet(value: GrantSet): GrantSet {
  return value === 'all'
    ? value
    : value.map(canonicalizeKortixCliAction);
}
...
function validateKortixAction(action: string): string | null {
  const canonical = canonicalizeKortixCliAction(action);
  if (GRANTABLE_KORTIX_CLI.has(canonical)) return null;
  if (VALID_ACTIONS.has(action)) {
    return `\`kortix_cli\` action "${action}" is account-scoped and can never be granted to an agent — only workspace-scoped actions are allowed`;
  }
  return `\`kortix_cli\` has unknown action "${action}" — see the grantable list (workspace.*)`;
}
