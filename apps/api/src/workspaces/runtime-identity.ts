...
function sessionIsNotDeleted() {
  return sql`coalesce(${workspaceSessions.metadata}->>'deletedAt', '') = ''`;
}
  ...
  try {
    const claimed = await db.transaction(async (tx) => {
      const [liveSession] = await tx
        ...
        .where(and(eq(workspaceSessions.sessionId, row.sessionId), sessionIsNotDeleted()))
  ...
  try {
    const updated = await db.transaction(async (tx) => {
      const [liveSession] = await tx
        ...
        .where(and(eq(workspaceSessions.sessionId, claim.row.sessionId), sessionIsNotDeleted()))
        .returning({ sessionId: workspaceSessions.sessionId });
      if (!liveSession) return null;
