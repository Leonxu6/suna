...
function workspaceSecretKey(workspaceId: string): Buffer {
  ...
  const key = hkdfSync(
    ...
    Buffer.from('kortix-workspace-secret-v1', 'utf8'),
    32,
  );
...
/**
 * Upsert the SHARED (owner_user_id IS NULL) row for a workspace secret to a new
...
export async function listWorkspaceSecrets(workspaceId: string): Promise<Record<string, string>> {
  const rows = await db
    .select({
      identifier: workspaceSecrets.identifier,
    ...
    .where(and(eq(workspaceSecrets.workspaceId, workspaceId), isNull(workspaceSecrets.ownerUserId)))
...
/**
 * One workspace secret resolved for a specific launching user: the shared
 ...
 * per-user provider login — see workspace_secrets.ownerUserId doc comment).
 */
...
/**
 * Every runtime-scope workspace secret, resolved AS a specific user (their own
...
export async function listResolvedWorkspaceSecrets(
  ...
  const rows = await db
    .select({
      ...
      active: workspaceSecrets.active,
    })
    .from(workspaceSecrets)
    .where(and(
      eq(workspaceSecrets.workspaceId, workspaceId),
      eq(workspaceSecrets.scope, 'runtime'),
      userId ? or(isNull(workspaceSecrets.ownerUserId), eq(workspaceSecrets.ownerUserId, userId)) : isNull(workspaceSecrets.ownerUserId),
    ));
  ...
  for (const [identifier, slot] of byIdentifier) {
    ...
    if (!chosen) continue;
    out.push({ identifier, key: chosen.name, value: decryptWorkspaceSecret(workspaceId, chosen.valueEnc) });
  }
  return out;
}
 ...
 *     set). `[]` therefore means inject ZERO workspace secrets.
...
export async function getWorkspaceSecretValue(
  ...
  const rows = await db
    .select({ identifier: workspaceSecrets.identifier, valueEnc: workspaceSecrets.valueEnc, updatedAt: workspaceSecrets.updatedAt })
    .from(workspaceSecrets)
    .where(and(
      eq(workspaceSecrets.workspaceId, workspaceId),
      eq(workspaceSecrets.name, normalizedName),
