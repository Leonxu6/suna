/**
 * `connectors` list parsing for the workspace manifest (`kortix.yaml`; a legacy
 * v1 workspace may instead declare `[[connectors]]` in `kortix.toml`).
 *
 ...
 * (provider, endpoint/spec, auth method + which workspace-secret to use) and,
 ...
 * Connectors are workspace-wide visible — the only access gate is which AGENTS
 ...
 *       auth:
 *         type: bearer
 *         secret: STRIPE_API_KEY     # workspace-secret NAME; value set in dashboard
 ...
 *
 * Parser mirrors `workspaces/agents.ts` + `workspaces/triggers.ts`: never throws on
...
interface ConnectorAuthSpec {
  ...
  prefix: string | null;
  /** Name of the workspace secret holding the credential. Value set in the dashboard. */
  secret: string | null;
}
...
export interface ConnectorSpec {
  /** URL-safe slug — unique per workspace. Also the tool namespace. */
  slug: string;
  /** e.g. `kortix.yaml#connectors.<slug>` (or the workspace's actual manifest filename) for UI / error reporting. */
  path: string;
  ...
  raw.forEach((entry, index) => {
    ...
    if (seenSlugs.has(result.spec.slug)) {
      errors.push({
        ...
        error: `Duplicate connector slug "${result.spec.slug}" — slugs must be unique within a workspace`,
      });
      return;
    }
...
function parseAuth(
  ...
  // not as a named workspace secret. If present it's validated for back-compat.
  const secret = typeof row.secret === 'string' && row.secret.trim() ? row.secret.trim() : null;
  if (secret && !isValidSecretName(secret)) {
    return err(slug, `[connectors.auth].secret "${secret}" must look like a workspace-secret name (^[A-Z_][A-Z0-9_]{0,63}$)`, filename);
  }
