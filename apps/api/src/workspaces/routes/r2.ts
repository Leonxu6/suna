import { ACCOUNT_ACTIONS, WORKSPACE_ACTIONS, assertAuthorized } from '../../iam';
import { auth, errors, json } from '../../openapi';
import { DEFAULT_SANDBOX_SLUG, deleteSandboxImage, kickPreBuild, kickWorkspaceTemplatePrebuilds, kickRoutedPreBuild, listSandboxTemplates, listSnapshotBuilds, reconcileStaleBuilds, templateBuildProviders } from '../../snapshots/builder';
...
import { buildWorkspaceSeedFilesFromItem } from '../seed-files';
...
import { AnyObject, SandboxTemplateSchema, SnapshotSchema, workspacesApp } from '../lib/app';
import { GitHubInstallationRequiredError, createGitHubInstallationInstallUrl, getWorkspaceGitConnection, loadGitWorkspace, resolveGitHubImport, resolveGitHubImportWithPat, resolveGitHubRepoAuth } from '../lib/git';
import { registerGitHubLinkedWorkspace, registerPatLinkedWorkspace } from '../lib/workspace-registration';
import { PAT_MANAGED_GIT_INSTALLATION_ID, deriveWorkspaceName, isRepoNameTakenError, normalizeString, readBody, requestAuditContext, serializeBuildSummary, serializeWorkspace, serializeWorkspaceGitConnection, serializeTemplate } from '../lib/serializers';
import { createWorkspaceSession, sendSessionCreateError } from '../lib/sessions';
import { resolveManifestValidateFormat } from '../lib/manifest-format';
import { resolveConfiguredWorkspaceProviderPin } from '../../snapshots/provider-coverage';
...
function templateProviderObservation(metadata: unknown) {
  const selectedProvider = resolveConfiguredWorkspaceProviderPin(
...
workspacesApp.openapi(
  ...
  // GET github/installations). Everything downstream (`resolveWorkspaceGitAuth`
  // → `workspace_credential`) already consumes the stored PAT either way.
  ...
  if (patToken) {
    ...
    kickWorkspaceTemplatePrebuilds(
      { workspaceId: row.workspaceId, repoUrl: row.repoUrl, defaultBranch: row.defaultBranch, manifestPath: row.manifestPath, gitAuthToken: patToken },
      { accountId: scope.accountId, source: 'workspace-create' },
    );
    return c.json({
      workspace: serializeWorkspace(row, { workspaceRole: 'manager', effectiveRole: 'manager' }),
      git_connection: serializeWorkspaceGitConnection(await getWorkspaceGitConnection(row.workspaceId)),
    }, 201);
  }
...
// then registers it as a Kortix workspace.
...
workspacesApp.openapi(
  ...
  if (!githubAuth.installation || !githubAuth.auth) {
    return c.json({
      error: 'Install the Kortix GitHub App before creating GitHub-backed workspaces',
  ...
  const createRepoQuota = await enforceWorkspaceQuota(c, scope.accountId);
  ...
  const workspaceName = normalizeString(body.workspace_name ?? body.workspaceName) ?? deriveWorkspaceName(repo.full_name);
  const defaultBranch = repo.default_branch || 'main';

  // Commit the Kortix starter into the fresh repo so users land with a
  // working workspace shape on first session boot. GitHub's Contents API
  // updates the branch tip on every write, so these must be sequential.
  // A partial starter is not a usable workspace.
  ...
  for (const file of starter) {
    try {
      ...
      console.warn(`[workspaces/create-repo] Failed to scaffold ${file.path} into ${repo.full_name}:`, message);
  ...
  return c.json(serializeWorkspace(row, { workspaceRole: 'editor', effectiveRole: 'editor' }), 201);
},
);
...
// order (workspace manifestPath > body `format` > `toml` default).
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/manifest/validate',
    tags: ['workspaces'],
    summary: 'POST /:workspaceId/manifest/validate',
    ...auth,
      request: {
...
// The append-only `workspace_snapshot_builds` log feeds the UI but is never
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/sandboxes',
    tags: ['sandboxes'],
    summary: 'GET /:workspaceId/sandboxes',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string() }),
      },
  ...
  try {
    const templates = await listSandboxTemplates(workspace, observation.listOptions);
    return c.json({
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/snapshots',
    tags: ['sandboxes'],
    summary: 'GET /:workspaceId/snapshots',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string() }),
      },
  ...
  try {
    templates = await listSandboxTemplates(workspace, observation.listOptions);
  } catch (err) {
  ...
  await reconcileStaleBuilds({ workspaceId }).catch(() => {});
  const builds = await listSnapshotBuilds(workspaceId, { limit: 25 }).catch(() => []);
  return c.json({
...
async function buildSandboxHealth(
  loaded: NonNullable<Awaited<ReturnType<typeof loadWorkspaceForUser>>>,
  workspaceId: string,
  ...
  const builds = await listSnapshotBuilds(workspaceId, { limit: 10 }).catch(() => []);
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/sandbox-health',
    tags: ['sandboxes'],
    summary: 'GET /:workspaceId/sandbox-health',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string() }),
      },
  ...
  try {
    payload = await withTimeout(
      buildSandboxHealth(loaded, workspaceId),
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/snapshots/rebuild',
    tags: ['sandboxes'],
    summary: 'POST /:workspaceId/snapshots/rebuild',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string() }),
        body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  try {
    const attempts = await runProviderActions(
      providers,
      async (provider) => {
        const deleted = await deleteSandboxImage(workspace, { slug, provider });
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/snapshots/fix-with-agent',
    tags: ['sandboxes'],
    summary: 'POST /:workspaceId/snapshots/fix-with-agent',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string() }),
      },
  ...
  const builds = await listSnapshotBuilds(workspaceId, { limit: 50 }).catch(() => []);
  const templateBuilds = sessionTemplateBuilds(builds);
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/sandbox-templates',
    tags: ['sandboxes'],
    summary: 'GET /:workspaceId/sandbox-templates',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string() }),
      },
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/sandbox-templates',
    tags: ['sandboxes'],
    summary: 'POST /:workspaceId/sandbox-templates',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string() }),
        body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    if (message.toLowerCase().includes('duplicate') || message.includes('idx_sandbox_templates_workspace_slug')) {
...
// PATCH /v1/workspaces/:workspaceId/sandbox-templates/:templateId — update fields.

workspacesApp.openapi(
  createRoute({
    method: 'patch',
    path: '/{workspaceId}/sandbox-templates/{templateId}',
    tags: ['sandboxes'],
    summary: 'PATCH /:workspaceId/sandbox-templates/:templateId',
    ...auth,
      request: {
  ...
  // templateId alone, so without this a manager of their own workspace could
  // mutate any other tenant's template by id (the post-write workspaceId check
  ...
  if (existing.workspaceId !== workspaceId) return c.json({ error: 'Not found' }, 404);

  try {
    const updated = await updateTemplate(templateId, patch, workspaceId);
    if (!updated) return c.json({ error: 'Not found' }, 404);
    if (updated.workspaceId !== workspaceId) return c.json({ error: 'Not found' }, 404);
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/sandbox-templates/{templateId}',
    tags: ['sandboxes'],
    summary: 'DELETE /:workspaceId/sandbox-templates/:templateId',
    ...auth,
      request: {
        params: z.object({ workspaceId: z.string(), templateId: z.string() }),
      },
  ...
  if (row.workspaceId !== workspaceId) return c.json({ error: 'Not found' }, 404);
  ...
  try {
    ...
    await Promise.all(
      templateBuildProviders().map((provider) =>
        deleteSandboxImage(workspace, { slug: row.slug, provider }).catch(() => null),
      ),
    );
