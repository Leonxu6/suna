 ...
 *  item into an EXISTING workspace. This is judgment-heavy (does the incoming
 *  agent persona collide with one that already exists? does the target workspace
 *  even want a new default agent?) — so an agent reads both sides and opens
 *  a change request rather than a blind file overwrite. */
export function buildRegistryWorkspaceInstallPrompt(
  ...
  // `files[].content` is always populated here. If an EXTERNAL workspace item
  ...
  if ((item.files ?? []).length > 0 && ownFiles.length === 0) {
    throw new Error(
      `Workspace template "${item.name}" has no resolvable file content (likely an external registry item) — install-session merge only supports base workspace items today.`,
    );
  }
  const deps = item.registryDependencies ?? [];

  const lines: string[] = [
    `Integrate the "${item.title ?? item.name}" workspace template into THIS workspace — without breaking anything already here.`,
    '',
    item.description ?? '',
    '',
    "This workspace's current kortix.yaml:",
    '```yaml',
    targetManifestRaw ?? '(no manifest found)',
    '```',
    '',
    'The template contributes these files. Its own kortix.yaml is a reference for what agent it expects to exist — do NOT overwrite this workspace\'s kortix.yaml with it verbatim.',
  ];
  ...
  lines.push(
    '',
    'Steps:',
    "1. Read this workspace's current kortix.yaml and .kortix/opencode/agents/ to see what already exists.",
    ...
    "3. Merge the template's kortix.yaml `agents:` entry for that agent into this workspace's kortix.yaml. Leave default_agent and every other existing agent untouched unless the user asks otherwise.",
...
export function buildTemplateInstallPrompt(entry: TemplateCatalogEntry, id: string): string {
  ...
  return [
    `Install the "${item.title ?? item.name}" automation into THIS workspace — as a short guided conversation, not a form.`,
    '',
    item.description ?? '',
    '',
