...
import { changeRequests, workspaceSessions, sessionSandboxes } from '@kortix/db';
import { and, desc, eq } from 'drizzle-orm';
import { loadWorkspaceForUser, loadVisibleSession, assertWorkspaceCapability } from '../lib/access';
import { assertAgentScope } from '../../iam/agent-scope';
import { WORKSPACE_ACTIONS } from '../../iam';
import { AnyObject, ChangeRequestSchema, SessionStartResultSchema, workspacesApp } from '../lib/app';
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/sessions/{sessionId}/start',
    tags: ['sessions'],
    summary: 'POST /:workspaceId/sessions/:sessionId/start',
    ...auth,
    request: {
  ...
  async (c) => {
    ...
    // Floor 'session' (= workspace.session.start) so the human gate matches
    // restart/stop and a custom role that withholds session.start is denied here
    // (was 'read', which let any workspace-reader start sessions).
    ...
    // token must hold workspace.session.start (no-op for human/PAT tokens).
    ...
    const result = await startSession({ source: 'ui', loaded, visible, workspaceId, sessionId, waitMs });
    return c.json(
      {
        ...result.start,
        runtime_transport: resolveWorkspaceRuntimeTransport(loaded.row.metadata),
      },
      200,
    );
  },
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/sessions/{sessionId}/restart',
    tags: ['sessions'],
    summary: 'POST /:workspaceId/sessions/:sessionId/restart',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), sessionId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    // hold workspace.session.start (no-op for human/PAT tokens).
    assertAgentScope(c, WORKSPACE_ACTIONS.WORKSPACE_SESSION_START);
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/sessions/{sessionId}/stop',
    tags: ['sessions'],
    summary: 'POST /:workspaceId/sessions/:sessionId/stop',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), sessionId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    assertAgentScope(c, WORKSPACE_ACTIONS.WORKSPACE_SESSION_START);
    // Human gate: stopping has its own leaf (workspace.session.stop), distinct from
    ...
    await assertWorkspaceCapability(c, loaded.userId, loaded.row.accountId, workspaceId, WORKSPACE_ACTIONS.WORKSPACE_SESSION_STOP);
...
// ─── Change Requests ────────────────────────────────────────────────────────
// Kortix-native PR layer. The CR is metadata stored alongside the workspace;
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/change-requests',
    tags: ['change-requests'],
    summary: 'GET /:workspaceId/change-requests',
    ...auth,
    request: {
  ...
  async (c: any) => {
    ...
    const whereClauses = [eq(changeRequests.workspaceId, workspaceId)];
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/change-requests',
    tags: ['change-requests'],
    summary: 'POST /:workspaceId/change-requests',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
  ...
  async (c: any) => {
    ...
    // role omits workspace.gitops.push to take Git-Ops away from a department.
    ...
    // Default-deny — a scoped agent must be granted workspace.cr.open.
    assertAgentScope(c, 'workspace.cr.open');
    ...
    if (originSessionId) {
      const [sessionRow] = await db
        ...
        .where(
          and(
            eq(workspaceSessions.sessionId, originSessionId),
    ...
    // Atomically allocate the next per-workspace number and insert. Retry once on
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const number = await getNextCrNumber(workspaceId);
      try {
        const [row] = await db
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/sessions/{sessionId}/commit-push',
    tags: ['sessions'],
    summary: 'POST /:workspaceId/sessions/:sessionId/commit-push',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), sessionId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/change-requests/{crId}',
    tags: ['change-requests'],
    summary: 'GET /:workspaceId/change-requests/:crId',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), crId: z.string() }),
    },
    responses: {
  ...
  async (c: any) => {
    ...
    let cr = await getCrById(crId, workspaceId);
    if (!cr) return c.json({ error: 'Change request not found' }, 404);

    await refreshCrTips({
      cr,
      workspace: await withWorkspaceGitAuth(loaded.row),
    });
    cr = (await getCrById(crId, workspaceId))!;
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/change-requests/{crId}',
    tags: ['change-requests'],
    summary: 'PATCH /:workspaceId/change-requests/:crId',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), crId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/change-requests/{crId}/request-changes',
    tags: ['change-requests'],
    summary: 'POST /:workspaceId/change-requests/:crId/request-changes',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), crId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    // gate it on workspace.review.act (the same leaf as /review/items/{id}/act),
    ...
    const row = await recordRequestedChange(crId, workspaceId, {
      text: feedback,
      by: loaded.userId,
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/change-requests/{crId}/diff',
    tags: ['change-requests'],
    summary: 'GET /:workspaceId/change-requests/:crId/diff',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), crId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    try {
      ...
      const diff = useSnapshot
        ...
        : await getBranchDiff(workspaceForGit, cr.baseRef, cr.headRef);
      return c.json({
        cr_id: cr.crId,
        base_ref: cr.baseRef,
        head_ref: cr.headRef,
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/change-requests/{crId}/merge-preview',
    tags: ['change-requests'],
    summary: 'GET /:workspaceId/change-requests/:crId/merge-preview',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), crId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    try {
      const preview = await previewMerge(
        await withWorkspaceGitAuth(loaded.row),
        cr.baseRef,
        cr.headRef,
      );
