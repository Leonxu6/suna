...
import { setWorkspaceBotName } from '../../channels/voice-identity';
...
import {
  bindChatThread,
  resolveProviderWorkspaceIdForChannel,
...
import { workspacePickerCatalog } from '../../llm-gateway/models/picker-catalog';
...
import { listWorkspaceSecretsSnapshot } from '../secrets';
import { reconcileWorkspaceTriggerRuntime } from '../trigger-runtime-catalog';
import { type ParsedManifest, extractTriggers, loadWorkspaceTriggers } from '../triggers';
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/connector-profiles',
    tags: ['connectors'],
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/connector-profiles/me',
    tags: ['connectors'],
...
for (const operation of ['credential', 'revoke', 'activate', 'default'] as const) {
  workspacesApp.openapi(
    createRoute({
      method: 'put',
      path: `/{workspaceId}/connector-profiles/{profileId}/${operation}`,
      tags: ['connectors'],
...
workspacesApp.openapi(
  createRoute({
    ...
    tags: ['triggers'],
    summary: 'GET /:workspaceId/triggers',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    // Leaf-gate the read (a custom role can omit workspace.trigger.read) — and, via
    ...
    return c.json(await loadTriggersForResponse(workspaceId, loaded.row));
  },
);
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/triggers',
    tags: ['triggers'],
    summary: 'POST /:workspaceId/triggers',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
  ...
  async (c: any) => {
    ...
    // A `pinned` trigger may only target a session that belongs to THIS workspace —
    // never a nonexistent or another workspace's session.
    if (draft.sessionMode === 'pinned' && draft.pinnedSessionId) {
      const [pinned] = await db
        ...
        .where(
          and(
            eq(workspaceSessions.sessionId, draft.pinnedSessionId),
      ...
      if (!pinned) {
        return c.json(
          { error: `Pinned session "${draft.pinnedSessionId}" was not found in this workspace.` },
          400,
        );
      }
    }
    ...
    await reconcileWorkspaceTriggerRuntime(workspaceId, extractTriggers(next).specs);

    return c.json(await loadTriggersForResponse(workspaceId, loaded.row), 201);
  },
);

// PATCH /:workspaceId/triggers/activation — server-side, per-workspace trigger
// kill-switch. Body { paused: boolean }. When paused, the platform won't
// auto-run any of this workspace's triggers (the cron sweep skips it, inbound
...
workspacesApp.openapi(
  createRoute({
    ...
    tags: ['triggers'],
    summary: "Pause or resume all of a workspace's triggers server-side",
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    return c.json(await loadTriggersForResponse(workspaceId, row));
  },
);
...
workspacesApp.openapi(
  createRoute({
    ...
    tags: ['triggers'],
    summary: 'PATCH /:workspaceId/triggers/:slug',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), slug: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
    },
    responses: {
      200: json(z.any(), 'OK'),
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/triggers/{slug}',
    tags: ['triggers'],
    summary: 'DELETE /:workspaceId/triggers/:slug',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), slug: z.string() }),
    },
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/slack/installation',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/slack/installation',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    const install = await loadSlackInstall(workspaceId);
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/slack/mode',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/slack/mode',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    try {
      const installUrl = buildSlackInstallUrl(workspaceId, loaded.userId);
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/channels/slack/connect',
    tags: ['channels'],
    summary: 'POST /:workspaceId/channels/slack/connect',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/channels/slack/installation',
    tags: ['channels'],
    summary: 'DELETE /:workspaceId/channels/slack/installation',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    await deleteSlackInstall(workspaceId);
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/teams/installation',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/teams/installation',
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    const install = await loadTeamsInstall(workspaceId);
    return c.json(install ?? null);
  },
);
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/teams/mode',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/teams/mode',
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
    responses: { 200: json(z.any(), 'OK'), ...errors(404) },
  }),
  async (c: any) => {
    ...
    const byoAppId = await loadTeamsAppIdForWorkspace(workspaceId);
    const install = await loadTeamsInstall(workspaceId).catch(() => null);
    return c.json({
      ...teamsMode(baseUrl, { workspaceId, byoAppId }),
      orgConsentUrl: byoAppId ? null : teamsOrgConsentUrl({ workspaceId, baseUrl }),
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/teams/manifest',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/teams/manifest',
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
  ...
  async (c: any) => {
    ...
    const byoAppId = await loadTeamsAppIdForWorkspace(workspaceId);
    const baseUrl = resolveBaseUrl(new URL(c.req.url), teamsPublicBaseUrl());
    const mode = teamsMode(baseUrl, { workspaceId, byoAppId });
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/channels/teams/connect',
    tags: ['channels'],
    summary: 'POST /:workspaceId/channels/teams/connect',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/channels/teams/installation',
    tags: ['channels'],
    summary: 'DELETE /:workspaceId/channels/teams/installation',
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
    responses: { 200: json(z.any(), 'OK'), ...errors(404) },
  }),
  async (c: any) => {
    ...
    await deleteTeamsInstall(workspaceId);
    void reconcileChannelConnectors(workspaceId);
    return c.json({ status: 'disconnected' });
  },
);
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/teams/file',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/teams/file (download proxy)',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    const result = await downloadTeamsFile(workspaceId, c.req.query('url') ?? '');
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/channels/teams/file/upload',
    tags: ['channels'],
    summary: 'POST /:workspaceId/channels/teams/file/upload (consent-card upload)',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    // Posting a consent card drives the workspace bot to SEND into the customer's
    ...
    if (!teamsChannelEnabled()) return c.json({ error: 'Not found' }, 404);
    const body = await readBody(c);
    const result = await initiateTeamsUpload(workspaceId, {
...
// ─── Email install — AgentMail-backed inbox per workspace ─────────────────────
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/email/installation',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/email/installation',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    const install = await loadAgentMailInstall(workspaceId, connectorSlug);
    if (!install) return c.json(null);
    return c.json({
      ...install,
      profile_id: await loadEmailInstallProfileId(workspaceId, install.inboxId),
    });
  },
);
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/email/mode',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/email/mode',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/channels/email/connect',
    tags: ['channels'],
    summary: 'POST /:workspaceId/channels/email/connect',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    // workspace.write. Built-in editor/manager hold the leaf.
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    await assertWorkspaceCapability(
      c,
    ...
    const clientId = `kortix-workspace-${workspaceId}`;
    ...
    try {
      const webhook = await createAgentMailWebhook({
        ...
        clientId: `kortix-email-${workspaceId}`,
      });
    ...
    return c.json({
      ...summary,
      profile_id: await loadEmailInstallProfileId(workspaceId, summary.inboxId),
    });
  },
);
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/channels/email/installation',
    tags: ['channels'],
    summary: 'PATCH /:workspaceId/channels/email/installation',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    const workspaceId = c.req.param('workspaceId');
    // Floor 'read'; workspace.connector.write is the real gate (see /email/connect).
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    await assertWorkspaceCapability(
      c,
    ...
    const summary = await updateAgentMailSenderPolicy(workspaceId, connectorSlug, senderPolicy);
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/channels/email/installation',
    tags: ['channels'],
    summary: 'DELETE /:workspaceId/channels/email/installation',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    const connectorSlug =
      c.req.query('connector_slug') || c.req.query('profile_slug') || 'kortix_email';
    await deleteAgentMailInstall(workspaceId, connectorSlug);
    await reconcileChannelConnectors(workspaceId, {
      platform: 'email',
      slug: connectorSlug,
    });
...
// POST /v1/workspaces/:workspaceId/execution-lease
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/execution-lease',
    tags: ['workspaces'],
    summary: 'POST /:workspaceId/execution-lease',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: {
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/turn-stream',
    tags: ['workspaces'],
    summary: 'POST /:workspaceId/turn-stream',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    // Two valid callers: a workspace/session-scoped PAT (dashboard, operator, or
    // in-sandbox agent CLI) and the session sandbox's own service credential.
    // Each is scoped back to this workspaceId before a turn event is accepted.
    ...
    if (authType === 'apiKey' && apiKeyType === 'sandbox') {
      ...
      if (legacyExecutionLeaseKind) {
        const target = { sandboxId, sessionId, workspaceId, accountId };
    ...
    // session_id is caller-supplied — scope it back to :workspaceId so a caller
    // authed for their own workspace can't relay turn events into another
    // tenant's live session (IDOR).
    const [turnStreamSession] = await db
      .select({ sessionId: workspaceSessions.sessionId })
      .from(workspaceSessions)
      .where(
        and(eq(workspaceSessions.sessionId, sessionId), eq(workspaceSessions.workspaceId, workspaceId)),
      )
      .limit(1);
    ...
    // is already scoped to this workspace (checked above); the daemon only ever
    ...
    if (body.kind === 'opencode_session') {
      ...
      const updated = await db
        ...
        .returning({ sessionId: workspaceSessions.sessionId });
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/channels/slack/file',
    tags: ['channels'],
    summary: 'GET /:workspaceId/channels/slack/file (download proxy)',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      query: z.object({ url: z.string() }),
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    const result = await downloadSlackFile(workspaceId, c.req.query('url') ?? '');
    if (!result.ok) return c.json({ error: result.error }, result.status as 400 | 404);
    c.header('Content-Type', result.contentType);
    return c.body(result.body);
  },
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/channels/slack/file/upload',
    tags: ['channels'],
    summary: 'POST /:workspaceId/channels/slack/file/upload (upload proxy)',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    // This is a SEND primitive (posts to Slack with the workspace's bot token), not
    // a read — a bare workspace-read gate let ANY workspace-read caller post
    ...
    // through assertWorkspaceCapability's workspace-scoped fold (nothing asserts it
    ...
    const body = await readBody(c);
    const result = await uploadSlackFile(workspaceId, {
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/channels/slack/bind-thread',
    tags: ['channels'],
    summary: 'POST /:workspaceId/channels/slack/bind-thread',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    // back to this workspace) or a workspace/session-scoped user PAT.
    const authType = (c as any).get('authType') as string | undefined;
    if (authType === 'apiKey' && (c as any).get('apiKeyType') === 'sandbox') {
      ...
      // it, is denied. The sandbox-token branch above is already workspace-scoped.
      await assertWorkspaceCapability(
        c,
        loaded.userId,
        loaded.row.accountId,
    ...
    // the session must belong to this workspace
    const [sess] = await db
      .select({ sessionId: workspaceSessions.sessionId })
      .from(workspaceSessions)
      .where(
    ...
    if (!providerWorkspaceId) {
      return c.json(
        {
          error:
            'could not resolve Slack workspace for channel (is the channel bound to this workspace?)',
        },
        400,
      );
    }
    await bindChatThread({ workspaceId, providerWorkspaceId, threadId: threadTs, sessionId });
...
workspacesApp.openapi(
  createRoute({
    method: 'put',
    path: '/{workspaceId}/channels/meet/name',
    tags: ['channels'],
    summary: 'PUT /:workspaceId/channels/meet/name',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    const workspaceId = c.req.param('workspaceId');
    // Floor 'read'; workspace.customize.write is the real gate (setting the bot
    // name is workspace customization). Built-in editor/manager hold the leaf.
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    await assertWorkspaceCapability(
      c,
    ...
    const saved = await setWorkspaceBotName(workspaceId, name);
...
// full picker into opencode config. The web UI also reads it with normal workspace
// auth so the model picker is available before the sandbox runtime answers.
// The catalog is non-secret; access is still scoped to this workspace.
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/llm-catalog',
    tags: ['workspaces'],
    summary: 'GET /:workspaceId/llm-catalog',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
    },
  ...
  async (c: any) => {
    ...
    if (authType === 'apiKey' && apiKeyType === 'sandbox' && accountId && sandboxId) {
      ...
      const [workspace] = await db
        ...
        .from(workspaces)
        .where(and(eq(workspaces.workspaceId, workspaceId), eq(workspaces.accountId, accountId)))
        .limit(1);
      ...
      const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
      if (!loaded) return c.json({ error: 'Not found' }, 404);
      workspaceMetadata = loaded.row.metadata;
      ownerAccountId = loaded.row.accountId as string | undefined;
    }
    if (!workspaceLlmGatewayEnabled(workspaceMetadata)) {
      return c.json(
        { error: 'LLM gateway is disabled for this workspace', code: 'llm_gateway_disabled' },
        404,
      );
    }
    ...
    const freeManagedOnly =
      config.KORTIX_BILLING_INTERNAL_ENABLED && ownerAccountId
        ...
        : false;
    const models = gatewayModelCatalog(workspaceId, { freeManagedOnly });
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/model-picker',
    tags: ['workspaces'],
    summary: 'GET /:workspaceId/model-picker',
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
    responses: {
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    if (!workspaceLlmGatewayEnabled(loaded.row.metadata)) {
      return c.json(
        { error: 'LLM gateway is disabled for this workspace', code: 'llm_gateway_disabled' },
        404,
      );
    ...
    const [secrets, defaults, routing] = await Promise.all([
      listWorkspaceSecretsSnapshot(workspaceId).catch(() => ({ names: [] as string[] })),
      getAccountModelDefaults(accountId, workspaceId),
      getWorkspaceRoutingPolicy(workspaceId),
    ]);
    const requiredModels = [
      defaults.workspaces[workspaceId],
      defaults.account,
    ...
    const models = workspacePickerCatalog(
      gatewayModelCatalog(workspaceId, { freeManagedOnly }),
...
// has, no reshaping.
//
// Deliberately NOT gated by workspaceLlmGatewayEnabled unlike /llm-catalog and
...
// workspace, including native (non-gateway) ones — that's the majority of
// workspaces and exactly the surface the connect modal serves. Non-secret
// model metadata; scoped to workspace auth only for consistency with the
// other catalog routes, not because it needs to be secret.
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/llm-catalog/providers',
    tags: ['workspaces'],
    summary: 'GET /:workspaceId/llm-catalog/providers',
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
    responses: {
      200: { description: 'OK', content: { 'application/json': { schema: z.any() } } },
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/model-defaults',
    tags: ['workspaces'],
    summary: 'GET /:workspaceId/model-defaults',
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
    responses: {
      200: {
  ...
  async (c: any) => {
    ...
    return c.json({
      ...
      workspaceDefault: defaults.workspaces[workspaceId] ?? null,
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/model-defaults',
    tags: ['workspaces'],
    summary: 'PUT /:workspaceId/model-defaults',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
  ...
  async (c: any) => {
    const workspaceId = c.req.param('workspaceId');
    // Floor 'read'; workspace.customize.write is the real gate.
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    await assertWorkspaceCapability(
      c,
...
// DELETE /v1/workspaces/:workspaceId/model-defaults?scope=account|agent&agentName=
workspacesApp.openapi(
  createRoute({
    method: 'delete',
    path: '/{workspaceId}/model-defaults',
    tags: ['workspaces'],
    summary: 'DELETE /:workspaceId/model-defaults',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      query: z.object({
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/turn-question',
    tags: ['workspaces'],
    summary: 'POST /:workspaceId/turn-question',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    // session_id is caller-supplied — scope it back to :workspaceId so a caller
    // authed for their own workspace can't relay a question into another
    // tenant's live session (IDOR).
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/triggers/{slug}/fire',
    tags: ['triggers'],
    summary: 'POST /:workspaceId/triggers/:slug/fire',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), slug: z.string() }),
    },
  ...
  async (c: any) => {
    const workspaceId = c.req.param('workspaceId');
    const slug = c.req.param('slug');
    // Floor 'read' (membership); workspace.trigger.fire is the real gate. The floor
    // was 'manage' (= workspace.write) — which the floor `member` role LACKS even
    ...
    const { specs } = await loadWorkspaceTriggers(await withWorkspaceGitAuth(loaded.row));
    ...
    if (result.status === 'queued') {
      await markGitTriggerFired(workspaceId, slug, now);
      return c.json(
        {
          status: 'queued',
