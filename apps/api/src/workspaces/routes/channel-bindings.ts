...
// workspace agent / model / join-policy is the in-Slack `/kortix agent|model|policy`
...
const CONVERSATION_POLICIES = ["owner_approval", "owner_only", "workspace_open"] as const;
...
workspacesApp.openapi(
  ...
  async (c: any) => {
    ...
    // Floor 'read'; workspace.connector.write below is the real gate (was 'manage'
    // → workspace.write, which over-gated a custom connector.write-only role).
    const loaded = await loadWorkspaceForUser(c, workspaceId, "read");
    if (!loaded) return c.json({ error: "Not found" }, 404);
    ...
    if (body.agentName !== undefined) {
      ...
      if (body.agentName !== null) {
        ...
        if (trimmed.toLowerCase() !== "default") {
          ...
          // has adopted `[[agents]]` — a legacy (undeclared) workspace has no fixed
