// ─── Review Center ──────────────────────────────────────────────────────────
// Per-workspace human-in-the-loop inbox. Agents submit outputs / decisions /
...
import { assertWorkspaceCapability, loadWorkspaceForUser } from '../lib/access';
import { AnyObject, workspacesApp } from '../lib/app';
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/review/items',
    tags: ['review'],
    summary: 'GET /:workspaceId/review/items',
    ...auth,
    request: {
  ...
  async (c: any) => {
    ...
    await assertWorkspaceCapability(c, loaded.userId, loaded.row.accountId, workspaceId, WORKSPACE_ACTIONS.WORKSPACE_REVIEW_READ);
    ...
    const items = await listInboxItems(workspaceId, {
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/review/items/{reviewItemId}',
    tags: ['review'],
    summary: 'GET /:workspaceId/review/items/:reviewItemId',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), reviewItemId: z.string() }),
    },
    responses: {
  ...
  async (c: any) => {
    ...
    const item = await getReviewItemById(c.req.param('reviewItemId'), workspaceId);
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/review/items',
    tags: ['review'],
    summary: 'POST /:workspaceId/review/items',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
  ...
  async (c: any) => {
    ...
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    // Human gate: submitting a reviewable needs workspace.review.submit. Every
    // built-in role holds it; a custom role that unchecks it is denied here.
    await assertWorkspaceCapability(c, loaded.userId, loaded.row.accountId, workspaceId, WORKSPACE_ACTIONS.WORKSPACE_REVIEW_SUBMIT);
    // Agent-side gate: submitting a reviewable is the agent's intended path.
    assertAgentScope(c, 'workspace.review.submit');
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/review/items/{reviewItemId}/act',
    tags: ['review'],
    summary: 'POST /:workspaceId/review/items/:reviewItemId/act',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), reviewItemId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    const existing = await getReviewItemById(reviewItemId, workspaceId);
    if (!existing) return c.json({ error: 'Review item not found' }, 404);

    const row = await applyVerdict(reviewItemId, workspaceId, {
      verdict,
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/review/bulk',
    tags: ['review'],
    summary: 'POST /:workspaceId/review/bulk',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    const rows = await bulkApplyVerdict(ids, workspaceId, { verdict, actingUserId: loaded.userId });
