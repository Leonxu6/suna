...
// for the governance field space, so PUT refuses a v1 workspace with a clear
// 400 (the UI degrades to the limited scope editor + an "upgrade to v2"
// hint instead of ever calling PUT here). GET still works on a v1 workspace — it
// reports schemaVersion:1 + a null block so the UI can branch.
//
// Manager-gated on workspace.customize.write (same leaf the model/scope editors
// and every other customize mutation use), threaded through
// assertWorkspaceCapability so the agent-grant fold fires.
...
async function readAgentMarkdown(
  loadedRow: Parameters<typeof withWorkspaceGitAuth>[0],
  branch: string,
  mdPath: string,
): Promise<{ frontmatter: Record<string, unknown>; body: string }> {
  try {
    const gitWorkspace = await withWorkspaceGitAuth(loadedRow);
    const content = await readRepoFile(gitWorkspace, mdPath, branch);
...
/** Workspace the recognized behavior fields out of a `.md`'s parsed
...
workspacesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/agents/{agentName}/config',
    tags: ['workspaces'],
    summary: 'GET /:workspaceId/agents/:agentName/config',
    ...auth,
    request: { params: z.object({ workspaceId: z.string(), agentName: z.string() }) },
...
// `kortix.yaml.default_agent` is durable truth; workspace.metadata.default_agent
// is the read-optimized mirror used by session creation and channel surfaces.
workspacesApp.openapi(
  createRoute({
    method: 'put',
    path: '/{workspaceId}/default-agent',
    tags: ['workspaces'],
    ...
    responses: {
      200: json(DefaultAgentResponseSchema, 'Updated workspace default agent'),
      ...errors(400, 403, 404),
    },
  }),
  async (c: any) => {
    ...
    try {
      ...
      await commitMultipleFilesToBranch(gitWorkspace, {
...
workspacesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/agents/{agentName}/config',
    tags: ['workspaces'],
    summary: 'PUT /:workspaceId/agents/:agentName/config',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string(), agentName: z.string() }),
