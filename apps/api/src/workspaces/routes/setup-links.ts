 ...
 * short-lived link it can hand to a human to (a) enter a workspace secret value,
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/secret-requests',
    tags: ['secrets'],
    summary: 'POST /:workspaceId/secret-requests — mint a secret-entry link',
    ...auth,
    request: {
  ...
  async (c: any) => {
    ...
    // Floor 'read'; workspace.secret.write is the real gate — the same leaf as
    // POST /secrets. Was 'manage' → workspace.write, so unchecking secret.write
    // did nothing here.
...
workspacesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/connect-requests',
    tags: ['connectors'],
    summary: 'POST /:workspaceId/connect-requests — mint a Pipedream Quick Connect link',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    const workspaceId = c.req.param('workspaceId');
    const body = await readBody(c);
    // Floor 'read'; workspace.connector.write is the real gate (minting a Pipedream
    // Quick Connect link is a connector operation). Was 'manage' → workspace.write.
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    ...
    if (!conn) {
      return c.json(
        { error: `"${slug}" is not a connected-via-Pipedream connector on this workspace. Add it to kortix.yaml first.` },
        404,
      );
    }
