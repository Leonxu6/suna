 ...
 *     workspace (skills/agents/tools/kortix.yaml), then opens a CR.
 *
 ...
 * docs/specs/2026-07-13-marketplace-as-workspaces.md. Adding a marketplace item
 * to an existing workspace is now always an agent import; no file is ever
...
async function handleMarketplaceInstallSession(c: any) {
  ...
  let prompt: string;
  try {
    // Whole workspaces get merged (judgment-heavy, guards the target's kortix.yaml);
