...
// with "agent not found" for v2 workspaces (see below).
//
...
workspacesApp.openapi(
  createRoute({
    method: 'put',
    path: '/{workspaceId}/agents/{agentName}/scope',
    tags: ['workspaces'],
    summary: 'PUT /:workspaceId/agents/:agentName/scope',
    ...auth,
    request: {
  ...
  async (c: any) => {
    ...
    // Floor 'read' (membership); the real gate is workspace.agent.write below.
    ...
    // 'manage' → workspace.write, so unchecking agent.write did nothing here.
    const loaded = await loadWorkspaceForUser(c, workspaceId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    await assertWorkspaceCapability(c, loaded.userId, loaded.row.accountId, workspaceId, WORKSPACE_ACTIONS.WORKSPACE_AGENT_WRITE);
    ...
    // edit on a YAML workspace 404'd "agent not found" — branch on the schema.
