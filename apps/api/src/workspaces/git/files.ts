...
export async function listRepoFiles(
  ...
  const treeRef = validateRef(ref || workspace.defaultBranch);
  ...
  return result.stdout
    .split('\n')
    .map<WorkspaceFileEntry | null>((line) => {
...
export async function searchRepoFileNames(
  ...
  if (!q) return [];
  const files = await listRepoFiles(workspace, ref);
  return files
    .map((f) => {
    ...
    .filter((x): x is { f: WorkspaceFileEntry; score: number } => Boolean(x))
...
export async function readRepoFile(workspace: GitBackedWorkspace, filePath: string, ref?: string) {
...
/**
 * Resolve + read the workspace's manifest, preferring the first candidate path
