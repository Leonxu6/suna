// Workspace config introspection: parses the workspace manifest (kortix.yaml,
// falling back to legacy kortix.toml) + the OpenCode config dir
// (agents/skills/commands) out of the repo into a WorkspaceConfigSummary.
...
import type { GitBackedWorkspace, WorkspaceConfigSummary, WorkspaceFileEntry } from './types';

async function optionalFile(workspace: GitBackedWorkspace, filePath: string) {
  try {
    return await readRepoFile(workspace, filePath, workspace.defaultBranch);
  } catch {
    return null;
  }
}
...
type NativeAgentSummary = Omit<WorkspaceConfigSummary['agents'][number], 'source' | 'enabled'>;

export function resolveConfigAgents(
  nativeAgents: NativeAgentSummary[],
  loadedAgents: LoadedAgents,
): Pick<WorkspaceConfigSummary, 'agent_discovery' | 'agents'> {
...
export async function loadWorkspaceConfig(
  ...
  const repoFiles = files ?? (await listRepoFiles(workspace, workspace.defaultBranch));
  // Dual-format: resolve kortix.yaml (preferred) or kortix.toml, then parse in
  // the matched format. Without this, a yaml-only workspace reads no manifest here
  // → its [[agents]] scoping silently vanishes from the config introspection.
  const resolved = await readManifestFromRepo(
    workspace,
    manifestCandidatePaths(workspace.manifestPath).map((c) => c.path),
  ...
  const manifestFilePath = resolved?.path ?? workspace.manifestPath;
  ...
  const loadedAgents = parsedManifest
    ? extractAgents({
        ...
        // declared `kortix_version`, not a hardcoded v1, or a v2 workspace's
  ...
  const openCodeRaw = await optionalFile(workspace, `${opencodeDir}/opencode.jsonc`);
  ...
  const nativeAgents = await Promise.all(
    agentPaths.map(async (path) => {
      const raw = await optionalFile(workspace, path);
  ...
  return {
    ...
    // v2 makes the manifest's declared default authoritative. Legacy workspaces
...
function resolveOpencodeDir(manifest: Record<string, unknown>): string {
  ...
  if (opencode && typeof opencode === 'object' && !Array.isArray(opencode)) {
    ...
    if (typeof raw === 'string' && raw.trim()) {
      ...
      // belt-and-suspenders since loadWorkspaceConfig uses its own parser.
