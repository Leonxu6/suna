// Shared core for the git-backed-workspace operations module.
//
...
import type { GitBackedWorkspace } from './types';
...
/**
 * The per-workspace bare mirror is a SHARED resource: session provisioning, the
 ...
 * `workspace.gitAuthToken`, and several of those callers legitimately resolve it
 * to null (e.g. `.catch(() => null)` on a transient resolve, or a workspace shape
 * built before auth is known). Because `refreshMirror()` dedups concurrent
 * refreshes by workspaceId, a single tokenless caller winning the lock makes the
 ...
 * resolve it lazily from the workspace's stored credentials. Dynamic import keeps
...
async function ensureMirrorAccess(workspace: GitBackedWorkspace): Promise<ResolvedMirrorAccess> {
  if (workspace.gitAuthToken || Object.keys(workspace.gitAuthHeaders ?? {}).length > 0) {
    return {
      repoUrl: workspace.repoUrl,
      token: workspace.gitAuthToken ?? null,
      headers: workspace.gitAuthHeaders ?? {},
    };
  }
  try {
    const { resolveWorkspaceGitAccessById } = await import('../lib/git');
    const resolved = await resolveWorkspaceGitAccessById(workspace.workspaceId);
    if (resolved) return resolved;
  } catch (err) {
    console.warn(
      `[git-mirror] lazy auth resolve failed for ${workspace.workspaceId}:`,
      err instanceof Error ? err.message : err,
    );
  }
  return { repoUrl: workspace.repoUrl, token: null, headers: {} };
}
...
export function repoCachePath(workspace: GitBackedWorkspace) {
  const id = createHash('sha256').update(workspace.workspaceId).digest('hex').slice(0, 32);
...
export async function makeSessionBranchRepo(workspaceId: string) {
...
/** Per-op timeout (ms) for a cold bare `git clone --bare` of a workspace mirror. */
...
async function doRefreshMirror(workspace: GitBackedWorkspace, force = false) {
  ...
  if (!needsClone && !looksLikeBareMirror(repoPath)) {
    ...
    await rm(repoPath, { recursive: true, force: true });
    lastRefreshAt.delete(workspace.workspaceId);
    needsClone = true;
  }
  const lastRefresh = lastRefreshAt.get(workspace.workspaceId) || 0;
  ...
  // refresh lock, the shared clone/fetch is authenticated whenever the workspace
  ...
  const access = await ensureMirrorAccess(workspace);
  ...
  if (needsClone) {
    ...
    lastRefreshAt.set(workspace.workspaceId, Date.now());
    return repoPath;
  }
...
export async function refreshMirror(workspace: GitBackedWorkspace, force = false) {
  const current = refreshLocks.get(workspace.workspaceId);
  if (current) return current;
  const next = doRefreshMirror(workspace, force)
    .then(async (repoPath) => {
    ...
    .finally(() => refreshLocks.delete(workspace.workspaceId));
  refreshLocks.set(workspace.workspaceId, next);
  return next;
}
 ...
 * sweeps walk EVERY active workspace's manifest through `refreshMirror()`, so the
...
/**
 * Drop the "recently refreshed" marker for this workspace so the next mirror
 ...
 * upstream change.
 */
export function invalidateWorkspaceMirror(workspaceId: string): void {
  lastRefreshAt.delete(workspaceId);
}
