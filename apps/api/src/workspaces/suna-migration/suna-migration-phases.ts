 ...
 * --push-repo ran on real data), but the `db` phase (workspace + sessions inserts)
...
import { workspaces, workspaceGitConnections, workspaceSessions } from '@kortix/db';
...
/** extract: discover the account's Suna workspaces, pull each sandbox's files into
...
export async function extractStep(ctx: SunaMigrationContext): Promise<void> {
  ...
  // Window over the account's workspaces, newest-first: plan.{limit,offset}.
  ...
  const sunaWorkspaces = (await ctx.database.execute(sql`
    SELECT p.workspace_id, COALESCE(NULLIF(p.name,''),'Untitled') AS name, r.external_id
  ...
  `)) as unknown as Array<{ workspace_id: string; name: string; external_id: string | null }>;

  const used = new Set<string>();
  const sessions: SessionToWrite[] = [];
  const slugs: string[] = [];
  for (const p of sunaWorkspaces) {
    const threads = (await ctx.database.execute(sql`
      SELECT thread_id FROM public.threads WHERE workspace_id = ${p.workspace_id}
    ...
    let slug = slugify(p.name, p.workspace_id.slice(0, 8));
    if (used.has(slug)) slug = `${slug}-${p.workspace_id.slice(0, 6)}`;
    used.add(slug);
 ...
 *  on-open rehydrate can ship it (keyed by the new workspaceId). */
export async function repoStep(ctx: SunaMigrationContext): Promise<void> {
  if (typeof ctx.progress.workspace_id === 'string') { ctx.log('repo: already done'); return; }
  ...
  // and key the aside to the stable migrationId — NOT the workspaceId, which
  ...
  // use, which is what rehydrateSessionChat opens) and upload keyed by workspaceId.
  const stageDir = mkdtempSync(join(tmpdir(), `suna-oc-${repo.workspaceId}-`));
  copyFileSync(dbAside, join(stageDir, 'opencode.db'));
  const tar = Bun.spawnSync(['tar', 'czf', '-', '-C', stageDir, 'opencode.db']);
  if (tar.exitCode === 0) await uploadOpencodeArchive(repo.workspaceId, Buffer.from(tar.stdout));
  rmSync(stageDir, { recursive: true, force: true });

  await ctx.checkpoint({
    workspace_id: repo.workspaceId, repo_url: repo.upstreamUrl, repo_owner: repo.repoOwner,
...
/** db: create the workspace + git connection + N dormant sessions, each pinned to
...
export async function dbStep(ctx: SunaMigrationContext): Promise<void> {
  ...
  const workspaceId = ctx.progress.workspace_id as string;
  ...
  await (ctx.database as any).transaction(async (tx: any) => {
    await tx.insert(workspaces).values({
      workspaceId, accountId: ctx.accountId, name: 'Legacy (Suna) workspaces',
      ...
      metadata: {
        ...
        // allocateRuntimeOnOpen reads source_sandbox_id from WORKSPACE metadata and
        // passes the rehydrate hook through the runtime allocator. The archive is
        // keyed by workspaceId and shared by every session, so it is workspace-level.
        legacy_migration: { run_id: ctx.runId, source_sandbox_id: workspaceId, migrated_at: now.toISOString() },
      },
    }).onConflictDoNothing({ target: workspaces.workspaceId });

    await tx.insert(workspaceGitConnections).values({
      accountId: ctx.accountId, workspaceId, provider, repoUrl, upstreamUrl: repoUrl, managed: true,
      ...
      visibility: 'private',
      status: 'connected',
    } as any).onConflictDoNothing({ target: workspaceGitConnections.workspaceId });

    for (const s of specs) {
      await tx.insert(workspaceSessions).values({
        sessionId: crypto.randomUUID(), accountId: ctx.accountId, workspaceId,
        ...
        agentName: 'default', status: 'stopped', createdBy: ctx.accountId, visibility: 'workspace',
        metadata: {
          legacy_migration: {
            run_id: ctx.runId,
            // Reuse the legacy on-open ship: archive is keyed by workspaceId.
            source_sandbox_id: workspaceId,
            rehydrate: { opencode_session_id: s.opencodeSessionId },
          },
        },
      } as any).onConflictDoNothing({ target: workspaceSessions.sessionId });
    }
  });
  ...
  ctx.log('db: committed', { workspace_id: workspaceId, sessions: specs.length });
}
