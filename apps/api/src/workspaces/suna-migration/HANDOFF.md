...
legacy-sandbox migration). One row per **account** → one new workspace with N
...
## Pieces (all in `apps/api/src/workspaces/suna-migration/`)
| file | status |
|---|---|
...
| `suna-migration-routes.ts` | ✅ eligibility/start/status, **registered** at `/v1/workspaces/suna-migration/*` |
...
1. **Validate the `db` phase** (`dbStep`) — the `workspaces` / `workspace_git_connections`
   / `workspace_sessions` inserts. Mirrors legacy `dbStep`; confirm columns/enums.
2. **Validate the on-open chat ship.** Design choice: store the one opencode.db via
   `uploadOpencodeArchive(workspaceId, tar)`; each session's
   `metadata.legacy_migration.source_sandbox_id = workspaceId` so the EXISTING
...
4. **Frontend button** — mirror `legacy-machine-card.tsx`: show when
   ...
   `…/status`, hide on `completed`. (eligible = has public.workspaces rows AND no
...
per-workspace tarballs to object storage between extract→repo.
