 ...
 *
 *   GET  /v1/workspaces/suna-migration/eligibility  → drives the "Migrate" button
 *   POST /v1/workspaces/suna-migration/start        → start (durable, no cancel)
 *   GET  /v1/workspaces/suna-migration/status       → poll progress
 *
 * eligible = the account has OLD Suna data (public.workspaces rows) AND no
...
// `countSunaWorkspaces`, an un-LIMITed `count(*)` over the legacy `public.workspaces`
...
async function countSunaWorkspaces(accountId: string): Promise<number> {
  try {
    const rows = (await db.execute(
      sql`SELECT count(*)::int AS n FROM public.workspaces WHERE account_id = ${accountId}`,
...
export async function buildEligibility(
  ...
  const eligible = (await countSunaWorkspaces(accountId)) > 0;
...
export function registerSunaMigrationRoutes(app: OpenAPIHono<AppEnv>): void {
  app.openapi(
    createRoute({
      method: 'get', path: '/suna-migration/eligibility', tags: ['workspaces'],
  ...
  app.openapi(
    createRoute({
      method: 'post', path: '/suna-migration/start', tags: ['workspaces'],
    ...
    async (c: any) => {
      ...
      if ((await countSunaWorkspaces(accountId)) === 0) {
        return c.json({ error: 'No Suna workspaces found for this account' }, 400);
      }
  ...
  app.openapi(
    createRoute({
      method: 'get', path: '/suna-migration/status', tags: ['workspaces'],
