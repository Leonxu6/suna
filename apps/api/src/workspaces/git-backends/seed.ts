 ...
 *
 * Used by the web "Create workspace" flow — there's no local working tree to push
...
export async function seedRepoViaGitPush(input: {
   ...
   * dates so every workspace of the same starter shares an identical root SHA.
  ...
  // Pinned identity + dates → deterministic commit SHA across workspaces.
  const PINNED = {
  ...
  try {
    ...
    if (input.baseFiles?.length) {
      ...
      await run(['add', '-A']);
      await execFileAsync('git', ['commit', '-m', 'chore: scaffold Kortix workspace'],
        { cwd: dir, timeout: 60_000, env: { ...env, ...PINNED } });
    }
    await writeFiles(input.files);
    await run(['add', '-A']);
    // Commit only if the per-workspace files differ from the base (avoids an
    // empty second commit when baseFiles === files).
    const status = await run(['status', '--porcelain']);
    if (status.stdout.toString().trim().length > 0) {
      await run(['commit', '-m', input.baseFiles?.length ? 'chore: workspace setup' : (input.commitMessage || 'chore: scaffold Kortix workspace')]);
    } else if (!input.baseFiles?.length) {
      await run(['commit', '-m', input.commitMessage || 'chore: scaffold Kortix workspace']);
    }
