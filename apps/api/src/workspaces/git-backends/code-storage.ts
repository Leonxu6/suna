 ...
 * (types.ts docstring: token resolution stays with the workspace layer for
 ...
 * normalization as GitHub App keys (workspaces/github.ts's
...
/** Repo path segment (e.g. `team/workspace-alpha`) parsed from a code.storage `http_url` response field. */
...
export const codeStorageBackend: GitHostBackend = {
  ...
  buildUpstream(ref: GitConnectionRef, token: string | null, scope: GitScope): UpstreamGit {
    ...
    // (`resolveWorkspaceGitAuth` in workspaces/lib/git.ts is GitHub-only), so
  ...
  async seedFiles(
    ...
    // deterministic base-scaffold commit first, then the per-workspace files —
    ...
    await commitPack(repoPath, token, {
      branch: opts.branch,
      message: opts.baseFiles?.length ? 'chore: workspace setup' : opts.message,
      files,
    });
  },
};
