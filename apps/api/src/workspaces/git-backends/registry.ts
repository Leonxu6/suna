/**
 * Git backend registry. The provider is stored per-workspace
 * (`workspaceGitConnections.provider`), so backends can run simultaneously: a
 * GitHub-managed workspace resolves through the GitHub backend while a future
 * Forgejo/Artifacts workspace resolves through its own — all behind the same
 * Kortix git proxy.
 *
...
/** The backend NEW managed workspaces are provisioned on. */
