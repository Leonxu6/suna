...
// DB-first, env-fallback — see workspaces/github.ts for the matching App
 ...
 * credential model as `resolveWorkspaceGitAuth`'s managed-GitHub branch: the org
 ...
 * scope before the repo exists). Per-workspace RUNTIME tokens are minted
 * repo-scoped separately in `resolveWorkspaceGitAuth`.
 */
...
export const githubBackend: GitHostBackend = {
  ...
  async seedFiles(
    ...
    await seedRepoViaGitPush({
      ...
      // every workspace of this starter shares an identical root SHA with the
      // image-baked scaffold (snapshots/build-context.ts). Without forwarding
      // this, the workspace root was the workspace-named files commit → unrelated
