...
mock.module('@kortix/db', () => ({ workspaceSessions: {}, workspaces: {} }));
...
// give it an empty-result chain so runWorkspaceMaintenance can complete.
...
const { shouldForceResetStaleLock, runWorkspaceMaintenance, __isMaintenanceRunningForTest } =
...
describe('runWorkspaceMaintenance stale-lock generation guard', () => {
  test("an abandoned run settling late does not release a newer run's lock", async () => {
    process.env.KORTIX_WORKSPACE_MAINTENANCE_STALL_MS = '20';
    ...
    const runA = runWorkspaceMaintenance();
    ...
    const runB = runWorkspaceMaintenance();
    ...
    delete process.env.KORTIX_WORKSPACE_MAINTENANCE_STALL_MS;
  });
});
