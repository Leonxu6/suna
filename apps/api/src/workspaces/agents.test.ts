/**
 * `loadWorkspaceAgents` — the declared-agent READ path `sessions.ts` actually
 ...
 * declared default agent for a blank managed-git workspace (no kortix.yaml/
 * kortix.toml committed yet — provisioned without `seed_starter:true`), but
 * left THIS path untouched: `loadWorkspaceAgents` → `readManifest` (the plain
 * `../triggers.ts` reader, not the edit-time synthesis) returned a literal
 * `null` for a blank workspace, so `extractAgents` never ran and
...
const { loadWorkspaceAgents } = await import('./agents');
...
describe('loadWorkspaceAgents — blank managed workspace (no manifest committed yet)', () => {
  test('synthesizes the same declared "kortix" default agent loadManifestForEdit promises', async () => {
    ...
    const loaded = await loadWorkspaceAgents(fakeWorkspace());
  ...
  // exactly this loadWorkspaceAgents → resolveGovernedAgentGrant chain, with
  // `subject: true` (every POST /workspaces/provision workspace stamps
  // `metadata.require_declared_agents = true`) and `workspaceDefaultAgent: null`
  // (workspace.metadata.default_agent is never stamped at provision time). This
  // is the REAL call shape session-create hits on a blank workspace's first
