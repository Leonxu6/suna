 ...
 * dedup key (workspace + target + commit + base runtime).
 */
export interface PrepIdentity {
  ...
  /** Resulting per-workspace ppwarm image name (perWorkspaceWarmImageName). */
  snapshotName: string;
}

/**
 * Stable dedup key for a prep identity — repeated switch calls for the same
 * (workspace, target, commit, base runtime) must collapse onto ONE build. Mirrors
...
/**
 * Monotonic-generation CAS. Activation flips the workspace's active provider ONLY
 * when this transition's generation is STRICTLY GREATER than the generation the
 * workspace last recorded — so an older transition that settles late can never
...
export function canActivateGeneration(opts: {
  ...
  workspaceRecordedGeneration: number;
}): boolean {
  return opts.transitionGeneration > opts.workspaceRecordedGeneration;
}
...
/** The next monotonic generation for a workspace, given the max already seen. */
 ...
 * per-workspace warm image may not exist yet). Switching to null (clear), to the
