...
/**
 * The prepare-branch body of PATCH /:workspaceId/sandbox-provider (the durable
 * transition the UI polls). Carries an explicit `kind` discriminant so the PATCH
 * response union ({ kind:'workspace' } | { kind:'preparation' }) is unambiguous — a
...
/**
 * The PUBLIC workspaceion served by GET /:workspaceId/sandbox-provider/transition.
 ...
 * first place, so they cannot leak through this workspaceion either.) No `kind`
