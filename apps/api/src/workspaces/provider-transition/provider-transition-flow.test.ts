...
import { createDb, accounts, workspaces, sandboxTemplates, type Database } from '@kortix/db';
...
async function freshWorkspace(defaultProvider = 'daytona'): Promise<string> {
  const [row] = await db
    .insert(workspaces)
...
function makeDeps(world: FakeWorld, now: () => Date = () => new Date()): TransitionDeps {
  return {
    ...
    ensureWarmImage: async (_workspace, opts) => {
      world.buildCount += 1;
...
function identity(workspaceId: string, commit: string, base: string): ResolvedPrepIdentity {
  return { commitSha: commit, baseRuntimeIdentity: base, snapshotName: perWorkspaceWarmImageName(workspaceId, commit, base) };
}
...
d('provider transition — durable flow (throwaway Postgres)', () => {
  test('request records a pending transition and does NOT flip the active provider', async () => {
    ...
    const id = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const res = await reserveSwitchTransition(db, {
      accountId,
      sourceProvider: 'daytona',
      identity: { workspaceId, targetProvider: 'platinum', ...id },
    });
    expect(res.created).toBe(true);
    const routing = await readActiveRouting(db, workspaceId);
  ...
  test('drive does not activate while the image is not yet ready', async () => {
    ...
    expect((await readActiveRouting(db, workspaceId))?.activeProvider).toBeNull();
  });
  ...
  test('duplicate switch requests dedup to ONE transition and ONE build', async () => {
    const workspaceId = await freshWorkspace();
    const id = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const a = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...id } });
    const b = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...id } });
  ...
  test('a new commit during prep prevents a stale activation (forks a new identity)', async () => {
    const workspaceId = await freshWorkspace();
    const orig = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const drifted = identity(workspaceId, 'commit-b', 'kortix-default-r1');
    const world = makeWorld(orig);
    world.identityByCall = [drifted];
    const res = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...orig } });
    const outcome = await driveProviderTransition(makeDeps(world), res.row.transitionId);
  ...
  test('a runtime/base-image change during prep prevents a stale activation', async () => {
    const workspaceId = await freshWorkspace();
    const orig = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const drifted = identity(workspaceId, 'commit-a', 'kortix-default-r2');
    const world = makeWorld(orig);
    world.identityByCall = [drifted];
    const res = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...orig } });
    const outcome = await driveProviderTransition(makeDeps(world), res.row.transitionId);
  ...
  test('a newer request supersedes the older; only the newest can activate', async () => {
    const workspaceId = await freshWorkspace();
    const idA = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const idB = identity(workspaceId, 'commit-b', 'kortix-default-r1');
    const a = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...idA } });
    const b = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...idB } });
    ...
    expect((await readActiveRouting(db, workspaceId))?.activeProvider).toBe('platinum');
  });

  test('concurrent activation CAS: only the generation matching the workspace wins (clobber race)', async () => {
    const workspaceId = await freshWorkspace();
    const idA = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const idB = identity(workspaceId, 'commit-b', 'kortix-default-r1');
    const a = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...idA } });
    const b = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...idB } });
    const now = new Date();
    const older = await activateWithCas(db, { workspaceId, transitionId: a.row.transitionId, targetProvider: 'platinum', generation: a.row.generation!, snapshotName: idA.snapshotName, externalTemplateId: 'tpl_a', now });
    const newer = await activateWithCas(db, { workspaceId, transitionId: b.row.transitionId, targetProvider: 'platinum', generation: b.row.generation!, snapshotName: idB.snapshotName, externalTemplateId: 'tpl_b', now });
    expect(older.activated).toBe(false);
    expect(older.reason).toBe('lost_cas');
    expect(newer.activated).toBe(true);
    expect((await readActiveRouting(db, workspaceId))?.activeExternalTemplateId).toBe('tpl_b');
  });

  test('the prepared snapshot name equals the session warm-lookup name (first session does not clone)', async () => {
    const workspaceId = await freshWorkspace();
    const id = identity(workspaceId, 'deadbeef', 'kortix-default-r1');
    expect(id.snapshotName).toBe(perWorkspaceWarmImageName(workspaceId, 'deadbeef', 'kortix-default-r1'));
    const world = makeWorld(id);
    const res = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...id } });
  ...
  test('switching back (setPin + generation bump) supersedes an in-flight prepare — no late re-flip', async () => {
    ...
    const world = makeWorld(id);
    const res = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...id } });
    await setPinWithGenerationBump(db, { workspaceId, pin: 'daytona', now: new Date() });
  ...
  test('a prebuild row builds the image but stays invisible to routing until adopted', async () => {
    ...
    const id = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const world = makeWorld(id);
    const { row, created } = await insertPrebuildTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...id } });
    ...
    const adopt = await reserveSwitchTransition(db, { accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...id } });
  ...
  test('scattered indeterminate blips interspersed with `building` NEVER dead-letter (attempts reset)', async () => {
    ...
    const res = await reserveSwitchTransition(db, {
      accountId, sourceProvider: 'daytona', identity: { workspaceId, targetProvider: 'platinum', ...id },
    });
  ...
  test('a zombie drive with a STALE epoch is fenced out: it ceases silently and clobbers no state', async () => {
    ...
    expect((await readActiveRouting(db, workspaceId))?.activeProvider).toBeNull(); // pin never flipped
  });
  ...
  test('activation is REJECTED for a stale lease epoch even at a MATCHING generation', async () => {
    ...
    const zombie = await activateWithCas(db, {
      workspaceId, transitionId: res.row.transitionId, targetProvider: 'platinum',
  ...
  //
  // The prepared warm image covers ONLY the default template. A workspace that
  ...
  // a broken custom template would wedge the workspace forever); its custom
  ...
  // workspaceDeclaresCustomTemplates (a workspace-scoped, non-default-slug row).
  const customTemplateProbe = (database: Database) => async (workspace: { workspaceId: string }) => {
    const rows = await database
      .select({ slug: sandboxTemplates.slug })
      .from(sandboxTemplates)
      .where(and(eq(sandboxTemplates.workspaceId, workspace.workspaceId), ne(sandboxTemplates.slug, 'default')))
      .limit(1);
    return rows.length > 0;
  };

  test('FIX-M1: a custom-template workspace activates on the DEFAULT warm image (non-blocking) and emits custom_template_cold_boot', async () => {
    resetProviderTransitionMetricsForTest();
    const workspaceId = await freshWorkspace();
    // Declare a custom (non-default-slug) template for this workspace.
  ...
  test('FIX-M1: a default-only workspace activates with NO custom_template_cold_boot event', async () => {
    resetProviderTransitionMetricsForTest();
    const workspaceId = await freshWorkspace(); // no custom template rows
    const id = identity(workspaceId, 'commit-a', 'kortix-default-r1');
    const world = makeWorld(id);
    world.state.set(id.snapshotName, 'active');
    world.externalIds.set(id.snapshotName, 'tpl_default_warm');
  ...
  test('heartbeat threaded through ensureWarmImage renews the lease during a long build', async () => {
    ...
    deps.ensureWarmImage = async (workspace, opts) => {
      for (let i = 0; i < 3; i++) {
        await opts.heartbeat?.(); // resolves while owned
        beats += 1;
      }
      return origEnsure(workspace, opts);
    };
    const outcome = await driveProviderTransition(deps, res.row.transitionId);
    expect(outcome).toBe('activated');
