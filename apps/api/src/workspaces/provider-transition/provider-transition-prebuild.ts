 ...
 * workspaces by a CONFIGURABLE fan-out policy, builds their target ppwarm image in
 * the background at a bounded concurrency (real Platinum build capacity), and
 * records readiness per workspace — WITHOUT shifting any traffic. Traffic only
 * moves for a workspace when an on-demand switch adopts its ready prebuild row (a
 * workspace that gets a new commit before activation is rebuilt: the prebuild row
...
import { workspaceSessions, workspaces, type Database } from '@kortix/db';
...
export function parsePrebuildConfig(
  ...
  return {
    ...
    workspaceIds: (pick('workspaces', 'PREBUILD_WORKSPACE_IDS') ?? '')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
...
/** Resolve the workspace cohort for the configured policy. */
export async function selectCohort(db: Database, cfg: PrebuildConfig): Promise<string[]> {
  if (cfg.policy === 'selected') return cfg.workspaceIds.slice(0, cfg.limit);

  if (cfg.policy === 'recently-active') {
    const cutoff = new Date(Date.now() - cfg.sinceMs);
    const rows = await db
      .selectDistinct({ workspaceId: workspaceSessions.workspaceId, updatedAt: workspaceSessions.updatedAt })
      .from(workspaceSessions)
      .innerJoin(workspaces, eq(workspaces.workspaceId, workspaceSessions.workspaceId))
      .where(and(eq(workspaces.status, 'active'), gt(workspaceSessions.updatedAt, cutoff)))
    ...
    const rows = await db
      ...
      .where(
        and(
          ...
          sql`${workspaces.metadata} @> ${JSON.stringify({ prebuild_platinum: true })}::jsonb`,
        ),
      )
      .limit(cfg.limit);
...
/**
 * Bounded worker pool: `width` runners each pull the NEXT workspace and drive it to
 ...
 * a concurrency that bounds ACTUAL builds (not just DB inserts). Every workspace's
 ...
 * ceiling. `allSettled` semantics — one workspace's failure never aborts a sibling,
