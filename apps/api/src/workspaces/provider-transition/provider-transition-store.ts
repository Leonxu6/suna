 ...
 *  - the generation is reserved ON THE WORKSPACE ROW (workspaces.sandbox_provider_generation)
 ...
 *    workspace's generation still equalling the transition's stamped generation;
...
import { providerTransitions, workspaces, type Database } from '@kortix/db';
...
/** Compact marker mirrored into workspace metadata so the existing workspace
...
/**
 * Read the active routing identity from a SINGLE workspace row — pin + activated
 ...
 */
export async function readActiveRouting(db: Database, workspaceId: string): Promise<ActiveRouting | null> {
  const [row] = await db
    .select({ metadata: workspaces.metadata, generation: workspaces.sandboxProviderGeneration })
    .from(workspaces)
...
export async function findLiveTransitionByIdentity(
  db: Queryable,
  identity: Pick<PrepIdentity, 'workspaceId' | 'targetProvider' | 'commitSha' | 'baseRuntimeIdentity'>,
): Promise<ProviderTransitionRow | null> {
  const [row] = await db
    .select()
    .from(providerTransitions)
    .where(
      and(
        eq(providerTransitions.workspaceId, identity.workspaceId),
...
export async function latestTransitionForWorkspace(
  db: Database,
  workspaceId: string,
): Promise<ProviderTransitionRow | null> {
  const [row] = await db
    .select()
    .from(providerTransitions)
    .where(eq(providerTransitions.workspaceId, workspaceId))
    .orderBy(desc(providerTransitions.requestedAt))
    .limit(1);
  return row ?? null;
}

export async function listTransitionsForWorkspace(
  db: Database,
  workspaceId: string,
  limit = 20,
 ...
 *     bump the workspace generation, stamp it onto the row, flip mode→switch
 *     (a ready prebuild then goes straight to verify→activate, no rebuild);
 *   - else bump the workspace generation, supersede older live switches, and
 *     insert a fresh pending switch row.
 * All under a single workspace-row FOR UPDATE lock, so concurrent requests
...
export async function reserveSwitchTransition(
  ...
  return db.transaction(async (tx) => {
    const [workspace] = await tx
      .select({ generation: workspaces.sandboxProviderGeneration })
      .from(workspaces)
      .where(eq(workspaces.workspaceId, input.identity.workspaceId))
      .for('update')
      .limit(1);
    if (!workspace) throw new Error(`workspace ${input.identity.workspaceId} not found`);
    ...
    const newGen = (workspace.generation ?? 0) + 1;
    await tx
      .update(workspaces)
    ...
    await tx
      ...
      .where(
        and(
          eq(providerTransitions.workspaceId, input.identity.workspaceId),
          inArray(providerTransitions.status, LIVE),
 ...
 * index). Does NOT touch the workspace generation, so a background prebuild can
...
/** Highest live SWITCH generation for a workspace (0 if none) — supersession check. */
export async function maxLiveSwitchGeneration(db: Database, workspaceId: string): Promise<number> {
  const [row] = await db
    .select({ max: sql<number | null>`max(${providerTransitions.generation})` })
    .from(providerTransitions)
    .where(
      and(eq(providerTransitions.workspaceId, workspaceId), inArray(providerTransitions.status, LIVE)),
    );
  return row?.max ?? 0;
}
...
/**
 * Atomic activation. Locks the workspace row, then flips the active pin ONLY if
 * the workspace's generation still equals this transition's stamped generation
 ...
 * the epoch does. Under the workspace lock we re-read the row's `lease_epoch`; if it
...
): Promise<{ activated: boolean; reason: 'won' | 'lost_cas' | 'lost_lease' | 'workspace_missing' }> {
  return db.transaction(async (tx) => {
    const [workspace] = await tx
      .select({ metadata: workspaces.metadata, generation: workspaces.sandboxProviderGeneration })
      .from(workspaces)
      ...
      .for('update')
      .limit(1);
    if (!workspace) return { activated: false, reason: 'workspace_missing' as const };

    // Lease fence: a fenced-out zombie must not win activation even at a matching
    // generation. Read the row's epoch under the workspace lock; a mismatch means a
    ...
    const recorded = workspace.generation ?? 0;
    // Reserved-at-request semantics: the winning transition's generation EQUALS
    // the workspace's current generation; a newer request would have bumped it
    ...
    await tx
      ...
      .where(
        and(
          eq(providerTransitions.workspaceId, args.workspaceId),
          inArray(providerTransitions.status, LIVE),
 ...
 * Locks the workspace, bumps the generation (so a late-settling prepared
...
export async function setPinWithGenerationBump(
  db: Database,
  args: { workspaceId: string; pin: string | null; now: Date },
  ...
  return db.transaction(async (tx) => {
    const [workspace] = await tx
      ...
      .for('update')
      .limit(1);
    if (!workspace) return { generation: 0 };
    const newGen = (workspace.generation ?? 0) + 1;
    ...
    await tx
      ...
      .where(and(eq(providerTransitions.workspaceId, args.workspaceId), inArray(providerTransitions.status, LIVE)));
    return { generation: newGen };
  });
}

/** Mirror a compact live-transition marker into workspace metadata for polling. */
