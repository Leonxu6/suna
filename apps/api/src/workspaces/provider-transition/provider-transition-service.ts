...
import { workspaces, sandboxTemplates, type Database } from '@kortix/db';
 ...
 * construction (the platform default is a shared row with a null workspace_id and
 * the reserved `DEFAULT_SANDBOX_SLUG`); the `ne(slug, DEFAULT_SANDBOX_SLUG)`
 * guard is belt-and-suspenders so a workspace row that somehow shadows the default
 ...
 *  custom-template workspace therefore migrates on the default warm image and its
...
export function defaultTransitionDeps(database: Database = appDb): TransitionDeps {
  return {
    ...
    loadWorkspace: async (workspaceId) => {
      const [row] = await database.select().from(workspaces).where(eq(workspaces.workspaceId, workspaceId)).limit(1);
      if (!row || row.status === 'archived') return null;
      return toGitBackedWorkspace(row);
    },
    // FIX-M1: default-only workspaces get the full warm hit; a custom-template
    // workspace still migrates safely on the default warm image, and the runner
...
// serializeTransition, the PUBLIC workspaceion) live in the pure, config-free
 ...
 * workspace so the route maps it to 400/404.
 */
export async function requestProviderTransition(input: {
  ...
  const [row] = await database.select().from(workspaces).where(eq(workspaces.workspaceId, input.workspaceId)).limit(1);
  ...
  if (kind === 'immediate_clear' || kind === 'immediate_set' || kind === 'noop') {
    ...
    const [updated] = await database.select().from(workspaces).where(eq(workspaces.workspaceId, input.workspaceId)).limit(1);
