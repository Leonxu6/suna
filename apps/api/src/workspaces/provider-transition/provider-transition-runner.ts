...
export interface TransitionDeps {
  ...
  resolvePrepIdentity: (workspace: GitBackedWorkspace, targetProvider: string) => Promise<ResolvedPrepIdentity>;
  /** Load a workspace as GitBackedWorkspace + accountId (null ⇒ gone). */
  loadWorkspace: (workspaceId: string) => Promise<(GitBackedWorkspace & { accountId: string }) | null>;
  /** FIX-M1: true iff the workspace declares custom (non-default-slug) templates.
   *  The prepared warm image covers ONLY the default template, so a switch that
   *  activates while the workspace has custom templates leaves those custom-template
   ...
   *  workspaces, and every existing caller, are unaffected). */
  hasCustomTemplates?: (workspace: GitBackedWorkspace) => Promise<boolean>;
  leaseTtlMs?: number;
...
export async function driveProviderTransition(
  ...
  try {
    const workspace = await deps.loadWorkspace(leased.workspaceId);
    if (!workspace) {
      await mustOwn(
        failTransition(deps.db, transitionId, {
          attempts: leased.attempts ?? 0,
          lastError: 'workspace no longer exists',
          errorClass: 'gone',
        }, myEpoch),
      );
      return 'gone';
    ...
    const current = await deps.resolvePrepIdentity(workspace, leased.targetProvider);
    ...
    if (leased.mode === 'switch' && leased.generation != null) {
      const maxLive = await maxLiveSwitchGeneration(deps.db, leased.workspaceId);
    ...
    if (readiness !== 'ready') {
      ...
      try {
        const buildResult = await deps.ensureWarmImage(workspace, {
    ...
    const current2 = await deps.resolvePrepIdentity(workspace, leased.targetProvider);
    const maxLive2 = await maxLiveSwitchGeneration(deps.db, leased.workspaceId);
    ...
    if (result.activated) {
      ...
      // If this workspace also declares custom (non-default-slug) templates, those
      // were deliberately NOT pre-baked for the target (Fable REJECTS a blocking
      // prepare-all — a broken/unused custom template would wedge the workspace
      ...
      if (deps.hasCustomTemplates) {
        const custom = await deps.hasCustomTemplates(workspace).catch(() => false);
        if (custom) {
    ...
    await mustOwn(failTransition(deps.db, transitionId, {
      ...
      lastError: 'workspace missing at activation',
      errorClass: 'gone',
    }, myEpoch));
    return 'gone';
  } catch (err) {
...
async function writeMarker(
  ...
  await writeTransitionMarker(deps.db, row.workspaceId, {
    status,
