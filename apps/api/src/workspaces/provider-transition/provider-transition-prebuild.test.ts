...
const makeRow = (workspaceId: string, status = 'pending') => ({ transitionId: `t-${workspaceId}`, workspaceId, status }) as never;
...
describe('prebuild config parsing', () => {
  ...
  test('argv overrides env overrides defaults', () => {
    const cfg = parsePrebuildConfig(
      ...
      ['--policy=selected', '--workspaces=a,b,c', '--dry-run=true'],
    );
    expect(cfg.policy).toBe('selected');
    expect(cfg.workspaceIds).toEqual(['a', 'b', 'c']);
...
describe('runPrebuildMigration — bounded worker pool over actual drives', () => {
  test('never exceeds cfg.concurrency concurrent drives, and drains every drive', async () => {
    ...
    let driven = 0;
    const deps = {
      requestPrebuild: async ({ workspaceId }: { workspaceId: string }) => makeRow(workspaceId),
    ...
    const result = await runPrebuildMigration({} as never, { ...baseCfg, workspaceIds: ids(20), concurrency: 4 }, { deps });
  ...
  test('rerun is idempotent — an already-ready row is skipped, never re-driven', async () => {
    let driveCalls = 0;
    const deps = {
      requestPrebuild: async ({ workspaceId }: { workspaceId: string }) => makeRow(workspaceId, 'ready'),
      driveToTerminal: async () => {
        driveCalls += 1;
        return 'prebuilt' as const;
      },
    ...
    const result = await runPrebuildMigration({} as never, { ...baseCfg, workspaceIds: ids(5), concurrency: 3 }, { deps });
  ...
  test('exit code reflects failures; a failure never aborts siblings', async () => {
    ...
    const result = await runPrebuildMigration({} as never, { ...baseCfg, workspaceIds: ids(6), concurrency: 2 }, { deps });
  ...
  test('a thrown drive is counted, not propagated (allSettled semantics)', async () => {
    ...
    const result = await runPrebuildMigration({} as never, { ...baseCfg, workspaceIds: ids(4), concurrency: 4 }, { deps });
  ...
  test('SIGINT stops launching NEW drives but drains the in-flight ones', async () => {
    ...
    const result = await runPrebuildMigration({} as never, { ...baseCfg, workspaceIds: ids(10), concurrency: 2 }, { deps });
  ...
  test('a dry run selects but drives nothing', async () => {
    ...
    const result = await runPrebuildMigration({} as never, { ...baseCfg, workspaceIds: ids(3), concurrency: 2, dryRun: true }, { deps });
    expect(driveCalls).toBe(0);
