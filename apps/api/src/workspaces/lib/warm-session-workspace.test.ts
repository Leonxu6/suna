...
const workspace = {
  ...
  repoUrl: 'https://git.example.test/workspace-1.git',
  defaultBranch: 'main',
...
describe('refreshWarmSessionWorkspace', () => {
  test('skips when the warm session has no active sandbox', async () => {
    let resolved = false;
    const result = await refreshWarmSessionWorkspace(workspace, 'session-1', {
  ...
  test('refreshes the active workspace to the latest base without restarting OpenCode', async () => {
    ...
    const resolvedWorkspaces: typeof workspace[] = [];
    const result = await refreshWarmSessionWorkspace(workspace, 'session-1', {
    ...
    expect(resolvedWorkspaces).toEqual([workspace]);
