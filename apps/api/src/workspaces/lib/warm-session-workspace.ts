...
import { resolveCommitSha, type GitBackedWorkspace } from '../git';
...
interface WarmSessionWorkspaceDependencies {
  ...
  resolveBaseSha: (workspace: GitBackedWorkspace) => Promise<string>;
  resolveIngress: (
    externalId: string,
...
const defaultDependencies: WarmSessionWorkspaceDependencies = {
  loadActiveSandbox,
  resolveBaseSha: (workspace) =>
    resolveCommitSha(workspace, workspace.defaultBranch),
...
export async function refreshWarmSessionWorkspace(
  ...
  try {
    ...
    const [baseSha, ingress] = await Promise.all([
      dependencies.resolveBaseSha(workspace),
