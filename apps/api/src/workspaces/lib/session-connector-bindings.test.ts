...
describe('session connector binding security contracts', () => {
  ...
  test('Supabase authentication session identity is not a Kortix workspace session identity', () => {
    expect(workspaceSessionIdForWorkspacePrincipal(undefined, 'supabase-auth-session')).toBeNull();
    expect(
      workspaceSessionIdForWorkspacePrincipal('11111111-1111-4111-a111-111111111111', 'kortix-session'),
    ).toBe('kortix-session');
  });
