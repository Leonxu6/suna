...
export async function resolveSandboxEnvSnapshot(
  ...
  const raw = await resolveOwnerRawEnv(workspaceId, sessionId, requestedAgent);
  if (!raw) return null;
...
export async function pushSessionModelToSandbox(input: {
  ...
  try {
    ...
    const snapshot = await resolveSandboxEnvSnapshot(input.workspaceId, input.sessionId);
