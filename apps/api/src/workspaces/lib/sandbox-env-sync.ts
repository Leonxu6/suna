...
import { workspaces, workspaceSessions, sessionSandboxes } from '@kortix/db';
...
import {
  ...
  workspaceSecretsRevision,
} from '../secrets';
import { grantFromLoadedAgents, loadWorkspaceAgents } from '../agents';
 ...
 * site here recomputes the LLM-gateway base URL from scratch (a workspace's
...
async function resolveOwnerRawEnv(
  ...
  if (workspace?.defaultBranch) {
    const loadedAgents = await loadWorkspaceAgents({
      ...
      manifestPath: workspace.manifestPath ?? 'kortix.yaml',
      gitAuthToken: null,
    }).catch(() => null);
  ...
  return (await listWorkspaceSecretsSnapshotForUser(workspaceId, row.createdBy, grantEnvForSession)).env;
}
...
export async function resolveSandboxEnvSnapshot(
  ...
  const raw = await resolveOwnerRawEnv(workspaceId, sessionId);
  if (!raw) return null;
  const { env, names } = sanitizeSandboxEnv(raw);
  return { env, names, revision: workspaceSecretsRevision(env) };
}
...
export async function syncSandboxEnvForPrompt(args: {
  ...
  const snapshot = await resolveSandboxEnvSnapshot(args.workspaceId, args.sessionId);
  if (!snapshot) return;
  const llmGatewayEnabled = await resolveWorkspaceLlmGatewayEnabled(args.workspaceId);
...
export async function propagateWorkspaceSecretsToActiveSandboxes(
  workspaceId: string,
  ...
  try {
    const rows = await db
      ...
      .where(and(eq(sessionSandboxes.workspaceId, workspaceId), eq(sessionSandboxes.status, 'active')));
    ...
    await runBounded(targets, FANOUT_CONCURRENCY, async (row) => {
      ...
      try {
        const snapshot = await resolveSandboxEnvSnapshot(workspaceId, row.sessionId);
        if (!snapshot) return;
    ...
    console.warn(
      `[env-sync] hot fan-out failed for workspace ${workspaceId}:`,
      err instanceof Error ? err.message : err,
    );
  }
}
...
export async function propagateLlmGatewayModeToActiveSandboxes(
  ...
  try {
    ...
    // Computed PER ROW (not once, hoisted) — a workspace's active sandboxes can
    ...
    await runBounded(targets, FANOUT_CONCURRENCY, async (row) => {
      ...
      try {
        const snapshot =
          (await resolveSandboxEnvSnapshot(workspaceId, row.sessionId)) ??
    ...
    console.warn(
      `[env-sync] LLM gateway mode fan-out failed for workspace ${workspaceId}:`,
      err instanceof Error ? err.message : err,
    );
  }
}

async function resolveWorkspaceLlmGatewayEnabled(workspaceId: string): Promise<boolean> {
  const [workspace] = await db
    .select({ metadata: workspaces.metadata })
    .from(workspaces)
    .where(eq(workspaces.workspaceId, workspaceId))
    .limit(1);
  return workspaceLlmGatewayEnabled(workspace?.metadata);
}
