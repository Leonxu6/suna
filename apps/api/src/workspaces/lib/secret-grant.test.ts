...
let loadWorkspaceAgentsImpl: () => Promise<LoadedAgents> = async () => ({
  specs: [],
  errors: [],
  defaultAgent: null,
});
...
mock.module('../agents', () => ({
  ...actualAgents,
  loadWorkspaceAgents: () => loadWorkspaceAgentsImpl(),
}));
...
beforeEach(() => {
  loadWorkspaceAgentsImpl = async () => ({ specs: [], errors: [], defaultAgent: null });
});
...
describe('resolveSessionSecretGrant', () => {
  test('fails closed when the manifest loader throws', async () => {
    loadWorkspaceAgentsImpl = async () => {
      throw new Error('git unreachable');
    };
    await expect(resolveSessionSecretGrant({ ...WORKSPACE, sessionAgent: 'kortix' })).rejects.toThrow(
      SecretGrantResolutionError,
    );
  });
  ...
  test('a workspace without git context is unrestricted', async () => {
    loadWorkspaceAgentsImpl = async () => {
      throw new Error('should not be called');
    };
    await expect(
      resolveSessionSecretGrant({ ...WORKSPACE, defaultBranch: null, sessionAgent: 'kortix' }),
    ).resolves.toBeUndefined();
  });

  test('a workspace that has not adopted agents is unrestricted', async () => {
    loadWorkspaceAgentsImpl = async () => loaded([]);
    await expect(
      resolveSessionSecretGrant({ ...WORKSPACE, sessionAgent: 'kortix' }),
    ).resolves.toBeUndefined();
  });

  test('resolves the declared grant for the session agent', async () => {
    loadWorkspaceAgentsImpl = async () => loaded([spec('release-bot', ['NPM_TOKEN'])]);
    await expect(
      resolveSessionSecretGrant({ ...WORKSPACE, sessionAgent: 'release-bot' }),
  ...
  test('resolves from the requested agent, not the session column', async () => {
    ...
    await expect(
      resolveSessionSecretGrant({ ...WORKSPACE, sessionAgent: 'a', requestedAgent: 'b' }),
    ).resolves.toEqual(['npm_token']);
  });
  ...
  test('refuses a requested agent whose grant differs from the session agent', async () => {
    ...
    await expect(
      resolveSessionSecretGrant({ ...WORKSPACE, sessionAgent: 'narrow', requestedAgent: 'broad' }),
