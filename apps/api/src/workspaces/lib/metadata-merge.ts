/**
 * SQL-side ATOMIC merge helpers for `workspaces.metadata` (FIX-J).
 *
 * The provider-routing pin lives in `workspaces.metadata` (`default_sandbox_provider`
 ...
 * EVERY other `workspaces.metadata` writer used to read the whole jsonb object into
...
function base(): SQL {
  return sql`coalesce(${workspaces.metadata}, '{}'::jsonb)`;
}
...
export function metadataMergeSubtree(topKey: string, subPatch: Record<string, unknown>): SQL {
  return sql`${base()} || jsonb_build_object(${topKey}::text, coalesce(${workspaces.metadata} -> ${topKey}, '{}'::jsonb) || ${JSON.stringify(subPatch)}::jsonb)`;
}
...
export function metadataClearSubtreeKey(topKey: string, subKey: string): SQL {
  const remaining = sql`(coalesce(${workspaces.metadata} -> ${topKey}, '{}'::jsonb) - ${subKey})`;
