export interface SessionRuntimeEnvInput {
   ...
   *  2` workspace — see `compile-agent-config.ts`. `null`/omitted for a v1
   *  workspace: no key is emitted, so v1 sandbox env is byte-for-byte unchanged. */
...
export function buildSessionRuntimeEnv(input: SessionRuntimeEnvInput): Record<string, string> {
  return {
    ...
    KORTIX_WORKSPACE_ID: input.workspaceId,
