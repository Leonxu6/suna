 ...
 *
 * `resolveCompiledAgentConfigForSession` is the I/O half: reads the workspace's
 ...
 * than waiting on that cap to move). It never throws: a v1 workspace (or any
...
import { type GitBackedWorkspace, readManifestFromRepo, readRepoFile } from '../git';
 ...
 *  (e.g. `parseManifestString` in workspaces/triggers.ts), which coerce a
...
/** The workspace's OpenCode config directory — the SAME top-level `[opencode]
 *  config_dir` v1 already reads (unrelated to per-agent behavior; this is
 *  just "where does `.kortix/opencode/...` live for this workspace"). Defaults
 *  to `.kortix/opencode`. */
 ...
 *  not a second hand-maintained literal, so the editor's merge/GET-workspaceion
 ...
 * workspaces keep depending on hand-authored `.md` frontmatter exactly as before
 ...
 * workspace's repo). When an agent's file is present, its frontmatter is
...
/**
 * Read a workspace's manifest straight from git + compile it (I/O half). Never
 ...
 * `KORTIX_COMPILED_AGENT_CONFIG` carries), or `null` for a v1 workspace / no
...
export async function resolveCompiledAgentConfigForSession(
  ...
  try {
    ...
    const found = await readManifestFromRepo(workspace, candidates, workspace.defaultBranch);
    ...
    await Promise.all(
      Object.keys(agents).map(async (name) => {
        ...
        try {
          agentMdFiles[path] = await readRepoFile(workspace, path, workspace.defaultBranch);
        } catch (err) {
          console.warn(
            `[compile-agent-config] workspace ${workspace.workspaceId}: failed to read agent "${name}"'s behavior file "${path}": ${(err as Error).message}`,
          );
        }
      }),
    );
    ...
    console.warn(
      `[compile-agent-config] workspace ${workspace.workspaceId}: compile failed, session boots without a compiled agent config: ${(err as Error).message}`,
    );
    return null;
  }
}
