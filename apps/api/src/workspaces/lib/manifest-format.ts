 ...
 *   1. the workspace's configured `manifestPath` (via `manifestFormatForPath`)
 *      — so a workspace explicitly created against `kortix.yaml` validates its
 *      YAML without the caller having to say so;
 *   2. an explicit `format` in the body, when the workspace has no
 ...
 * (or for an older workspace created before the default flipped to
