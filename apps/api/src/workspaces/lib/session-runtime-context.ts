...
import { workspaceSessionRuntimeContexts } from '@kortix/db';
...
export async function loadSessionRuntimeContext(
  ...
  const [row] = await db
    .select({ context: workspaceSessionRuntimeContexts.context })
    .from(workspaceSessionRuntimeContexts)
    .where(eq(workspaceSessionRuntimeContexts.sessionId, sessionId))
    .limit(1);
  if (!row) return null;
