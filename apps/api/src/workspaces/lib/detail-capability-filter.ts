// GET /:workspaceId/detail bundles several read surfaces (the config summary's
// agents/skills/commands + raw customization config, and the repo file list)
// behind ONE coarse workspace.read floor. To keep each capability checkbox
...
export interface DetailCaps {
  /** workspace.file.read — the repo file list. */
  canFiles: boolean;
  /** workspace.agent.read — the agents summary + discovery mode. */
  canAgents: boolean;
  /** workspace.skill.read — the skills summary. */
  canSkills: boolean;
  /** workspace.command.read — the slash-commands summary. */
  canCommands: boolean;
  /** workspace.customize.read — the raw workspace config (kortix.toml / opencode). */
...
// concrete interface (WorkspaceConfigSummary), which TS won't implicitly widen to
