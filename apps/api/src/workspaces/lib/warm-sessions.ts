export interface WarmWorkspaceSessionRecord {
  sessionId: string;
  status: string;
  baseRef: string;
...
export interface WarmWorkspaceSessionConfiguration {
  baseRef: string;
  agentName: string;
  sandboxSlug: string;
}

export interface ClaimWarmWorkspaceSessionConfiguration {
  sessionId: string;
  agentName?: string;
  sandboxSlug?: string;
}

interface WarmWorkspaceSessionMarker {
...
interface WarmWorkspaceSessionDependencies<T extends WarmWorkspaceSessionRecord> {
...
function markerOf(metadata: Record<string, unknown> | null): WarmWorkspaceSessionMarker | null {
  const value = metadata?.warm_session;
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const marker = value as Partial<WarmWorkspaceSessionMarker>;
  if (
  ...
  return marker as WarmWorkspaceSessionMarker;
}
...
function withMarker(
  session: WarmWorkspaceSessionRecord,
  marker: WarmWorkspaceSessionMarker,
): Record<string, unknown> {
  return {
    ...(session.metadata ?? {}),
    warm_session: marker,
...
function compatible(
  session: WarmWorkspaceSessionRecord,
  configuration: WarmWorkspaceSessionConfiguration,
): boolean {
...
export class WarmWorkspaceSessionError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = 'WarmWorkspaceSessionError';
  }
}

export function createWarmWorkspaceSessionCoordinator<T extends WarmWorkspaceSessionRecord>(
  dependencies: WarmWorkspaceSessionDependencies<T>,
) {
  const now = dependencies.now ?? (() => new Date());

  return {
    async ensure(configuration: WarmWorkspaceSessionConfiguration) {
    ...
    async claim(configuration: ClaimWarmWorkspaceSessionConfiguration) {
      const available = await dependencies.findAvailable();
      if (!available || available.sessionId !== configuration.sessionId) {
        throw new WarmWorkspaceSessionError(
