 ...
 *
 * Two paths inject workspace secrets into a sandbox — boot
 ...
 *
 *   1. FAIL-OPEN. Both wrapped `loadWorkspaceAgents` in `.catch(() => null)`, so
 *      any throw from the loader collapsed the grant to `undefined` — which
 *      `listWorkspaceSecretsSnapshotForUser` reads as "all". A transient git/parse
 *      failure silently handed the agent every workspace secret. Resolution now
 ...
 * (`grantFromLoadedAgents` returns null → `undefined` for an ungoverned workspace
...
export interface SessionSecretGrantInput {
  workspaceId: string;
  repoUrl: string;
  /** Git context. Absent (a workspace with no default branch) means the manifest
...
export async function resolveSessionSecretGrant(
  ...
  try {
    ...
    // loadWorkspaceAgents never throws: it swallows an unreadable manifest into a
    ...
    // would hand an ordinary prompt every workspace secret on a transient git
    // blip — the exact fail-open this module exists to close.
    loaded = await loadWorkspaceAgents(
      {
