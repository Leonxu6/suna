...
// ─── Guard (FIX-J adjustment #4): no whole-object RMW writer on workspaces.metadata ─
//
...
// file for `.update(workspaces)` and asserts any `metadata:` it SETs goes through
...
/** Every `metadata:` assignment inside a `.update(workspaces)…set({…})` window. */
function workspacesMetadataWrites(source: string): string[] {
  const values: string[] = [];
  const re = /\.update\(\s*workspaces\s*\)/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(source)) !== null) {
    // Window from the .update(workspaces) to the closing of its .set({…}) (bounded
...
describe('FIX-J guard — no whole-object RMW writer on workspaces.metadata', () => {
  const files = tsFiles(SRC_ROOT);

  test('the scan actually finds the workspaces.metadata writers', () => {
    const total = files.reduce((n, f) => n + workspacesMetadataWrites(readFileSync(f, 'utf8')).length, 0);
  ...
  test('every workspaces.metadata SET uses the atomic-merge helpers, never a raw object', () => {
    const offenders: string[] = [];
    for (const file of files) {
      for (const value of workspacesMetadataWrites(readFileSync(file, 'utf8'))) {
...
async function readMeta(workspaceId: string): Promise<Record<string, unknown>> {
  const [row] = await db.select({ metadata: workspaces.metadata }).from(workspaces).where(eq(workspaces.workspaceId, workspaceId)).limit(1);
  return (row?.metadata ?? {}) as Record<string, unknown>;
}

/** Reserve a switch (gen=1) and activate it, pinning the workspace to `provider`. */
async function pinWorkspace(workspaceId: string, provider: string, snapshotName: string): Promise<void> {
  const res = await reserveSwitchTransition(db, {
    accountId,
    sourceProvider: 'daytona',
    identity: { workspaceId, targetProvider: provider, commitSha: 'c1', baseRuntimeIdentity: 'kortix-default-r1', snapshotName },
  });
...
d('atomic merge preserves the routing pin (throwaway Postgres)', () => {
  test('a metadata writer holding a PRE-activation snapshot does NOT revert the pin', async () => {
    ...
    const staleSnapshot = await readMeta(workspaceId);
    expect(staleSnapshot[PIN_META_KEY]).toBeUndefined();

    // …the pin is activated in the gap…
    await pinWorkspace(workspaceId, 'platinum', snap);
    ...
    const after = await readMeta(workspaceId);
  ...
  test('the delete-key path removes a key while preserving the pin', async () => {
    const workspaceId = await freshWorkspace();
    await pinWorkspace(workspaceId, 'platinum', 'kortix-ppwarm-bbbb2222-000011112222');
    await db.update(workspaces).set({ metadata: metadataMerge({ triggers_paused: true }) }).where(eq(workspaces.workspaceId, workspaceId));

    await db.update(workspaces).set({ metadata: metadataMerge({}, ['triggers_paused']) }).where(eq(workspaces.workspaceId, workspaceId));
  ...
  test('nested experimental toggles never clobber each other or the pin, and cleanup drops the empty object', async () => {
    const workspaceId = await freshWorkspace();
    await pinWorkspace(workspaceId, 'platinum', 'kortix-ppwarm-cccc3333-000011112222');

    // Two DIFFERENT experimental sub-keys — a shallow `||` of the whole
    // `experimental` object would lose one; the nested merge keeps both.
    await db.update(workspaces).set({ metadata: metadataMergeSubtree('experimental', { agent_tunnel: true }) }).where(eq(workspaces.workspaceId, workspaceId));
    await db.update(workspaces).set({ metadata: metadataMergeSubtree('experimental', { llm_gateway: false }) }).where(eq(workspaces.workspaceId, workspaceId));

    let meta = await readMeta(workspaceId);
    ...
    await db.update(workspaces).set({ metadata: metadataClearSubtreeKey('experimental', 'agent_tunnel') }).where(eq(workspaces.workspaceId, workspaceId));
    meta = await readMeta(workspaceId);
    ...
    await db.update(workspaces).set({ metadata: metadataClearSubtreeKey('experimental', 'llm_gateway') }).where(eq(workspaces.workspaceId, workspaceId));
    meta = await readMeta(workspaceId);
  ...
  test('the generation CAS still rejects a stale activation (unchanged by the merge conversion)', async () => {
    ...
    const first = await reserveSwitchTransition(db, {
      accountId,
      sourceProvider: 'daytona',
      identity: { workspaceId, targetProvider: 'platinum', commitSha: 'c1', baseRuntimeIdentity: 'kortix-default-r1', snapshotName: snap1 },
    });
    // A newer request bumps the workspace generation to 2 (supersedes gen 1).
    await reserveSwitchTransition(db, {
      accountId,
      sourceProvider: 'daytona',
      identity: { workspaceId, targetProvider: 'platinum', commitSha: 'c2', baseRuntimeIdentity: 'kortix-default-r1', snapshotName: 'kortix-ppwarm-dddd4444-333344445555' },
    });
    ...
    expect((await readActiveRouting(db, workspaceId))?.activeProvider).toBeNull(); // pin untouched
  });
  ...
  test('the lease-epoch fence still rejects a zombie activation at a matching generation', async () => {
    ...
    const res = await reserveSwitchTransition(db, {
      accountId,
      sourceProvider: 'daytona',
      identity: { workspaceId, targetProvider: 'platinum', commitSha: 'c1', baseRuntimeIdentity: 'kortix-default-r1', snapshotName: snap },
    });
