...
// resolves `./git` to. `withWorkspaceGitAuth` is `./git` relative to this file,
...
mock.module('./git', () => ({
  ...realLibGit,
  withWorkspaceGitAuth: async (workspace: unknown) => ({
...
const { DEFAULT_AGENT_SENTINEL, extractAgents, resolveGovernedAgentGrant, loadWorkspaceAgents } =
  await import('../agents');
const { applyDefaultAgentV2, applyAgentBlockV2 } = await import('./agent-config-v2');

const fakeWorkspace = (overrides: Record<string, unknown> = {}) =>
  ({
    workspaceId: 'proj_blank',
...
describe('loadManifestForEdit — blank managed workspace (no kortix.yaml on disk yet)', () => {
  test('synthesizes a valid v2 manifest with a declared, resolvable default agent', async () => {
    ...
    const manifest = await loadManifestForEdit(fakeWorkspace());
    ...
    // manifest could ever land in git, so the workspace was permanently stuck
    // un-declarable.
  ...
  // — `loadWorkspaceAgents` (../agents.ts) → `readManifest` (../triggers.ts) —
  ...
  // subject/workspaceDefaultAgent shape sessions.ts actually passes.
  test('gap 1: loadWorkspaceAgents (the session-create read path) resolves the same way, given the same "no manifest" repo', async () => {
    manifestFile = null;

    const loaded = await loadWorkspaceAgents({
      workspaceId: 'proj_blank',
  ...
  // object `loadManifestForEdit` hands them for a blank workspace — the exact
  ...
  test('an already-committed manifest is read as-is, untouched (v1 stays v1)', async () => {
    ...
    const manifest = await loadManifestForEdit(fakeWorkspace({ metadata: {} }));
