...
// throughout apps/api/src/workspaces/maintenance.test.ts and friends.
...
mock.module('../git', () => ({
  ...
  readRepoFile: async (_workspace: unknown, path: string) => {
...
describe('compileAgentConfig — v1 is a compiler no-op', () => {
  ...
  test('returns null when kortix_version is missing entirely', () => {
    expect(compileAgentConfig({ workspace: { name: 'x' } })).toBeNull();
  });
...
describe('resolveCompiledAgentConfigForSession', () => {
  test('returns null when no manifest is found', async () => {
    manifestFile = null;
    expect(await resolveCompiledAgentConfigForSession(WORKSPACE)).toBeNull();
  });

  test('returns null for a v1 manifest — v1 workspaces are unaffected', async () => {
  ...
  test("reads each declared agent's conventional .md and returns the compiled JSON for a v2 manifest", async () => {
    ...
    readRepoFileCalls = [];
    const result = await resolveCompiledAgentConfigForSession(WORKSPACE);
