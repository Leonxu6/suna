...
import { decryptWorkspaceSecret, encryptWorkspaceSecret, getWorkspaceSecretValue } from '../secrets';
import { accountGithubInstallationStates, accountGithubInstallations, accountMembers, workspaceGitConnections, workspaceGitCredentials, workspaces, sessionSandboxes } from '@kortix/db';
...
// Memoized briefly (positive hits only): this runs on every workspace-scoped
...
export async function resolveGitHubRepoAuth(accountId: string, installationId?: string | null): Promise<{
  ...
  // that stores a workspace_credential — see resolveGitHubImportWithPat.)
...
export interface WorkspaceGitRemote {
  ...
  provider: string;
  /** managed | github_app | pat | workspace_credential | none */
  authMethod: string;
...
export async function getWorkspaceGitConnection(workspaceId: string): Promise<WorkspaceGitConnectionRow | null> {
  const [row] = await db
    .select()
    .from(workspaceGitConnections)
    .where(eq(workspaceGitConnections.workspaceId, workspaceId))
    .limit(1);
  return row ?? null;
}


export async function upsertWorkspaceGitConnection(input: {
  accountId: string;
  workspaceId: string;
  provider: string;
  repoUrl: string;
...
}): Promise<WorkspaceGitConnectionRow> {
  const now = new Date();
  const values = {
...
export async function getWorkspaceGitCredential(
  workspaceId: string,
  provider: string,
): Promise<WorkspaceGitCredentialRow | null> {
  const [row] = await db
    .select()
    .from(workspaceGitCredentials)
    .where(and(
      eq(workspaceGitCredentials.workspaceId, workspaceId),
      eq(workspaceGitCredentials.provider, provider),
    ))
    .limit(1);
  return row ?? null;
}
...
export async function upsertWorkspaceGitCredential(input: {
  accountId: string;
  workspaceId: string;
  provider: string;
  token: string;
  createdBy: string;
}): Promise<WorkspaceGitCredentialRow> {
  const now = new Date();
  const values = {
    accountId: input.accountId,
    workspaceId: input.workspaceId,
    provider: input.provider,
    authMethod: 'token',
    valueEnc: encryptWorkspaceSecret(input.workspaceId, input.token),
...
export function emptyGitRemote(): WorkspaceGitRemote {
  return {
    provider: 'generic',
    authMethod: 'none',
    repoId: null,
...
export function getWorkspaceGitRemote(workspace: WorkspaceRow, connection?: WorkspaceGitConnectionRow | null): WorkspaceGitRemote {
  if (connection) {
    return {
...
/**
 * Real upstream host git URL for a workspace. Prefers the explicit `upstreamUrl`
 * (set once the git-proxy refactor lands), then derives it from the provider's
 * coordinates, and finally falls back to `workspace.repoUrl` — correct for every
 * pre-refactor workspace, where repoUrl IS the real URL.
 */

export function resolveUpstreamUrl(workspace: WorkspaceRow, remote: WorkspaceGitRemote): string {
...
export function buildConnectionRef(workspace: WorkspaceRow, remote: WorkspaceGitRemote): GitConnectionRef {
  return {
    provider: remote.provider,
    upstreamUrl: resolveUpstreamUrl(workspace, remote),
...
export async function hasServerManagedGitAuth(workspace: WorkspaceRow): Promise<boolean> {
  const remote = getWorkspaceGitRemote(workspace, await getWorkspaceGitConnection(workspace.workspaceId));
...
export async function resolveWorkspaceGitAuth(workspace: WorkspaceRow): Promise<{
  auth?: GitHubAuthContext;
  authSource: 'app_installation' | 'pat' | 'managed' | 'workspace_credential' | 'none';
}> {
  const remote = getWorkspaceGitRemote(workspace, await getWorkspaceGitConnection(workspace.workspaceId));
  ...
  // adds remote latency to every Git-backed workspace read. The PAT is internal
  ...
  if (remote.provider === 'github' && remote.managed) {
    ...
    if (installId && isGithubAppConfigured()) {
      const repoName = remote.repoName ?? parseGitHubRepoUrl(remote.upstreamUrl ?? workspace.repoUrl)?.repo;
      try {
        ...
        console.warn(
          `[workspaces] failed to mint managed GitHub installation token for ${workspace.workspaceId}:`,
          err,
        );
      }
    }
  ...
  if (remote.provider === 'github' && remote.authMethod === 'github_app') {
    const repo = parseGitHubRepoUrl(remote.upstreamUrl ?? workspace.repoUrl);
    if (!repo) return { authSource: 'none' };
    const installation = remote.installationId
      ? await getAccountGitHubInstallation(workspace.accountId, remote.installationId)
      : (await listAccountGitHubInstallations(workspace.accountId)).find(
  ...
  if (remote.authMethod === 'workspace_credential') {
    const credential = await getWorkspaceGitCredential(workspace.workspaceId, remote.provider);
    if (credential) {
      return {
        auth: {
          token: decryptWorkspaceSecret(workspace.workspaceId, credential.valueEnc),
          source: 'workspace_credential',
        },
        authSource: 'workspace_credential',
      };
    }
  }

  const legacyToken = await getWorkspaceSecretValue(workspace.workspaceId, WORKSPACE_GIT_AUTH_SECRET_NAME);
  if (legacyToken) {
    return {
      auth: {
        token: legacyToken,
...
export async function withWorkspaceGitAuth(workspace: WorkspaceRow): Promise<WorkspaceRow & {
  gitAuthToken: string | null;
  gitAuthHeaders: Record<string, string>;
}> {
  const upstream = await resolveWorkspaceUpstream(workspace, 'write');
  const credential = parseBasicAuthHeader(upstream?.headers.Authorization);
  return {
    ...workspace,
    repoUrl: upstream?.url ?? workspace.repoUrl,
...
/**
 * Resolve a workspace to a real upstream git endpoint + short-lived host auth
 * headers — the single seam consumed by the Kortix git proxy (and, post-M2,
 * server-side git). Token resolution reuses `resolveWorkspaceGitAuth` (managed +
 * BYO GitHub / workspace credential); the backend formats
...
export async function resolveWorkspaceUpstream(
  ...
  const gitAuth = await resolveWorkspaceGitAuth(workspace);
  const ref = buildConnectionRef(workspace, remote);
 ...
 * git Basic/Bearer header) + the target workspace + the operation scope.
 *
  ...
  if (isKortixToken(token)) {
    ...
    if (result.type === 'sandbox') {
      ...
      const [sandbox] = await db
        ...
        .limit(1);
      if (!sandbox) {
        return { ok: false, status: 403, message: 'sandbox token is not scoped to this workspace' };
      }
      return { ok: true, workspace };
    }
...
export async function loadGitWorkspace(loaded: { row: WorkspaceRow }) {
  const resolved = await withWorkspaceGitAuth(loaded.row);
  return {
...
/** Lazy provider-neutral mirror access resolution from only a workspace id. */
export async function resolveWorkspaceGitAccessById(workspaceId: string): Promise<{
  repoUrl: string;
  token: string | null;
  ...
  try {
    const [row] = await db
      ...
      .limit(1);
    if (!row) return null;
    const resolved = await withWorkspaceGitAuth(row);
    return {
      repoUrl: resolved.repoUrl,
    ...
    console.warn(
      `[workspaces] lazy git-access resolve failed for ${workspaceId}:`,
...
/**
 * Resolve a workspace's upstream git auth token from just its id — loads the
 * workspace row, then runs the normal `resolveWorkspaceGitAuth` resolution
 * (managed App/PAT, BYO App, workspace_credential, legacy secret).
 *
 * This is the lazy fallback the shared mirror layer (`git/mirror.ts`) calls
 * when a caller reaches `refreshMirror()` without a token. The per-workspace
 * mirror is a shared resource hit by ~15 code paths; relying on every caller to
 * thread a non-null token is fragile, and `refreshMirror`'s per-workspace dedup
 ...
 *
 * Returns null when the workspace is gone or has no resolvable git auth. Never
 ...
 */
export async function resolveWorkspaceGitAuthTokenById(workspaceId: string): Promise<string | null> {
