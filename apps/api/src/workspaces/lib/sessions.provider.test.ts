...
// Per-workspace sandbox-provider override — precedence unit test. Deterministic:
...
//   explicit request › per-workspace pin (if enabled) › fallback (weighted balancer).
const ALLOWED = ['daytona', 'platinum', 'e2b'] as const;
const bothEnabled = (_p: string) => true;

describe('resolveSessionProvider (per-workspace provider override)', () => {
  test('explicit request wins over the pin + is used verbatim', () => {
    expect(
      resolveSessionProvider({ requested: 'platinum', workspacePin: 'daytona', allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual({ provider: 'platinum' });
  });

  test('explicit request not in ALLOWED → badRequest (becomes 400 upstream)', () => {
    expect(
      resolveSessionProvider({ requested: 'gcp', workspacePin: null, allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual({ badRequest: 'gcp' });
  });

  test('per-workspace pin is used when set + enabled + no explicit request', () => {
    expect(
      resolveSessionProvider({ requested: null, workspacePin: 'platinum', allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual({ provider: 'platinum' });
  });
  ...
  test('stale pin (provider since removed from ALLOWED) is ignored → fallback', () => {
    expect(
      resolveSessionProvider({ requested: null, workspacePin: 'platinum', allowed: ['daytona'], isEnabled: (p) => p === 'daytona' }),
    ).toEqual({ fallback: true });
  });

  test('pin allowed but keyless (isEnabled=false) → fallback, never a hard create failure', () => {
    expect(
      resolveSessionProvider({ requested: null, workspacePin: 'platinum', allowed: ALLOWED, isEnabled: (p) => p === 'daytona' }),
    ).toEqual({ fallback: true });
  });

  test('no request + no pin → fallback (weighted balancer runs)', () => {
    expect(
      resolveSessionProvider({ requested: null, workspacePin: null, allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual({ fallback: true });
  });
});

// Build-on-push warm-prebake target selection. A git push carries no per-session
// context, so it must warm the provider(s) a session on this workspace COULD land
// on: an enabled pin ⇒ that one; no/stale pin ⇒ every enabled provider. This is
// the parity fix — an unpinned workspace (or one that never used to warm Platinum)
// now pre-warms BOTH backends, while a pinned workspace warms only what it uses.
describe('warmPrebakeProviders (build-on-push provider parity)', () => {
  test('no pin → EVERY enabled provider (daytona + platinum + e2b) — full parity', () => {
    expect(
      warmPrebakeProviders({ workspacePin: null, allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual(['daytona', 'platinum', 'e2b']);
  });

  test('enabled pin → ONLY that provider (no wasted bake on the one it never uses)', () => {
    expect(
      warmPrebakeProviders({ workspacePin: 'platinum', allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual(['platinum']);
    expect(
      warmPrebakeProviders({ workspacePin: 'daytona', allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual(['daytona']);
  });
  ...
  test('Daytona pre-warm is NEVER dropped — daytona is warmed with no pin and when pinned', () => {
    ...
    // include 'daytona'.
    expect(warmPrebakeProviders({ workspacePin: null, allowed: ALLOWED, isEnabled: bothEnabled })).toContain('daytona');
    expect(warmPrebakeProviders({ workspacePin: 'daytona', allowed: ALLOWED, isEnabled: bothEnabled })).toContain('daytona');
  });
  ...
  test('stale/disabled/absent pin degrades to all-enabled (never bakes a provider that cannot run)', () => {
    ...
    expect(
      warmPrebakeProviders({ workspacePin: 'platinum', allowed: ALLOWED, isEnabled: (p) => p === 'daytona' }),
    ).toEqual(['daytona']);
    // pin a provider that has since left ALLOWED → all-enabled.
    expect(
      warmPrebakeProviders({ workspacePin: 'gcp', allowed: ALLOWED, isEnabled: bothEnabled }),
    ).toEqual(['daytona', 'platinum', 'e2b']);
  });

  test('single-provider deploy → exactly that one (platinum-only)', () => {
    expect(
      warmPrebakeProviders({ workspacePin: null, allowed: ['platinum'], isEnabled: (p) => p === 'platinum' }),
    ).toEqual(['platinum']);
  });

  test('only enabled providers are warmed — a listed-but-keyless provider is skipped', () => {
    expect(
      warmPrebakeProviders({ workspacePin: null, allowed: ALLOWED, isEnabled: (p) => p === 'daytona' }),
    ).toEqual(['daytona']);
  });
});
