// POST /workspaces/:id/manifest/validate historically always parsed `raw` as
// TOML, so a `kortix.yaml` workspace silently mis-parsed. This is a regression
// guard for the resolution rule: workspace manifestPath wins (via
// manifestFormatForPath), an explicit body `format` is the fallback for a
// workspace with no manifestPath on record, and `toml` is the last-resort
