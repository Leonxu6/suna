...
import {
  ...
  workspaceRequiresDeclaredAgents,
...
import { withWorkspaceGitAuth } from './git';
import { resolveSessionProvider } from './provider-precedence';
import { toPersistedSessionVisibility } from './persistence';
...
export async function countActiveWorkspaceSessions(accountId: string): Promise<number> {
  const [row] = await db
 ...
 */
export async function countActiveWorkspaceSessionsForOrigin(
  accountId: string,
  originRef: string,
): Promise<number> {
  const [row] = await db
    ...
    .where(
      and(
        eq(workspaceSessions.accountId, accountId),
        eq(workspaceSessions.originRef, originRef),
        inArray(workspaceSessions.status, [...ACTIVE_SESSION_STATUSES]),
      ),
    )
    .limit(1);
...
export async function enforcePerOriginSessionCap(
  ...
  const active = await countActiveWorkspaceSessionsForOrigin(accountId, originRef);
...
export async function countProvisioningWorkspaceSessions(workspaceId: string): Promise<number> {
  const [row] = await db
...
export async function enforceConcurrentSessionCap(
  ...
  const activeSessions = await countActiveWorkspaceSessions(accountId);
  if (activeSessions < limit) return null;

  recordAuditEvent({
    accountId,
    actorUserId: userId,
    action: `RATE_LIMIT ${request?.method ?? 'SYSTEM'} ${request?.path ?? 'workspace_session'}`,
    ...
    console.error('[workspaces] Failed to record session cap audit event:', error);
  });
...
export async function buildSessionSandboxEnvVars(input: {
  ...
  /** Resolved per-workspace `llm_gateway` experimental flag. Gateway ON →
  ...
  /** The workspace's base-branch tip SHA, resolved server-side (no tunnel). When
   *  it equals the image-baked scaffold's root SHA — true for a fresh workspace
   *  seeded from the starter with no per-workspace commit — the daemon skips the
  ...
  baseSha?: string;
  /** Workspace git context, so the running agent's `secrets` grant in `agents:`
  ...
  // secretsPrincipalUserId below. Every OTHER secret is workspace-wide (secret
  ...
  // for a v1 workspace (no `kortix_version: 2`) or any read/parse failure — no
  // KORTIX_COMPILED_AGENT_CONFIG key is emitted below in that case, so a v1
  // workspace's sandbox env is byte-for-byte unaffected by this. Gated on the
  ...
  const [sessionKaabRow] = await db
    .select({
      secretsAllowlist: workspaceSessions.secretsAllowlist,
      originRef: workspaceSessions.originRef,
  ...
  // behalf of any workspace manager/admin, and a per-user secret override (today
  // CODEX_AUTH_JSON) resolves per principal (`listResolvedWorkspaceSecrets`). If a
  ...
  try {
    runtimeSecrets = await listWorkspaceSecretsSnapshotForUser(
      input.workspaceId,
  ...
  // Guardrail: drop any workspace secret whose name would clobber the sandbox's
  ...
  if (droppedReserved.length > 0) {
    console.warn(
      `[session ${input.sessionId}] ignored ${droppedReserved.length} workspace secret(s) with reserved env names: ${droppedReserved.join(', ')}`,
    );
  }
  ...
  return {
    ...
    ...channelEnv,
    ...sessionContextEnv,
    KORTIX_WORKSPACE_SECRET_NAMES: runtimeSecrets.names.join(','),
    KORTIX_WORKSPACE_SECRETS_REVISION: runtimeSecrets.revision,
    ...
    ...buildSessionRuntimeEnv({
      ...
      repoUrl: config.KORTIX_GIT_PROXY ? proxyGitUrl(input.workspaceId) : input.repoUrl,
      baseRef: input.baseRef,
      agentName: input.agentName,
      apiUrl: deriveKortixApiBase(),
      frontendUrl: sandboxFrontendBaseUrl(),
      initialPrompt: input.initialPrompt,
      // Concrete session model after explicit → agent → workspace → account →
      ...
      // continue to work while the workspace experiment selects the ACP client.
...
/**
 * The Kortix git-proxy origin for a workspace — the UNIVERSAL client-facing git
 * URL. Clients clone/push this with a Kortix token; the API resolves the real
 * upstream + mints the host credential server-side.
 */

export function proxyGitUrl(workspaceId: string): string {
  return `${deriveKortixApiRoot(config.KORTIX_URL)}/v1/git/${workspaceId}.git`;
}
...
export async function createWorkspaceSession(input: {
   ...
   * Slack/Telegram channels) pass 'workspace' — those sessions belong to the
   * workspace, not to the stand-in owner they're attributed to, and would
  ...
  const { workspace, userId, body } = input;
  ...
  // connector, unbound aliases keep resolving to the WORKSPACE DEFAULT instead of
  // failing closed. It can only ever inherit the workspace default (never another
  ...
  // oversized handle can't reach the workspace_sessions.origin_ref column.
  ...
  if (secretsAllowlist && secretsAllowlist.length > 0) {
    const resolvedWorkspaceSecrets = await listResolvedWorkspaceSecrets(workspaceId, userId);
    // Every allowlisted identifier must name an existing runtime secret in the
    // workspace (KORTIX_*/connector rows are already excluded by the resolver), so
    // a typo fails fast at create rather than silently injecting nothing.
    const known = new Set(resolvedWorkspaceSecrets.map((r) => r.identifier.toUpperCase()));
    ...
    const collision = secretKeyCollisionInAllowlist(resolvedWorkspaceSecrets, secretsAllowlist);
    if (collision) {
      return {
        error: {
          status: 409,
  ...
  const baseRef = normalizeString(body.base_ref ?? body.baseRef) ?? workspace.defaultBranch;
  // The literal "default" is a non-binding legacy sentinel. It must not block
  // the configured workspace default. This rule applies to every caller,
  ...
  const freeModelsOnly = config.KORTIX_BILLING_INTERNAL_ENABLED
    ...
    : false;
  const llmGatewayEnabled = workspaceLlmGatewayEnabled(workspace.metadata);
  ...
  if (requestedModel) {
    ...
    try {
      ...
      console.error('[workspaces] Failed to resolve the session default model:', error);
      return {
        error: {
          status: 503,
          body: {
  ...
  // unification.md §2.1/§3 Phase 2). Only workspaces "subject" to enforcement (the
  // platform-wide flag, or a workspace stamped `metadata.require_declared_agents`
  ...
  // fail-safe for NON-subject workspaces). Non-subject workspaces take the exact
  // same path as before this flag existed (zero added I/O, zero behavior change).
  if (workspaceRequiresDeclaredAgents(workspace.metadata, config.KORTIX_REQUIRE_DECLARED_AGENTS)) {
  ...
  // workspace default and platform default remain the final fallbacks.
  const workspaceDefaultSandboxSlug = normalizeString(
    (workspace.metadata as Record<string, unknown> | null | undefined)?.default_sandbox_slug,
  );
  const sandboxSlug = resolveSessionSandboxSlug({
    ...
    workspace: workspaceDefaultSandboxSlug,
  });
  // Sandbox provider: explicit request › per-workspace pin (Customize → Settings) ›
  // weighted balancer. The pin lets you put ONE workspace on e.g. platinum regardless
  ...
  const picked = resolveSessionProvider({
    ...
    workspacePin:
      normalizeString(
        (workspace.metadata as Record<string, unknown> | null | undefined)?.default_sandbox_provider,
      ) ?? null,
    ...
    return {
      error: {
        ...
        body: {
          ...
          // The account that actually needs the upgrade — the workspace's owning
  ...
  let sessionRow: WorkspaceSessionRow | null = null;
  try {
    sessionRow = await db.transaction(async (tx) => {
      ...
      .values({
        ...
        // session-header control (visibility = workspace | restricted).
        createdBy: userId,
        visibility: toPersistedSessionVisibility(visibility),
        origin,
        originRef,
        secretsAllowlist,
    ...
    if (parsedRuntimeContext.context !== undefined) {
        await tx
          .insert(workspaceSessionRuntimeContexts)
          .values({
            sessionId,
          ...
          .returning({ sessionId: workspaceSessionRuntimeContexts.sessionId });
    }
      if (validatedConnectorBindings.bindings.length > 0) {
        await tx
          ...
          .returning({ sessionId: workspaceSessionConnectorBindings.sessionId });
      }
      return row;
    });
  } catch (error) {
    // Besides a randomUUID() collision on the PK / (workspace_id, branch_name)
    ...
    // create on a workspace pinned to it.) verify-live-schema.ts now gates that drift.
  ...
  void (async () => {
    ...
    try {
      ...
      const workspaceWithGitAuthPromise = withWorkspaceGitAuth(workspace).then((gitWorkspace) => {
        tl.mark('git-auth');
        return gitWorkspace;
      });
      ...
      const baseShaPromise = Promise.race([
        resolveCommitSha(workspace, baseRef).catch(() => undefined),
      ...
      const branchPromise: Promise<void> = branchAlreadyCreated
        ? Promise.resolve()
        : workspaceWithGitAuthPromise
            .then((workspaceWithGitAuth) =>
            createRemoteSessionBranch(workspaceWithGitAuth, sessionId, baseRef),
            )
            .then(() => {
      ...
      branchPromise.catch((err) => {
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[workspaces] Remote branch creation failed for session ${sessionId}:`, err);
        void mergeSessionMetadata({
          remote_branch: {
            status: 'failed',
            branch: sessionId,
      ...
      const provisionPromise = provisionSessionSandbox({
        ...
        metadata: { session_id: sessionId, workspace_id: workspaceId, ...(input.metadata ?? {}) },
        extraEnvVars,
        ...
        resolveGitWorkspace: async () => workspaceWithGitAuthPromise,
        baseRef,
        sandboxSlug,
      });
      ...
      console.error(`[workspaces] Failed to kick off sandbox for session ${sessionId}:`, err);
      try {
        await db
          .update(workspaceSessions)
          .set({
        ...
        console.error(`[workspaces] Failed to mark session ${sessionId} failed:`, markErr);
      }
