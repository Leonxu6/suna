import { workspaceSessions, workspaces } from '@kortix/db';
...
import { type GitBackedWorkspace, deleteRemoteSessionBranch } from './git';
import {
...
const globalForWorkspaceMaintenance = globalThis as typeof globalThis & {
  __kortixWorkspaceMaintenanceTimer?: MaintenanceTimer | null;
};
...
function maintenanceIntervalMs(): number {
  return positiveInt(
    process.env.KORTIX_WORKSPACE_MAINTENANCE_INTERVAL_MS,
...
function stallThresholdMs(): number {
  return positiveInt(process.env.KORTIX_WORKSPACE_MAINTENANCE_STALL_MS, maintenanceIntervalMs() * 3);
}
...
export async function sweepExpiredSessionBranches(now = new Date()): Promise<{
  ...
  const rows = await db
    .select({
      ...
      baseRef: workspaceSessions.baseRef,
      metadata: workspaceSessions.metadata,
      workspaceId: workspaces.workspaceId,
      repoUrl: workspaces.repoUrl,
      defaultBranch: workspaces.defaultBranch,
      manifestPath: workspaces.manifestPath,
    })
    .from(workspaceSessions)
    .innerJoin(workspaces, eq(workspaceSessions.workspaceId, workspaces.workspaceId))
    .where(
      and(
        inArray(workspaceSessions.status, [...TERMINAL_SESSION_STATUSES]),
        lt(workspaceSessions.updatedAt, cutoff),
        ne(workspaceSessions.branchName, workspaces.defaultBranch),
        ...
        sql`(${workspaceSessions.metadata}->'branch_gc'->>'deleted_at') IS NULL`,
        sql`NOT (
          COALESCE(${workspaceSessions.metadata}->>'open_pr', 'false') = 'true'
          OR COALESCE(${workspaceSessions.metadata}->>'has_open_pr', 'false') = 'true'
          OR lower(COALESCE(${workspaceSessions.metadata}->'pull_request'->>'state', '')) = 'open'
          OR lower(COALESCE(${workspaceSessions.metadata}->'github_pull_request'->>'state', '')) = 'open'
          OR lower(COALESCE(${workspaceSessions.metadata}->'pr'->>'state', '')) = 'open'
        )`,
      ),
    )
    // Oldest deletable rows first — deterministic drain so the planner's scan
    // order can't crowd them out of the batch.
    .orderBy(asc(workspaceSessions.updatedAt))
    .limit(GC_BATCH_SIZE);
  ...
  for (const row of rows) {
    ...
    try {
      const workspace: GitBackedWorkspace = {
        workspaceId: row.workspaceId,
        repoUrl: row.repoUrl,
        defaultBranch: row.defaultBranch,
        manifestPath: row.manifestPath,
      };
      const remoteDeleted = await deleteRemoteSessionBranch(workspace, row.branchName);
      await db
      ...
      errors += 1;
      console.error(`[workspace-maintenance] Failed to GC branch ${row.branchName}:`, err);
    }
  }
...
export async function runWorkspaceMaintenance(): Promise<void> {
  if (maintenanceRunning) {
    ...
    console.error(
      `[workspace-maintenance] STALLED — lock held for ${heldForMs}ms (threshold ${stallThresholdMs()}ms), forcing reset. ` +
  ...
  try {
    const [
      ...
      reapAndReconcileSandboxes().catch((err) => {
        console.warn(
          '[workspace-maintenance] reaper failed:',
      ...
      reconcileOrphanComputeSessions().catch((err) => {
        console.warn(
          '[workspace-maintenance] orphan-compute reconcile failed:',
          err instanceof Error ? err.message : err,
        );
        return { checked: 0, closed: 0, errors: 0 };
      }),
      // Session-side leak fix: reconcile workspace_sessions stuck in an ACTIVE
      ...
      reconcileStuckActiveSessions().catch((err) => {
        console.warn(
          '[workspace-maintenance] stuck-session reconcile failed:',
          err instanceof Error ? err.message : err,
        );
      ...
      reconcileUndeliveredPrompts().catch((err) => {
        console.warn(
          '[workspace-maintenance] undelivered-prompt reconcile failed:',
          err instanceof Error ? err.message : err,
        );
      ...
      reapOrphanProviderBoxes().catch((err) => {
        console.warn(
          '[workspace-maintenance] orphan-box reaper failed:',
          err instanceof Error ? err.message : err,
        );
      ...
      tickRunningComputeCharges().catch((err) => {
        console.warn(
          '[workspace-maintenance] compute tick failed:',
          err instanceof Error ? err.message : err,
        );
        return { settled: 0 };
      }),
      // Heal snapshot build-log rows orphaned at "building" by a process
      // restart/crash, globally across all workspaces.
      reconcileStaleBuilds().catch((err) => {
        console.warn(
          '[workspace-maintenance] stale-build reconcile failed:',
          err instanceof Error ? err.message : err,
        );
      ...
      reconcileSnapshotQuota().catch((err) => {
        console.warn(
          '[workspace-maintenance] snapshot quota GC failed:',
          err instanceof Error ? err.message : err,
        );
        return {
          orgTotal: 0,
    ...
    if (hadAction) {
      console.log('[workspace-maintenance] completed', {
        idle,
        orphanCompute,
        stuckSessions,
        undeliveredPrompts,
    ...
    console.log(
      `[workspace-maintenance] heartbeat idle_candidates=${idle.candidates} action=${hadAction}`,
    );
    ...
    try {
      ...
      if (billingLeak > 0) {
        console.warn(
          `[workspace-maintenance] BILLING INVARIANT VIOLATED: ${billingLeak} active compute session(s) with a non-running box`,
        );
      }
    } catch (err) {
      console.warn(
        '[workspace-maintenance] billing invariant check failed:',
        err instanceof Error ? err.message : err,
      );
    }
  } finally {
...
export function startWorkspaceMaintenance(): void {
  if (process.env.KORTIX_WORKSPACE_MAINTENANCE_ENABLED === 'false') return;
  if (globalForWorkspaceMaintenance.__kortixWorkspaceMaintenanceTimer) {
    clearInterval(globalForWorkspaceMaintenance.__kortixWorkspaceMaintenanceTimer);
  }
  maintenanceTimer = setInterval(() => {
    runWorkspaceMaintenance().catch((err) => {
      console.error('[workspace-maintenance] run failed:', err);
    });
  }, maintenanceIntervalMs());
  globalForWorkspaceMaintenance.__kortixWorkspaceMaintenanceTimer = maintenanceTimer;
}

export function stopWorkspaceMaintenance(): void {
  if (maintenanceTimer) {
  ...
  if (globalForWorkspaceMaintenance.__kortixWorkspaceMaintenanceTimer) {
    clearInterval(globalForWorkspaceMaintenance.__kortixWorkspaceMaintenanceTimer);
    globalForWorkspaceMaintenance.__kortixWorkspaceMaintenanceTimer = null;
  }
}
