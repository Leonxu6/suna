/**
 * Kortix trigger DSL — lives inside the workspace manifest (kortix.yaml; a
 * legacy v1 workspace may instead use kortix.toml) as a `triggers:` list of
 ...
 * `workspace_trigger_runtime` DB table.
 *
...
import { type GitBackedWorkspace, readManifestFromRepo } from './git';
 ...
 *  A workspace may instead use `kortix.yaml` ({@link MANIFEST_FILENAME_YAML}) —
 ...
 * or every v2 workspace's session grant resolution would fail closed/open
...
export interface GitTriggerSpec {
   ...
   * now that triggers are centralized — `kortix.yaml` for v2 workspaces,
   ...
   * "Default" — resolve the chain at fire time (agent → workspace → account →
  ...
  timezone: string;
  /**
   * For type=webhook only — the workspace_secrets key that holds the HMAC
  ...
  /**
   * For `sessionMode === 'pinned'` only — the exact `workspace_sessions.session_id`
   * this trigger loops. Null for `'fresh'`/`'reuse'`. Stored in the manifest as
   * `session_id` (portable) AND persisted on `workspace_trigger_runtime.session_id`.
   */
...
/** Result of `loadWorkspaceTriggers` — same shape callers got pre-refactor. */
...
/**
 * Read + parse the workspace's manifest. Returns null if no manifest file is
 * present (so the caller can treat the repo as "not a Kortix workspace yet").
...
export async function readManifest(
  ...
  try {
    // manifest_path can still say kortix.toml (an older workspace, or a stale
    ...
    // per-agent env/connector scoping ON for a yaml-only workspace (a missing
    // `agents:` read = grants resolve to null = unrestricted).
    const candidates = manifestCandidatePaths(workspace.manifestPath).map((c) => c.path);
    found = await readManifestFromRepo(workspace, candidates, workspace.defaultBranch);
  } catch (err) {
    ...
    // transient git failure then looks identical to "blank workspace", which
    // `loadWorkspaceAgents` answers with a synthesized `secrets: 'all'` manifest.
    // Callers that must fail CLOSED on an unreadable manifest opt into the
    // distinction here. See workspaces/lib/secret-grant.ts.
 ...
 * `agents.kortix`). Keeping the two in sync means a blank managed-git workspace
 ...
 * write path) AND `loadWorkspaceAgents` (./agents.ts, the session-create
 ...
 * literal `null` and 400'd AGENT_NOT_DECLARED on a blank workspace's very
 * first session).
 */
export const SYNTHESIZED_DEFAULT_AGENT_NAME = 'kortix';

/**
 * Build the synthesized v2 manifest for a workspace with no kortix.yaml/
 ...
 * the v2 body validators — the exact 400 a blank workspace's first
 * PUT /default-agent hit.
 */
export function synthesizeBlankManifest(workspace: {
  name?: string;
  ...
  return {
    ...
    raw: {
      kortix_version: 2,
      workspace: { name: workspace.name ?? '', description: '' },
    ...
    format: 'yaml',
    path: manifestCandidatePaths(workspace.manifestPath ?? undefined)[0].path,
  };
}
  ...
  rawTriggers.forEach((entry, index) => {
    ...
    if (seenSlugs.has(result.spec.slug)) {
      errors.push({
        ...
        error: `Duplicate trigger slug "${result.spec.slug}" — slugs must be unique within a workspace`,
      });
      return;
    }
...
/**
 * Walk a workspace: read its manifest, extract triggers. Convenience for
 ...
 * never throws.
 */
export async function loadWorkspaceTriggers(workspace: GitBackedWorkspace): Promise<LoadedTriggers> {
 ...
 * workspace manifest after a UI edit.
 */
...
function parseTriggerEntry(entry: unknown, index: number, filename: string = MANIFEST_FILENAME): ParseOk | ParseErr {
  ...
  if (!secretEnv) {
    return err(
      slug,
      'webhook triggers must declare `secret_env` pointing at a workspace_secrets entry',
    );
  }
  if (!/^[A-Z_][A-Z0-9_]*$/.test(secretEnv)) {
    return err(slug, `secret_env must look like a workspace_secrets name (got "${secretEnv}")`);
  }
  return {
    ok: true,
    spec: {
