...
export interface WorkspaceSeedFilesInput {
  workspaceName: string;
  repoFullName: string;
  template: StarterTemplateId;
  /** Accepted for API back-compat; no longer deterministically installed at
   *  provision time — see docs/specs/2026-07-13-marketplace-as-workspaces.md.
   *  Adding a marketplace item to a workspace is now an agent import
   *  (POST /:workspaceId/marketplace/install-session), which needs a session
   *  (and therefore an already-existing workspace) to run. */
...
/**
 * Seed a brand-new workspace's deterministic scaffold: just the starter's own
 ...
 * installed at provision time; adding one to a workspace is an agent import.
 */
export async function buildWorkspaceSeedFiles(input: WorkspaceSeedFilesInput): Promise<{
  files: Array<{ path: string; content: string }>;
  baseFiles: Array<{ path: string; content: string }>;
}> {
  const baseFiles = buildStarterFiles({
    workspaceName: 'kortix-workspace',
    repoFullName: 'kortix/kortix-workspace',
...
export interface WorkspaceSeedFilesFromItemInput {
   ...
   *  see `WorkspaceSeedFilesInput.marketplaceItems`. */
  extraMarketplaceItems: string[];
  now: string;
}

/**
 * Seed a brand-new workspace by cloning a `registry:project` marketplace item.
 * The minimal starter gives the new repo its opencode runtime (tools,
 * plugins, opencode.jsonc, base skills) exactly like any other new workspace;
 * the workspace item's own files (its kortix.yaml, agent personas, …) are
 * already inline on the catalog entry (`entry.item.files`, see
 * `buildWorkspaceTemplateRegistry` in apps/api/src/marketplace/catalog.ts) — no
 * install engine, no lock, just a plain file union with the destination
 * workspace's name interpolated in.
 */
export async function buildWorkspaceSeedFilesFromItem(input: WorkspaceSeedFilesFromItemInput): Promise<{
  files: Array<{ path: string; content: string }>;
  baseFiles: Array<{ path: string; content: string }>;
}> {
  const baseFiles = buildStarterFiles({
  ...
  // Workspace-item catalog content ships with `{{var}}` placeholders unresolved
  // (see `buildWorkspaceTemplateRegistry` in apps/api/src/marketplace/catalog.ts)
  // so they can be resolved here against the real destination workspace's name
  ...
  const vars = {
    workspaceName: input.workspaceName,
    projectName: input.workspaceName,
    repoFullName: input.repoFullName,
  };
 ...
 * `workspace.metadata.default_agent` (the read-optimized mirror session creation
 * and the LLM gateway's model-default resolution consult — see
 * workspaces/lib/sessions.ts and llm-gateway/resolution/default-model.ts) at
 * workspace-creation time, the same value `PUT /:workspaceId/default-agent`
 * writes later. Without this, a brand-new workspace's manifest declares a
 ...
 *
 * Never throws: a parse failure here must not fail workspace creation —
 * session creation's own manifest read (for MANDATORY DECLARED AGENTS
 * workspaces) is the actual source of truth and will surface a real error if
