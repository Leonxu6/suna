...
// `default_agent: kortix`, but never stamped workspace.metadata.default_agent
// with it — every session then stored the non-binding 'default' sentinel
// (see sessions.ts createWorkspaceSession), and an agent-scope model pin set on
...
// workspace.metadata at creation time, same as PUT /:workspaceId/default-agent.
describe('defaultAgentFromSeedFiles', () => {
  ...
  test('no default_agent declared → null (not every workspace needs one)', () => {
  ...
  test('malformed YAML → null, never throws (workspace creation must not fail over this)', () => {
