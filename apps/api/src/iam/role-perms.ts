...
//   workspace: manager > editor > member
//
...
//
// `member` is the floor workspace role (read + run sessions + fire triggers).
 ...
 *  still gates access by membership, so reads are scoped per-workspace. */
...
/** Manager-only actions on a workspace: delete + member management. */
...
/** Actions an editor gets on top of member. Editing the workspace and triggers
...
const EDITOR_EXTRAS: readonly string[] = [
  ...
  // today (all of these previously collapsed to workspace.write); a custom role
  ...
  // use the workspace (run the agent/chat) but can't browse the file tree via the
  // files page or view secret values; editor+ retains both. Makes "workspace
...
/** Baseline for the floor workspace role. `member` is the base *usable* role:
 *  it can read the workspace and start / run / stop sessions — i.e. actually use
 ...
 *  drive Kortix. What it CANNOT do is customize the workspace (edit settings,
...
const WORKSPACE_MEMBER_BASELINE: readonly string[] = [
  ...
  // Per-capability read leaves (IAM v1). NOTE: workspace.file.read and
  // workspace.secret.read are DELIBERATELY NOT here — they moved to EDITOR_EXTRAS
 ...
 *  FIRE the workspace's triggers (operate the automations) — still no editing,
...
/** Return the higher-ranked of two workspace roles. Used when a user's
 *  effective workspace role comes from multiple sources (direct membership
...
/** Owner/admin get implicit Manager on every workspace in their account.
 *  Member does not — they only see workspaces they've been added to. */
...
/** Does the workspace role grant this action? */
export function workspaceRoleAllows(role: WorkspaceRole, action: string): boolean {
