...
accountsRouter.openapi(
  ...
  return c.json({
    ...
    token_context: {
      auth_type: authType,
      workspace_id: (c.get('tokenWorkspaceId') as string | undefined) ?? null,
...
accountsRouter.openapi(
  ...
  // Optional workspace scope. A workspace-scoped key only ever works on that one
  // workspace (the auth middleware enforces the binding); it never widens access.
  const workspaceId =
  ...
  return c.json(
    {
      ...
      name: created.name,
      workspace_id: created.workspaceId ?? null,
