import { workspaceSessions, usageEvents } from '@kortix/db';
...
export const resolveSessionOriginRef = ttlMemo({
  ...
  loader: async (accountId: string, sessionId: string): Promise<string | null> => {
    const [row] = await db
      .select({ originRef: workspaceSessions.originRef })
      .from(workspaceSessions)
      .where(
        and(eq(workspaceSessions.sessionId, sessionId), eq(workspaceSessions.accountId, accountId)),
      )
      .limit(1);
