...
export interface AccountTokenValidationResult {
  ...
  userId?: string;
  tokenId?: string;
  /** Non-null = this token is scoped to one workspace; the auth
...
export interface CreateAccountTokenParams {
  ...
  userId: string;
  name: string;
  /** Non-null = token is scoped to one workspace. Session executor tokens also
 ...
 *  tokens scoped to that workspace (useful for the per-workspace token
