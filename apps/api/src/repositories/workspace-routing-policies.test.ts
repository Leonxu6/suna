...
const { getWorkspaceRoutingPolicy } = await import('./workspace-routing-policies');
...
test('reads current routing policy from the shared DB on every request', async () => {
  selectRows = [];
  expect(await getWorkspaceRoutingPolicy('multi-replica-workspace')).toBeNull();
  ...
  expect(await getWorkspaceRoutingPolicy('multi-replica-workspace')).toEqual({
    visionModel: 'glm-5.2',
...
test('round-trips a stored modelGenerationConfig blob verbatim', async () => {
  ...
  const policy = await getWorkspaceRoutingPolicy('gen-config-workspace');
