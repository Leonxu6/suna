...
import { PPWARM_PREFIX, excludePinnedTargets, legacyPerWorkspaceWarmImageName, perWorkspaceWarmImageName, ppwarmReapTargets, proj8, tpl8 } from './ppwarm-names';
...
describe('perWorkspaceWarmImageName — template-scoped naming', () => {
  test('produces the new (proj8, tpl8, hash12) shape', () => {
    const name = perWorkspaceWarmImageName(PROJ_A, 'tip1', BASE, 'my-template');
  ...
  test('omitting templateSlug defaults to the platform default template', () => {
    const withDefault = perWorkspaceWarmImageName(PROJ_A, 'tip1', BASE);
    const explicitDefault = perWorkspaceWarmImageName(PROJ_A, 'tip1', BASE, 'default');
    expect(withDefault).toBe(explicitDefault);
  });

  test('two templates for the same (workspace, tip, base) get different names', () => {
    const a = perWorkspaceWarmImageName(PROJ_A, 'tip1', BASE, 'default');
    const b = perWorkspaceWarmImageName(PROJ_A, 'tip1', BASE, 'custom-tpl');
    expect(a).not.toBe(b);
  ...
  test('the same template always yields the same tpl8, independent of tip/base', () => {
    const a = perWorkspaceWarmImageName(PROJ_A, 'tipX', 'kortix-default-r1', 'default');
    const b = perWorkspaceWarmImageName(PROJ_A, 'tipY', 'kortix-default-r2', 'default');
    const aTpl = a.slice(PPWARM_PREFIX.length).split('-')[1];
    const bTpl = b.slice(PPWARM_PREFIX.length).split('-')[1];
...
describe('ppwarmReapTargets — template-scoped reap', () => {
  test('two templates in one workspace never reap each other', () => {
    const defaultCur = perWorkspaceWarmImageName(PROJ_A, 'tipB', BASE, 'default');
    const defaultOld = perWorkspaceWarmImageName(PROJ_A, 'tipA', BASE, 'default');
    const customCur = perWorkspaceWarmImageName(PROJ_A, 'tipB', BASE, 'custom-tpl');
    const customOld = perWorkspaceWarmImageName(PROJ_A, 'tipA', BASE, 'custom-tpl');
  ...
  test('a moved tip of the SAME template is still reaped (regression guard)', () => {
    const cur = perWorkspaceWarmImageName(PROJ_A, 'tipB', BASE, 'default');
    const old = perWorkspaceWarmImageName(PROJ_A, 'tipA', BASE, 'default');
  ...
  test('an old-format currentName falls back to the pre-migration proj8-only scope', () => {
    ...
    const newFormatSibling = perWorkspaceWarmImageName(PROJ_A, 'tipZ', BASE, 'default');
  ...
  test('a proj8 collision between two different workspaces is still harmless', () => {
    expect(proj8(PROJ_A)).toBe(proj8(PROJ_B_COLLIDING));
    const aCur = perWorkspaceWarmImageName(PROJ_A, 'tipB', BASE, 'default');
    const bPinnedLive = perWorkspaceWarmImageName(PROJ_B_COLLIDING, 'tipQ', BASE, 'default');
...
describe('legacyPerWorkspaceWarmImageName — migration fallback', () => {
  ...
  test('reproduces the pre-migration two-segment shape', () => {
    const legacy = legacyPerWorkspaceWarmImageName(PROJ, TIP, BASE);
  ...
  test('is a different name from the new template-scoped one', () => {
    expect(legacyPerWorkspaceWarmImageName(PROJ, TIP, BASE)).not.toBe(
      perWorkspaceWarmImageName(PROJ, TIP, BASE, 'default'),
    );
  });

  test('hash excludes the template slug, unlike the new format', () => {
    const a = legacyPerWorkspaceWarmImageName(PROJ, TIP, BASE);
    const b = legacyPerWorkspaceWarmImageName(PROJ, TIP, BASE);
    expect(a).toBe(b);
    expect(perWorkspaceWarmImageName(PROJ, TIP, BASE, 'default')).not.toBe(
      perWorkspaceWarmImageName(PROJ, TIP, BASE, 'custom'),
    );
  });

  test('moves with the tip, so a stale legacy name is never served for a moved branch', () => {
    expect(legacyPerWorkspaceWarmImageName(PROJ, TIP, BASE)).not.toBe(
      legacyPerWorkspaceWarmImageName(PROJ, 'b'.repeat(40), BASE),
    );
  });

  test('a legacy name is not an on-bake reap target of a new-format tip', () => {
    const legacy = legacyPerWorkspaceWarmImageName(PROJ, TIP, BASE);
    const current = perWorkspaceWarmImageName(PROJ, 'c'.repeat(40), BASE, 'default');
    expect(ppwarmReapTargets(PROJ, current, [legacy, current])).not.toContain(legacy);
  });

  test('a legacy name is still recognised as this workspace ppwarm by the quota sweeps', () => {
    const legacy = legacyPerWorkspaceWarmImageName(PROJ, TIP, BASE);
