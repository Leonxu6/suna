 ...
 * SOME workspace, so neither the on-bake warm reaper nor the snapshot quota GC ever
 * deletes a LIVE pinned image.
 *
 * Both reapers group ppwarm images by the 8-hex `proj8` prefix over an ORG-WIDE
 * snapshot list, so a proj8 collision (two workspaces whose ids share the first 8
 * hex) could select another workspace's live cache as a "superseded" tip and delete
 ...
 *
 * A workspace's pin is written together at activation (see
 ...
 * across all workspaces in THIS environment's database. Best-effort: the callers
