...
const prov:any=await(await fetch(`${BASE}/v1/workspaces/provision`,{method:'POST',headers:H,body:JSON.stringify({name:`mk3-${Date.now()}`,seed_starter:true,account_id:ACC})})).json();
const exts:string[]=[];
for(let n=1;n<=3;n++){
