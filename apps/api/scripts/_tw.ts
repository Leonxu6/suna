...
const prov=await (await fetch('http://localhost:8008/v1/workspaces/provision',{method:'POST',headers:H,body:JSON.stringify({name:'tw2',seed_starter:true})})).json() as any;
const t0=Date.now();
const ses=await (await fetch(`http://localhost:8008/v1/workspaces/${prov.project_id}/sessions`,{method:'POST',headers:H,body:JSON.stringify({branch_already_created:false})})).json() as any;
