...
const prov: any = await (await fetch('http://localhost:8008/v1/workspaces/provision', { method: 'POST', headers: H, body: JSON.stringify({ name: `coldbranch-${Date.now()}`, seed_starter: true }) })).json();
const ses: any = await (await fetch(`http://localhost:8008/v1/workspaces/${prov.project_id}/sessions`, { method: 'POST', headers: H, body: JSON.stringify({ branch_already_created: false }) })).json();
