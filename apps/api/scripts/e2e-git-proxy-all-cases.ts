...
import { encryptWorkspaceSecret } from '../src/workspaces/secrets';
...
function ok(m: string) {
  process.stdout.write(`  \x1b[32mok\x1b[0m   ${m}\n`);
}
...
async function acct(): Promise<{ accountId: string; userId: string }> {
  const r: any = await db.execute(
    sql`select account_id, user_id from kortix.account_members order by joined_at limit 1`,
  );
...
function cloneThroughProxy(projectId: string, token: string): { ok: boolean; detail: string } {
  ...
  try {
    ...
    const res = spawnSync(
      ...
      [
        '-c',
        `http.${scheme}://${host}/.extraheader=AUTHORIZATION: basic ${b64}`,
        'clone',
        '--depth=1',
        url,
        join(dir, 'repo'),
...
async function main() {
  ...
  const A = await mkProject(
    'e2e-managed',
    aMeta,
    { managed: true, authMethod: 'github_app', installationId: INSTALL_ID },
    accountId,
  );
  ...
  const B = await mkProject(
    'e2e-byo-app',
    bMeta,
    { managed: false, authMethod: 'github_app', installationId: INSTALL_ID },
    accountId,
  );
  ...
  const C = await mkProject(
    'e2e-byo-pat',
    cMeta,
    { managed: false, authMethod: 'project_credential' },
    accountId,
  );
  if (PAT) {
    await db.execute(sql`
      insert into kortix.project_git_credentials (account_id, project_id, provider, auth_method, value_enc, created_by)
      values (${accountId}, ${C}, 'github', 'token', ${encryptWorkspaceSecret(C, PAT)}, ${userId})`);
  }
  ...
  const cases: Array<{ label: string; id: string; skip?: string }> = [
    ...
    {
      label: 'C byo-pat (project_credential)',
      id: C,
      skip: PAT ? undefined : 'no E2E_PAT provided',
    },
  ];
  ...
  for (const cs of cases) {
    process.stdout.write(`\n• ${cs.label}\n`);
    if (cs.skip) {
      process.stdout.write(`  \x1b[33mskip\x1b[0m ${cs.skip}\n`);
      continue;
    }
    ...
    cSb.ok
      ? ok(`clone via SANDBOX token (kortix_sb_)`)
      : bad(`clone via SANDBOX token — ${cSb.detail}`);
  ...
  cross === '403'
    ? ok(`\ncross-project: A-scoped PAT on B → 403`)
...
async function cleanup() {
  for (const name of created.tokenNames)
    await db.execute(sql`delete from kortix.account_tokens where name=${name}`).catch(() => {});
  await db
    .execute(sql`delete from kortix.account_tokens where name like 'e2e-allcases-%'`)
    .catch(() => {});
  for (const id of created.projects) {
    await db
      ...
      .catch(() => {});
    await db
      .execute(sql`delete from kortix.session_sandboxes where project_id=${id}`)
      .catch(() => {});
    await db
  ...
  if (created.installRow)
    await db
      .execute(
        sql`delete from kortix.account_github_installations where installation_id=${INSTALL_ID}`,
      )
      .catch(() => {});
}
