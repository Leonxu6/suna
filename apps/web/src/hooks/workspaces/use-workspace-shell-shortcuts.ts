...
import {
  useWorkspaceSessionTabsStore,
  CUSTOMIZE_TAB_ID,
} from '@/stores/workspace-session-tabs-store';
import { useCloseWorkspaceTab } from '@/hooks/workspaces/use-close-workspace-tab';
import { isDesktop } from '@/lib/desktop';

/**
 * Workspace-shell keyboard shortcuts — equivalents to the legacy dashboard's
 * tab-bar shortcuts, scoped to the workspace's open session tabs.
 *
 * | Shortcut              | Action                              |
 * |-----------------------|-------------------------------------|
...
export function useWorkspaceShellShortcuts({
  ...
  const reopenLastClosed = useWorkspaceSessionTabsStore((s) => s.reopenLastClosed);
  const closeWorkspaceTab = useCloseWorkspaceTab(workspaceId);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const sessMatch = pathname?.match(/^\/workspaces\/([^/]+)\/sessions\/([^/]+)/);
      const custMatch = pathname?.match(/^\/workspaces\/([^/]+)\/customize/);
      const urlWorkspace = sessMatch?.[1] ?? custMatch?.[1] ?? null;
      const activeTabId = sessMatch?.[2] ?? (custMatch ? CUSTOMIZE_TAB_ID : null);
      const tabs =
        useWorkspaceSessionTabsStore.getState().tabsByWorkspace[workspaceId] ?? [];

      const hrefForTab = (id: string) =>
        id === CUSTOMIZE_TAB_ID
          ? `/workspaces/${workspaceId}/customize`
      ...
      if (
        ...
        if (urlWorkspace !== workspaceId || !activeTabId) return;
        e.preventDefault();
  ...
  }, [workspaceId, pathname, router, tabSwitchModifier, onNewSession, closeWorkspaceTab, reopenLastClosed]);
}
