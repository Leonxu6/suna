...
export function useNewWorkspaceSession(
  ...
  resolveWarmSession?: () => Promise<Pick<WorkspaceSession, 'session_id'> | undefined>,
) {
      ...
      claimOrCreate()
        ...
        .catch((err) => {
          ...
          if (action === 'upgrade') {
            ...
            if (workspaceId && connector) {
              openConnectorGate({ workspaceId, connector, retry: () => startRef.current(opts) });
            } else {
