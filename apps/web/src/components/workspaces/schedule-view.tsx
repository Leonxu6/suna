...
function getTriggerName(t: WorkspaceTrigger): string {
...
function getTriggerSubtitle(t: WorkspaceTrigger): string {
...
/** workspace_secrets keys are UPPER_SNAKE_CASE — mirror the API's own validation
...
type SessionMode = WorkspaceTrigger['session_mode'];
...
function describeSessionStrategy(trigger: WorkspaceTrigger): string {
...
export function ScheduleView({ workspaceId, type }: { workspaceId: string; type: TriggerKind }) {
  const meta = TYPE_META[type];

  const tHardcodedUi = useTranslations('hardcodedUi');
  const queryClient = useQueryClient();
  const canWrite =
    useWorkspaceCan(workspaceId, WORKSPACE_ACTIONS.WORKSPACE_TRIGGER_CREATE).allowed === true;
  const queryKey = useMemo(() => ['workspace-triggers', workspaceId], [workspaceId]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<WorkspaceTrigger | null>(null);
    ...
    <>
      <CustomizeSectionWrapper
        ...
        description={
          type === 'cron'
            ? tHardcodedUi(
                'componentsWorkspacesTriggersView.line253JsxTextCronDrivenEntryPointsWhenAScheduleFires',
              )
            : tHardcodedUi(
                'componentsWorkspacesTriggersView.line272JsxTextSignedHttpEntryPointsWhenARequestHits',
              )
        }
        action={
        ...
        <div className="space-y-4">
          {triggersPaused && showContent && (
            <InfoBanner tone="warning" icon={AlertTriangle}>
              Triggers are paused for this workspace — scheduled runs and incoming webhooks are
          ...
          {triggersQuery.isLoading ? (
            ...
            <InfoBanner
              ...
              title={tHardcodedUi.raw(
                'componentsWorkspacesTriggersView.line1438JsxTextAccessRequired',
              )}
            >
              {tHardcodedUi.raw(
                'componentsWorkspacesTriggersView.line1440JsxTextYouDonAposTHavePermissionToView',
              )}
            </InfoBanner>
            ...
            <ErrorState
              size="sm"
              title={tHardcodedUi.raw(
                'componentsWorkspacesTriggersView.line1450JsxTextFailedToLoadTriggers',
              )}
...
function TriggerDetailSheet({
  ...
  const fire = useMutation({
    mutationFn: () => fireWorkspaceTrigger(workspaceId, trigger!.slug),
    onSuccess: (res) => {
      if (res.status === 'fired') {
  ...
  const toggle = useMutation({
    mutationFn: (enabled: boolean) => updateWorkspaceTrigger(workspaceId, trigger!.slug, { enabled }),
          ...
          <header className="space-y-2">
            ...
            {canWrite ? (
              <TriggerDetailToolbar
                ...
                fireLabel={tHardcodedUi.raw('componentsWorkspacesTriggersView.line623JsxTextFireNow')}
                onFire={() => fire.mutate()}
...
function AgentModelSection({
  ...
  const saveAgent = useMutation({
    mutationFn: (agent: string) => updateWorkspaceTrigger(workspaceId, trigger.slug, { agent }),
    onSuccess: () => {
  ...
  const saveModel = useMutation({
    mutationFn: (model: ModelKey | null) =>
      updateWorkspaceTrigger(workspaceId, trigger.slug, {
  ...
  const pinnableSessions = useQuery({
    queryKey: ['workspace-sessions', workspaceId, 'trigger-pin'],
    queryFn: () => listWorkspaceSessions(workspaceId),
    enabled: canWrite && modeDraft === 'pinned',
    staleTime: 30_000,
  });
  const saveSession = useMutation({
    mutationFn: (input: UpdateWorkspaceTriggerInput) =>
      updateWorkspaceTrigger(workspaceId, trigger.slug, input),
    onSuccess: () => {
...
function DeliveryFilterSection({
  ...
  const save = useMutation({
    mutationFn: () => updateWorkspaceTrigger(workspaceId, trigger.slug, { filter: rowsToFilter(rows) }),
    onSuccess: () => {
...
function NameSection({
  ...
  const save = useMutation({
    mutationFn: () => updateWorkspaceTrigger(workspaceId, trigger.slug, { name: draft.trim() }),
    onSuccess: () => {
          ...
          rows={[
            ...
            },
            {
              ...
              value: tI18nHardcoded.raw(
                'autoComponentsWorkspacesScheduleViewJsxTextOneOffFiresAacd67796',
              ),
            },
          ]}
        />
...
function WebhookSection({
  ...
  // `secret_env` names the workspace_secrets entry holding the HMAC secret — the
  ...
  const saveSecretEnv = useMutation({
    mutationFn: () =>
      updateWorkspaceTrigger(workspaceId, trigger.slug, { secret_env: secretEnvDraft.trim() }),
    onSuccess: () => {
    ...
    <div className="space-y-8">
      <section className="space-y-2">
        ...
        {trigger.secret_env ? (
          <InfoBanner tone="success" className="text-xs">
            {tHardcodedUi.raw(
              'componentsWorkspacesTriggersView.line744JsxTextSignedViaWorkspaceSecret',
            )}{' '}
          ...
          <InfoBanner tone="warning" className="text-xs">
            {tHardcodedUi.raw(
              'componentsWorkspacesTriggersView.line749JsxTextNoSigningSecretConfigured',
            )}
          </InfoBanner>
        )}
      </section>
      ...
      {canWrite && (
        <section className="space-y-2">
          ...
          />
          <p className="text-muted-foreground/70 text-xs leading-relaxed text-pretty">
            The workspace secret holding this webhook&apos;s HMAC key — the value itself never lives
      ...
      <section className="space-y-2">
        ...
        <p className="text-muted-foreground/70 text-xs leading-relaxed text-pretty">
          ...
          <code className="font-mono">
            {tHardcodedUi.raw('componentsWorkspacesTriggersView.line776JsxTextBody')}
          </code>{' '}
          and{' '}
          <code className="font-mono">
            {tHardcodedUi.raw('componentsWorkspacesTriggersView.line777JsxTextSecret')}
          </code>
          {tHardcodedUi.raw(
            'componentsWorkspacesTriggersView.line777JsxTextTheSignatureMustCoverTheRawRequestBody',
          )}
        </p>
      </section>
    </div>
...
function PromptTemplateSection({
  ...
  const save = useMutation({
    mutationFn: () => updateWorkspaceTrigger(workspaceId, trigger.slug, { prompt_template: draft }),
    onSuccess: () => {
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <Label>
          {tHardcodedUi.raw('componentsWorkspacesTriggersView.line817JsxAttrTitlePromptTemplate')}
        </Label>
        {!canWrite ? null : editing ? (
          <ButtonGroup>
            <Button
...
function MetaSection({ trigger }: { trigger: WorkspaceTrigger }) {
  return (
    <section className="space-y-2">
...
function CreateTriggerModal({
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      return createWorkspaceTrigger(workspaceId, {
        name: trimmedName,
        slug,
        type: sourceType,
              ...
              <div className="overflow-hidden rounded-md">
                <SourceCard
                  ...
                  description={tHardcodedUi.raw(
                    'componentsWorkspacesTriggersView.line1085JsxAttrDescriptionTimeBasedSchedule',
                  )}
                ...
                <SourceCard
                  ...
                  title="Webhook"
                  description={tHardcodedUi.raw(
                    'componentsWorkspacesTriggersView.line1092JsxAttrDescriptionFiresOnHttpRequest',
                  )}
            ...
            <>
              ...
              >
                <Input
                  ...
                  placeholder={tHardcodedUi.raw(
                    'componentsWorkspacesTriggersView.line1151JsxAttrPlaceholderDailyStandupDigest',
                  )}
                  maxLength={64}
                  autoFocus
                />
              ...
              >
                <Textarea
                  ...
                  placeholder={tHardcodedUi.raw(
                    'componentsWorkspacesTriggersView.line1162JsxAttrPlaceholderGenerateTheDailyStatusReportAndSaveIt',
                  )}
                  rows={5}
                />
                <p className="text-muted-foreground text-xs leading-relaxed text-pretty">
                  {tHardcodedUi.raw(
                    'componentsWorkspacesTriggersView.line1166JsxTextMustachePlaceholdersSupported',
                  )}{' '}
...
function WebhookSourceConfig({
  ...
  /** Optional override for the workspace_secrets key the manifest references
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <Label>
          {tHardcodedUi.raw('componentsWorkspacesTriggersView.line1325JsxTextSigningSecret')}
        </Label>
        <div className="flex gap-2">
          <Input
            value={secret}
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-muted-foreground text-xs leading-relaxed text-pretty">
          The workspace-secret key the manifest points at (
      ...
      <div className="bg-muted/40 space-y-1.5 rounded-md p-4">
        <div className="text-muted-foreground text-xs font-medium">
          {tHardcodedUi.raw('componentsWorkspacesTriggersView.line1349JsxTextExternalUrl')}
        </div>
        ...
        <p className="text-muted-foreground text-xs leading-relaxed text-pretty">
          {tHardcodedUi.raw(
            'componentsWorkspacesTriggersView.line1355JsxTextWeGenerateTheTriggerIdOnCreatePost',
          )}{' '}
          <code className="font-mono text-xs">X-Kortix-Signature</code>
          {tHardcodedUi.raw('componentsWorkspacesTriggersView.line1357JsxTextHeaderSetTo')}{' '}
          <code className="font-mono text-xs">
            {tHardcodedUi.raw('componentsWorkspacesTriggersView.line1359JsxTextSha256LtHmacGt')}
          </code>{' '}
          {tHardcodedUi.raw(
            'componentsWorkspacesTriggersView.line1360JsxTextComputedOverTheRawBodyWithTheSecret',
          )}
        </p>
      </div>
    </div>
...
function DeleteDialog({
  ...
  workspaceId: string;
  target: WorkspaceTrigger | null;
  onClose: () => void;
  onDeleted: () => void;
}) {
  const tHardcodedUi = useTranslations('hardcodedUi');
  const remove = useMutation({
    mutationFn: (trigger: WorkspaceTrigger) => deleteWorkspaceTrigger(workspaceId, trigger.slug),
    onSuccess: () => {
  ...
  return (
    <AlertDialog open={!!target} onOpenChange={(open) => !open && onClose()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            {tHardcodedUi.raw('componentsWorkspacesTriggersView.line1394JsxTextDeleteTrigger')}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {target && (
              <>
                ...
                {tHardcodedUi.raw(
                  'componentsWorkspacesTriggersView.line1402JsxTextAndStopAnyFutureRunsFromItPast',
                )}
              </>
            )}
          </AlertDialogDescription>
