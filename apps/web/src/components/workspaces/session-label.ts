import type { WorkspaceRuntimeSession, WorkspaceSession } from '@kortix/sdk';

/**
 * Canonical helpers for resolving a workspace session's display label and its
...
/** The root opencode session a workspace session is pinned to (if synced). */
export function rootOpenCodeSession(session: WorkspaceSession): WorkspaceRuntimeSession | null {
...
export function directSubsessions(session: WorkspaceSession): WorkspaceRuntimeSession[] {
...
export function sessionSource(session: WorkspaceSession): SessionSource {
...
export function matchesSessionFilter(session: WorkspaceSession, filter: SessionFilterValue): boolean {
 ...
 */
export function sessionDisplayLabel(session: WorkspaceSession): string {
  const metadataName =
