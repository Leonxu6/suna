 ...
 */
export function SessionCacheWarmer({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    for (const session of sessions ?? []) {
      const seed = workspaceSessionStartSeed(session);
      if (!seed) continue;
      const key = sessionStartKey(workspaceId, session.session_id);
  ...
  }, [workspaceId, queryClient, sessions, targets]);
