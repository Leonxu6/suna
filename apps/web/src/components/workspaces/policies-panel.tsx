...
/**
 * Workspace-wide approval rules for tool calls. Source of truth = `kortix.yaml`;
 ...
 * `listWorkspacePolicies` response.
 *
 ...
 * (reading 'map')` in the workspace layout's `?c=global` → `GlobalRulesPanel` →
 * `PoliciesPanel` path) can be unit-tested directly without a react-query
 * harness. `listWorkspacePolicies` is typed to always return `policies` /
 * `defaultMode`, but a 200 with a missing/`null` `policies` field (empty/new
 * workspace, a backend shape gap, a partial response) is a valid HTTP outcome; the
...
export function PoliciesPanel({ workspaceId }: { workspaceId: string }) {
  ...
  const queryKey = useMemo(() => ['workspace-policies', workspaceId] as const, [workspaceId]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      const payload: WorkspacePolicy[] = draft
        .map((d) => ({ match: d.match.trim(), action: d.action }))
        .filter((p) => p.match.length > 0);
      return setWorkspacePolicies(workspaceId, payload, defaultMode);
    },
    onSuccess: () => {
    ...
    return (
      <InfoBanner
        ...
        title={tI18nHardcoded.raw(
          'autoComponentsWorkspacesPoliciesPanelJsxAttrTitleAdminAccessRequired34f66101',
        )}
      >
        {tI18nHardcoded.raw(
          'autoComponentsWorkspacesPoliciesPanelJsxTextOnlyWorkspaceManagersCan603f67fc',
        )}
      </InfoBanner>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard
        title={tI18nHardcoded.raw(
          'autoComponentsWorkspacesPoliciesPanelJsxAttrTitleDefaultBehavior16e16541',
        )}
        description={tI18nHardcoded.raw(
          'autoComponentsWorkspacesPoliciesPanelJsxAttrDescriptionWhatHappensWhen4b8b2205',
        )}
      >
        {query.isLoading ? (
      ...
      <SectionCard
        title="Rules"
        description={tI18nHardcoded.raw(
          'autoComponentsWorkspacesPoliciesPanelJsxAttrDescriptionTopToBottom4926dcea',
        )}
        ...
        action={
          ...
          >
            ...
            {tI18nHardcoded.raw('autoComponentsWorkspacesPoliciesPanelJsxTextAddRulea294c4bd')}
          </Button>
        }
        flush
      >
        {parseErrors.length > 0 && (
          <div className="px-4 pt-3">
            <InfoBanner
              tone="warning"
              title={tI18nHardcoded.raw(
                'autoComponentsWorkspacesPoliciesPanelJsxAttrTitleKortixTomlHad8db85c74',
              )}
            >
        ...
        {query.isLoading ? (
          ...
          <EmptyState
            ...
            size="sm"
            title={tI18nHardcoded.raw(
              'autoComponentsWorkspacesPoliciesPanelJsxAttrTitleNoRulesYet795c2fb5',
            )}
            description={tI18nHardcoded.raw(
              'autoComponentsWorkspacesPoliciesPanelJsxAttrDescriptionRequireApprovalFor7e7b9477',
            )}
            action={
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Input
                      ...
                      placeholder={tI18nHardcoded.raw(
                        'autoComponentsWorkspacesPoliciesPanelJsxAttrPlaceholderStripeChargesCreate0ff0fa54',
                      )}
      ...
      {dirty && (
        ...
        >
          <div className="text-muted-foreground text-xs">
            {tI18nHardcoded.raw(
              'autoComponentsWorkspacesPoliciesPanelJsxTextUnsavedChangesSavingCommitse36b25bc',
            )}{' '}
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={() => save.mutate()} disabled={save.isPending}>
              ...
              {tI18nHardcoded.raw('autoComponentsWorkspacesPoliciesPanelJsxTextSaveChanges3a1ef407')}
            </Button>
          </div>
        </div>
      )}
