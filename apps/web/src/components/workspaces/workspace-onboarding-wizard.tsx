...
/**
 * Workspace onboarding — a full-screen, guided setup flow for a brand-new workspace.
 *
 ...
 *
 * Self-gates: only renders while the workspace's onboarding status is 'pending'
 ...
 * mounted wizard adds ~nothing to the workspace bundle until it actually opens.
 *
...
import { useWorkspaceOnboarding } from '@/hooks/workspaces/use-workspace-onboarding';
...
/** Lazy — keeps the giant connectors-view module out of the workspace bundle. */
...
export function WorkspaceOnboardingWizard({ workspaceId }: { workspaceId: string }) {
  ...
  const onboarding = useWorkspaceOnboarding(workspaceId);
  ...
  const refreshConnectors = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['workspace-connectors', workspaceId] });
      ...
      >
          ...
          <div className="w-full max-w-2xl py-6">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {stepId === 'slack' && <SlackStep workspaceId={workspaceId} />}
        ...
        {stepId !== 'done' && (
          <div className="border-border/60 bg-muted/20 flex items-center justify-between border-t px-5 py-3.5 md:px-8">
            ...
            <StepPrimaryAction stepId={stepId} workspaceId={workspaceId} onNext={next} />
          </div>
        )}
      </motion.div>
...
function StepPrimaryAction({
  ...
  const slackInstall = useSlackInstall(stepId === 'slack' ? workspaceId : null);
...
function ToolsStep({
  ...
  const connect = useToolConnect(workspaceId, onConnected);

  const appsQuery = useInfiniteQuery({
    queryKey: ['onboarding-tools', workspaceId, q],
    queryFn: ({ pageParam }) =>
...
function SlackStep({ workspaceId }: { workspaceId: string }) {
          ...
          {customOpen && (
            <div className="mt-3">
              <Suspense fallback={<Skeleton className="h-24 w-full rounded-2xl" />}>
                <SlackConnectForm workspaceId={workspaceId} onConnected={() => install.refetch()} />
              </Suspense>
            </div>
          )}
        </div>
