...
const DEFAULT_WORKSPACE_SHARING: SharingSelection = { mode: 'workspace', memberIds: [], groupIds: [] };
...
export function useChatGptSubscriptionConnected(workspaceId: string, enabled = true) {
...
export function useShowChatGptConnectPrompt(workspaceId: string) {
...
export function ChatGptSubscriptionConnect({
  workspaceId,
  sharing = DEFAULT_WORKSPACE_SHARING,
  ...
  }, [workspaceId, sharing, queryClient, onConnected]);
    ...
    <div className="bg-popover rounded-md border p-4">
      ...
      {showSharingPicker ? null : (
        <p className="text-muted-foreground mt-3 text-xs">
          Saved for everyone on this workspace. Restart a running session sandbox to pick it up.
        </p>
      )}
    </div>
  );
