// Tests for workspace-role-descriptors.ts.
//
...
describe('WORKSPACE_ROLE_DESCRIPTORS', () => {
  test('covers every workspace role', () => {
    expect(Object.keys(WORKSPACE_ROLE_DESCRIPTORS).sort()).toEqual(
      ['editor', 'manager', 'member'],
    );
  });

  test('every descriptor has non-empty label, blurb, and summary', () => {
    for (const [key, d] of Object.entries(WORKSPACE_ROLE_DESCRIPTORS)) {
  ...
  test('label is title-cased (so badges look right)', () => {
    for (const d of Object.values(WORKSPACE_ROLE_DESCRIPTORS)) {
  ...
  test('member (floor role) blurb communicates it can use the workspace (start sessions)', () => {
    ...
    // is useless).
    expect(WORKSPACE_ROLE_DESCRIPTORS.member.blurb.toLowerCase()).toMatch(/session|chat|use/);
  });

  test('manager blurb communicates member management', () => {
    // The thing that distinguishes Manager from Editor in real use.
    const text = WORKSPACE_ROLE_DESCRIPTORS.manager.blurb.toLowerCase();
  ...
  test('editor blurb references the member role (additive framing)', () => {
    ...
    expect(WORKSPACE_ROLE_DESCRIPTORS.editor.blurb.toLowerCase()).toContain('member');
  });
  ...
  test('member copy calls out firing triggers (its defining capability over read-only)', () => {
    ...
    expect(WORKSPACE_ROLE_DESCRIPTORS.member.summary.toLowerCase()).toContain('trigger');
    expect(WORKSPACE_ROLE_DESCRIPTORS.member.blurb.toLowerCase()).toContain('trigger');
  });
});

describe('WORKSPACE_ROLES_ASCENDING', () => {
  test('matches role hierarchy member → editor → manager', () => {
    expect(WORKSPACE_ROLES_ASCENDING).toEqual(['member', 'editor', 'manager']);
  });
});
...
describe('ACCOUNT_ROLE_DESCRIPTORS', () => {
  ...
  test('member blurb explains they need explicit workspace access', () => {
    ...
    expect(text).toMatch(/workspace|group|added/);
  });
});
