...
/* The Workspaces tab — driven by the CLI: `kortix init` adds a `draft`, `kortix
 * ship` flips it `shipping` → `live`. Shared by the standalone hero section and
 * the modular demo (where it renders its empty state). */

function WorkspaceStatusBadge({ status }: { status: WorkspaceCard['status'] }) {
  if (status === 'draft') {
    return (
...
function WorkspaceRow({ workspace }: { workspace: WorkspaceCard }) {
  const { name, status, files, branch, repo, url } = workspace;
...
export function WorkspacesPage({ workspaces }: { workspaces: WorkspaceCard[] }) {
    ...
    <div>
      <PageHead
        ...
        sub={tI18nHardcoded.raw(
          'autoComponentsHomeInteractiveDemoPagesWorkspacesPageJsxAttrSub90e41adc',
        )}
        action={
          <Button variant="default" size="sm">
            <Plus className="size-3.5" />{' '}
            {tI18nHardcoded.raw(
              'autoComponentsHomeInteractiveDemoPagesWorkspacesPageJsxTextNewba012455',
            )}
          </Button>
        }
      />
      ...
      {workspaces.length === 0 ? (
        <div className="border-border/60 text-muted-foreground flex min-h-[18rem] flex-col items-center justify-center gap-3 rounded-md border border-dashed py-12 text-center">
          ...
          <div className="space-y-1">
            <div className="text-foreground text-sm font-medium">
              {tI18nHardcoded.raw(
                'autoComponentsHomeInteractiveDemoPagesWorkspacesPageJsxTextNo21ae83eb',
              )}
            </div>
            <div className="text-muted-foreground text-xs">
              ...
              <span className="text-foreground font-mono">
                {tI18nHardcoded.raw(
                  'autoComponentsHomeInteractiveDemoPagesWorkspacesPageJsxTextKortixa20df451',
                )}
              </span>{' '}
              {tI18nHardcoded.raw(
                'autoComponentsHomeInteractiveDemoPagesWorkspacesPageJsxTextIn2531c25e',
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2.5">
          <AnimatePresence initial={false}>
            {workspaces.map((p) => (
              <WorkspaceRow key={p.name} workspace={p} />
            ))}
          </AnimatePresence>
        </div>
      )}
