...
export interface MarketplaceItem {
  ...
  /** Set when this item also ships inside a whole workspace (e.g. a starter skill);
   *  the card badges it "Part of <workspace>". */
...
export function defaultWorkspaceMarketplaceItems(
  ...
  return (items ?? [])
    .filter((item) => item.defaultWorkspaceInstall)
    .sort(
      (a, b) =>
        (a.defaultWorkspaceInstallOrder ?? 999) - (b.defaultWorkspaceInstallOrder ?? 999) ||
 ...
 *  an existing workspace's kortix.yaml, which isn't safe to do deterministically.
