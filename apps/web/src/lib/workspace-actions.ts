 ...
 * (role-perms.ts `WORKSPACE_MEMBER_BASELINE`) are visible to every workspace role;
 * `secrets` gates on workspace.secret.read, which is DELIBERATELY editor-tier
 ...
 *   actual Slack connect/disconnect routes assert workspace.connector.write.
 * - `git` surfaces repository metadata and clone instructions; pushes remain
 *   separately gated by workspace.gitops.push.
 * - sandbox/settings/marketplace/computers have no dedicated read leaf, so
 *   they stay visible on workspace.read and gate writes on the closest real leaf
...
> = {
  agents: { read: WORKSPACE_ACTIONS.WORKSPACE_AGENT_READ, write: WORKSPACE_ACTIONS.WORKSPACE_AGENT_WRITE },
  skills: { read: WORKSPACE_ACTIONS.WORKSPACE_SKILL_READ, write: WORKSPACE_ACTIONS.WORKSPACE_SKILL_WRITE },
  commands: {
  ...
  // (workspace triggers, filtered client-side by `type`) — there is no
  ...
  // enforcement point: workspace.trigger.read/create. Update/delete stay gated
  ...
  git: { read: WORKSPACE_ACTIONS.WORKSPACE_GITOPS_READ, write: WORKSPACE_ACTIONS.WORKSPACE_GITOPS_PUSH },
  review: { read: WORKSPACE_ACTIONS.WORKSPACE_REVIEW_READ, write: WORKSPACE_ACTIONS.WORKSPACE_REVIEW_ACT },
  members: {
    read: WORKSPACE_ACTIONS.WORKSPACE_MEMBERS_READ,
    write: WORKSPACE_ACTIONS.WORKSPACE_MEMBERS_MANAGE,
  },
  marketplace: { read: WORKSPACE_ACTIONS.WORKSPACE_READ, write: WORKSPACE_ACTIONS.WORKSPACE_GITOPS_PUSH },
  ...
  // on each mutation route, so visibility gates on workspace.read.
  'llm-management': { read: WORKSPACE_ACTIONS.WORKSPACE_READ, write: WORKSPACE_ACTIONS.WORKSPACE_WRITE },
  'llm-overview': { read: WORKSPACE_ACTIONS.WORKSPACE_READ, write: WORKSPACE_ACTIONS.WORKSPACE_WRITE },
  'llm-providers': { read: WORKSPACE_ACTIONS.WORKSPACE_READ, write: WORKSPACE_ACTIONS.WORKSPACE_WRITE },
  'llm-logs': { read: WORKSPACE_ACTIONS.WORKSPACE_READ, write: WORKSPACE_ACTIONS.WORKSPACE_WRITE },
  'llm-budgets': { read: WORKSPACE_ACTIONS.WORKSPACE_READ, write: WORKSPACE_ACTIONS.WORKSPACE_WRITE },
  ...
  sandbox: { read: WORKSPACE_ACTIONS.WORKSPACE_READ, write: WORKSPACE_ACTIONS.WORKSPACE_CUSTOMIZE_WRITE },
 ...
 * (Previously this ALSO required `workspace.customize.write`, which blanked the
 ...
 * here — it's the standalone /workspaces/[id]/files page, gated on workspace.file.read.
 */
