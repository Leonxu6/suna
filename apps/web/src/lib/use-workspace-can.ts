...
/**
 * useWorkspaceCan — per-action capability gating for a Workspace, for the current
 ...
 * for the exact leaf action a route asserts (e.g. workspace.gitops.push), so a
 ...
 * the workspace itself (`['workspace', workspaceId]`), so callers pass only workspaceId.
 *
 * NOT a security boundary — the API re-checks every mutating route via
 * assertWorkspaceCapability. This only decides what to show/enable. Probes are
 ...
 * that loaded the workspace under a DIFFERENT query key like ['workspace-detail'])
 ...
 * shared ['workspace', workspaceId] cache.
 */
...
/**
 * Batch variant — one HTTP roundtrip for N workspace actions. Returns a map keyed
