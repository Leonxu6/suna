...
/**
 * Cap how many workspaces we retain tab state for. The maps below are keyed by
 * workspaceId and would otherwise grow one entry per workspace ever opened, with no
 * eviction. Keep the N most-recently-touched workspaces.
 */
...
interface Actions {
  ...
  /** Bulk-replace tabs (e.g. when workspace loads). */
