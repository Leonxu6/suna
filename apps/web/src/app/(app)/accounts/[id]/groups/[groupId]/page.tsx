...
function GroupMembersCard({
  ...
  // and admins implicitly have Manager on every workspace, which overrides
  // any group-level role on that workspace. Worth flagging here so an
...
function GroupWorkspaceGrantsCard({
  ...
  const detachMutation = useMutation({
    // detach the grant via the per-workspace route — that's the one gated
    // by workspace.members.manage and the canonical write surface.
    ...
    <div className="space-y-4">
      ...
      <AttachToWorkspaceDialog
        ...
        onAttached={(attachedWorkspaceId) => {
          ...
          queryClient.invalidateQueries({ queryKey: ['workspace-access', attachedWorkspaceId] });
          queryClient.invalidateQueries({ queryKey: ['workspace', attachedWorkspaceId] });
          setAttachOpen(false);
        }}
      />
...
// per-workspace group-grants endpoint (server-side gate on
// workspace.members.manage matches our client-side filter).
...
function AttachToWorkspaceDialog({
  ...
  const workspacesQuery = useQuery({
    queryKey: ['workspaces-for-account', accountId],
  ...
  const candidates = useMemo(() => {
    ...
    return all
      .filter(
        (p) => p.effective_workspace_role === 'manager' && !attachedWorkspaceIds.has(p.workspace_id),
      )
