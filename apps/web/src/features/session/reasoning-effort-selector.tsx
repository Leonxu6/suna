 ...
 *
 * *** WHY THIS WRITES A WORKSPACE-LEVEL SETTING, NOT A PER-MESSAGE ONE ***
 ...
 *
 * The one path that reliably reaches the wire today is the per-workspace
 ...
 * `workspace_llm_routing_policies.model_generation_config` (same table/API the
 * Gateway → Routing settings page's "Generation defaults" panel writes).
 * This component reads/writes that same config, scoped to
 * (this workspace, this exact wire model) — every session in the workspace
 ...
 * (`WORKSPACE_CUSTOMIZE_WRITE`, editor+) — a plain workspace member can see the
...
import type { GatewayWorkspaceRoutingPolicy } from '@kortix/sdk';
...
export interface ReasoningEffortControl {
   ...
   *  workspace to scope the setting to — render nothing. */
  visible: boolean;
  /** The model's own effort labels (e.g. ['none','low','medium','high','xhigh','max']). */
  values: string[];
  /** Currently configured effort for this (workspace, model), or null = model default. */
  current: string | null;
  /** Whether the current user can change it (editor+ on the workspace). */
  canWrite: boolean;
 ...
 * workspace's `modelGenerationConfig`, preserving any other generation-config
...
export function applyReasoningEffort(
  modelGenerationConfig: GatewayWorkspaceRoutingPolicy['modelGenerationConfig'],
  wireModel: string,
  next: string | null,
): GatewayWorkspaceRoutingPolicy['modelGenerationConfig'] {
 ...
 * reasoning-effort control on a given (model, workspace). Thin React/query
...
export function useReasoningEffortControl(
  ...
  const current =
    wireModel && routing.data
      ? (routing.data.workspace.modelGenerationConfig?.[wireModel]?.reasoningEffort ?? null)
      : null;
  const canWrite = routing.data?.capabilities?.write ?? false;

  const setEffort = (next: string | null) => {
    if (!wireModel || !workspaceId || !routing.data) return;
    const policy: GatewayWorkspaceRoutingPolicy = routing.data.workspace;
    routing.set.mutate({
      ...policy,
  ...
  return {
    visible: !!workspaceId && values.length > 0,
    values,
    current,
    canWrite,
    pending: !!workspaceId && (routing.isLoading || routing.set.isPending),
    wireModel,
    setEffort,
  };
}
...
export function ReasoningEffortSelector({
  ...
  const { visible, values, current, canWrite, pending, wireModel, setEffort } =
    useReasoningEffortControl(model, workspaceId);
    ...
    <CommandPopover open={open} onOpenChange={(next) => setOpen(locked || pending ? false : next)}>
      <Tooltip>
        ...
        <TooltipContent side="top" className="max-w-[240px]">
          {locked ? (
            <p>Only workspace editors can change reasoning effort for this model.</p>
          ) : (
            <p>
              Reasoning effort for <span className="font-mono">{wireModel}</span> — applies to every
              session in this workspace using this model.
            </p>
          )}
        </TooltipContent>
      </Tooltip>
