...
// co-worker session page (`/workspaces/:id/sessions/:sessionId`). A render-path
...
// `SessionCacheWarmer` → `workspaceSessionStartSeed` cache seed: the
// `workspace_sessions.sandbox_id` column is nullable (legacy Suna-migration rows
...
const pageSource = readFileSync(
  join(
    import.meta.dir,
    '../../app/(app)/workspaces/[id]/sessions/[sessionId]/page.tsx',
  ),
  'utf8',
);
