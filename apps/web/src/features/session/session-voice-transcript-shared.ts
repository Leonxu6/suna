...
export function voiceTranscriptKey(workspaceId: string | undefined, sessionId: string | undefined) {
  return ['voice-transcript', workspaceId ?? '', sessionId ?? ''] as const;
}
...
export function useVoiceTranscript(
  ...
  return useQuery<VoiceTranscript>({
    queryKey: voiceTranscriptKey(workspaceId, sessionId),
    // `enabled` guards presence, so the `?? ''` fallbacks are never exercised.
    queryFn: () =>
      getVoiceTranscript(workspaceId ?? '', sessionId ?? '', {
