...
export function ConnectorConnectionGateDialog() {
  const { isOpen, workspaceId, connector, retry, closeConnectorGate } = useConnectorGateStore();
  const queryClient = useQueryClient();
  const connect = usePipedreamConnectMember(workspaceId ?? '', connector ?? '', () => {
    if (workspaceId) {
      queryClient.invalidateQueries({ queryKey: ['connector-profiles', workspaceId] });
    }
    ...
    <Modal open={isOpen} onOpenChange={(open) => !open && closeConnectorGate()}>
      <ModalContent className="lg:max-w-md">
        ...
        <ModalFooter>
          ...
          <Button
            className="gap-2"
            onClick={() => workspaceId && connector && connect.mutate(undefined)}
            disabled={connect.isPending || !workspaceId || !connector}
          >
