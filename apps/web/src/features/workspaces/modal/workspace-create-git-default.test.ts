...
const source = readFileSync(join(import.meta.dir, 'workspace-create-modal.tsx'), 'utf8');

describe('new workspace git provider default', () => {
  test('starts with a managed repository and keeps user GitHub explicit', () => {
    ...
    expect(source).toContain('createWorkspaceRepo');
  ...
  test('uses current workspace-modal list and radius primitives', () => {
