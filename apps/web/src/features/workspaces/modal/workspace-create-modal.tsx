...
import { isManagedGitUnavailableError, isWorkspaceLimitError } from '@/lib/onboarding/ensure-first-workspace';
import {
  getMarketplaceItem,
...
import {
  startWorkspaceOnboardingSession,
  startTemplateSetupSession,
} from './template-setup-session';

const sanitizeWorkspaceName = (value: string) => value.replace(/[^a-zA-Z0-9._ -]+/g, '').trim();

// Mirrors the API's WORKSPACE_NAME_MAX_LENGTH (workspaces.name is varchar(255);
// pasted prompts used to sail past the charset regex and 500 on the insert).
const WORKSPACE_NAME_MAX_LENGTH = 120;

const managedWorkspaceSchema = z.object({
  name: z
    .string()
    .transform(sanitizeWorkspaceName)
    .pipe(
      z
        .string()
...
type ManagedWorkspaceFormValues = z.infer<typeof managedWorkspaceSchema>;
...
interface WorkspaceCreateModalProps {
   ...
   *  starter — set from `/workspaces?clone=<item-id>` (marketplace "Clone"). */
...
function rememberGitHubSetupReturn(path: string) {
  try {
    ...
    // Non-critical: the setup page still falls back to the workspaces flow.
  }
}

function upsertWorkspace(workspaces: KortixWorkspace[] | undefined, workspace: KortixWorkspace) {
  const current = workspaces ?? [];
  const existingIndex = current.findIndex((item) => item.workspace_id === workspace.workspace_id);
  if (existingIndex === -1) return [workspace, ...current];
...
export const WorkspaceCreateModal = ({
  ...
  // Cloning a workspace template comes from two places: the marketplace's
  ...
  // Pre-check whether managed git (the "Create workspace" quick path, backed by
  // POST /workspaces/provision) is usable BEFORE the user hits its 503 —
  ...
  const managedForm = useForm<ManagedWorkspaceFormValues>({
    resolver: zodResolver(managedWorkspaceSchema),
    defaultValues: {
      name: '',
    },
  });
  ...
  async function finishCreatedWorkspace(workspace: KortixWorkspace) {
      successToast('Workspace created');
      queryClient.setQueryData<KortixWorkspace[]>(['workspaces', workspace.account_id], (workspaces) =>
        upsertWorkspace(workspaces, workspace),
      );
      queryClient.setQueryData<KortixWorkspace[]>(['workspaces'], (workspaces) =>
        upsertWorkspace(workspaces, workspace),
      );
      void queryClient.invalidateQueries({ queryKey: ['workspaces'] });
    void queryClient.refetchQueries({ queryKey: ['workspaces'], type: 'active' });

      if (effectiveSourceItemId) {
        const sessionId = await startTemplateSetupSession(workspace, {
          itemId: effectiveSourceItemId,
          title: sourceItemQuery.data?.title ?? 'this workspace',
        });
        if (sessionId) {
          resetAndClose();
          router.replace(`/workspaces/${workspace.workspace_id}/sessions/${sessionId}`);
          return;
        }
      }

      const onboardingSessionId = await startWorkspaceOnboardingSession(workspace);
      if (onboardingSessionId) {
        resetAndClose();
        router.replace(`/workspaces/${workspace.workspace_id}/sessions/${onboardingSessionId}`);
        return;
      }
  ...
  const createMutation = useMutation({
    ...
    // `provisionWorkspace(input, options?: ApiClientOptions)`. Pre-existing on
    // main after the react-query bump; fixed here because it blocks typecheck.
    mutationFn: (input: Parameters<typeof provisionWorkspace>[0]) => provisionWorkspace(input),
    onSuccess: finishCreatedWorkspace,
    onError: async (error: Error) => {
      if (effectiveAccountId && isWorkspaceLimitError(error)) {
        try {
          const existing = await listWorkspacesForAccount(effectiveAccountId);
      ...
      if (isManagedGitUnavailableError(error)) {
        ...
        errorToast("Managed git isn't set up on this server", {
          description: 'An admin needs to connect GitHub in Git settings before workspaces can be created.',
  ...
  const githubCreateMutation = useMutation({
    mutationFn: createWorkspaceRepo,
    onSuccess: finishCreatedWorkspace,
  ...
  const templatesQuery = useQuery({
    queryKey: ['marketplace-workspace-templates'],
    queryFn: () => listMarketplaceItems({ type: 'workspace' }),
  ...
  const linkMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.setQueryData<KortixWorkspace[]>(
        ['workspaces', result.workspace.account_id],
        (workspaces) => upsertWorkspace(workspaces, result.workspace),
      );
      queryClient.setQueryData<KortixWorkspace[]>(['workspaces'], (workspaces) =>
        upsertWorkspace(workspaces, result.workspace),
      );
      void queryClient.invalidateQueries({ queryKey: ['workspaces'] });
      ...
      resetAndClose();
      router.replace(`/workspaces/${result.workspace.workspace_id}`);
    },
    onError: (error: Error) => {
  ...
  function handleCreate(values: ManagedWorkspaceFormValues) {
    ...
    createMutation.mutate({
      account_id: effectiveAccountId,
      name: values.name,
      // One starter kit: every new workspace ships the full Kortix skill kit (the
  ...
  return (
    <Modal open={open} onOpenChange={(o) => (!o ? resetAndClose() : onOpenChange(o))}>
      <ModalContent className={cn('space-y-6 lg:max-w-lg')}>
        <ModalHeader>
          <ModalTitle>
            {tHardcodedUi.raw('componentsWorkspacesWorkspaceCreateModal.line237JsxTextNewWorkspace')}
          </ModalTitle>
          {/* <ModalDescription>
            {tHardcodedUi.raw(
              'componentsWorkspacesWorkspaceCreateModal.line240JsxTextStartWithAPrivateManagedRepoExistingGithub',
            )}
          </ModalDescription> */}
        </ModalHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {tHardcodedUi.raw(
                            'componentsWorkspacesWorkspaceCreateModal.line258JsxTextWorkspaceName',
                          )}
                        </FormLabel>
                        <FormControl>
                          <Input
                      ...
                      <div className="flex items-start gap-3 px-3.5 py-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="text-foreground text-sm font-medium">
                            Cloning {sourceItemQuery.data?.title.replaceAll('-', ' ') ?? 'workspace'}
                          </div>
                          <p className="text-muted-foreground mt-0.5 text-xs leading-relaxed">
                            {sourceItemQuery.data?.description ??
                              'Your new workspace starts with everything this workspace ships.'}
                          </p>
                        </div>
            ...
            <form onSubmit={githubForm.handleSubmit(handleLinkGitHub)} className="w-full">
                ...
                <div className="min-h-[430px] space-y-5">
                  {githubInstallationsQuery.isLoading ? (
                    <div className="text-muted-foreground flex h-28 items-center justify-center text-sm">
                      <Loading />
                      {tHardcodedUi.raw(
                        'componentsWorkspacesWorkspaceCreateModal.line352JsxTextLoadingGithubConnections',
                      )}
                    </div>
                        ...
                        render={({ field }) => (
                          <FormItem className="space-y-1.5">
                            <div className="flex items-center justify-between gap-3">
                              <FormLabel>
                                {tHardcodedUi.raw(
                                  'componentsWorkspacesWorkspaceCreateModal.line391JsxTextGitAccount',
                                )}
                              </FormLabel>
                              {githubInstallationsQuery.data?.configured !== false ? (
                                <Button
                        ...
                        <EmptyState
                          ...
                          title={
                            debouncedRepositorySearch
                              ...
                              : tHardcodedUi.raw(
                                  'componentsWorkspacesWorkspaceCreateModal.line484JsxAttrTitleNoRepositoriesAvailable',
                                )
                          }
                          description={
                            debouncedRepositorySearch
                              ...
                              : tHardcodedUi.raw(
                                  'componentsWorkspacesWorkspaceCreateModal.line485JsxAttrDescriptionUpdateTheGithubAppInstallationToGrantKortix',
                                )
                          }
                          size="sm"
                          action={
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="space-y-1.5">
                            <FormLabel>
                              {tHardcodedUi.raw(
                                'componentsWorkspacesWorkspaceCreateModal.line511JsxTextWorkspaceName',
                              )}
                            </FormLabel>
                            <FormControl>
                              <Input
                                placeholder={tHardcodedUi.raw(
                                  'componentsWorkspacesWorkspaceCreateModal.line516JsxAttrPlaceholderUseRepositoryName',
                                )}
                                autoCapitalize="none"
                                autoCorrect="off"
                                {...field}
              ...
              <ModalFooter>
                ...
                >
                  ...
                  {tHardcodedUi.raw(
                    'componentsWorkspacesWorkspaceCreateModal.line549JsxTextImportRepo',
                  )}
                </Button>
              </ModalFooter>
            </form>
...
/** Account switcher for the new-workspace target. Only rendered by the caller
...
function CreateAccountField({
  ...
  return (
    <div className="space-y-1.5 px-5" data-testid="workspace-create-account">
      <Label>Account</Label>
      <DropdownMenu>
 ...
 *  each mode's form body, right above the footer — never above the workspace
...
/** "Clone from a template" step — pick a `registry:workspace` marketplace item
 *  to seed the new workspace from, right inside the New Workspace flow (the
