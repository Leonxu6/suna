 ...
 * surface (`workspaceId`/`installedNames` on workspace, `itemHref` on public)
 * without non-null assertions.
 *
 * - **public** (`/marketplace`): route-based navigation (`itemHref` → real
 *   `<Link>`), no workspace bound, no installed state.
 * - **workspace** (Customize → Marketplace): in-panel overlay navigation
...
export type MarketplaceSurface =
  ...
  | {
       ...
       *  `workspace` variant still compile without a shape change. */
