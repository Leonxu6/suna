/**
 * The prompt a freshly-installed workspace/template gets seeded with as its first
 * session, so an agent wires up the thing you just installed instead of dumping
 * you on an empty workspace. Deliberately generic — the agent reads the workspace's
...
export function buildTemplateSetupPrompt(title: string): string {
  ...
  return [
    `You were just created from the "${name}" template. Everything it needs — its agent(s), skills, triggers, and the integrations it depends on — is already in this workspace's files.`,
    '',
...
/**
 * The first-session prompt for a brand-new workspace (NOT cloned from a marketplace
 * item). This is the "agent creation" default — rather than dropping the user on
 * an empty workspace, the first session onboards + personalizes: it reads the
