...
describe('workspaceMarketplaceHref', () => {
  test('builds the customize marketplace deep link for a workspace', () => {
    expect(workspaceMarketplaceHref('proj_123')).toBe('/workspaces/proj_123/customize/marketplace');
  });
...
describe('isInstallDisabled', () => {
  test('disabled when no item is resolved', () => {
    expect(isInstallDisabled({ hasItem: false, targetWorkspaceId: 'p1', pending: false })).toBe(true);
  });
  ...
  test('disabled while a request is pending', () => {
    expect(isInstallDisabled({ hasItem: true, targetWorkspaceId: 'p1', pending: true })).toBe(true);
  });

  test('enabled once an item, workspace, and idle state all line up', () => {
    expect(isInstallDisabled({ hasItem: true, targetWorkspaceId: 'p1', pending: false })).toBe(false);
  });
});
