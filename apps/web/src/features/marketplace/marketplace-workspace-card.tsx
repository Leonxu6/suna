...
/**
 * A whole installable workspace — the primary surface of the marketplace, so
 ...
 * just a link to the item's detail page — the real "Add to a workspace" button
 * lives there (`AddToWorkspaceModal`); this is a preview + fast path into it.
 *
 ...
 * workspaces" on a workspace's own detail page).
 */
export function MarketplaceWorkspaceCard({
  item,
  size = 'default',
}: {
  ...
  const banner = workspaceBannerClass(item.name || item.id);
