/** Deterministic per-workspace banner gradient — kortix-* tokens only, no raw
 *  palette. Shared by the card and the detail page so the same workspace gets
...
export function workspaceBannerClass(seed: string): string {
