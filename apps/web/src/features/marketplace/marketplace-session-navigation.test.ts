...
describe('marketplace install session navigation', () => {
  ...
  test('prepares the returned install session before navigation without marking it fresh', () => {
    ...
    expect(href).toBe('/workspaces/workspace-123/sessions/session-123');
    ...
    expect(prefetchedRoutes).toEqual(['/workspaces/workspace-123/sessions/session-123']);
    expect(prefetchedQueries).toHaveLength(1);
    expect(invalidatedQueries).toEqual([{ queryKey: ['workspace-sessions', 'workspace-123'] }]);
  });
