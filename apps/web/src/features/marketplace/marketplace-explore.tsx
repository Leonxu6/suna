...
// Only skills are browseable alongside Workspaces today (agents/commands/
...
export function MarketplaceExplore({
   ...
   *  the Workspaces showcase is fully indexed/static. */
  ...
  syncUrl?: boolean;
  /** Unauthenticated catalog reads (public). Off for the in-workspace view. */
  publicOnly?: boolean;
  /** Ancestor scroll element to virtualize the grids against (in-workspace). */
  ...
  // by id and addable individually (workspace detail, add-to-workspace), just not as
  // their own tiles on the landing grid.
  const componentItems = useMemo(
    () => catalogItems.filter((it) => it.type !== 'registry:project' && !it.partOfWorkspace),
    [catalogItems],
  );
        ...
        {showWorkspaces ? (
          <section className="scroll-mt-28">
            <SectionHeading
              ...
              subtitle="A full, working Kortix workspace — spun up as its own workspace and set up for you in one session."
            />
