...
import { getWorkspaceLlmCatalogProviders } from '@kortix/sdk';
 ...
 *
 * Not gated by the workspace's llm_gateway flag — see the route's doc comment;
 * this must work for native-mode workspaces too, which is the majority case
 ...
 */
export function useLiveLlmProviderCatalog(workspaceId: string, enabled: boolean) {
  const query = useQuery({
    queryKey: ['llm-catalog-providers', workspaceId],
    queryFn: () => getWorkspaceLlmCatalogProviders(workspaceId),
