...
import { useReviewCenterEnabled } from '@/hooks/workspaces/use-review-center-enabled';
...
function WorkspaceSessionListSkeleton() {
  return (
...
export function WorkspaceSessionList({ workspaceId, filter = 'all' }: WorkspaceSessionListProps) {
  ...
  const [sessionToShare, setSessionToShare] = useState<WorkspaceSession | null>(null);
  ...
  const reviewEnabled = useReviewCenterEnabled(workspaceId);
  ...
  const restartMutation = useMutation({
    mutationFn: ({ sessionId }: { sessionId: string; label: string }) =>
      restartWorkspaceSession(workspaceId, sessionId),
  ...
  const stopMutation = useMutation({
    mutationFn: ({ sessionId }: { sessionId: string; label: string }) =>
      stopWorkspaceSession(workspaceId, sessionId),
    onSuccess: (_data, { label }) => {
  ...
  // Filtering itself lives in the SESSIONS header dropdown (workspace-sidebar);
  ...
  if (viewState === 'loading') {
    return <WorkspaceSessionListSkeleton />;
  }
    ...
    return (
      <div className="space-y-1.5 px-2 py-2">
        <p className="text-destructive/80 text-xs">
          {tHardcodedUi.raw(
            'componentsWorkspacesWorkspaceSessionList.line120JsxTextFailedToLoadSessions',
          )}
        </p>
        {message && (
  ...
  if (viewState === 'empty') {
    return (
      <div className="text-muted-foreground/60 px-2 pt-1 pb-2 text-xs">
        {tHardcodedUi.raw('componentsWorkspacesWorkspaceSessionList.line132JsxTextNoSessionsYet')}
      </div>
    );
  }
  ...
  if (viewState === 'no-matches') {
    return (
      <div className="text-muted-foreground/60 px-2 pt-1 pb-2 text-xs">
        {tI18nHardcoded.raw('autoFeaturesCoWorkerWorkspaceSidebarWorkspaceSessionListJsxText1fba7ca0')}
      </div>
    );
  }
    ...
    <>
      <FadedScrollArea className="h-full min-h-0 space-y-px">
        {visibleSessions.map((session) => {
          const href = `/workspaces/${session.workspace_id}/sessions/${session.session_id}`;
      ...
      <ShareSessionModal
        ...
        onSaved={() => queryClient.invalidateQueries({ queryKey: ['workspace-sessions', workspaceId] })}
      />
...
interface WorkspaceSessionRowProps {
  ...
  onShare: (session: WorkspaceSession) => void;
              ...
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  aria-label={tHardcodedUi.raw(
                    'componentsWorkspacesWorkspaceSessionList.line312JsxAttrAriaLabelSessionActions',
                  )}
                  className={cn(
...
function WorkspaceSubsessionRow({
  title,
  href,
  isActive,
  updatedAt,
