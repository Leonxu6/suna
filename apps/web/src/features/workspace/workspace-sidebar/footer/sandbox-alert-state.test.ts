...
function build(overrides: Partial<WorkspaceSnapshotBuild>): WorkspaceSnapshotBuild {
  return {
    build_id: 'build-1',
    slug: 'default',
...
function health(overrides: Partial<WorkspaceSandboxHealth>): WorkspaceSandboxHealth {
  return {
...
describe('selectCurrentSandboxFailure', () => {
  ...
  test('drops a failed optional workspace accelerator', () => {
    const failed = build({
      build_id: 'accelerator-failed',
      slug: 'default-warm',
      snapshot_name: 'kortix-ppwarm-workspace-hash',
      status: 'failed',
    });
