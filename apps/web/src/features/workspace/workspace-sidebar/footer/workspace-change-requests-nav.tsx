...
import { WorkspaceFilesProvider } from '@/features/workspace-files/context';
import { useChangeRequests } from '@/features/workspace-files/hooks/use-change-requests';
        ...
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            <h3 className="text-foreground truncate text-sm font-medium">
              {tI18nHardcoded.raw(
                'autoFeaturesCoWorkerWorkspaceSidebarFooterWorkspaceChangeRequestsNav63c4c66f',
              )}
            </h3>
            <p className="text-muted-foreground truncate text-xs">
              {crs.length}{' '}
              {tI18nHardcoded.raw(
                'autoFeaturesCoWorkerWorkspaceSidebarFooterWorkspaceChangeRequestsNavafed4c1a',
              )}
              {baseRef || 'main'}
            </p>
          </div>
...
function NavItemInner({ workspaceId }: { workspaceId: string }) {
  const c = useOpenCrController();
  const isMobile = useIsMobile();
  const openCustomize = useCustomizeStore((s) => s.openCustomize);
  // When the Review Center is enabled for this workspace, this pill becomes the
...
export function WorkspaceChangeRequestsNavItem({ workspaceId }: { workspaceId: string }) {
  return (
    <WorkspaceFilesProvider value={{ workspaceId, ref: '' }}>
      <NavItemInner workspaceId={workspaceId} />
