...
export const SANDBOX_HEALTH_QUERY_KEY = (workspaceId: string) => ['sandbox-health', workspaceId];
...
export function useSandboxHealth(workspaceId: string) {
  return useQuery<WorkspaceSandboxHealth>({
    queryKey: SANDBOX_HEALTH_QUERY_KEY(workspaceId),
    queryFn: () => getWorkspaceSandboxHealth(workspaceId),
    staleTime: 30_000,
...
export function useSandboxRecovery(workspaceId: string) {
  const router = useRouter();
  const queryClient = useQueryClient();

  const invalidate = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: SANDBOX_HEALTH_QUERY_KEY(workspaceId) });
...
function SandboxAlertContent({
  ...
  workspaceId: string;
  health: WorkspaceSandboxHealth;
  severity: SandboxAlertSeverity;
}) {
  const tI18nHardcoded = useTranslations('hardcodedUi');
  const openCustomize = useCustomizeStore((s) => s.openCustomize);
  const { retry, fixWithAgent } = useSandboxRecovery(workspaceId);
      ...
      <div className="border-border flex flex-col gap-2 border-t p-3">
        {canFixWithAgent && (
          ...
          >
            ...
            {tI18nHardcoded.raw(
              'autoFeaturesCoWorkerWorkspaceSidebarFooterWorkspaceSandboxAlertJsxe7d8ac75',
            )}
          </Button>
        )}
        <Button
        ...
        >
          ...
          {tI18nHardcoded.raw(
            'autoFeaturesCoWorkerWorkspaceSidebarFooterWorkspaceSandboxAlertJsx8794c0a3',
          )}
        </Button>
      </div>
    </div>
...
export function WorkspaceSandboxAlert({ workspaceId }: { workspaceId: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const { data } = useSandboxHealth(workspaceId);
    ...
    <SidebarMenuItem>
      ...
      >
        ...
        <DisclosureContent variant="outline">
          <SandboxAlertContent workspaceId={workspaceId} health={data} severity={severity} />
        </DisclosureContent>
      </Disclosure>
    </SidebarMenuItem>
  );
...
export function WorkspaceSandboxAlertRailItem({ workspaceId }: { workspaceId: string }) {
  const { data } = useSandboxHealth(workspaceId);
  const severity = resolveSandboxAlertSeverity(data);
  if (!severity || !data) return null;
  const tone = SEVERITY_TONE[severity];
