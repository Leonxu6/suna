...
export function useFilesActivate() {
  ...
  return useCallback(() => {
    ...
    if (isMobile) setOpenMobile(false);
  }, [router, workspaceId, isMobile, setOpenMobile]);
}
...
export function WorkspaceCustomizeNavItem() {
...
export function WorkspaceCustomizeRailItem() {
  const onClick = useCustomizeActivate();
 ...
 *  customization). Hidden when the caller lacks `workspace.file.read`: that leaf
...
export function WorkspaceFilesNavItem() {
  const onClick = useFilesActivate();
  const pathname = usePathname();
  const params = useParams<{ id: string }>();
  const canReadFiles = useWorkspaceCan(params?.id, WORKSPACE_ACTIONS.WORKSPACE_FILE_READ);
  const isActive = !!pathname && /^\/workspaces\/[^/]+\/files(\/|$)/.test(pathname);
...
export function WorkspaceFilesRailItem() {
  const onClick = useFilesActivate();
