...
import { setWorkspaceSessionSharing, type WorkspaceSession } from '@kortix/sdk';
...
const SESSION_SHARING_COPY = {
  ...
  workspace: { label: 'Whole team', desc: 'Everyone in this workspace' },
  private: { label: 'Only you', desc: 'Private — just you' },
  members: { label: 'Select members', desc: 'A chosen list of members' },
};

export function sessionVisibilityMeta(session: Pick<WorkspaceSession, 'visibility'>) {
...
export function SessionVisibilityBadge({ session }: { session: WorkspaceSession }) {
...
export function ShareSessionModal({
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return setWorkspaceSessionSharing(workspaceId, session!.session_id, selectionToIntent(sharing));
    },
    onSuccess: () => {
    ...
    >
      <ModalContent className="lg:max-w-md">
        <ModalHeader>
          <ModalTitle>
            {tI18nHardcoded.raw(
              'autoFeaturesCoWorkerWorkspaceSidebarModalShareSessionModalJsxc5c9cc41',
            )}
          </ModalTitle>
          <ModalDescription>
            {tI18nHardcoded.raw(
              'autoFeaturesCoWorkerWorkspaceSidebarModalShareSessionModalJsxb29062b4',
            )}
          </ModalDescription>
        </ModalHeader>
