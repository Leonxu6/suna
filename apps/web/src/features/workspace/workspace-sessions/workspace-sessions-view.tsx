...
import { sessionSource, type SessionSourceKind } from '@/components/workspaces/session-label';
...
import {
  ...
  ShareSessionModal,
} from '@/features/workspace/workspace-sidebar/modal/share-session-modal';
import {
...
import {
  ...
  WORKSPACE_SESSIONS_FILTERS,
  workspaceSessionsFilterCounts,
  sessionAccessMeta,
  sessionOwnerLabel,
...
function SessionRow({
  ...
  const href = `/workspaces/${workspaceId}/sessions/${session.session_id}`;
      ...
      <DisclosureContent variant="outline" contentClassName="border-border border-t">
        <div className="space-y-5 px-4 py-5">
          ...
          <dl className="grid gap-x-6 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            />
            <DetailItem label="Agent" value={session.agent_name || 'Workspace default'} />
...
export function WorkspaceSessionsView({ workspaceId }: { workspaceId: string }) {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState<WorkspaceSessionsFilter>('all');
  ...
  const sessionsQuery = useQuery({
    queryKey: ['workspace-session-inventory', workspaceId],
    queryFn: () => listWorkspaceSessions(workspaceId, { scope: 'workspace' }),
    staleTime: 10_000,
  ...
  const counts = useMemo(() => workspaceSessionsFilterCounts(sessions), [sessions]);
  const visibleSessions = useMemo(
    () => filterWorkspaceSessions(sessions, filter, search),
  ...
  const restartMutation = useMutation({
    ...
    onSuccess: (_data, { label }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['workspace-session-inventory', workspaceId] });
      ...
      <div className="space-y-4">
        ...
        <div className="overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <FilterBar className="h-8 rounded-md">
            {WORKSPACE_SESSIONS_FILTERS.map((option) => (
              <FilterBarItem
                key={option.value}
        ...
        {sessionsQuery.isLoading ? (
          ...
          <EmptyState
            ...
            description="Start a session to give this workspace its first task."
            action={
