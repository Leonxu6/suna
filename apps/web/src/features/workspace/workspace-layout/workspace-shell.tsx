...
import { parseSidebarStateCookie } from '@/features/workspace/workspace-layout/sidebar-cookie';
import { WorkspaceSidebar } from '@/features/workspace/workspace-sidebar/workspace-sidebar';
...
import { useWorkspaceShellShortcuts } from '@/hooks/workspaces/use-workspace-shell-shortcuts';
...
export function WorkspaceShell({ workspaceId, initialSidebarOpen, children }: WorkspaceShellProps) {
  ...
  useGatewayCatalogSync(workspaceId);
  ...
  useEffect(() => {
    ...
    if (searchParams.get('customize') === 'files') {
      router.replace(`/workspaces/${workspaceId}/files`);
      return;
    }
  ...
  // useNewWorkspaceSession). Shared with the sidebar / ⌘T-⌘J / command palette.
  const newSession = useNewWorkspaceSession(workspaceId);
  const handleNewSession = useCallback(() => {
    newSession();
  }, [newSession]);

  useWorkspaceShellShortcuts({ workspaceId, onNewSession: handleNewSession });

  const params = useParams<{ sessionId?: string }>();
  const activeSessionId = params?.sessionId ?? null;

  const openTab = useWorkspaceSessionTabsStore((s) => s.openTab);

  useLayoutEffect(() => {
    if (activeSessionId) openTab(workspaceId, activeSessionId);
  }, [workspaceId, activeSessionId, openTab]);
  ...
  return (
    <BillingAccountProvider accountId={workspaceDetail?.workspace?.account_id ?? null}>
      <AppProviders
        showSidebar
        ...
        sidebarContent={<WorkspaceSidebar workspaceId={workspaceId} />}
      >
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          <WorkspaceSheelLayout>{children}</WorkspaceSheelLayout>
        </div>

        <CustomizPanel workspaceId={workspaceId} />

        <Suspense fallback={null}>
          <PresentationViewerWrapper />
        </Suspense>

        <WorkspaceOnboardingWizard workspaceId={workspaceId} />

        <PersonalOnboardingWelcome workspaceId={workspaceId} />
      </AppProviders>
    </BillingAccountProvider>
  );
}

const WorkspaceSheelLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  // the web. Client-only tree (WorkspaceShell gates on auth), so reading the UA
  ...
  return (
    ...
    >
      ...
      {/* Mobile: the sidebar is a sheet with no docked affordance, and view
          ...
          the opener lives here, always mounted, on every workspace view. The
...
export default WorkspaceSheelLayout;
