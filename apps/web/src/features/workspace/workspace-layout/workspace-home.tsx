...
export interface WorkspaceHomeSendOptions extends ComposerOptions {
  sandbox_slug?: string;
}
...
export function WorkspaceHome({
  ...
  onSend: (
    ...
    options?: WorkspaceHomeSendOptions,
  ) => void;
  busy: boolean;
}) {
  ...
  const sandboxesQuery = useQuery({
    ...
    queryFn: () => listWorkspaceSandboxes(workspaceId),
    ...Q,
  });
  ...
  const accessRequests = useQuery({
    ...
    queryFn: () => listWorkspaceAccessRequests(workspaceId, { showErrors: false }),
    retry: false,
    ...Q,
  });
  const pendingAccessCount = accessRequests.data?.requests.length ?? 0;

  const pendingPrefill = useComposerPrefillStore((s) => s.prefillByWorkspace[workspaceId]);
  const consumePrefill = useComposerPrefillStore((s) => s.consume);

  useEffect(() => {
    if (!pendingPrefill) return;
    consumePrefill(workspaceId);
    setPrefill({ text: pendingPrefill, id: Date.now() });
  }, [pendingPrefill, workspaceId, consumePrefill]);
        ...
        composer={
          <ComposerChatInput
            ...
            placeholder={tI18nHardcoded.raw(
              'autoFeaturesCoWorkerWorkspaceLayoutWorkspaceHomeJsxAttrPlaceholder115e6c2d',
            )}
            prefill={prefill}
            toolbarSlot={
              showSandboxPicker ? (
...
/**
 * The workspace-home empty-state body, laid out like Perplexity's home: the
 ...
 * the quiet "set up your workspace" pills sit at the bottom of the viewport.
 * Shared by the workspace index page AND the instant session shell's empty state
 * so a brand-new session opens onto the identical surface.
 */
export function WorkspaceHomeWelcomeBody({
  workspaceId,
  composer,
  onPickSuggestion,
}: {
  ...
  const name = detail.data?.workspace?.name ?? '';
  const displayName = name.trim() || 'this workspace';
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
        <div className="m-auto flex w-full max-w-[52rem] flex-col items-center gap-8 px-2 py-8 sm:px-4">
          <h1 className="text-muted-foreground max-w-2xl text-center text-4xl leading-[1.2] tracking-tight text-balance max-sm:text-3xl">
            ...
            {tI18nHardcoded.raw(
              'autoFeaturesCoWorkerWorkspaceLayoutWorkspaceHomeJsxTextSomething18ab9904',
            )}
          </h1>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          aria-label={tI18nHardcoded.raw(
            'autoFeaturesCoWorkerWorkspaceLayoutWorkspaceHomeJsxAttrAria4acf4ecd',
          )}
      ...
      <DropdownMenuContent align="start" className="w-80">
        <DropdownMenuLabel>
          {tI18nHardcoded.raw('autoFeaturesCoWorkerWorkspaceLayoutWorkspaceHomeJsxTextSandboxe9c5fbaa')}
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
...
const WORKSPACE_SETUP_TILES: SetupTile[] = [
  {
  ...
  },
  {
    icon: Icon.Slack,
    title: 'Slack',
    desc: 'Run this workspace right from chat.',
    section: 'channels',
  },
  {
...
function WorkspaceHomeSections() {
  const openCustomize = useCustomizeStore((s) => s.openCustomize);
  const tiles = WORKSPACE_SETUP_TILES;
