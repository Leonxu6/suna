  ...
  if (error || !diff)
    return (
      <div className="text-muted-foreground p-2 text-xs">
        {tHardcodedUi.raw(
          'featuresWorkspaceFilesComponentsFileHistoryPopover.line72JsxTextFailedToLoadDiff',
        )}
      </div>
    );
    ...
    <div className="border-border/30 bg-muted/20 border-t">
      ...
      {patchContent ? (
        ...
        <div className="text-muted-foreground p-2 text-center text-xs">
          {tHardcodedUi.raw(
            'featuresWorkspaceFilesComponentsFileHistoryPopover.line102JsxTextNoDiff',
          )}
        </div>
      )}
    </div>
      ...
      >
        ...
        <Button
          ...
          title={tHardcodedUi.raw(
            'featuresWorkspaceFilesComponentsFileHistoryPopover.line160JsxAttrTitleCopyCheckpointId',
          )}
        >
          {copied ? (
      ...
      <ScrollArea className="flex-1 overflow-hidden">
          ...
          <div className="flex flex-col items-center justify-center gap-2 p-6 text-center">
            ...
            <p className="text-muted-foreground text-xs">
              {error instanceof Error && error.message.includes('not a git repository')
                ...
                : tHardcodedUi.raw(
                    'featuresWorkspaceFilesComponentsFileHistoryPopover.line216JsxTextFailedToLoadHistory',
                  )}
            </p>
          </div>
        )}
        ...
        {!isLoading && !error && totalCheckpoints === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 p-6 text-center">
            ...
            <p className="text-muted-foreground text-xs">
              {tHardcodedUi.raw(
                'featuresWorkspaceFilesComponentsFileHistoryPopover.line224JsxTextNoCheckpointsYet',
              )}
            </p>
          </div>
        )}
            ...
            {history?.hasMore && (
              <div className="py-1 text-center">
                <span className="text-muted-foreground/50 text-xs">
                  {tHardcodedUi.raw(
                    'featuresWorkspaceFilesComponentsFileHistoryPopover.line240JsxTextShowingTheMostRecent',
                  )}{' '}
