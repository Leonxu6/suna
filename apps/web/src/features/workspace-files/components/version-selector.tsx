...
import type { WorkspaceBranch } from '@kortix/sdk';
...
export function VersionSelector() {
  ...
  const { primary, others } = useMemo(() => {
    const primary: WorkspaceBranch[] = [];
    const others: WorkspaceBranch[] = [];
  ...
  const handleSelect = (branchName: string) => {
    setVersion(workspaceId, branchName === defaultBranch ? null : branchName);
    setOpen(false);
    setQuery('');
  };
      ...
      <PopoverTrigger asChild>
        <Button
          ...
          title={tHardcodedUi.raw(
            'featuresWorkspaceFilesComponentsVersionSelector.line88JsxAttrTitleSwitchVersion',
          )}
        >
        ...
        <div className="border-border flex shrink-0 items-center gap-1.5 border-b px-3 py-2">
          ...
          <input
            ...
            placeholder={tHardcodedUi.raw(
              'featuresWorkspaceFilesComponentsVersionSelector.line112JsxAttrPlaceholderFindAVersion',
            )}
          ...
          <FadedScrollArea fadeColor="from-popover" className="h-full overscroll-contain">
            {isLoading && (
              <div className="text-muted-foreground flex items-center justify-center gap-2 py-8 text-xs">
                ...
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsVersionSelector.line122JsxTextLoadingVersions',
                )}
              </div>
            )}
            ...
            {error && !isLoading && (
              <div className="text-muted-foreground px-4 py-8 text-center text-xs">
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsVersionSelector.line128JsxTextFailedToLoadVersions',
                )}
              </div>
            )}
            ...
            {!isLoading && !error && filtered.length === 0 && (
              <div className="text-muted-foreground px-4 py-8 text-center text-xs">
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsVersionSelector.line134JsxTextNoVersionsMatch',
                )}{' '}
                {query ? `“${query}”` : ''}
              </div>
            )}
            ...
            {primary.length > 0 && (
              <div className={cn('space-y-3 p-3', others.length > 0 ? 'pb-0' : 'pb-3')}>
                <Label>
                  {tHardcodedUi.raw(
                    'featuresWorkspaceFilesComponentsVersionSelector.line140JsxTextMainVersion',
                  )}
                </Label>
                {primary.map((b) => (
                  <VersionRow
            ...
            {others.length > 0 && (
              <div className="space-y-3 p-3 pb-3">
                <Label>
                  {tHardcodedUi.raw(
                    'featuresWorkspaceFilesComponentsVersionSelector.line154JsxTextOtherVersions',
                  )}
                </Label>
                {others.map((b) => (
                  <VersionRow
          ...
          {!branch.is_default && branch.ahead != null && branch.behind != null && (
            <>
              ...
              <span
                ...
                title={tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsVersionSelector.line234JsxAttrTitleAheadBehindMainVersion',
                )}
              >
