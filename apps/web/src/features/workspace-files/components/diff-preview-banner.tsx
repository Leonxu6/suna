...
export function DiffPreviewBanner({
    ...
    return (
      <InfoBanner tone="neutral" className={className}>
        <span className="flex items-center gap-2">
          <Loading className="h-3.5 w-3.5 shrink-0" />{tHardcodedUi.raw('featuresWorkspaceFilesComponentsDiffPreviewBanner.line33JsxTextCalculatingTheDiff')}</span>
      </InfoBanner>
    );
  }
  ...
  if (error) {
    return (
      <InfoBanner
        tone="warning"
        icon={AlertCircle}
        title={tHardcodedUi.raw('featuresWorkspaceFilesComponentsDiffPreviewBanner.line44JsxAttrTitleCouldnTComputeTheDiff')}
        className={className}
      >
        {error.message}
      </InfoBanner>
  ...
  if (preview.is_same_ref) {
    return (
      <InfoBanner tone="warning" icon={Minus} className={className}>{tHardcodedUi.raw('featuresWorkspaceFilesComponentsDiffPreviewBanner.line57JsxTextSameVersionOnBothSidesPickDifferentVersions')}</InfoBanner>
    );
  }

  if (preview.is_up_to_date || preview.files_changed === 0) {
    return (
      <InfoBanner tone="warning" icon={Minus} className={className}>{tHardcodedUi.raw('featuresWorkspaceFilesComponentsDiffPreviewBanner.line65JsxTextNoChangesBetweenTheseVersionsTheSourceNeeds')}</InfoBanner>
    );
  }
