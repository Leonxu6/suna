...
import type { WorkspaceCommitFile } from '@kortix/sdk';
...
function statusIconFor(status: WorkspaceCommitFile['status'], className = 'size-3.5') {
  switch (status) {
    case 'added':
...
function statusBadgeFor(status: WorkspaceCommitFile['status']) {
  const map: Record<
    WorkspaceCommitFile['status'],
  ...
  if (!file) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-2 text-center">
        ...
        <p className="text-sm text-muted-foreground">{tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line166JsxTextSelectAFileToViewTheDiff')}</p>
      </div>
    );
  }
      ...
      <ScrollArea className="flex-1 min-h-0">
        ...
        {error && !isLoading && (
          <div className="flex flex-col items-center justify-center gap-2 p-10 text-center">
            ...
            <p className="text-xs text-muted-foreground">{tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line220JsxTextFailedToLoadDiff')}</p>
          </div>
        )}
        {data && !data.patch && !isLoading && (
          <div className="flex flex-col items-center justify-center gap-2 p-10 text-center">
            <History className="h-6 w-6 text-muted-foreground/30" />
            <p className="text-xs text-muted-foreground">{tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line226JsxTextNoTextualDiff')}</p>
            <p className="text-xs text-muted-foreground/60">{tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line228JsxTextFileMayBeBinaryOrUnchangedInThis')}</p>
          </div>
        )}
        {data?.patch && (
          ...
          {shaList.length > 1 && (
            <div className="flex items-center gap-0.5 shrink-0">
              <Button
                ...
                disabled={!hasPrevSha}
                onClick={gotoPrevSha}
                title={tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line407JsxAttrTitlePreviousCheckpoint')}
              >
              ...
              <Button
                ...
                disabled={!hasNextSha}
                onClick={gotoNextSha}
                title={tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line417JsxAttrTitleNextCheckpoint')}
              >
          ...
          {data && (
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                ...
                title={tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line482JsxAttrTitleCopyCheckpointId')}
              >
                {copied ? (
            ...
            <div className="px-3 py-3 border-b border-border/40 shrink-0 space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder={tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line528JsxAttrPlaceholderFilterFiles')}
            ...
            <ScrollArea className="flex-1 min-h-0">
              ...
              {error && !isLoading && (
                <div className="flex flex-col items-center justify-center gap-2 p-6 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">{tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line546JsxTextFailedToLoadCheckpoint')}</p>
                </div>
              )}
          ...
          <main className="flex-1 min-w-0 flex flex-col">
            {isLoading && !data && (
              <div className="flex flex-col items-center justify-center h-full gap-2">
                ...
                <p className="text-xs text-muted-foreground">{tHardcodedUi.raw('featuresWorkspaceFilesComponentsCheckpointDetailDialog.line575JsxTextLoadingCheckpoint')}</p>
              </div>
            )}
            {sha && (data || !isLoading) && (
