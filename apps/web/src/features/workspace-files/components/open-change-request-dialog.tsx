...
import type { WorkspaceBranch, WorkspaceSession } from '@kortix/sdk';
...
function BranchRow({ branch }: { branch: WorkspaceBranch }) {
  return (
...
export function OpenChangeRequestDialog({
  ...
  const lastSessionRef = useRef<WorkspaceSession | null>(session);
  ...
  const branchesQuery = useBranches({ enabled: open && !sessionMode, workspaceId });
  ...
  const openMutation = useOpenChangeRequest({ workspaceId });
    ...
    <Modal open={open} onOpenChange={onOpenChange}>
      <ModalContent className="gap-0 overflow-hidden p-0 lg:max-w-lg">
        <ModalHeader>
          <ModalTitle>
            {tHardcodedUi.raw(
              'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line226JsxTextOpenChangeRequest',
            )}
          </ModalTitle>
          <ModalDescription className="text-balance">
            {sessionMode ? (
              <>
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line231JsxTextProposeMergingThisSessionAposSWorkInto',
                )}{' '}
                <span className="text-foreground font-mono">{defaultBranch}</span>
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line232JsxTextTheSessionNeedsToHaveCommittedAndPushed',
                )}
              </>
            ) : (
              tHardcodedUi.raw(
                'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line237JsxTextProposeMergingOneVersionIntoAnotherTheMerge',
              )
            )}
          </ModalDescription>
        </ModalHeader>
            ...
            <ModalBody className="space-y-3 pt-0">
              <InfoBanner
                ...
                title={tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line246JsxAttrTitleNoNonDefaultVersionsYet',
                )}
              >
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line247JsxTextStartASessionEachSessionLivesOnIts',
                )}
              </InfoBanner>
            </ModalBody>
          ...
          <>
            <ModalBody className="space-y-4 pt-0">
                          ...
                          {selectedHeadBranch ? (
                            ...
                            <SelectValue
                              placeholder={tHardcodedUi.raw(
                                'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line305JsxAttrPlaceholderPickAVersion',
                              )}
                            />
                          )}
                        </SelectTrigger>
              ...
              {!sessionMode && headRef && baseRef && headRef === baseRef && (
                <InfoBanner tone="warning">
                  {tHardcodedUi.raw(
                    'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line354JsxTextPickTwoDifferentVersionsYouCanAposT',
                  )}
                </InfoBanner>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder={tHardcodedUi.raw(
                    'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line369JsxAttrPlaceholderContextForReviewersWhatChangedAndWhy',
                  )}
                  rows={3}
            ...
            <ModalFooter className="sm:justify-between">
              ...
              >
                ...
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsOpenChangeRequestDialog.line386JsxTextOpenChangeRequest',
                )}
              </Button>
            </ModalFooter>
          </>
