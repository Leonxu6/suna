...
export function ChangeRequestDetailDialog({ crId, onClose }: ChangeRequestDetailDialogProps) {
  ...
  const handleFixWithAgent = () => {
    if (!cr || !workspaceId || manifestIssues === null || fixing) return;
    setFixing(true);
    ...
    if (cr?.status === 'open') {
      return (
        <div className={cn(isFooter ? 'flex w-full gap-2' : 'flex shrink-0 items-center gap-1.5')}>
          ...
          >
            {tI18nHardcoded.raw(
              'autoFeaturesWorkspaceFilesComponentsChangeRequestDetailDialogJsxTexta48f1c83',
            )}
          </Button>
          <Button
            size={buttonSize}
            ...
            {!cr ? (
              <div className="space-y-1.5">
                <ModalTitle className="sr-only">
                  {tI18nHardcoded.raw(
                    'autoFeaturesWorkspaceFilesComponentsChangeRequestDetailDialogJsxText9f1f82e8',
                  )}
                </ModalTitle>
        ...
        {manifestIssues !== null && (
          <div className="border-border shrink-0 border-b px-4 py-3 sm:px-5">
            <InfoBanner
              tone="destructive"
              icon={AlertTriangle}
              title="Workspace config check failed, so this change can't be applied yet"
              action={
          ...
          {diffQuery.isLoading ? (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="text-foreground text-sm font-medium">
                      {tI18nHardcoded.raw(
                        'autoFeaturesWorkspaceFilesComponentsChangeRequestDetailDialogJsxText7ba43c2e',
                      )}
                    </h3>
                ...
                <div className="border-border bg-card rounded-md border px-3 py-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">
                      {tI18nHardcoded.raw(
                        'autoFeaturesWorkspaceFilesComponentsChangeRequestDetailDialogJsxText561449ba',
                      )}
                    </span>
                    <DiffStat
                    ...
                    <SelectTrigger className="h-10 w-full justify-between">
                      <SelectValue
                        placeholder={tI18nHardcoded.raw(
                          'autoFeaturesWorkspaceFilesComponentsChangeRequestDetailDialogJsxAttr29624185',
                        )}
                      />
                    </SelectTrigger>
                    ...
                    <div className="flex items-center justify-between gap-3 px-3 py-2.5">
                      <div className="min-w-0">
                        <div className="text-foreground text-sm font-medium">
                          {tI18nHardcoded.raw(
                            'autoFeaturesWorkspaceFilesComponentsChangeRequestDetailDialogJsxTexte014913b',
                          )}
                        </div>
                      </div>
                      {mergeReadiness && (
                        ...
                        !preview.can_merge && (
                          <InfoBanner
                            ...
                            title={
                              <>
                                {tHardcodedUi.raw(
                                  'featuresWorkspaceFilesComponentsChangeRequestDetailDialog.line283JsxTextConflictsIn',
                                )}{' '}
                            ...
                            <DisclosureBody className="border-t">
                              {patch ? (
                                ...
                                <div className="text-muted-foreground px-3 py-6 text-center text-xs">
                                  {tI18nHardcoded.raw(
                                    'autoFeaturesWorkspaceFilesComponentsChangeRequestDetailDialogJsxTextec240dc2',
                                  )}
                                </div>
                              )}
                            </DisclosureBody>
            ...
            <div className="p-5">
              <p className="border-border text-muted-foreground bg-background rounded-lg border border-dashed p-5 text-center text-xs">
                {tHardcodedUi.raw(
                  'featuresWorkspaceFilesComponentsChangeRequestDetailDialog.line356JsxTextNoChangesDetected',
                )}
              </p>
            </div>
          )}
