...
/**
 * Workspace-files view: no "uncommitted changes" concept — files are already
...
export const gitStatusKeys = {
  all: ['workspace-files', 'git-status'] as const,
  status: (workspaceId: string, ref: string) =>
    ['workspace-files', 'git-status', workspaceId, ref] as const,
};
