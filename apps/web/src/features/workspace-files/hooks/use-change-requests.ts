...
export function useVersionDiff(
  ...
  return useQuery<VersionDiffPreview>({
    queryKey: canRun
      ? ['workspace-files', 'version-diff', workspaceId, input!.from, input!.into]
      : ['workspace-files', 'version-diff', 'idle'],
...
function useInvalidateAll(workspaceIdArg?: string) {
  ...
  return () => {
    qc.invalidateQueries({ queryKey: ['workspace-files', 'change-requests', workspaceId] });
    // Branches list shows ahead/behind that may shift after a merge.
    qc.invalidateQueries({ queryKey: ['workspace-files', 'branches', workspaceId] });
    // The merge commit lands on the default branch — commit list goes stale.
    qc.invalidateQueries({ queryKey: ['workspace-files', 'commits', workspaceId] });
    ...
    qc.invalidateQueries({ queryKey: ['workspace-files', 'version-diff', workspaceId] });
    qc.invalidateQueries({ queryKey: ['workspace', 'session', workspaceId] });
  };
}
