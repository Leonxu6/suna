/**
 * Git history API for the workspace-files feature.
 *
 * Backed by `/v1/workspaces/:workspaceId/commits[/...]` and `.../files/history`.
...
function toGitCommit(c: WorkspaceCommit): GitCommit {
...
export async function getFileHistory(
  ...
  const path = filePath.startsWith('/workspace/')
    ...
    : filePath.replace(/^\/+/, '');
  const result = await getWorkspaceFileHistory(workspaceId, path, {
    ref,
    limit: options?.limit,
    skip: options?.skip,
  });
...
export async function getFileCommitDiff(
  ...
  const [commit, diff] = await Promise.all([
    getWorkspaceCommit(workspaceId, commitHash),
    getWorkspaceCommitDiff(workspaceId, commitHash, { path }),
  ]);
