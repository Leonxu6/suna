/**
 * Workspace Files API — Git-backed, read-only.
 *
 ...
 * every function dispatches against `/v1/workspaces/:workspaceId/files` instead
 * of the OpenCode SDK.
 *
 * Write operations (`writeFile`, `uploadFile`, `deleteFile`, `mkdirFile`,
 * `renameFile`, `createFile`, `copyFile`) throw — workspace files are
 * immutable from this view; users mutate via a session sandbox + commit.
 */

import { fetchWorkspaceArchive, listWorkspaceFiles, readWorkspaceFile } from '@kortix/sdk';
import type {
  FileContent,
  FileNode,
  FindMatch,
...
const READ_ONLY = 'Read-only — workspace files come from Git';
 ...
 * OpenCode "absolute under /workspace" convention; the workspace API uses
 * repo-relative paths.
 */
...
export async function readFile(
  ...
  const result = await readWorkspaceFile(workspaceId, relativePath, ref);
  return {
    type: 'text',
...
/**
 * Workspace API returns text only; binary preview is unsupported.
...
export async function downloadFile(
  ...
  const blob = await readFileAsBlob(workspaceId, ref, filePath);
...
export async function downloadDirectory(
  ...
  const blob = await fetchWorkspaceArchive(workspaceId, ref, relative || undefined);
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
...
// ---------------------------------------------------------------------------
// Git status / search / workspace info — no-op stubs for the read-only view
// ---------------------------------------------------------------------------
...
export async function getCurrentWorkspace(): Promise<RuntimeWorkspaceInfo> {
  return {
    id: 'workspace-files',
    worktree: '/workspace',
    vcs: 'git',
...
export async function getServerHealth(): Promise<ServerHealth> {
  return { healthy: true, version: 'workspace-files' };
}
