...
async function mockAccounts(page: Page) {
  await page.route(/\/accounts$/, async (route) => {
    assert(
      route.request().headers().authorization === 'Bearer debug-workspace-create-token',
...
async function openHarness(page: Page) {
  ...
  await page.goto(`${baseUrl}/debug/workspace-create-modal`, { waitUntil: 'domcontentloaded' });
  await page.getByRole('dialog', { name: /new workspace/i }).waitFor({ state: 'visible', timeout: 30_000 });
  // One starter kit — every workspace ships the full skill pack; there's no toggle.
...
function defaultMarketplaceItem(name: string, title: string, order: number) {
  return {
    ...
    defaultWorkspaceInstall: true,
    defaultWorkspaceInstallOrder: order,
  };
}

async function submitWorkspaceCreate(page: Page, name: string): Promise<ProvisionPayload> {
  let payload: ProvisionPayload | null = null;
  await page.route('**/workspaces/provision', async (route) => {
  ...
  await page.getByRole('textbox', { name: /workspace name/i }).fill(name);
  const request = page.waitForRequest((req) => req.url().includes('/workspaces/provision'));
  await page.getByRole('button', { name: /^create workspace$/i }).click();
  await request;
  const workspacePath = `/workspaces/proj_${name}`;
  await page.waitForURL(
    (url) =>
      url.pathname === workspacePath ||
      (url.pathname === '/auth' && url.searchParams.get('redirect') === workspacePath),
    { timeout: 10_000 },
  );
  await page.unroute('**/workspaces/provision');
  assert(payload, 'expected a /workspaces/provision request payload');
  return payload;
}
...
async function main() {
  ...
  try {
    ...
    const defaultPayload = await submitWorkspaceCreate(page, 'default-full');
    ...
    // One starter kit: every workspace scaffolds with the full general-knowledge-worker starter.
    assert(
    ...
    await openHarness(page);
    const accountField = page.getByTestId('workspace-create-account');
    ...
    const defaultAccountPayload = await submitWorkspaceCreate(page, 'default-account-visible');
    assert(
    ...
    assert(
      (await page.getByTestId('workspace-create-account').textContent())?.includes('Acme Team'),
      'account field should show the switched account',
    );
    const switchedPayload = await submitWorkspaceCreate(page, 'switched-account');
    assert(
      switchedPayload.account_id === TEAM_ACCOUNT_ID,
      'payload should target the account picked in the modal',
    );

    console.log('[workspace-create-modal] ok: starter template, marketplace item, and account picker payloads verified');
  } finally {
    await browser.close();
  }
}
