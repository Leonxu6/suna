...
A **cron trigger** fires the workspace once a day, re-prompting the same
...
is encrypted in the secrets manager and injected at runtime, never shown to the
...
- **Isolation.** Every run happens in its own microVM sandbox, and only the
  Slack digest it's explicitly allowed to send leaves the sandbox.
- **Scoped secrets.** Any credential the agent uses is encrypted in the secrets
  manager and injected into the sandbox at runtime, never exposed to the model
  or the logs.
