...
The CLI follows one hierarchy: host → account → workspace → session. You sign in to a host, pick an account inside it, pick a workspace inside that account, and open sessions inside the workspace. `kortix hosts login` walks the first three steps in order: it signs you in, picks the account, then sets a default workspace.

Every token starts with `kortix_pat_`. A user token, from `kortix login`, sees every account and workspace you belong to. A workspace token is auto-minted for a session sandbox and scoped to one workspace. See [Token scope](#token-scope).
...
Start a new workspace, or link an existing repo folder to one.
...
`kortix init` creates a workspace directory with the `general-knowledge-worker` template.

To link an existing cloned repo to a workspace you already created:

```sh
kortix workspaces link <workspace-id>
```

This command writes `.kortix/link.json` in the current directory. Kortix reads this file to find your workspace on every command run from this folder. If you plan to run `kortix ship` first, skip this step. It links a new workspace for you when none exists.
</Step>
...
`kortix ship` lints your `kortix.yaml`, commits local changes, pushes your branch, and prompts for any missing secret or connector connection. Run it each time you want your local changes on the cloud workspace. The first run also creates the cloud workspace and repo if you have not linked one yet.
</Step>
...
`kortix cr ls` lists change requests for the linked workspace. `kortix cr diff <cr>` shows the unified patch. `kortix cr merge <cr>` merges it into the workspace's default branch. Accept a CR number or its full id.
</Step>
...
| Command | Effect |
| --- | --- |
| `kortix login [--host <name>] [--api <url>] [--token <pat>] [--account <slug>] [--no-workspace]` | Sign in to the active host, or the named one. Opens a browser by default; `--token` signs in headless. `--no-workspace` skips the default-workspace pick. |
...
| `kortix hosts login [<name>] [--token <pat>] [--api <url>] [--account <slug>] [--no-workspace]` | Sign in to a host. An unknown name registers the host first. |
...
A command resolves "the workspace" in this order:

1. The `--workspace` flag.
2. The `KORTIX_WORKSPACE_ID` environment variable.
3. `.kortix/link.json` in the exact working directory.
4. The global default set by `kortix workspaces use`.

| Command | Effect |
| --- | --- |
| `kortix workspaces ls [--all] [--json]` | List workspaces on the active account. `--all` spans every account you belong to. |
| `kortix workspaces info [<id>] [--json]` | Show one workspace. Default: the linked or default workspace. |
| `kortix workspaces use [<id>]` | Set the global default workspace. Switches the active account if the workspace lives elsewhere. |
| `kortix workspaces unset` | Clear the global default workspace. |
| `kortix workspaces link [<id>]` | Bind the current directory to a workspace. Writes `.kortix/link.json`. |
| `kortix workspaces unlink` | Remove `.kortix/link.json`. |
| `kortix workspaces open [<id>]` | Open a workspace's dashboard page in your browser. |
| `kortix workspaces clone [<id>] [dir]` | Clone a workspace's repo through the authenticated Kortix git proxy. |
| `kortix workspaces rm [<id>] [--purge] [-y\|--yes]` | Archive a workspace. `--purge` also deletes its managed git repo. |

`kortix projects` remains a deprecated compatibility alias for `kortix workspaces`. `KORTIX_PROJECT_ID` remains a fallback after `KORTIX_WORKSPACE_ID`.

### Workspace scaffold

`kortix init [workspace-name] [options]` creates a new, empty workspace directory. It writes `kortix.yaml`, `.kortix/Dockerfile`, and the OpenCode config directory. It does not write `.kortix/link.json` — `kortix ship` or `kortix workspaces link` create that later.
...
| `--template <general-knowledge-worker\|minimal>` | Workspace template. Default: `general-knowledge-worker`. |
...
`kortix ship` stages, commits, and pushes your current branch to the workspace's git repo. Run it once to create the workspace. Run it again any time to sync. Alias: `kortix deploy`.
...
| Option | Effect |
| --- | --- |
| `--name <workspace>` | Display name for a new workspace. |
...
| `--workspace <id>` / `--host <name>` | Target a non-default workspace or host. |
...
| Command | Effect |
| --- | --- |
| `kortix sessions ls` | List every session on the workspace. |
...
| `kortix sessions new [--prompt "<text>"] [--agent <name>] [--model <id>] [--wait] [--json]` | Start a session. `--model <id>` overrides the workspace's default model. `--wait` blocks until it is running (up to ~5 minutes); a timeout is a hard failure (exit 1), not a silent success. Backend-origin overrides require a backend token (PAT or service account) and 403 otherwise: `--origin-ref <user-id>` (attribute to your end user), `--secret <id>` (narrow injected secrets, repeatable), `--no-secrets` (inject zero workspace secrets), `--connector <alias>=<profile-id>` (bind a connector to a profile, repeatable), `--context <key>=<value>` (runtime context, repeatable). |
...
| Command | Effect |
| --- | --- |
| `kortix cr ls [--status open\|merged\|closed\|all] [--workspace <id>]` | List CRs. Default: `--status open`. |
| `kortix cr show <cr> [--workspace <id>]` | Show one CR, including its merge preview. Alias: `info`. |
| `kortix cr diff <cr> [--no-color] [--json]` | Print a CR's unified diff. |
| `kortix cr open --title "<text>" [--description "<text>"] [--head <ref>] [--session <id>] [--base <ref>]` | Open a CR. Aliases: `new`, `create`. Inside a sandbox, `--head` and `--session` default automatically. `--base` defaults to the workspace's default branch. |
...
`<cr>` accepts a per-workspace number (`3`) or the full id. Inside a sandbox, the CLI reads its token automatically — no login or link needed.

### Secrets

Encrypted values stored on the workspace and injected as plain environment variables into every session sandbox at boot.
...
Per-agent model settings on the linked workspace.
...
Manages the workspace's connection to a chat platform. Tokens are stored encrypted in the workspace's secrets and resolved server-side — they are never injected into the sandbox.
...
| `kortix channels disconnect` | Remove the workspace's channel secrets. |
...
Connectors an agent calls as tools. `add`, `rm`, and `policy set` edit the local `kortix.yaml`; run `kortix ship` to apply, unless you pass `--apply` to change the cloud workspace immediately.
...
| `kortix connectors policy ls [--json]` | Show workspace-wide execution policy. |
...
Manages the workspace's sandbox images. A template defines an image or Dockerfile plus resources; a build produces the snapshot sessions boot from.
...
Read-only discovery of marketplace items. Installing an item into a workspace is an agent import, done from inside a session — there is no install command.
...
The Kortix system skills are the platform's own documentation — sessions, sandboxes, the executor, memory, channels. The API serves them live, so they always match the version of Kortix you are signed into. The binary and a token are all any agent needs to learn how to drive Kortix: no repo checkout, no baked image.
...
| `kortix system-skills path [name]` | Print a skill's on-disk directory in this workspace. |
...
| `kortix executor rm <slug>` | Remove a connector from the workspace. |
...
Read-only view of the workspace's git repo. Operates on the default branch unless `--ref` names another branch, tag, or commit.
...
| `kortix triggers pause` / `resume` | Deactivate or reactivate every trigger on the workspace, server-side. |
...
Who can use the linked workspace. Roles: `manager`, `editor`, `member`.

| Command | Effect |
| --- | --- |
| `kortix access ls [--json]` | List members and their workspace roles. |
| `kortix access invite <email> --role <r>` | Invite someone to the workspace. |
| `kortix access grant <user-id> --role <r>` | Set a member's workspace role. |
| `kortix access revoke <user-id>` | Remove a member's workspace access. |
...
| `kortix roles assignments [--workspace <id>] [--json]` | List policy bindings. |
...
| `kortix roles export [--workspace <id>] [--out <file>] [--format toml\|json]` | Dump roles and bindings to a file. |
| `kortix roles import <file>` | Apply a roles and bindings file. |

### Grants

Assigns workspace resources to people. Secrets and connectors live on agents; assigning an agent to a member grants everything that agent declares.
...
See [Manifest reference](/docs/workspace/manifest).
...
Every token starts with `kortix_pat_`. A user token is scoped to every workspace on your accounts. A workspace token is scoped to one workspace and auto-injected into that workspace's sandboxes. See the full token-family reference at [Session runtime](/docs/work/runtime).
