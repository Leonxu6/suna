...
`kortix.session(workspaceId, sessionId)` returns the handle for everything a
...
|---|---|---|
| `s.get(opts?)` | `GET /workspaces/:workspaceId/sessions/:sessionId` | Reads session details |
| `s.update(input)` | `PATCH …/sessions/:sessionId` | Renames the session or updates metadata |
| `s.start(waitMs?)` | `POST …/sessions/:sessionId/start` | Provisions and boots the runtime |
| `s.restart()` | `POST …/sessions/:sessionId/restart` | Restarts the runtime; keeps the same sandbox |
| `s.stop()` | `POST …/sessions/:sessionId/stop` | Stops the runtime; the session stays |
| `s.delete()` | `DELETE …/sessions/:sessionId` | Deletes the session |
...
- `s.transcript(options?)` — a compact server-side transcript (text and tool calls, no tool inputs or outputs). This works with a workspace-scoped session token.
...
`kortix.session(workspaceId, sessionId)`. The SDK resolves it during
`ensureReady()` and caches it on the handle.

## Warm a workspace session

Use the server-owned warm-session operations when a workspace landing page needs
one empty runtime before the first prompt.

```ts
const workspace = kortix.workspace(workspaceId);
const warm = await workspace.sessions.ensureWarm();

await kortix.session(workspaceId, warm.session.session_id).ensureReady();

const claimed = await workspace.sessions.claimWarm({
...
try {
  await kortix.session(workspaceId, sessionId).start();
} catch (err) {
...
```tsx
const s = useSession(workspaceId, sessionId);
