...
| Scope | Reach |
|---|---|
| Account (default) | Every workspace in the account |
| Workspace | One workspace only; every other workspace returns `403` |

Choose the workspace scope for CI and other narrow-purpose credentials.
...
<Callout type="warn">
  A new service account has no workspace access. If you point `getToken` at one
  before you grant it access, every call returns `403 "You do not have access
  to this workspace"`. Grant the service account access first, or use a
...
|---|---|
| A backend, script, or CI job | A personal access token, account-wide or workspace-scoped |
| The Kortix CLI | `kortix login` (mints a personal access token) |
| A web app on Kortix sign-in | A Supabase JWT |
| An automated caller with a granted workspace | A service account |
