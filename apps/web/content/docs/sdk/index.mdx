...
1. Create a session in your workspace.

   ```ts
   const created = await kortix.workspace(workspaceId).sessions.create();
   const session = kortix.session(workspaceId, created.session_id);
   ```
...
`createKortix` gives you an imperative client: call methods for every action, like workspaces, sessions, secrets, and triggers. `@kortix/sdk/react` gives you hooks for live UI data — `useSession` runs a whole session in one hook.
