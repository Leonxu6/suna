...
the platform API. The `workspace(id)` and `session(workspaceId, sessionId)` handles bind ids
...
kortix.workspaces; // top-level workspace operations
kortix.workspace(id); // id-bound workspace handle
kortix.session(workspaceId, sessionId); // id-bound session handle → see Sessions
...
| `tokens.create(input)` | mint one — `{ accountId?, name, expiresAt?, workspaceId? }` |
...
### Workspaces — `kortix.workspaces`

| method | what |
| --- | --- |
| `list()` · `listForAccount(accountId)` | your workspaces · workspaces in an account |
| `get(id)` · `detail(id)` | summary · full detail |
| `create(input)` · `createRepo(input)` | from an existing `repo_url` · new empty GitHub repo |
| `provision(input)` | new workspace on a new Kortix-managed repo, seeded with a starter template — `{ name, account_id?, seed_starter?, starter_template?, marketplace_items?, source_item_id? }` |
...
`provision` creates a new workspace; it does not start an existing workspace's
sandbox. Start a session instead — see [Sessions](/docs/sdk/sessions).

`workspace(id).sessions.list({ scope: 'workspace' })` is a manager-only inventory:
private, unavailable, and soft-deleted sessions, with ownership and
runtime-state metadata.

The deprecated `kortix.projects` collection and `kortix.project(id)` handle delegate to these Workspace APIs. They remain available for existing SDK consumers.

### GitHub — `kortix.github`

Account-scoped GitHub App install and repo linking, not workspace-scoped.
...
| `linkRepository(input)` | import a repo as a workspace |
...
Public catalog browsing, read-only — distinct from `workspace(id).marketplace`,
which installs an item onto a workspace.
...
### The workspace handle — `kortix.workspace(id)`

Binds the workspace id; every sub-resource hangs off it.

```ts
const p = kortix.workspace(workspaceId);
await p.detail();
await p.update({ name });
await p.llmCatalog();
```
...
| `onboardingComplete` | mark workspace onboarding done |
...
#### `p.tokens` — workspace-scoped API keys
...
#### `p.secrets` — workspace secrets

| method | what |
| --- | --- |
| `list()` · `upsert({ name, value })` | all workspace secrets · create or update one |
...
agent, a skill, a secret) instead of the whole workspace.
...
`p.connectors.discover` browses the direct-integration catalog. It is **experimental** and off by default — enable it per workspace under Customize → Settings → Experimental → "Connectors API Discover". Easy Connect (Pipedream) remains the default connector marketplace.
...
#### `p.policies` — workspace policies

| method | what |
| --- | --- |
| `list()` · `set(policies)` | the workspace's policies · replace the set |
...
| `setActivation(paused)` | pause or resume every trigger on the workspace |
...
Installs a catalog item's files onto the workspace's default branch.
...
Read-only access to the workspace's git tree. To read and write files inside a
...
| `session(sessionId)` | the session handle (same as `kortix.session(id, sessionId)`) |
...
Account, agent, and workspace-scoped model defaults, resolved by the gateway.

| method | what |
| --- | --- |
| `get()` · `set(input)` · `clear(params)` | read · set a default · clear an override |

#### `p.setDefaultAgent` — workspace default agent

`p.setDefaultAgent(agentName)` checks that the agent is declared and enabled,
then sets it as `default_agent` in the workspace's `kortix.yaml`. New sessions
...
feature for the workspace. Pass `enabled: null` to clear the override.

#### `p.sandbox` — templates and snapshot builds

Sandbox build config beyond `sandboxHealth`/`sandboxTemplates` on the workspace
handle: Dockerfile/image/warm-pool templates and their snapshot builds.

| method | what |
| --- | --- |
| `list()` · `snapshots()` | sandboxes for this workspace · built snapshots |
...
| `setProvider(provider)` | request a provider switch. `null` (or the platform default / the already-active provider) applies immediately; switching to a different enabled provider starts a durable prepare→verify→activate transition — the current provider keeps serving while the target warm image is built and verified, then activated. The return is a tagged union: `kind:'workspace'` (immediate) or `kind:'preparation'` (poll `getWorkspaceSandboxProviderTransition()` until `activated`/`failed`) |
...
`kortix.session(workspaceId, sessionId).runtime` (call `ensureReady()` first) instead, which
...
| Workspaces REST | The raw REST functions the facade wraps |
...
with auth already injected. Prefer `kortix.session(workspaceId, sessionId).runtime`, the
...
`kortix.session(workspaceId, sessionId).runtime` after `await s.ensureReady()`.

### Deprecated aliases

About twenty old subpaths still work: `/files`, `/turns`, `/session`, `/auth`,
`/workspaces-client`, `/api-client`, `/config`, `/event-stream`,
...
(`agent_*`, `session_*`, `task_*`, `trigger_*`, `workspace_*`, `pty_*`) resolve
...
| Legacy subpaths (`/workspaces-client`, `/turns`, `/files`, and more) | Deprecated | Still work. Each re-exports from the root. Import from the root instead. |
