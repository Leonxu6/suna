---
title: Full example
description: One file that lists workspaces, starts a session, and streams a reply.
---
...
async function main() {
  ...
  // 2. Platform REST: list workspaces, pick one (or provision your first).
  const workspaces = await kortix.workspaces.list();
  const workspace = workspaces[0] ?? (await kortix.workspaces.provision({ name: 'sdk-quickstart' }));
  ...
  const created = await kortix.workspaces.createSession(workspace.workspace_id, {
    name: 'sdk full example',
  });
  const session = kortix.session(workspace.workspace_id, created.session_id);
  ...
  //    prompt only (ids come from workspaces.modelPicker() and
  //    workspaces.detail().config.agents).
...
| 2–3  | The platform REST surface: workspaces and sessions    | [Reference](/docs/sdk/reference)   |
...
// Workspace secrets — env vars every session's agent can read at runtime
await workspace.secrets.upsert({ name: 'MY_API_KEY', value: 'sk-…' });

// The workspace's agents and skills (config files in the repo)
const { config } = await kortix.workspaces.detail(workspaceId);
console.log(
...
const overview = await workspace.gateway.overview(7);
const routing = await workspace.gateway.routing.get();
```
