...
## useSession(workspaceId, sessionId, options?)
...
function Chat({ workspaceId, sessionId }: { workspaceId: string; sessionId: string }) {
...
| `runtimeTransport` | `'rest' \| 'acp'` | The server-selected transport. Display it for diagnostics only. Do not branch on it. |
...
| `runCommand(command, args)` | Run a workspace slash command. |
...
The workspace experiment `acp_runtime` selects ACP. The default is REST.
`useSession` owns both paths. The host does not construct `/kortix/acp/` URLs
and does not call OpenCode prompt routes directly. Disable the experiment to
restore REST without changing the frontend.
...
| Hook | Reads |
|---|---|
| `useWorkspaceModels(workspaceId)` | Selectable models for the workspace. |
| `useVisibleAgents({ workspaceId })` | The workspace's visible agents. |
| `useWorkspaceConfig(workspaceId)` | The workspace's runtime config: default agent, commands. |
| `useWorkspaceSecrets(workspaceId)` | Secrets: list, add, remove, and personal overrides. |
| `useWorkspaceTriggers(workspaceId)` | Triggers: list, create, update, remove, fire. |
| `useChangeRequests(workspaceId, status?)` | Change requests: list, open, merge, close, request changes. |
