...
- Kortix cuts the branch from `base_ref`, which defaults to the workspace's default branch, at session-create time.
...
```
/workspace                       ← WORKDIR. The workspace repo is cloned here.
...
Kortix injects these variables at boot, alongside your workspace's [secrets](/docs/workspace/secrets), which arrive as plain environment variables.

| Variable | What |
| --- | --- |
| `KORTIX_WORKSPACE_ID` | UUID of this workspace. |
| `KORTIX_SESSION_ID` | UUID of this session. Also the branch name. |
| `KORTIX_BRANCH_NAME` | Same value as `KORTIX_SESSION_ID`. |
| `KORTIX_REPO_URL` | Clone URL for the workspace repo. |
| `KORTIX_DEFAULT_BRANCH` | The workspace's default branch. |
...
| `KORTIX_PROJECT_SECRET_NAMES` | Comma-separated names of the workspace's secrets. |
...
`KORTIX_PROJECT_ID` remains available as a deprecated compatibility alias. New integrations must read `KORTIX_WORKSPACE_ID`.
...
| `KORTIX_CLI_TOKEN` | A workspace-scoped [personal access token](/docs/sdk/auth) (PAT). | Lets the in-sandbox `kortix` CLI act as the launching user, scoped to the workspace. |
| `KORTIX_EXECUTOR_TOKEN` | The same workspace-scoped PAT as `KORTIX_CLI_TOKEN`. | Backs the `kortix-executor` MCP gateway. |

`KORTIX_SANDBOX_TOKEN` is not a workspace API token. The `kortix` CLI does not read it, and workspace-scoped API routes reject it. Use `KORTIX_CLI_TOKEN` or `KORTIX_EXECUTOR_TOKEN` for any call that should act as the launching user.

## Pushing from a session

The daemon fetches a short-lived clone credential from `GET /v1/workspaces/:id/git/clone-credential`, authenticated with `KORTIX_SANDBOX_TOKEN`. No static git token sits in the environment. With that credential, `git push origin HEAD` sends commits to the session branch. Landing on the default branch requires a merged [change request](/docs/work/change-requests).

## The agent runtime

The daemon launches OpenCode as `opencode serve --port 4096 --hostname 127.0.0.1`, with `OPENCODE_CONFIG_DIR` set to the workspace's config directory (default `.kortix/opencode`) inside the cloned repo. See [Agents](/docs/workspace/agents) for how a session picks an agent and its config.
...
If your workspace has no Dockerfile, Kortix builds sessions from a bare `ubuntu:24.04` image plus the runtime layer below.
...
| Don't bake credentials into the image. | Declare the name in `env:` and set the value as a workspace secret. Kortix injects it at session start. |
