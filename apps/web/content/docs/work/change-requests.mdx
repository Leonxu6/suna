...
A change request (CR) merges one git branch into another. Kortix creates a CR from a session's branch (`head_ref`) onto the workspace's default branch (`base_ref`, usually `main`). The CR row is metadata; the merge, diff, and conflict checks run as real git operations against the workspace's repository. A CR is the only way session work reaches the default branch.
...
An agent must open a CR to land any change on the workspace's default branch. The agent does not merge its own CR — merging is the user's decision. Follow this contract:
...
3. Open the CR: `kortix cr open --title "..." --description "..."`. Inside a session sandbox, `--head` and `--session` default to `$KORTIX_BRANCH_NAME` and `$KORTIX_SESSION_ID`. `--base` defaults to the workspace's default branch.
...
| `workspace_id` | uuid | The workspace the CR belongs to. |
| `number` | integer | Per-workspace display number (`#1`, `#2`…). Unique per workspace. Never recycles. |
| `title` | text | Required. |
...
A unique index on `(workspace_id, number)` lets you reference a CR by its short number instead of its UUID.
...
`POST /v1/workspaces/:workspaceId/change-requests/:crId/merge` runs these steps.
...
5. On success, Kortix sets `status` to `merged`, records `merged_at`, `merged_by`, and `merge_commit_sha`, and invalidates the workspace's git cache.
...
All routes sit under `/v1/workspaces/:workspaceId/change-requests`.
...
Read routes need read access to the workspace. Write routes need write access. Each write action also needs a capability, shown below for a full token and for a session's scoped token.

| Action | Full-token capability | Scoped-token capability |
| --- | --- | --- |
| Open a CR | `workspace.gitops.push` | `workspace.cr.open` |
| Request changes | `workspace.review.act` | `workspace.cr.open` |
| Merge | `workspace.gitops.merge` | `workspace.cr.merge` |
