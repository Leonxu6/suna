...
The manifest is the file the platform treats as authoritative for a workspace. This page lists every `kortix.yaml` (v2) key, its type, its default, and how the platform validates it. For the plain-language version, see [Your workspace](/docs/workspace).

## Two configuration surfaces

A Kortix workspace has two configuration surfaces with strict, non-overlapping ownership.
...
Any repo with a valid manifest at its root is a Kortix workspace. `kortix.yaml` (v2, YAML only) is the current format for new workspaces, created by the web "Create workspace" flow and by `kortix init`. `kortix.toml` (v1) still works for existing workspaces, but the platform accepts no new v2 features on it. See [Legacy TOML](/docs/workspace/legacy-toml) for the v1 schema and the v1-to-v2 migration steps.
...
| `project.name` / `project.description` | no | legacy manifest key for display metadata; the platform does not read `project.name` for the workspace's display name |
...
| `policy.default_mode` | no | `allow_all` (default) or `risk`; workspace-wide connector approval mode |
| `agents` | yes | name-to-block governance map; must not be empty |

## `project:`

This legacy manifest key remains unchanged for compatibility. It contains optional `name` and `description` metadata. The platform does not read this table. A workspace's display name comes from its database record.
...
Env var names match `^[A-Z_][A-Z0-9_]*$`. The Secrets Manager caps a name at 64 characters — a longer name parses here but can never get a value. Names starting with `KORTIX_` can never get a value either. Keep names at 64 characters or fewer, and avoid the `KORTIX_` prefix. Full contract: [Secrets](/docs/workspace/secrets).
...
|---|---|---|---|
| `slug` | string | — | Required. Unique per workspace. `default` is reserved. |
...
Set `default` on `sandbox` to make one template the workspace-wide default. Every session, trigger, and channel then boots it without naming a slug.
...
3. Workspace `sandbox.default`.
4. Platform `"default"`.

Cron triggers, webhook triggers, schedules, and channels use the selected agent's template.

A spec change (`cpu`, `memory`, `disk`, or the image itself) rebuilds the workspace snapshot. The new size applies on the next session, not a running one.
...
That directory holds agents, skills, commands, tools, plugins, and `opencode.jsonc`. `opencode.jsonc` stays the OpenCode-native registry for plugins, MCP servers, providers, and permissions — do not duplicate those settings in the manifest. See [Agents](/docs/workspace/agents).
...
| `model` | no | resolves at fire time | Wire form `provider/model`. Pins the fired session to that model. See [Models](/docs/workspace/models). |
...
| Field | Default | Notes |
|---|---|---|
| *(map key)* | — | `[a-z0-9][a-z0-9_-]{0,127}`, unique per workspace. Matches the `.md` filename it governs. |
...
| `secrets` | `none` | Which workspace secrets this agent receives as sandbox env vars. Renamed from v1's `env`. |
...
v2 is deny-by-default: an omitted `connectors`, `secrets`, `kortix_cli`, or `skills` on a declared agent resolves to `none`. Grant every permission an agent needs explicitly, as the starter's `kortix` agent does above. A workspace migrating from v1 must re-grant everything by hand — v1 defaults `env` (secrets) to `all` when omitted, the opposite of v2.

`kortix_cli` grants come from a fixed set of workspace-scoped actions (`workspace.read`, `workspace.cr.open`, `workspace.cr.merge`, `workspace.trigger.*`, `workspace.secret.*`, `workspace.connector.*`, and more). Account-scoped actions — `member.*`, `billing.*`, `workspace.create` — can never be granted to an agent. Run `kortix validate --scopes` from inside a session to print the live, grantable list.
