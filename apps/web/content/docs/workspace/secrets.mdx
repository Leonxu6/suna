---
title: Secrets
description: How Kortix stores, injects, and rotates encrypted workspace credentials.
---

import { Steps, Step } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

A secret is a per-workspace credential — an API key, token, or connection
string — that a session needs but that must not live in the repository.
Kortix stores secrets on the workspace, never on the account, and encrypts
every value with AES-256-GCM using a key derived per workspace.
...
`STRIPE_API_KEY`. In most workspaces the identifier and the name match. They
differ only when a workspace holds several candidate values for one name —
...
- **Shared** — the workspace-wide value. Every workspace member with read
...
the default when you omit it. See [Agents](/docs/workspace/agents).
...
Kortix saves it as a shared secret for the workspace. To store more than one
...
Open the workspace's Environment variables page, enter the key and value,
...
Run `kortix secrets ls` to see which secrets a workspace declares and which
...
Run the same command with the new value, or set it again on the workspace's
...
workspace, on a best-effort basis. If the key is a model or gateway
credential, Kortix also restarts the agent process so it picks up the new
value.
</Step>
...
delete it from the workspace's Environment variables page.
...
|---|---|
| `kortix secrets ls` | List secrets declared and set for the workspace |
...
Reading or writing a secret needs the `workspace.secret.read` or
`workspace.secret.write` permission. A custom workspace role can withhold
either permission, even from a manager.

## REST routes

All routes sit under `/v1/workspaces/{workspaceId}`.
...
change to every active sandbox in the workspace:
...
3. If the changed secret is an LLM provider credential, Kortix also
   restarts the sandbox's `opencode` process. The next prompt in the
   session uses the new credential.
