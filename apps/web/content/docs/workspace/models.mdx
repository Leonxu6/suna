...
This page applies to workspaces with the LLM Gateway on. LLM Gateway is an
experimental setting, and it is on by default for cloud workspaces. Check or
...
Connect a BYOK key on the workspace's Model settings page, or set the
provider's env var directly as a [secret](/docs/workspace/secrets).
...
2. The [agent's](/docs/workspace/agents) default for this workspace.
...
[manifest reference](/docs/workspace/manifest) for the trigger `model:`
field.
...
If you see credit charges on a BYOK-only workspace, check these two causes
before reporting a billing bug.
</Callout>

## Shared, not private, keys

A connected provider key applies to the whole workspace. There is no private,
per-user key — setting a personal override for a provider key fails with a
`llm_credentials_workspace_wide` error. Update the shared key on the
[secrets](/docs/workspace/secrets) page instead.
