...
description: Connect Slack to a workspace and control sessions from a channel.
---

import { Steps, Step } from 'fumadocs-ui/components/steps';

A channel connects a chat platform to a workspace. A message in a connected
...
workspace shows a workspace picker on the first mention.
...
[secret](/docs/workspace/secrets). It does not appear on the workspace's Secrets
...
From the dashboard, open the workspace's Channels page and connect Slack there
...
| `switch`, `unbind` | Rebind or unbind this channel from its workspace |
| `workspaces` | List workspaces you can bind to |
| `sessions` | List recent sessions started in this workspace |
| `whoami` | Show the channel panel: workspace, agent, model, policy, and your linked identity |
| `agent <name>`, `model <id>` | Set the agent or model this channel uses |
| `policy <mode>` | Set who can start sessions here |
| `help` | List all commands |

`policy` accepts `workspace_open` (default — any workspace member who mentions
