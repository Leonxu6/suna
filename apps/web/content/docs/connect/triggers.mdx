...
- **webhook** — runs when an external service sends a signed request to the workspace's webhook URL.

You define triggers in the workspace manifest, `kortix.yaml`. Each trigger holds a prompt that renders as the fired session's first message. Runtime state — such as the last fire time and status — lives outside the manifest, in the database. Firing a trigger does not create a commit.
...
`kortix ship` commits `kortix.yaml` and pushes it. The schedule goes live once this lands on your workspace's default branch.
</Step>
...
See [Secrets](/docs/workspace/secrets) for more on secrets.
</Step>
...
Kortix builds the webhook URL from your workspace id and the trigger's slug:

```
POST /v1/webhooks/workspaces/<workspace-id>/<slug>
```
...
# kortix.yaml
triggers:
  - slug: daily-digest # required, lowercase + dashes, unique per workspace
...
Legacy `kortix.toml` uses the same fields in a different container; see [legacy TOML](/docs/workspace/legacy-toml).

## Fields

| Field | Required | Default | Notes |
| --- | --- | --- | --- |
| `slug` | yes | — | `[a-z0-9][a-z0-9_-]{0,127}`, unique per workspace. |
...
| `secret_env` | required, webhook only | — | Name of a workspace secret holding the webhook signing key. |
...
Every trigger-fired session is created with workspace visibility (any workspace member can see it), not private to whoever configured or fired the trigger.
...
Fires on `POST /v1/webhooks/workspaces/{workspaceId}/{slug}`. Kortix checks the request in this order, with a constant-time comparison:
...
| 200 | Valid, but skipped — the workspace is paused, or the delivery did not match `filter`. |
| 400 | Malformed workspace ID or slug in the URL. |
| 401 | Signature and token both missing or wrong. |
| 404 | Trigger not found, disabled, not a webhook, or the workspace is not active. |
...
| --- | --- | --- |
| `GET /v1/workspaces/{workspaceId}/triggers` | `workspace.trigger.read` | Lists triggers, runtime state, and manifest parse errors. A bad entry appears in `errors[]`; it does not break the other triggers. |
| `POST /v1/workspaces/{workspaceId}/triggers` | `workspace.trigger.create` | Creates a trigger. Commits to the manifest directly. |
| `PATCH /v1/workspaces/{workspaceId}/triggers/{slug}` | `workspace.trigger.update` | Partial update, merged onto the current entry. |
| `DELETE /v1/workspaces/{workspaceId}/triggers/{slug}` | `workspace.trigger.delete` | Also clears the trigger's runtime state. |
| `PATCH /v1/workspaces/{workspaceId}/triggers/activation` | `workspace.trigger.update` | Body `{ paused: boolean }`. See [Pause and resume](#pause-and-resume). |
| `POST /v1/workspaces/{workspaceId}/triggers/{slug}/fire` | `workspace.trigger.fire` | Manual fire. Any workspace member can fire a trigger — this permission does not require the editor or manager role. |
| `POST /v1/webhooks/workspaces/{workspaceId}/{slug}` | signature or token | Public URL, gated by the webhook secret. |

## Pause and resume

A workspace-level switch stops every trigger in the workspace at once, independent of each trigger's own `enabled` field. While paused, the scheduler skips the workspace and inbound webhooks return `200` with `{ status: "skipped" }` — no session fires. A manual fire still works. Use this when the same repository runs on two control planes (for example, dev and production) so cron does not fire twice. CLI: `kortix triggers pause` and `kortix triggers resume`. See [CLI](/docs/cli) for the full `kortix triggers` command group.
...
- Each workspace allows 3 triggered sessions provisioning at once, by default. The account's plan-tier active-session cap can also apply. A fire past either limit returns `queued` (`202`) instead of failing, and runs once a slot frees up.
