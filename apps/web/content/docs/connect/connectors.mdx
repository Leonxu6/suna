...
A connector links your workspace to an external tool or service, such as Gmail, Stripe, a database, or any HTTP API. The agent calls it as a tool. Kortix brokers every call, so the sandbox never holds the connector's credential. The dashboard may label this area **Connections** — it is the same feature.

You declare most connectors in the workspace manifest, `kortix.yaml`. See the [manifest reference](/docs/workspace/manifest) for every field. Kortix declares channel and computer connectors for you, when you connect a chat platform or a machine.
...
- **API key or token** — you declare the connector in `kortix.yaml` and enter the credential yourself, or reference it from a workspace [secret](/docs/workspace/secrets) (legacy).
...
<Steps>
<Step>
### Open the workspace's Connectors page

From the dashboard, open the workspace's Connectors page and search for the app.
</Step>
<Step>
### Connect the app
...
Open the connector on the workspace's Connectors page and enter the credential value. Kortix stores it encrypted and never writes it to the manifest.
</Step>
</Steps>
