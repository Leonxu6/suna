 ...
 * the experimental flag, boot a sandbox, drive the voice connector the way an
 ...
 *
 * Step 4 calls the real `kortix_voice.spawn_room` connector action over the
 * Executor gateway. This is the same route and session token an in-sandbox
 * agent uses. The LiveKit worker calls the API's separate worker-facing MCP.
 */
...
const { workspaceSessions } = await loadFrom('@kortix/db', API_DIR);
...
function printHelp() {
  ...
  --skip-provision          reuse an existing workspace/session instead of minting one
  --workspace <id>            workspace id (required with --skip-provision unless derivable from --call)
  ...
  --skip-spawn              skip calling spawn_room (opening the LiveKit room)
  --skip-mcp-list           skip the Executor voice catalog assertion
  --skip-speak              skip joining + speaking into the room
  --skip-transcript         skip the voice_call_turns user+agent row assertion
  --skip-run-command        skip the run_command trigger-phrase + durable-turn check
  --skip-voice-read         skip the read_transcript connector assertion
  --skip-end                skip end_call + room-closed assertion
...
// ── step 1 + 3 helpers: provision a real user/workspace/session ──────────────
async function provisionUser(): Promise<{ workspaceId: string; userJwt: string }> {
  ...
  const workspaceRes = await fetch(`${API}/workspaces/provision`, {
    method: 'POST',
    headers: H,
    body: JSON.stringify({ name: `voice-full-flow-${stamp}`, seed_starter: true }),
  });
  const workspace = await j(workspaceRes);
  const workspaceId =
    workspace?.workspace_id ?? workspace?.workspace?.workspace_id ?? workspace?.id;
  if (!workspaceId) {
    throw new Error(`workspace create failed: ${JSON.stringify(workspace).slice(0, 400)}`);
  }
...
async function createSessionAndWaitReady(workspaceId: string, userJwt: string): Promise<string> {
  const H = { Authorization: `Bearer ${userJwt}`, 'content-type': 'application/json' };
  const sessRes = await fetch(`${API}/workspaces/${workspaceId}/sessions`, {
    method: 'POST',
    headers: H,
  ...
  for (let i = 0; i < 80; i++) {
    const s = await j(await fetch(`${API}/workspaces/${workspaceId}/sessions/${sessionId}`, { headers: H }));
...
// ── Executor voice connector over HTTP, as an in-sandbox agent calls it ───
async function executorCall(
  ...
  const res = await fetch(`${API}/executor/workspaces/${workspaceId}/call`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
      'content-type': 'application/json',
      // Defensive only — a workspace-scoped session token already carries this
      // session id server-side; see db-deps.ts's resolveWorkspacePrincipal.
      'X-Kortix-Session-Id': sessionId,
    },
    body: JSON.stringify({ connector: 'kortix_voice', action, args }),
  });
  const body = await j(res);
  return { status: res.status, body };
}

async function executorCatalog(
  workspaceId: string,
  token: string,
): Promise<{ status: number; body: any }> {
  const res = await fetch(`${API}/executor/workspaces/${workspaceId}/catalog`, {
...
async function main() {
  ...
  // ── step 1: provision a NEW user + workspace ────────────────────────────
  if (!skipProvision) {
    try {
      const p = await provisionUser();
      workspaceId = p.workspaceId;
      userJwt = p.userJwt;
      record('1-provision', 'PASS', `workspace=${workspaceId}`);
    } catch (err) {
    ...
    if (!workspaceId) {
      const [row] = await db
        .select({ workspaceId: workspaceSessions.workspaceId })
        .from(workspaceSessions)
        .where(eq(workspaceSessions.sessionId, sessionId))
        .limit(1);
      if (!row) abort('1-provision', new Error(`session ${sessionId} not found in DB — pass --workspace explicitly`));
      workspaceId = row!.workspaceId;
    }
    record('1-provision', 'SKIP', `reusing workspace=${workspaceId} call=${sessionId}`);
  }

  // ── step 2: enable the voice experimental flag ─────────────────────────
  // The built-in voice connector is materialized server-side. The flag,
  // read back from the same PATCH response, is the enablement assertion.
  if (flag('skip-voice-flag')) {
    record('2-voice-flag', 'SKIP');
  } else if (!userJwt) {
    record('2-voice-flag', 'SKIP', 'no fresh user token (reusing a workspace)');
  } else {
    try {
      const res = await fetch(`${API}/workspaces/${workspaceId}/experimental`, {
        method: 'PATCH',
  ...
  // would carry (KORTIX_EXECUTOR_TOKEN), needed for every real connector call
  ...
  // ── step 4: spawn — the real kortix_voice connector action ─────────────
  ...
  if (flag('skip-spawn')) {
    ...
    try {
      const { status, body } = await executorCall(
        workspaceId!,
        sessionId!,
        executorToken,
        'spawn_room',
      ...
      if (status !== 200 || body?.ok !== true || !data?.join_url) {
        record('4-spawn', 'FAIL', `HTTP ${status} ${JSON.stringify(body).slice(0, 300)}`);
      } else {
        joinUrl = data.join_url as string;
        roomOpened = true;
        record('4-spawn', 'PASS', `call_id=${data.call_id} join_url=${joinUrl.slice(0, 70)}…`);
      }
    } catch (err) {
      abort('4-spawn', err);
    }
  ...
  // ── step 5: catalog — exact voice action surface, no blocking action ───
  if (flag('skip-mcp-list')) {
    ...
    try {
      const { status, body } = await executorCatalog(workspaceId!, executorToken);
      if (status !== 200) {
        record('5-mcp-tools-list', 'FAIL', `HTTP ${status} ${JSON.stringify(body).slice(0, 200)}`);
      } else {
        const voice = (body?.connectors ?? []).find(
          (connector: { slug?: string }) => connector.slug === 'kortix_voice',
        );
        const names: string[] = (voice?.actions ?? []).map(
          (action: { path: string }) => action.path,
        );
        const sameSet =
          names.length === EXPECTED_ACTIONS.length &&
          EXPECTED_ACTIONS.every((name) => names.includes(name));
  ...
  // ── step 8: trigger run_command by speech; assert its durable turn ──────
  if (flag('skip-run-command')) {
    ...
    try {
      ...
      const page = await readTurns(sessionId!, 0, 500);
      const hit = page.turns.some(
        (turn) => turn.role === 'tool' && turn.speaker === 'run_command',
      );
      record(
        '8-run-command',
        hit ? 'PASS' : 'FAIL',
        hit
          ? 'durable run_command tool turn found'
          : 'no durable run_command tool turn found after 15s',
      );
    } catch (err) {
  ...
  // ── step 9: read_transcript — turns + a monotonic cursor ───────────────
  if (flag('skip-voice-read')) {
    ...
    try {
      ...
      if (status !== 200) {
        ...
        const data = body?.data;
        const turns: Array<{ cursor: number }> = data?.turns ?? [];
  ...
  // ── step 10: end_call — assert the room actually closes ───────────────
  if (flag('skip-end')) {
    record('10-end', 'SKIP');
  } else if (!executorToken) {
    ...
    try {
      ...
      const ended = status === 200 && body?.ok === true && body?.data?.ended === true;
      await sleep(1500);
      const after = await svc.listRooms([roomName]).catch(() => []);

      if (before.length === 0) {
        record('10-end', 'WARN', 'room did not exist before end_call — was step 4 skipped?');
      } else {
        const roomGone = after.length === 0;
        record(
          '10-end',
          ended && roomGone ? 'PASS' : 'FAIL',
          `connectorEnded=${ended} roomBefore=${before.length} roomAfter=${after.length}`,
        );
      }
    } catch (err) {
