...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (f === '--workspace' && v) (a.workspace = v), i++;
  }
  return a;
}
...
async function main() {
  ...
  // A stable, obviously-fake workspace id keeps things self-consistent without
  // needing a real workspace row; the LiveKit room and its metadata are only
  // ever checked against themselves and the live call registry.
  const workspaceId = args.workspace ?? '00000000-0000-4000-8000-000000000000';
  ...
  setTimeout(async () => {
    ...
    console.log(
      result.delivered
        ...
        : `  [kortix -> call] ${result.reason ?? 'call not live'}`,
    );
  }, 5000);
