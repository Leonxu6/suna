/**
 * 18 — marketplace: public catalog browse (top-level) + this workspace's
 * installed registry items and available updates. All reads — install/remove
 * commit onto the workspace branch, so they are not exercised here.
 *
...
run("marketplace", async () => {
  ...
  const registry = kortix.workspace(workspaceId).registry;
  const installed = await registry.list();
  console.log(
    `✓ workspace registry.list(): ${JSON.stringify(installed).slice(0, 250)}…`,
  );
