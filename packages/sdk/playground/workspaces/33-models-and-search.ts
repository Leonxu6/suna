/**
 * 33 — the remaining workspace-level reads: LLM catalog, resolved model
 ...
 * connect app catalog.
 *
 * Run (from packages/sdk):  bun run playground/workspaces/33-models-and-search.ts [workspaceId]
 */
...
run("models-and-search", async () => {
  ...
  const catalog = await kortix.workspaces.llmCatalog(workspaceId);
  console.log(
    `✓ llmCatalog(): ${Object.keys(catalog.models).length} model(s): ${Object.keys(catalog.models).join(", ")}`,
  );

  const defaults = await workspace.modelDefaults.get();
  console.log(
    `✓ modelDefaults.get(): ${JSON.stringify(defaults).slice(0, 250)}`,
  );

  const search = await workspace.files.search("kortix", { limit: 5 });
  console.log(
    `✓ files.search('kortix'): ${JSON.stringify(search).slice(0, 200)}…`,
  );

  const history = await workspace.files
  ...
  const commits = await workspace.git.commits();
  const sha = (
    Array.isArray(commits) ? (commits[0] as { sha?: string }) : undefined
  )?.sha;
  if (sha) {
    const commit = await workspace.git.commit(sha);
    console.log(
  ...
  if ((status as { enabled?: boolean }).enabled) {
    const apps = await workspace.connectors.pipedream.listApps("github");
    console.log(
