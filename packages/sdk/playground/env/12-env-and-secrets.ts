 ...
 * round-trip. Secrets are workspace-scoped env vars every session's agent can
...
run("env-and-secrets", async () => {
  ...
  const before = await workspace.secrets.list();
  console.log(
  ...
  const during = await workspace.secrets.list();
  ...
  await workspace.secrets.remove(TEST_SECRET);
  const after = await workspace.secrets.list();
