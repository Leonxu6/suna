...
run("sandbox", async () => {
  ...
  const health = await kortix.workspaces.sandboxHealth(workspaceId);
  console.log(`✓ sandboxHealth(): ${JSON.stringify(health).slice(0, 250)}…`);

  const templates = await kortix.workspaces.sandboxTemplates(workspaceId);
  console.log(
    `✓ sandboxTemplates(): ${JSON.stringify(templates).slice(0, 250)}…`,
  );

  const sandboxes = await workspace.sandbox.list();
  console.log(`✓ sandbox.list(): ${JSON.stringify(sandboxes).slice(0, 250)}…`);

  const snapshots = await workspace.sandbox.snapshots();
  console.log(
