...
| `09-kaab-backend-wrapper.ts` | **Kortix as a Backend, end-to-end** — mint a connector → per-user profile → backend-origin session (`end_user_ref` + `secrets` + `connector_bindings`) → stream; one-shot CLI **and** an SSE service | PAT + workspace |
