...
|---|---|---|
| **Kortix REST API** (`apps/api`, `/v1/*`) | `backendApi` (Supabase bearer) | control plane — workspaces, session lifecycle, sandbox provisioning, git/versions, secrets, billing |
...
imports `node:async_hooks`, respectively. The canonical Workspace compatibility
subpath is `@kortix/sdk/workspaces-client`. The legacy subpaths
...
| op | Kortix REST | SDK |
|---|---|---|
| list / get / create / update | `GET/POST /v1/workspaces`, `GET/PUT /v1/workspaces/:id` | ✅ |
| detail (config+agents+skills+files) | `GET /v1/workspaces/:id/detail` | ✅ |
| provision / import linked repo / create repo | `POST /v1/workspaces/{provision,link-repository,create-repo}` | ✅ |
| github installs / repos / repository branches / collaborators | `GET /v1/workspaces/github/*`, `/:id/git/collaborators` | ✅ |
| model catalogs | `GET /v1/workspaces/:id/llm-catalog` (full runtime), `GET /v1/workspaces/:id/model-picker` (compact connected UI picker) | ✅ |
| experimental flags / onboarding | `GET/PUT /v1/workspaces/:id/{experimental,onboarding}` | ✅ |

### 3. Workspace secrets / env  ✅
`GET/POST/PUT/DELETE /v1/workspaces/:id/secrets[/:name]` · personal overrides · OAuth credential flow (`/oauth/:provider/{start,poll}`) · git-credential. → SDK `workspaces-client/secrets.ts`.

### 4. Workspace access / IAM (workspace-scoped)  ✅
`/v1/workspaces/:id/access` (+ invite, remove, pending-invites, access-requests approve/reject, group-grants). → `workspaces-client/access.ts`.

### 5. Session lifecycle (Kortix side)  ✅
| op | REST |
|---|---|
| list / create | `GET/POST /v1/workspaces/:id/sessions` |
| get / update / delete | `GET/PUT/DELETE /v1/workspaces/:id/sessions/:sid` |
...
| sharing (workspace) | `GET/PUT .../sessions/:sid/sharing` |
| transcript | `GET .../sessions/:sid/transcript` → `workspaces-client/sessions.ts`'s `getSessionTranscript` ✅, facade `session(pid,sid).transcript()` ✅ (previously listed ✅ here with no client fn behind it — that was false; now genuinely wired) |
...
| op | REST | SDK |
|---|---|---|
| list / create / revoke (account-scoped) | `GET/POST /v1/accounts/tokens`, `DELETE /v1/accounts/tokens/:tokenId` | `workspaces-client/tokens.ts` ✅, facade `kortix.accounts.tokens.{list,create,revoke}` ✅ |
| list / create / revoke (workspace-scoped, `KORTIX_TOKEN`) | `GET/POST /v1/workspaces/:id/cli-token`, `DELETE .../cli-token/:tokenId` | ✅, facade `workspace(id).tokens.{list,create,revoke}` ✅ |
...
- catalog/budget: `GET /v1/llm/models`, `GET /v1/workspaces/:id/llm-catalog`
- selection + persistence: `useOpenCodeLocal`, `useModelStore` ✅
- **gateway observability** (`/v1/workspaces/:id/gateway/{overview,logs,keys,budgets,series,errors}`) → client fully in SDK (`workspaces-client/gateway.ts`) ✅; hooks still web-local 🟡
- **gateway playground** — `workspace(id).gateway.playground(prompt, models, system?)` → `POST /v1/workspaces/:id/gateway/playground` (run one prompt, plus an optional system prompt, against up to 6 models side by side) ✅; UI: Playground tab in `gateway-view.tsx` (`useGatewayPlayground` hook) ✅
...
React hooks are still web-local (`features/files/`, + duplicated in `features/workspace-files/` — collapsing that twin remains open). **`useWorkspaceSearch` is alive and consumed (`features/workspace/command-palette.tsx`) — not dead.** `useLssSearch` / `useTextSearch` are already gone.

### 11. Git / versions / change-requests  🟡
Client fns in SDK (`git-history.ts`, `change-requests.ts`), **hooks partial** (`useChangeRequests` in `@kortix/sdk/react` ✅; the rest of `features/workspace-files` is still web-local):
| op | REST |
|---|---|
| commits / commit / diff | `GET /v1/workspaces/:id/commits[/:sha][/diff]` |
| branches | `GET /v1/workspaces/:id/branches` |
| file history / version-diff | `GET /v1/workspaces/:id/files/history`, `/version-diff` |
| change-requests CRUD | `GET/POST/PUT /v1/workspaces/:id/change-requests[/:cr]` |
| merge / merge-preview / close / reopen | `POST .../change-requests/:cr/{merge,close,reopen}`, `GET .../merge-preview` |
| **request-changes** (Review Center feedback) | `POST .../change-requests/:cr/request-changes` → client fn already existed (`requestChangesOnChangeRequest`), now also on the facade: `workspace(id).changeRequests.requestChanges(crId, feedback)` ✅ |
| workspace files (git-backed) | `GET /v1/workspaces/:id/files`, `POST /files/{content,search}`, `GET /files/archive` |

### 12. Connectors / integrations  ✅ (workspace) · 🟡 (executor)
- workspace connectors + sharing/policies → `workspaces-client/{connectors,policies}.ts` ✅
- executor runtime (`/v1/executor/workspaces/:id/connectors/*`, Slack/Pipedream/CUA) → 🟡 web-local (`lib/*`)

### 13. Triggers / scheduled tasks  🟡
`workspaces-client/triggers.ts` ✅ (client) ; `useWorkspaceTriggers` now in `@kortix/sdk/react` ✅ (list + create/update/remove/fire, invalidation-wired); the web app's own `hooks/scheduled-tasks` hook hasn't migrated onto it yet.

### 13b. Marketplace / registry install (workspace-scoped)  ✅
Installing/updating/removing a catalog item onto a workspace's default branch (a commit, not a runtime call) — distinct from browsing the catalog itself (client fns in `workspaces-client/marketplace-catalog.ts`, now also wrapped on the facade as top-level `kortix.marketplace.*` — see §13c). `workspaces-client/marketplace.ts` ✅; facade `workspace(id).marketplace.{list,install,updates,update,updateAll,remove}` and the identical `workspace(id).registry.{...}` alias ✅:
| op | REST |
|---|---|
| install | `POST /v1/workspaces/:id/marketplace/install` (+ `/registry/install` alias) |
| list installed | `GET /v1/workspaces/:id/marketplace` (+ `/registry` alias) |
| check for updates | `GET /v1/workspaces/:id/marketplace/updates` (+ `/registry/updates` alias) |
| update one / update all | `POST /v1/workspaces/:id/marketplace/{update,update-all}` (+ `/registry/...` alias) |
| remove | `DELETE /v1/workspaces/:id/marketplace/:name` (+ `/registry/:name` alias) |

### 13c. Marketplace catalog browse (public) + sources  ✅
Previously OUT OF SCOPE ("Marketplace catalog browsing"). Now wrapped
end-to-end: client fns in `workspaces-client/marketplace-catalog.ts` are on the
...
`workspace(id).marketplace.*` in §13b):
| op | REST |
|---|---|
...
value/credential. `workspaces-client/setup-links.ts` ✅; facade
`workspace(id).setupLinks.{requestSecret, requestConnector}` ✅:
| op | REST |
|---|---|
| mint a secret-entry link | `POST /v1/workspaces/:id/secret-requests` |
| mint a Pipedream Quick Connect link | `POST /v1/workspaces/:id/connect-requests` |

### 13e. Manifest validate + git token  ✅
Two small workspace-scoped mutations, added to `workspaces-client/workspaces.ts`:
- `workspace(id).validateManifest(raw)` → `POST /v1/workspaces/:id/manifest/validate`
  (validates a `kortix.yaml` — or legacy `kortix.toml` — manifest's raw text
  server-side, format auto-resolved from the workspace's manifest path; same
...
- `workspace(id).gitToken()` → `POST /v1/workspaces/:id/git-token` (mints a
  fresh scoped git push token for a *managed* workspace; throws/409s for BYO
  repos).

### 14. Sandbox lifecycle  ✅ / 🟡
- session-sandbox status/metrics/instances → `workspaces-client/{sandbox,session-sandbox}.ts` ✅
- `GET /v1/workspaces/:id/{sandbox-health,sandboxes}`, snapshots, warm-pool, `GET /v1/platform/sandbox/version*` → 🟡 client in `@kortix/sdk/platform-client` ✅; hooks web-local (`hooks/platform`)
...
unwired) now live in `workspaces-client/billing.ts` and are grouped on the
...
`POST /v1/transcription` (voice) client now in SDK (`workspaces-client/transcription.ts`) ✅; hooks still web-local (`hooks/transcription`) 🟡.

### 17. Channels (workspace-scoped)  🟡
Slack/email inbound-outbound installs live in `workspaces-client/channels.ts`; hooks remain web-local.
Also now wrapped: Slack file download/upload proxies
(`workspace(id).channels.slack.{getFile, uploadFile}` →
`GET/POST /v1/workspaces/:id/channels/slack/file[/upload]`) and the Meet
"bot speaks" action (`workspace(id).channels.meet.speak(botId, text, voice?)`
→ `POST /v1/workspaces/:id/channels/meet/speak`).

### 18. Account audit log (Enterprise)  ✅ (client + facade) / 🟡 (hooks)
Event list + CSV/JSONL export + outbound SIEM webhook CRUD, gated server-side on `audit.read`/`account.write` + the account's `auditAccess` entitlement. `workspaces-client/audit.ts` ✅; facade `kortix.accounts.audit.{log, export, webhooks: {list,create,update,remove}}` ✅ (accountId-first, like the rest of `kortix.accounts.*`); no hooks yet (this is an admin-console surface, low priority for the agent-product hooks):
| op | REST |
|---|---|
...
| Domain | Status |
|---|---|
| Auth, Workspaces, Secrets, Access, Session lifecycle | ✅ complete |
...
| Token minting (account + workspace-scoped CLI PATs) | ✅ complete — `workspaces-client/tokens.ts`, facade `kortix.accounts.tokens.*` / `workspace(id).tokens.*` |
| Marketplace/registry install (workspace-scoped) | ✅ complete — `workspaces-client/marketplace.ts`, facade `workspace(id).marketplace.*` / `.registry.*` |
| Public marketplace catalog browse + sources | ✅ complete — `workspaces-client/marketplace-catalog.ts`, facade `kortix.marketplace.*` |
| Billing mutations (checkout/subscription/credits) | ✅ complete — `workspaces-client/billing.ts`, facade `kortix.billing.{checkout, subscription, credits}` |
| Setup links, manifest validate, git token | ✅ complete — facade `workspace(id).{setupLinks, validateManifest, gitToken}` |
...
| Triggers, workspace secrets, change-requests | 🟡→partial ✅ — `useWorkspaceTriggers`/`useWorkspaceSecrets`/`useChangeRequests` now in `@kortix/sdk/react`; the pre-existing web hooks for these haven't migrated onto them yet |
| Executor connectors runtime | 🟡 web-local |
| kortix-master daemon family (tasks/tickets/workspaces/milestones/credentials/services) | ✅ client in SDK (`opencode/kortix-master.ts`, re-exported via `@kortix/sdk/opencode-client`) + hooks in `@kortix/sdk/react` (`use-kortix-master.ts`); web's `hooks/kortix/*` files are now thin re-export wrappers over them. Not on the ROOT barrel (deliberate — it's an opencode-runtime surface, reached via the opencode-client subpath) |

### To make the SDK the whole data layer
1. ~~Add a `files` client to the SDK~~ — **done**: `@kortix/sdk/files` wraps the daemon `/file` + `/find` endpoints (12 ops). Remaining: move `features/files` hooks in; **collapse the `features/workspace-files` twin** into it (backend-parameterized).
2. **Wrap the existing client fns as hooks** in the SDK: git/versions/change-requests (`useChangeRequests` ✅ done; commits/branches/diff still web-local), triggers (`useWorkspaceTriggers` ✅ done), gateway-observability, sandbox-admin, billing/account-state.
