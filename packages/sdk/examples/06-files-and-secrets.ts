/**
 * 06 — Workspace files (session-scoped) + workspace secrets.
 *
 ...
 * `workspace(id).secrets.upsert` writes an env var a session's agent reads at
 ...
 *   KORTIX_WORKSPACE_ID=... KORTIX_SESSION_ID=... \
...
async function main() {
  ...
  if (!apiKey || !workspaceId || !sessionId) {
    console.error('Set KORTIX_API_KEY, KORTIX_WORKSPACE_ID, and KORTIX_SESSION_ID and re-run.');
    process.exit(1);
  }
  ...
  // Workspace secret — scoped to the workspace, available to every session's
  // agent (not just this one) unless further restricted via `agentScope`.
  await kortix.workspace(workspaceId).secrets.upsert({
    name: 'EXAMPLE_API_KEY',
    value: 'sk-example-do-not-use',
  });
  const secrets = await kortix.workspace(workspaceId).secrets.list();
  console.log(
