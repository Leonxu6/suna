 ...
 *
 * createKortix → workspaces.list() → session(pid, sid).send() → session.stream()
 ...
 * server-side* transcript read (`core/rest/workspaces-client/sessions.ts`'s
...
async function main() {
  ...
  if (!apiKey || !workspaceId || !sessionId) {
    console.error('Set KORTIX_API_KEY, KORTIX_WORKSPACE_ID and KORTIX_SESSION_ID.');
    process.exit(1);
  }
  ...
  console.log(`${workspaces.length} workspace(s); using ${workspaceId}`);
