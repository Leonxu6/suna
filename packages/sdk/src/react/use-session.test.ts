...
import { SessionStartError } from '../core/rest/workspaces-client';
...
describe('ACP session title synchronization', () => {
  test('refetches the matching Kortix list and detail queries for a real title', async () => {
    ...
    expect(calls).toEqual([
      { queryKey: ['workspace-sessions', 'workspace-1'], type: 'active' },
      { queryKey: ['workspace-session', 'workspace-1', 'session-1'], type: 'active' },
    ]);
  });
  ...
  test('does not refetch for missing or placeholder ACP titles', async () => {
    ...
    syncAcpSessionTitleQuery(titleQueryClient, 'workspace-1', 'session-1', null);
    syncAcpSessionTitleQuery(titleQueryClient, 'workspace-1', 'session-1', {
  ...
  test('refetches authoritative session queries until a generated title appears', async () => {
    ...
    const queryClient = {
      getQueryData: (queryKey: string[]) => {
        if (queryKey[0] === 'workspace-sessions') {
      ...
      refetchQueries: (input: { queryKey: string[]; type: string }) => {
        calls.push(input);
        if (
          input.queryKey[0] === 'workspace-sessions' &&
          calls.filter((call) => call.queryKey[0] === 'workspace-sessions').length === 2
        ) {
          title = 'Research Marko Kraemer';
        }
        return Promise.resolve();
