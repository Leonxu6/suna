...
export function useSession(workspaceId: string, sessionId: string, options: UseSessionOptions = {}) {
  ...
  useEffect(() => {
    ...
    syncAcpSessionTitleQuery(queryClient, workspaceId, sessionId, { title: acpSessionTitle });
  }, [queryClient, workspaceId, sessionId, usesAcp, acpSessionTitle]);
  useEffect(
    () => () => {
  ...
  const sendParts = async (
    ...
    void refreshSessionTitleQueryUntilResolved(queryClient, workspaceId, sessionId, {
