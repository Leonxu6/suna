...
test('native OAuth2 lifecycle methods use profile-scoped generic routes', async () => {
  ...
  await ensureWorkspaceConnectorProfile('P1', 'generic-api');
  expect(last()).toMatchObject({ method: 'POST' });
  expect(last().url).toContain('/workspaces/P1/connectors/generic-api/oauth2/profile');
  ...
  expect(last().url).toContain('/workspaces/P1/connector-profiles/profile-1/oauth2/application');
...
test('listConnectors GETs the workspace connectors list', async () => {
  nextResponse = { status: 200, body: { connectors: [] } };
  const result = await listConnectors('P1');
  expect(last().url).toContain('/executor/workspaces/P1/connectors');
  expect(last().method).toBe('GET');
...
test('listConnectors is a silent background read — a 403 never hits the global error sink', async () => {
  // Fired at workspace mount (workspace-home tiles, sidebar setup checklist);
...
test('syncConnectors POSTs an empty body to the sync endpoint', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/sync');
  expect(last().method).toBe('POST');
...
test('setConnectorCredentialMode PUTs { mode }', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/slack/credential-mode');
...
test('setConnectorSensitive PUTs { sensitive }', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/slack/sensitive');
  expect(last().method).toBe('PUT');
...
test('getConnectorPolicies GETs the policies list', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/slack/policies');
  expect(last().method).toBe('GET');
...
test('getConnectorConfig GETs the config, url-encoding a slug with special characters', async () => {
  ...
  expect(last().url).toContain(
    `/executor/workspaces/P1/connectors/${encodeURIComponent('my app/v1')}/config`,
  );
...
test('setConnectorName PUTs { name }', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/slack/name');
  expect(last().method).toBe('PUT');
...
test('pipedreamConnect POSTs an empty body to the connect endpoint', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/github/connect');
  expect(last().method).toBe('POST');
  expect(last().body).toEqual({});
...
test('discoverConnectorAuth POSTs a draft to the auth-discovery endpoint', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/auth-discovery');
  expect(last().method).toBe('POST');
  expect(last().body).toEqual(draft);
});
...
test('deleteConnector DELETEs the connector by slug', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/slack');
...
test('listPipedreamApps GETs with no query string when no optional params are given', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/pipedream/apps');
...
test('listDiscoverIntegrations GETs a searchable cursor page', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/discover/integrations?');
...
test('getDiscoverIntegration GETs detail by encoded catalogue id', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/discover/integrations/detail?');
...
test('setConnectorCredential PUTs { value }', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/slack/credential');
  expect(last().method).toBe('PUT');
...
test('setConnectorCredential PUTs a native OAuth2 client-credentials configuration', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/sharepoint/credential');
  expect(last().method).toBe('PUT');
...
test('pipedreamFinalize POSTs an empty body to the connect/finalize endpoint', async () => {
  ...
  expect(last().url).toContain('/executor/workspaces/P1/connectors/github/connect/finalize');
  expect(last().method).toBe('POST');
  expect(last().body).toEqual({});
  expect(result).toEqual({ connected: true, accountId: 'acc_1' });
});

test('connection profile lifecycle uses the typed workspace profile routes', async () => {
  nextResponse = { status: 200, body: { profiles: [] } };
  await listConnectionProfiles('P1');
  expect(last().url).toContain('/workspaces/P1/connector-profiles');
  expect(last().method).toBe('GET');
...
test('member profile creation is owner-scoped by the API and never accepts an owner id', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/connector-profiles/me');
  expect(last().method).toBe('POST');
...
test('profile-specific Pipedream connect and finalize bind the OAuth identity to the profile', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/connector-profiles/profile-member/connect');
  expect(last().method).toBe('POST');
  ...
  expect(last().url).toContain('/workspaces/P1/connector-profiles/profile-member/connect/finalize');
  expect(last().method).toBe('POST');
  expect(last().body).toEqual({});
});
