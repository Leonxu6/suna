...
// response/request shape — see apps/api/src/workspaces/routes/agent-config.ts.
...
// server-side (workspace.customize.write). v2-only: `editable:false` on the GET
// means a v1 workspace — the UI degrades to the limited scope editor.
...
export async function getAgentConfig(workspaceId: string, agentName: string) {
  return unwrap(
    await backendApi.get<AgentConfigResponse>(
      `/workspaces/${workspaceId}/agents/${encodeURIComponent(agentName)}/config`,
    ),
  );
}
...
export async function updateAgentConfig(
  ...
  return unwrap(
    await backendApi.put<{
      ...
      schema_version: number;
      block: AgentConfigBlock | null;
    }>(`/workspaces/${workspaceId}/agents/${encodeURIComponent(agentName)}/config`, block),
  );
}

export interface UpdateWorkspaceDefaultAgentResponse {
  ok: boolean;
  default_agent: string;
}

/** Set the declared workspace default in `kortix.yaml` (v2 workspaces). */
export async function updateWorkspaceDefaultAgent(workspaceId: string, agentName: string) {
  return unwrap(
    await backendApi.put<UpdateWorkspaceDefaultAgentResponse>(
