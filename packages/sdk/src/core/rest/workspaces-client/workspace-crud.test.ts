...
const input = { account_id: 'acc-1', name: 'My First Workspace', seed_starter: true };

test('GitHub workspace creation accepts a marketplace workspace template', () => {
  const createInput: CreateWorkspaceRepoInput = {
    account_id: 'acc-1',
    name: 'support-agent',
...
test('returns ok:true with the parsed workspace on a real 200 body', async () => {
  nextResponse = () =>
    new Response(JSON.stringify({ workspace_id: 'proj-1', name: 'My First Workspace' }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });

  const result = await provisionWorkspaceWithToken(opts, input);
...
test('reports limitReached on a 403 with the workspace_limit_reached code', async () => {
  nextResponse = () =>
    new Response(JSON.stringify({ code: 'workspace_limit_reached' }), {
      status: 403,
      headers: { 'content-type': 'application/json' },
    });
...
test('returns ok:false without hitting the network when credentials are missing', async () => {
  ...
  const result = await provisionWorkspaceWithToken({ backendUrl: '', accessToken: '' }, input);
  expect(result).toEqual({ ok: false, limitReached: false });
  expect(calls).toHaveLength(0);
});

test('normalizes the provider-neutral default_agent field from legacy workspace config', async () => {
  configureKortix({
    backendUrl: 'http://backend.test/v1',
    getToken: async () => 'tok',
  });
  ...
  const detail = await getWorkspaceDetail('proj-1');
