...
test('getSlackInstallation hits the installation endpoint and returns null on failure', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/slack/installation');
...
test('connectSlack posts bot token + signing secret', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/slack/connect');
...
test('getSlackManifest hits the webhooks manifest route (not /workspaces)', async () => {
...
test('getEmailInstallation forwards an optional connector_slug query param', async () => {
  ...
  expect(last().url).toContain(
    '/workspaces/P1/channels/email/installation?connector_slug=custom_inbox',
  );
});
...
test('connectEmail posts the connect payload', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/email/connect');
...
test('getMeetVoices hits the meet voices endpoint and returns null on failure', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/meet/voices');
  expect(result).toBeNull();
});
...
test('setMeetVoice PUTs the selected voice', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/meet/voice');
...
test('setMeetBotName PUTs the bot name', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/meet/name');
  expect(last().method).toBe('PUT');
...
test('previewMeetVoice posts to the per-voice preview endpoint and returns null on failure', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/meet/voices/voice-1/preview');
  expect(last().method).toBe('POST');
...
test('getSlackChannelFile GETs the file proxy with the url query param', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/slack/file?url=');
...
test('uploadSlackChannelFile posts channel/filename/content_base64 to the upload proxy', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/slack/file/upload');
  expect(last().method).toBe('POST');
...
test('speakInMeeting posts bot_id/text/voice to the meet speak endpoint', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/meet/speak');
  expect(last().method).toBe('POST');
      ...
      bindings: [
        {
          ...
          agentName: null,
          opencodeModel: null,
          conversationPolicy: 'workspace_open',
          installedAt: '2026-01-01',
          effectiveAgent: { agent: 'support', source: 'workspace' },
        },
      ],
    },
  };
  const result = await listChannelBindings('P1');
  expect(last().url).toContain('/workspaces/P1/channels/bindings');
  expect(last().method).toBe('GET');
  expect(result.workspaceDefaultAgent).toBe('support');
  expect(result.bindings).toHaveLength(1);
  expect(result.bindings[0]?.effectiveAgent).toEqual({ agent: 'support', source: 'workspace' });
});
...
test('updateChannelBinding PATCHes the binding by id', async () => {
  ...
  expect(last().url).toContain('/workspaces/P1/channels/bindings/b1');
