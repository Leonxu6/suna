 ...
 *  scopes the engine resolves at match time (currently: workspace_group). */
export type PolicyScopeType = ResourceType | 'workspace_group';
...
// V2-only: which workspaces is this group attached to + at what role?
// Backed by GET /accounts/:id/iam/groups/:gid/workspace-grants. Each row
// can be detached via the per-workspace DELETE /workspaces/:pid/group-grants/:gid
// endpoint (already in workspaces-client as detachGroupFromWorkspace).
export interface GroupWorkspaceGrant {
   ...
   *  backend's workspace_group_grants.expires_at. */
  expires_at?: string | null;
}

export async function listGroupWorkspaceGrants(accountId: string, groupId: string) {
  return unwrap(
    await iamGet<{ grants: GroupWorkspaceGrant[] }>(
      `/accounts/${accountId}/iam/groups/${groupId}/workspace-grants`,
    ),
  ).grants;
}
...
// V2-only: which workspaces can this member reach, at what role, and how?
// `sources` tells the UI why they have access (one or more of):
//   implicit — they're an account owner/admin (manager on every workspace)
//   direct   — explicit workspace_members row
//   group    — inherited from a workspace_group_grants attachment
...
export async function listMemberWorkspaceAccess(accountId: string, userId: string) {
  return unwrap(
    await iamGet<{ workspaces: MemberWorkspaceAccess[] }>(
      `/accounts/${accountId}/iam/members/${userId}/workspace-access`,
    ),
  ).workspaces;
}
...
export interface ListAuditFilter {
  ...
  actor?: string;
  /** Prefix match on resource_type (e.g. "workspace_session"). */
  resource_type?: string;
