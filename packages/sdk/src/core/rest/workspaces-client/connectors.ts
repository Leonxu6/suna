...
// workspace-wide visible; the only access gate is the agent's `connectors`
...
export async function listConnectionProfiles(workspaceId: string) {
  return unwrap(
...
export async function reconcileMemberConnectionProfile(
  ...
  return unwrap(
    await backendApi.post<ConnectionProfile>(`/workspaces/${workspaceId}/connector-profiles/me`, input),
  );
}
...
export async function updateConnectionProfileCredential(
  ...
  return unwrap(
    await backendApi.put<{ ok: true }>(
      `/workspaces/${workspaceId}/connector-profiles/${profileId}/credential`,
      input,
    ),
  );
}

function profileOAuth2Path(workspaceId: string, profileId: string, suffix: string): string {
  return `/workspaces/${workspaceId}/connector-profiles/${profileId}/oauth2/${suffix}`;
}

export async function ensureWorkspaceConnectorProfile(workspaceId: string, slug: string) {
  return unwrap(
    await backendApi.post<{ profile_id: string }>(
      `/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}/oauth2/profile`,
      {},
    ),
  );
}
...
export async function putConnectionProfileOAuth2Application(
  ...
  return unwrap(
    await backendApi.put<{ ok: true }>(
      profileOAuth2Path(workspaceId, profileId, 'application'),
      input,
    ),
  );
}

export async function getConnectionProfileOAuth2Application(workspaceId: string, profileId: string) {
  return unwrap(
...
export async function discoverConnectionProfileOAuth2(
  ...
  return unwrap(
    await backendApi.post<{ metadata: Partial<OAuth2ApplicationView> }>(
      profileOAuth2Path(workspaceId, profileId, 'discover'),
      input,
    ),
  );
}
...
export async function startConnectionProfileOAuth2Authorization(
  ...
  return unwrap(
    await backendApi.post<OAuth2AuthorizationStartResult>(
      profileOAuth2Path(workspaceId, profileId, 'authorize'),
      input,
    ),
  );
}
...
export async function startConnectionProfileOAuth2DeviceAuthorization(
  ...
  return unwrap(
    await backendApi.post<OAuth2DeviceAuthorizationStartResult>(
      profileOAuth2Path(workspaceId, profileId, 'device'),
      input,
    ),
  );
}
...
export async function pollConnectionProfileOAuth2DeviceAuthorization(
  ...
  return unwrap(
    await backendApi.post<OAuth2ConnectionStatus>(
      profileOAuth2Path(workspaceId, profileId, `device/${encodeURIComponent(sessionId)}`),
      {},
    ),
  );
}

export async function getConnectionProfileOAuth2Status(workspaceId: string, profileId: string) {
  return unwrap(
    await backendApi.get<OAuth2ConnectionStatus>(profileOAuth2Path(workspaceId, profileId, 'status')),
  );
}

export async function revokeConnectionProfile(workspaceId: string, profileId: string) {
  return unwrap(
    await backendApi.put<{ ok: true }>(
      `/workspaces/${workspaceId}/connector-profiles/${profileId}/revoke`,
      {},
    ),
  );
}

export async function activateConnectionProfile(workspaceId: string, profileId: string) {
  return unwrap(
    await backendApi.put<{ ok: true }>(
      `/workspaces/${workspaceId}/connector-profiles/${profileId}/activate`,
      {},
    ),
  );
}
...
export async function pipedreamConnectConnectionProfile(
  ...
  return unwrap(
    await backendApi.post<{
      ...
      app?: string;
      connectUrl?: string;
    }>(`/workspaces/${workspaceId}/connector-profiles/${profileId}/connect`, input),
  );
}

export async function pipedreamFinalizeConnectionProfile(workspaceId: string, profileId: string) {
  return unwrap(
    await backendApi.post<{ connected: boolean; accountId?: string }>(
      `/workspaces/${workspaceId}/connector-profiles/${profileId}/connect/finalize`,
      {},
    ),
  );
}
...
export async function listConnectors(workspaceId: string) {
  return unwrap(
    // Background read fired at workspace mount (workspace-home tiles, sidebar
    // setup checklist) — never global-toast; callers render their own state.
    await backendApi.get<ConnectorsResponse>(`/executor/workspaces/${workspaceId}/connectors`, {
      showErrors: false,
    }),
  );
}
...
export async function syncConnectors(workspaceId: string) {
  return unwrap(
    await backendApi.post<ConnectorSyncResult>(
      `/executor/workspaces/${workspaceId}/connectors/sync`,
      {},
    ),
  );
}
...
export async function setConnectorCredentialMode(workspaceId: string, slug: string, mode: 'shared') {
  return unwrap(
    await backendApi.put<{ ok: boolean; sync?: ConnectorSyncResult }>(
      `/executor/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}/credential-mode`,
      { mode },
    ),
  );
}
...
export async function setConnectorSensitive(workspaceId: string, slug: string, sensitive: boolean) {
  return unwrap(
    await backendApi.put<{ ok: boolean; sync?: ConnectorSyncResult }>(
      `/executor/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}/sensitive`,
      { sensitive },
    ),
  );
}
...
export async function getConnectorConfig(workspaceId: string, slug: string) {
  return unwrap(
    await backendApi.get<ConnectorConfig>(
      `/executor/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}/config`,
    ),
  );
}
...
export async function setConnectorName(workspaceId: string, slug: string, name: string) {
  return unwrap(
    await backendApi.put<{ ok: boolean; sync?: ConnectorSyncResult }>(
      `/executor/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}/name`,
      { name },
    ),
  );
}
...
export async function pipedreamConnect(workspaceId: string, slug: string) {
  return unwrap(
    await backendApi.post<{
      ...
      app?: string;
      connectUrl?: string;
    }>(`/executor/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}/connect`, {}),
  );
}
...
export async function createConnector(workspaceId: string, draft: ConnectorDraftInput) {
  return unwrap(
    ...
    }>(`/executor/workspaces/${workspaceId}/connectors`, draft),
  );
}
...
export async function discoverConnectorAuth(workspaceId: string, draft: ConnectorDraftInput) {
  return unwrap(
    await backendApi.post<ConnectorAuthDiscovery>(
      `/executor/workspaces/${workspaceId}/connectors/auth-discovery`,
      draft,
    ),
  );
}
...
export async function deleteConnector(workspaceId: string, slug: string) {
  return unwrap(
    await backendApi.delete<{ ok: boolean }>(
      `/executor/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}`,
    ),
  );
}
...
export async function listPipedreamApps(workspaceId: string, q?: string, cursor?: string) {
  ...
  return unwrap(
    await backendApi.get<{
      ...
      nextCursor?: string;
      hasMore: boolean;
    }>(`/executor/workspaces/${workspaceId}/pipedream/apps${qs ? `?${qs}` : ''}`),
  );
}
...
export async function listDiscoverIntegrations(workspaceId: string, q?: string, cursor?: string) {
  const params = new URLSearchParams();
  if (q) params.set('q', q);
  if (cursor) params.set('cursor', cursor);
  const qs = params.toString();
  return unwrap(
    await backendApi.get<DiscoverIntegrationsPage>(
      `/executor/workspaces/${workspaceId}/discover/integrations${qs ? `?${qs}` : ''}`,
    ),
  );
}

export async function getDiscoverIntegration(workspaceId: string, id: string) {
  const params = new URLSearchParams({ id });
  return unwrap(
    await backendApi.get<DiscoverIntegrationDetail>(
      `/executor/workspaces/${workspaceId}/discover/integrations/detail?${params.toString()}`,
    ),
  );
}
...
export async function setConnectorCredential(
  ...
  return unwrap(
    await backendApi.put<{ ok: boolean }>(
      `/executor/workspaces/${workspaceId}/connectors/${encodeURIComponent(slug)}/credential`,
      input,
    ),
  );
}
