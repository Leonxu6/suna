...
The Kortix Marketplace is the workspace skill library. It lets agents and humans
...
- **The workspace owns the install.** Installing copies files into the workspace
...
- **Every mutation is a git commit.** Install, update, update-all, and remove
  commit the changed files and lock update to the workspace repo. There is no
  hidden runtime-only install state.
- **Existing workspaces stay compatible.** New installs use `registry-lock.json`.
  Older `skills-lock.json` files are read through the lock parser so workspaces
  can migrate without breaking installed-status and update checks.

## Consumer CLI

Use these commands from a Kortix workspace or session:
...
kortix marketplace install <name> --workspace <workspace-id> --dry-run
kortix marketplace status --workspace <workspace-id>
kortix marketplace updates --workspace <workspace-id>
kortix marketplace update <name> --workspace <workspace-id>
kortix marketplace update --all --workspace <workspace-id>
kortix marketplace remove <name> --workspace <workspace-id>
```

Prefer `--json` when an agent needs to make a decision from command output.
Do not scrape tables.

Inside a Kortix session, the CLI is pre-authenticated for the current workspace.
If the command needs a workspace id, use `kortix workspaces info --json` first or
pass the id the user gave you.

## Web UI

The web equivalent is the workspace's Marketplace or Customize surface:

- Explore/search lists available skills from enabled marketplace sources.
- Installed/status views read the workspace's lock file.
...
3. Install it:
   ```sh
   kortix marketplace install presentations --workspace <workspace-id>
   ```
...
a normal workspace code change: review the diff when needed and revert through git
...
```sh
kortix marketplace update presentations --workspace <workspace-id>
```
...
New workspaces are scaffolded from the minimal runtime floor, then selected
...
workspaces, but users can remove, edit, or replace them like ordinary workspace
...
Normal workspace agents should not run registry build/validate/publish to install
...
4. If nothing fits, create a focused workspace-local skill under
   `.kortix/opencode/skills/<name>/SKILL.md`.
5. If the skill should be reused across workspaces, move it into a registry source
   and publish/index that source separately.

Keep workspace-local skills small and specific. Marketplace skills should carry
