...
description: "The Kortix marketplace — how to discover capabilities and IMPORT them into this workspace. The marketplace is a catalog of open git repos: whole ready-made WORKSPACES (`registry:project`) and individual skills/agents/tools. There is no deterministic installer. You import an item by reading its source, merging the files that fit this workspace, and opening a change request. Use this skill when the user wants to browse the Kortix catalog, clone a workspace, import a skill or agent, or add a registry source. For the open npx-skills ecosystem, see find-skills-sh."
---

<skill name="kortix-marketplace">

The **Kortix marketplace** is the Kortix-native catalog. Every item is an **open
git repo** made of plain files — a whole clonable **workspace** (its `kortix.yaml`,
...
workspace's own files, then opening a change request. Everything you import becomes
...
<importing>
**The standard import recipe.** To add a marketplace item to this workspace:
...
2. **Detect what it is** from its file structure:
   - a `kortix.yaml` at the repo root → a **workspace** (agents + skills + tools +
     config),
   - a `SKILL.md` → a **skill**,
   - an agent definition (an `agents/*.md`) → an **agent**,
   - a tool file (a `tools/*.ts`) → a **tool**.
3. **Merge what fits into this workspace**, in the right place:
   - skills → `.kortix/opencode/skills/<name>/`,
   - agents → `.kortix/opencode/agents/`, tools → `.kortix/opencode/tools/`,
   - for a **whole workspace**: pull in its skills/agents/tools and reconcile its
     `kortix.yaml` into this workspace's (merge connectors/triggers/agents — never
     blindly overwrite the user's manifest). Use judgment; keep what's relevant.
4. **Adapt it to this workspace.** If a skill carries a "TODO: make this yours"
   placeholder or generic copy, tailor it to what this workspace actually does.
...
truth** for what's in this workspace.
</importing>

<workspaces>
A whole **workspace** item is the richest thing in the marketplace — a working,
end-to-end Kortix workspace focused on a function. You can spin it up as its **own
new workspace** (from the web UI's "Add to a workspace" → new workspace, which seeds
the files deterministically), or **import/merge it into this existing workspace**
via the recipe above to make this workspace bigger. It's all just files.
</workspaces>
...
<vs-find-skills-sh>
- **`kortix-marketplace`** (this) — the Kortix catalog: whole workspaces + skills/
...
Reach for the Kortix marketplace first for anything workspace-shaped; reach for
