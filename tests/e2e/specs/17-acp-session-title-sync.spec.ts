...
async function waitForReadySession(
  ...
  while (Date.now() < deadline) {
    ...
    }>(token, 'POST', `/workspaces/${workspaceId}/sessions/${sessionId}/start?wait_ms=8000`, {});
    last = [
      result.stage,
...
test.describe.serial('17 — ACP session title synchronization', () => {
  ...
  test.beforeAll(async ({ browser: _browser }, testInfo) => {
    ...
    const workspace = await api<{ workspace_id: string }>(
      auth.access_token,
      'POST',
    ...
    await api(auth.access_token, 'PATCH', `/workspaces/${workspaceId}/onboarding`, {
      completed: true,
    });
    await api(auth.access_token, 'PUT', `/workspaces/${workspaceId}/model-defaults`, {
      scope: 'workspace',
  ...
  test.afterAll(async ({ browser: _browser }, testInfo) => {
    ...
    if (workspaceId) {
      await api(auth.access_token, 'DELETE', `/workspaces/${workspaceId}`).catch(() => {});
    }
    if (user?.id) {
  ...
  test('updates the sidebar from ACP and REST titles without a refresh', async ({ page }) => {
    ...
    const sessionLists: WorkspaceSession[][] = [];
    ...
    page.on('response', async (response) => {
      ...
      if (Array.isArray(body)) sessionLists.push(body as WorkspaceSession[]);
    });

    for (const transport of ['acp', 'rest'] as const) {
      await api(auth.access_token, 'PATCH', `/workspaces/${workspaceId}/experimental`, {
        feature: 'acp_runtime',
      ...
      await waitForReadySession(auth.access_token, workspaceId, sessionId, transport);

      const path = `/workspaces/${workspaceId}/sessions/${sessionId}${
        transport === 'acp' ? '?acp' : ''
      }`;
      ...
      const sessionTitle = page.locator(
        `a[href="/workspaces/${workspaceId}/sessions/${sessionId}"] span[title]`,
      );
      ...
      const persisted = await api<WorkspaceSession[]>(
        auth.access_token,
        'GET',
        `/workspaces/${workspaceId}/sessions`,
      );
