...
Single source of truth for the e2e suite. Every flow the platform supports, start→finish, enumerated. Each step is `METHOD /path → expected`. CLI steps are `kortix …`. Negatives (`→ 4xx`) are part of the flow, not optional. Each flow has a stable ID (`WS-3`, `IAM-7`) so a test maps 1:1 to a line here.
...
- Auth header: `Authorization: Bearer <token>`. Token types:
  ...
  - **PAT** — `kortix_pat_…` CLI personal access token (`account_tokens`). Carries real `userId`. May be **workspace-scoped** (`workspaceId` set).
...
- Auth middlewares: `supabaseAuth` (JWT or PAT) on `/v1/accounts/*`, `/v1/workspaces/*`, `/v1/platform/api-keys`. `combinedAuth` (JWT|token|PAT|cookie|`X-Kortix-Token`|`?token=`) on `/v1/p/*`, `/v1/servers/*`, `/v1/tunnel/*`. `apiKeyAuth` (kortix_ only) on `/v1/router/*`. `requireAdmin` (platform role) on `/v1/ops/*`. Webhooks = HMAC, no auth middleware.
- Workspace authz gate `loadWorkspaceForUser(c, id, level)`: `read`→`WORKSPACE_READ` (any workspace role), `write`→`WORKSPACE_WRITE` (editor/manager), `manage`→`WORKSPACE_DELETE` (manager only). Account owner/admin get implicit `manager` on every workspace.
...
| `MEMBER`                          | account `member`, **no** workspace grant               | account-reads only; cannot see workspaces                                                                                                                                                                                                                                                        |
| `M_VIEWER` `M_EDITOR` `M_MANAGER` | member + project_members row (viewer/editor/manager) | per-workspace (read + run-sessions) / +customize (write) / +manage. Viewer is the base _usable_ role: it can read AND start/run/stop sessions (use the agent chat) — it just can't customize the workspace. So POST `/workspaces/:id/sessions` is allowed for M_VIEWER; PATCH `/workspaces/:id` is not |
...
| `DENY_USER`                       | member with explicit `deny` policy on a workspace      | deny-wins over any allow                                                                                                                                                                                                                                                                       |
...
| `PAT_WORKSPACE`                        | workspace-scoped PAT (minted per session)              | hard-fenced to one workspace                                                                                                                                                                                                                                                                     |
...
3. `kortix ship -y` (no `origin` present) → managed path: `POST /workspaces/provision` → 201 `{push_token, repo_id, repo_url}`; `git remote add origin <managed GitHub URL>`; commit; token-header push; writes `.kortix/link.json`. (§14)
4. Poll `GET /workspaces/:id/snapshots` → wait for a `ready` snapshot. (§4)
5. `kortix secrets set STRIPE_API_KEY=sk_live_…` → `POST /workspaces/:id/secrets` → 200. (§6)
6. `kortix sessions new -p "add a README"` → `POST /workspaces/:id/sessions` → 201 status `provisioning`; branch `<sessionId>` created. (§7)
7. Poll `GET /workspaces/:id/sessions/:sid/sandbox` → status `provisioning`→`active`. (§8)
...
9. `POST /workspaces/:id/change-requests {head_ref:<sessionId>, title}` → 201; `GET …/:crId/merge-preview` → mergeable; `POST …/:crId/merge` → 200 status `merged`. (§11)
10. `DELETE /workspaces/:id/sessions/:sid` → 200 status `stopped` (branch preserved). (§7)
...
`MEM-1` `GET /accounts/:id/members` → every account member sees the teammate directory; PAT counts, MFA, groups, and workspace-grant counts are visible only on self unless the caller can manage members.
...
`MEM-3` `PATCH /accounts/:id/members/:userId {role}` → `MEMBER_UPDATE` → 200; same role → 200 `{unchanged:true}` (no-op); **promoting/demoting `owner` additionally requires `MEMBER_SUPER_ADMIN_GRANT`** (owner only) → ADMIN owner-grant → 403; **promotion to owner/admin deletes the member's `project_members` rows + workspace policies**.
...
`TOK-2` `POST /accounts/tokens {name}` → `TOKEN_CREATE` → 201, `secret_key` returned **once** (absent from list). Account-scoped only — this route does **not** accept `workspaceId`; workspace-scoped PATs are minted via the workspace cli-token route (GH-8).
`TOK-3` `DELETE /accounts/tokens/:tokenId` → `TOKEN_REVOKE` → 200; unknown/already-revoked → 404; revoked token on any route → 401.
`TOK-4` workspace-scoped PAT (`workspaceId` set): allowed only on its own workspace + `/accounts/me`; **everything else → 403** (other workspace, `/accounts/*`, workspace-list, and all other surfaces — `enforceTokenWorkspaceScope`).
...
`IAM-4` Effective probe: `GET …/iam/members/:userId/effective?action=…[&resourceType=&resourceId=]` (`MEMBER_READ`; self-probe always allowed) → `{allowed, reason, action, resource_type}`. Built-in account/workspace membership remains the default decision source; custom policies are additive and covered in `IAM-25/26`.
...
`IAM-9` **super-admin bypass** — the account creator is super-admin; their effective probe (`…/members/:userId/effective`) is `allowed:true reason:super_admin` for every action (account-write, workspace.create, and any workspace action on any/unknown workspace) regardless of policies or workspace membership. A revoked-super-admin owner still passes via `account_role`/`workspace_role`, never `super_admin`. Asserted via the effective endpoint.
`IAM-10` **no deny precedence** — V2 has NO deny rules (engine: "No deny precedence"; access is allow-by-role only, max-role-wins across direct+group sources). There is no constructible allow+deny conflict via real routes. Closest assertion: stack a low (viewer) direct role and a high (manager) group grant on the same workspace — effective `workspace.delete` is `allowed:true` (max wins, never denied by the lower grant). NOTE: classic deny-wins is unverifiable black-box because the feature does not exist.
`IAM-11` **PATs inherit the minter (no token-only policy eval)** — V2 has no per-token policies; a PAT carries no narrowing policy set, it only optionally binds to one workspace (`account_tokens.project_id`). An unscoped account PAT's effective access equals its minter's (owner → super-admin set). Asserted by exercising the same `…/effective` reads as the JWT owner. NOTE: per-token policy evaluation is unverifiable black-box because the feature does not exist; workspace-bound-PAT scope narrowing is covered indirectly by the token/scope flows, not here.
`IAM-12` **legacy role bridge** — `account_role` maps to the V2 action set: a plain `member` gets account-reads only — `account.read` allowed but `account.write`/`workspace.create` denied (`reason:account_role_insufficient`), and a workspace action on a workspace they're not on is denied (`reason:no_workspace_membership`), so they cannot reach all workspaces. owner/admin → Administrator-level set (`account.write` allowed; implicit Manager on every workspace). Asserted via the effective endpoint.
`IAM-13` **scope match** — a workspace group-grant matches only its own workspace. Grant a group Manager on workspace A; a member of that group probed with `resourceType=workspace&resourceId=A` → `workspace.delete` allowed (`reason:workspace_role`); the same probe against workspace B (no grant) → denied (`reason:no_workspace_membership`). Asserted via the effective endpoint with/without the matching `resourceId`.
...
### Approval control-plane (workspace access-requests, approvals, agent/connector scoping)

The human-in-the-loop surface an agent's write/destructive tool calls gate on, plus its adjacent per-agent scoping and the Enterprise preview/import surfaces. `GET /workspaces/:id/approvals[/needs-input]` and `POST /workspaces/:id/approvals/:id` gate on plain IAM capability (`workspace.members.manage`/`workspace.read`), never a billing tier — see PR #4117 (a prior 402 regression on the per-session audit poll); these must never start 402ing.

`IAM-27` `POST /workspaces/:id/access-requests {message?}` (any signed-in caller; already-has-access short-circuits `{status:"already_has_access"}`) → 201 `{status:"created",request}`; re-request while pending → 200 `{status:"pending",request}`. `GET /workspaces/:id/access-requests` (`workspace.members.manage`) → 200 `{requests:[...]}` pending only; caller with no workspace grant → 403; unknown workspace → 404.
`IAM-28` `POST /workspaces/:id/access-requests/:rid/approve {role?}` / `.../reject` (`workspace.members.manage` — stricter than plain `manage`/`workspace.write`, so an editor without members-manage → 403) → 200 grants the workspace role (`ensureOrgMembership` + `grantWorkspaceRole`) and marks the request `approved`/`rejected`; invalid `role` → 400; already-reviewed → 409; unknown request id → 404.
`IAM-29` `GET /workspaces/:id/approvals` (manager-only inbox of unresolved `pending_approval` executor actions) → 200 `{count,approvals}`; out-of-range `limit` → 400; non-manager with no grant → 403. `GET /workspaces/:id/approvals/needs-input` (`read` — any workspace member) → 200 `{total,sessions}`; a manager sees every session's pending count, a non-manager only their own launched sessions; non-member → 403.
`IAM-30` `POST /workspaces/:id/approvals/:executionId {decision:"approve"|"deny",scope?}` (manager OR the session launcher) → resolves a pending executor action atomically (TOCTOU-safe); malformed execution id → 400; invalid `decision` → 400 (validated before the row lookup); unknown execution id → 404; non-workspace-member → 403; already-resolved → 409 (happy-path resolve of a REAL pending row needs a live governed connector call from an agent session — not black-box reproducible here, same constraint as `SESS-11`).
`IAM-31` `PUT /workspaces/:id/agents/:agentName/scope {env?,connectors?}` (`manage`) — writes the `[[agents]].env`/`.connectors` allowlists into `kortix.yaml` (or legacy `kortix.toml`); empty body (`nothing_to_update`) → 400; malformed grant set → 400; unknown agent name → 404 (`agent_not_found`); caller with no workspace grant → 403.
...
## 6. Workspaces — CRUD + access

DB `projects` (`status active|archived`, unique `(account_id, repo_url)`). Soft delete → `archived`.

`WS-1` `GET /workspaces` → OWNER/ADMIN: all account workspaces; `MEMBER`: only `project_members` grants; `NONMEMBER`: empty/own only.
`WS-2` `POST /workspaces {repo_url,name}` (BYO) → `WORKSPACE_CREATE` (OWNER/ADMIN) → 201, creator granted `manager`, snapshot build kicked. `MEMBER` → 403. Non-GitHub `repo_url` → 400.
`WS-3` `POST /workspaces/provision {name,provider?:github}` (managed) → `WORKSPACE_CREATE` → 201 `{push_token,repo_id,repo_url}`. Unconfigured managed GitHub backend → 503.
`WS-4` `POST /workspaces/create-repo {name,private?}` (new GitHub repo) → `WORKSPACE_CREATE` → 201; no account GitHub App install → 409 + `install_url`; auto-dedupes name collision.
`WS-5` `GET /workspaces/:id` → `read` → 200 (bumps `last_opened_at`); archived → 404; `NONMEMBER` → 403.
`WS-6` `GET /workspaces/:id/detail` → `read` → 200 workspace + parsed `kortix.yaml` (agents/skills/env) + file list.
`WS-7` `PATCH /workspaces/:id {name,default_branch,manifest_path}` → `manage` (M_MANAGER/OWNER/ADMIN) → 200; M_EDITOR/M_VIEWER → 403.
`WS-8` `DELETE /workspaces/:id` → `manage` → 200 status `archived`; M_EDITOR → 403.

### Workspace access (membership)

`WACC-1` `GET /workspaces/:id/access` → `read` → members + effective workspace roles.
`WACC-2` `POST /workspaces/:id/access/invite {email,role}` → `manage`. **Existing Kortix user → 200** — `ensureOrgMembership` auto-adds them to the org as `member` then grants the workspace role (account-manager target → implicit access, `workspace_role:null`). **Email with no Kortix account yet → 201 `{status:"invited", invite_id, invite_url, workspace_role}`** — an account invitation with a `bootstrap_grant` is created/merged idempotently so they land on the workspace at signup. Missing email / bad role → 400; non-account-member caller → 403 (`loadWorkspaceForUser` — 404 only when the workspace row is missing/archived).
`WACC-3` `PUT /workspaces/:id/access/:userId {role}` → `manage`.
`WACC-4` `DELETE /workspaces/:id/access/:userId` → `manage`.
`WACC-7` `GET/POST/DELETE /workspaces/:id/resource-grants[/:grantId]` → manager-only per-resource scoping. **AGENT-ONLY (resource-model simplification): `agent` is the only member/department-scopable resource** — assigning an agent lets the assignee USE it and inherit its declared skills/connectors/secrets (to USE, not edit; editing needs the editor role). A POST with `resource_type=skill` or `secret` → **400** (agent-only; the guard runs before any config/DB load, so no existing resource is needed). Reading/listing/revoking pre-existing skill/secret grant rows still works (back-compat), but none can be CREATED. GET lists grantable resources (`$.resources.agents`) + existing grants. POST `resource_type=agent` with a real agent id + member/group principal → 201; unknown/invalid `resource_type` (e.g. `database`) → 400; invalid/foreign principal → 400/404; deleting unknown grant → 404.
...
`SESS-1` `POST /workspaces/:id/sessions {agent_name?,initial_prompt?,base_ref?,provider?,name?,session_id?,branch_already_created?,runtime_context?}` → `session` (any workspace member, **M_VIEWER included** — viewer is the base usable role) → 201 status `provisioning` (fire-and-forget sandbox). `runtime_context` is an optional non-secret scalar map (max 64 entries / 16 KiB UTF-8 JSON; lower-case semantic keys only; no credential-like keys); it is stored outside user-editable session metadata and restored into every replacement runtime as the single server-owned `KORTIX_SESSION_CONTEXT` JSON variable. Nested/oversize/reserved/credential-shaped input → 400 before session persistence or provisioning; raw env/MCP fields are unknown and rejected. MEMBER with no workspace grant / NONMEMBER → 403. (An invalid `provider` for an allowed caller → 400, proving the role gate passed before provider validation.)
`SESS-2` concurrency cap — Nth session over tier cap → **429** + `X-RateLimit-Limit/-Remaining` headers.
`SESS-3` CLI client-branch optimization — `kortix sessions new`: if server can't self-create a branch through its configured Git credentials AND local `origin` == `workspace.repo_url`, CLI mints uuid, `git push origin HEAD:refs/heads/<uuid>`, then posts `session_id`+`branch_already_created:true`+`base_ref`.
`SESS-4` `GET /workspaces/:id/sessions` → `read` → list (updatedAt desc).
`SESS-5` `GET /workspaces/:id/sessions/:sid` → `read` → 200; non-uuid `sid` → 400.
`SESS-6` `PATCH /workspaces/:id/sessions/:sid {name?,metadata?}` → `session` (any workspace member, M_VIEWER included); attempting `status`/`sandbox_url`/`error`/`opencode_session_id` → 400 (server-managed); any other field → 400 (not user-editable). `name` sets a sticky USER override stored in `metadata.custom_name` (NOT clobbered by the server-side OpenCode title mirror, which only writes the auto title `metadata.name` during session reads); `name:""`/null clears it. Response `name` = resolved display (`custom_name ?? metadata.name`); `custom_name` exposed separately (authoritative override or null).
`SESS-7` `DELETE /workspaces/:id/sessions/:sid` → `session` (then **owner or workspace manager** only — a viewer can stop sessions they own) → 200 soft-delete status `stopped`; **remote branch preserved**.
`SESS-8` `GET /workspaces/:id/sessions/:sid/sandbox` → `read` → `session_sandboxes` row; **404 while row not yet inserted** (frontend polls); then status `provisioning`→`active` with `base_url`/`external_id`.
`SESS-9` `POST /workspaces/:id/sessions/:sid/restart` → `session` (then **owner or workspace manager** only) → **202**; tears down container, revokes sandbox keys, re-provisions with rotated git/LLM/CLI tokens (status→`provisioning`); branch preserved.
`SESS-10` OpenCode title/tree mirror is server-owned: `GET /workspaces/:id/sessions` and `GET /workspaces/:id/sessions/:sid` read the sandbox's OpenCode sessions server-side and mirror `metadata.name`/`metadata.opencode_sessions`; there is no browser-write sync endpoint.
`SESS-12` `POST /workspaces/:id/sessions/:sid/stop` → `session` (then **owner or workspace manager** only) → **200** status `stopped`, sandbox paused in place (disk kept, no re-provision — same contract as an idle auto-stop); resumable via `/start`/`SESS-9`. Sandbox not `active` → 409; unsupported provider → 400.
`SESS-13` Public shares — `GET/POST /workspaces/:id/sessions/:sid/public-shares` and `DELETE …/public-shares/:shareId` → gated on `canManageSharing` (session owner or workspace manager; see `SESS-14`). POST body `{preview:{port,path?,label?},file?,mode?,expires_at?}` → 201 `{share:{share_id,session_id,project_id,resource_type,label,port,path,file_path,mode,allow_websocket,expires_at,revoked_at,created_at,updated_at,public_token,public_path,proxy_path}}` (`public_token` = `kps_<shareId no dashes>`); a blocked/out-of-range port or an invalid `expires_at` → 400. GET list → `{shares:[…]}`, **not filtered** — a revoked share stays in the list with `revoked_at` set (revoking never deletes the row). DELETE `:shareId` → 200 `{share:{…,revoked_at}}`; **no idempotency guard** — revoking an already-revoked share is 200 again, not 409; unknown `shareId` → 404; non-uuid `sid`/`shareId` → 400. The unauthenticated counterpart is `GET /p/public-share/:token` (§9, mounted public — no auth middleware at all): unknown token → 404 `Share link not found`; **revoked token → 410** `Share link revoked` (checked before the sandbox); a real token whose sandbox has no `external_id` yet → 503 `Sandbox is not ready` (`resolvePublicShare` LEFT JOINs `session_sandboxes` for exactly this case — a freshly-created, not-yet-provisioned session has no sandbox row at all; an INNER JOIN previously misread that as a 404 "not found" on a perfectly valid token); a resolvable `preview` share proxies to `GET {origin}/p/public-share/:token/:port/*` (redirect for the bare `:port` form), a `file` share to `.../file[/*]` (view-only — non-GET/HEAD/OPTIONS → 405).
`SESS-14` Public-share access gate (`loadSessionForSharing().canManageSharing = isOwner || canManageWorkspace` — workspaces/lib/access.ts; **not** `loadVisibleSession()`, which gates on session-content visibility and made a manager's `canManageWorkspace` half unreachable on the default-`private` session, 404ing before the sharing check ever ran) — the session **creator** may manage its shares regardless of workspace role; a workspace **manager** (or owner/admin) may manage ANY session's shares even if they didn't create it; a workspace **editor/viewer who is not the creator** → 403 (`"Only the session owner or a workspace manager can …"`, a real permission denial — they're a legitimate workspace member, so there's nothing to 404-hide); NONMEMBER → 403 (account-membership gate, before the sharing check); ANON → 401.
`SESS-15` `GET /workspaces/:id/sessions/:sid/audit` → `read` + session-visible → 200 `{session_id,agent,audit_access,count,actions:[{execution_id,action,connector_id,status,risk,acted_by,acted_by_email,resolved_by,resolved_by_email,result_summary,at,resolved_at}]}` (most-recent-first, `?limit=` 1–1000, default 200; invalid `limit` → 400). This is the always-on approval control plane the launcher polls from every open session — non-Enterprise accounts (`auditAccess` entitlement off) degrade to **unresolved pending approvals only**, never a 402. Non-uuid `sid` → 400; NONMEMBER → 403; ANON → 401.
...
`SESS-18` Server-owned warm session — `POST /workspaces/:id/sessions/warm {}` creates one private available session for the current user and returns `{session,reused:false,workspace_refresh:{status:"skipped"}}`; a second ensure returns the same `session_id` with `reused:true`. `POST /workspaces/:id/sessions/warm/claim {session_id,agent_name?,sandbox_slug?}` atomically changes its warm state to `claimed`; a second claim returns 409 `WARM_SESSION_ALREADY_CLAIMED`; the next ensure creates a different available session. A selected agent or sandbox mismatch returns 409 `WARM_SESSION_CONFIGURATION_MISMATCH`.

---

## 8. Sandbox lifecycle + snapshots

`SNAP-1` `GET /workspaces/:id/snapshots` → `read` → list `kortix-snap-…` images per baseRef. **Session boot requires a `ready` snapshot of baseRef** (no shared fallback → session `failed` if none).
`SNAP-2` `POST /workspaces/:id/snapshots/rebuild` → **`manage` AND account `ACCOUNT_WRITE` (owner/admin)** → rebuild image. A workspace `manager` who is not owner/admin → 403; M_EDITOR → 403.
...
Repo files are read-only over the workspace API; live edits happen in the sandbox (OpenCode file API via proxy) or via manifest commits. All git reads are `read`.

`FILE-1` `GET /workspaces/:id/files?ref=&path=` → file/dir listing.
`FILE-2` `GET /workspaces/:id/files/content?path=&ref=` → file text; **absent `path` param → 400**; non-existent file path is uncaught → surfaces 500 (not 404).
`FILE-3` `GET /workspaces/:id/files/search?q=&content=1&ref=&limit=` → filename + grep.
`FILE-4` `GET /workspaces/:id/files/history?path=` → commit history for path.
`FILE-5` `GET /workspaces/:id/files/archive?path=&ref=` → zip stream.
`FILE-6` `GET /workspaces/:id/branches` → branches.
`FILE-7` `GET /workspaces/:id/commits?ref=&path=` · `GET …/commits/:sha` · `GET …/commits/:sha/diff`.
`FILE-8` `GET /workspaces/:id/version-diff?from=|head=&into=|base=` → diff between two refs (params are `from`/`head` and `into`/`base` — there is **no `to`**).
...
DB `change_requests` (per-workspace `number`, `status open|merged|closed`).

`CR-1` `GET /workspaces/:id/change-requests?status=open|merged|closed|all` → `read`.
`CR-2` `POST /workspaces/:id/change-requests {title,head_ref,base_ref?,description?,session_id?}` → `write` → 201, head/base SHAs anchored. Missing `title` → 400; missing `head_ref` → 400; `base_ref==head_ref` → 400; head with no commits ahead of base (equal tip, or merge-base == head behind an advanced base) → 422 `CR_HEAD_NOT_AHEAD` — an empty CR can never be created (the resolver force-refreshes the mirror before rejecting, so a just-pushed head never bounces).
...
`CR-9` CLI mirror: `kortix cr ls|show|diff|open|merge|close|reopen` (reads `KORTIX_WORKSPACE_ID` inside sandbox; `KORTIX_PROJECT_ID` remains a deprecated fallback).
`CR-10` response envelopes (assert shape): list → `{change_requests:[…]}`, get → `{change_request:{…}}`, merge → `{change_request, merge}`. (Workspace DELETE returns `{ok:true}`, not an echoed status.)

---

## 11b. Review Center (per-workspace human-in-the-loop inbox)

DB `review_items` (per-workspace; `kind change|approval|output|decision|batch`, `status needs_you|waiting|approved|changes_requested|rejected|done|dismissed`, polymorphic `detail` jsonb). This pass: native items only (`output|decision|batch` via submit); `change`/`approval` are folded in by adapters later.

`RV-1` `GET /workspaces/:id/review/items?segment=needs_you|waiting|done&kind=…` → `read` → `{review_items:[…]}`. Invalid `segment` → 400; invalid `kind` → 400.
`RV-2` `GET …/review/items/:reviewItemId` → `read` → `{review_item:{…}}`; unknown id → 404.
`RV-3` `POST …/review/items {kind(output|decision|batch),title,summary?,risk?,detail?,agent?,session_id?}` → `read` + agent scope `workspace.review.submit` → 201. Missing `title` → 400; non-submittable `kind` (e.g. `change`) → 400; invalid `risk` → 400.
`RV-4` `POST …/review/items/:reviewItemId/act {verdict(approve|reject|changes|answer|dismiss),feedback?}` → `write` + `workspace.review.act` → 200 updated item; invalid `verdict` → 400; unknown id → 404; adapted (`cr:…`) id → 409 (act from the source view). The list read-model also folds in Change Requests as `kind:change` items (id `cr:<crId>`).
`RV-5` `POST …/review/bulk {ids:[…],verdict}` → `write` + `workspace.review.act` → 200 `{updated,review_items}`; empty/missing `ids` → 400.
...
`TRG-1` `GET /workspaces/:id/triggers` → `read` + leaf `workspace.trigger.read` → specs + `last_fired_at` + parse `errors` + `webhook_url`; non-member 403/404; ANON 401.
`TRG-2` `POST /workspaces/:id/triggers {name(required),slug?,type,agent?,enabled?,prompt_template,cron?,timezone?,secret_env?}` → `manage` → 201, manifest committed; `name` is required (slug derived from it when omitted); duplicate slug → 409. `webhook` requires `secret_env` (names a `project_secrets` key, regex `^[A-Z_][A-Z0-9_]*$`). `cron` requires 6-field croner expr + IANA `timezone` (default UTC).
`TRG-3` `PATCH /workspaces/:id/triggers/:slug` (e.g. `{enabled:false}`) → `manage`.
`TRG-4` `DELETE /workspaces/:id/triggers/:slug` → `manage` (also drops runtime row).
`TRG-5` `POST /workspaces/:id/triggers/:slug/fire` → `manage` → manual fire → 202 `{status:fired,session_id}`; under backpressure → 202 `{status:queued,reason}`.
`TRG-6` cron scheduler — global `setInterval` (default 60s), sweeps ≤200 active workspaces; due = `nextCronRun(cron,lastFired,tz) ≤ now`; **marks fired BEFORE firing** (no double-spawn per slot). Disabled via `KORTIX_TRIGGER_SCHEDULER_ENABLED=false`.
`TRG-7` webhook fire — `POST /webhooks/workspaces/:id/:slug` (**public, HMAC**). Sig header `X-Kortix-Signature` or `X-Hub-Signature-256` (`sha256=` stripped), HMAC-SHA256 over raw body vs `project_secrets[secret_env]`, constant-time. Valid → 202 fired/queued; malformed UUID/slug → 400; unknown workspace → 404; bad sig → 401; missing secret → 409; unknown/disabled/non-webhook trigger → 404; fire failure → 500.
`TRG-8` fire→run — `fireGitTrigger` → actor = account's first `owner` (no owner → silent fail), `createWorkspaceSession(enforceAccountCap:false, visibility:'workspace', metadata.trigger_*)` — trigger sessions are workspace-visible (any workspace member sees them in `GET /sessions`), not private to the stand-in actor. Backpressure: provisioning sessions ≥3 OR account at tier cap → queued.
...
`TRG-10` `GET /workspaces/:id/triggers` leaf gate — a member bound to a custom (Enterprise) workspace role granting `workspace.read` but NOT `workspace.trigger.read` loads the workspace yet is rejected 403 at `GET /triggers` (the `assertProjectCapability(workspace.trigger.read)` fires after the read passes); a floor `user` member (built-in role carries `workspace.trigger.read`) still gets 200. Scoped-agent-token variant proven at the API layer in `integration-workspace-read-leaf-gates-http.test.ts`.
`TRG-11` Triggers CRUD authz boundaries — `ANON → 401` on POST/PATCH/DELETE/fire/activation; a workspace `member` (floor role) holds `trigger.read` + `trigger.fire` but NOT `workspace.write` (the `manage` floor) nor `trigger.create/update/delete` → `GET 200`, `POST/PATCH/DELETE/activation 403`, `fire` unknown-slug `404` (NOT 403 — the fire leaf passes; the 404 is the slug lookup).
`TRG-12` `POST /workspaces/:id/triggers` input validation — missing `name`/`type`/`prompt_template` → `400`; bad `type` (not cron/webhook) → `400`; invalid `session_mode` → `400`; `pinned` without `session_id` → `400`; `pinned` with a `session_id` from another workspace → `400`; webhook without `secret_env` → `400`; webhook with bad `secret_env` (lowercase / leading digit, not `^[A-Z_][A-Z0-9_]*$`) → `400`; cron without `cron` AND without `run_at` → `400`; cron with non-ISO `run_at` → `400`; explicit invalid slug (uppercase / leading dash, not `^[a-z0-9][a-z0-9_-]{0,127}$`) → `400`.
...
Tokens stored as encrypted workspace secrets; webhooks public + signature-gated.

`CHN-1` `kortix channels connect --bot-token xoxb-… --signing-secret …` (manual/BYO mode) → validates `xoxb-` via `auth.test` → `POST /workspaces/:id/channels/slack/connect` (`manage`) → 200, prints webhook `$API/webhooks/slack/:id`.
`CHN-1b` `kortix channels connect` (no creds) → `GET /workspaces/:id/channels/slack/mode` (`read`) → `{oauth_available, install_url}`; when available the CLI prints the pre-signed one-click "Add to Slack" URL (signed `state`, 10-min TTL — same flow as CHN-7) instead of manual mode.
`CHN-2` `GET /workspaces/:id/channels/slack/installation` → `read` → workspace/team/bot/url or "not connected".
`CHN-3` `DELETE /workspaces/:id/channels/slack/installation` → `manage`.
`CHN-4` Slack inbound (OAuth mode) — `POST /webhooks/slack` (shared `SLACK_SIGNING_SECRET`): `v0=HMAC(v0:{ts}:{body})`, ±5min replay window; `url_verification` → echo `challenge`; `event_callback` routed by `team_id`→binding→workspace.
`CHN-5` Slack inbound (BYO mode) — `POST /webhooks/slack/:id` (unsigned `url_verification` bootstrap → 200 challenge before installation; real callbacks require the per-workspace signing secret).
`CHN-6` Slack dispatch — `app_mention`/IM/threaded `message` → existing thread session → deliver to sandbox `/kortix/prompt` (`delivered|transient|stale`); else `createWorkspaceSession` (actor=owner, agent `default`, `visibility:'workspace'` — channel sessions are team-visible) + record `chat_threads`.
`CHN-7` Slack OAuth — `GET /webhooks/slack/oauth/callback` (signed `state`, 10-min TTL) → exchange code → `saveSlackInstall`.
`CHN-8` Telegram inbound — `POST /webhooks/telegram/:id`: verify `x-telegram-bot-api-secret-token` (missing→404, mismatch→401) → `message`/`edited_message` → spawn session (actor=owner, `visibility:'workspace'`).
`CHN-9` bad sig on any channel webhook → 401. Not configured → **503 (Slack OAuth mode + OAuth callback)** but **404 (Slack BYO + Telegram)**.
`CHN-13` `POST /workspaces/:id/channels/email/connect {connector_slug?}` → `manage` + workspace experimental `agentmail_email` enabled → creates or attaches an AgentMail inbox + `message.received`/`message.received.unauthenticated` webhook, stores inbox/webhook metadata as encrypted per-profile workspace secrets, and marks that Email connector profile connected. Disabled workspaces return 403 before AgentMail key validation. Omit `connector_slug` for legacy `kortix_email`; provide an Email connector slug for multiple inboxes.
`CHN-14` `GET /workspaces/:id/channels/email/installation?connector_slug=...` → `read` → AgentMail inbox id/email/webhook id for that profile or null; disabled workspaces return null.
`CHN-15` `DELETE /workspaces/:id/channels/email/installation?connector_slug=...` → `manage` → removes that profile's inbox binding.
`CHN-16` AgentMail inbound — `POST /webhooks/email/agentmail`: Svix `svix-*` signature verified against the per-workspace webhook secret when configured; AgentMail's real unwrapped `message.received` or `message.received.unauthenticated` payload routes by `message.inbox_id` → workspace, maps `thread_id` 1:1 to a Kortix session, and follow-up emails continue that session.
`CHN-17` `GET /workspaces/:id/channels/email/mode` → `read` → `{provider:"agentmail",enabled:boolean,managed_available:boolean}` so the UI can hide Email until `agentmail_email` is enabled and require a workspace AgentMail key when no managed server key exists.
`CHN-18` `GET /workspaces/:id/channels/bindings` → `read` → `{projectDefaultAgent, bindings:[{bindingId,platform,workspaceId,channelId,channelName,channelType,agentName,opencodeModel,conversationPolicy,installedAt,effectiveAgent:{agent,source}}]}` — the web management surface for `chat_channel_bindings` (today populated only via Slack `/kortix agent|model|policy`); `effectiveAgent` resolves `agentName ?? workspace default ?? 'default'` the same way the Slack panel does.
`CHN-19` `PATCH /workspaces/:id/channels/bindings/:bindingId {agentName?,opencodeModel?,conversationPolicy?}` → `workspace.connector.write` (no dedicated channel-binding leaf exists; reuses the same capability that gates connecting/disconnecting the channel itself) → updates via the same `setChannelAgent`/`setChannelModel`/`setChannelConversationPolicy` helpers the Slack commands call; `agentName` validated against the workspace's declared `[[agents]]` when adopted (any name accepted for a legacy/undeclared workspace), `null` resets to the workspace default, `"default"` is an alias for `null`; `opencodeModel` validated via `isModelServableForAccount` (409 `model_not_servable` when not servable) and normalized to the opencode `kortix/…` ref before storing; unknown `bindingId` → 404 before body validation; empty body on an existing binding → 400 `empty_patch`.
`CHN-20` send-primitive IAM gate — `POST /workspaces/:id/channels/slack/file/upload` posts to a channel with the workspace's bot credentials and asserts leaf `workspace.connector.write` (IAM enforcement audit; previously gated by workspace-read only, so any read-capable caller could drive them). A floor `user` member (workspace.read, no connector.write) → 403 before any Slack call; an `editor` (holds connector.write) passes the gate (200/400/404/502/503, never 403); non-member 403/404; ANON 401. The `channel.*` catalog leaves were removed (never wired to a route). Scoped-agent-token variant proven at the API layer in `integration-workspace-read-leaf-gates-http.test.ts`. (The former `meet/speak` half of this gate went away with the notetaker — see §VOICE.)
`TRG-11` Triggers CRUD authz boundaries — `ANON → 401` on POST/PATCH/DELETE/fire/activation; a workspace `member` (floor role) holds `trigger.read` + `trigger.fire` but NOT `workspace.write` (the `manage` floor) nor `trigger.create/update/delete` → `GET 200`, `POST/PATCH/DELETE/activation 403`, `fire` unknown-slug `404` (NOT 403 — the fire leaf passes; the 404 is the slug lookup).
`TRG-12` `POST /workspaces/:id/triggers` input validation — missing `name`/`type`/`prompt_template` → `400`; bad `type` (not cron/webhook) → `400`; invalid `session_mode` → `400`; `pinned` without `session_id` → `400`; `pinned` with a `session_id` from another workspace → `400`; webhook without `secret_env` → `400`; webhook with bad `secret_env` (lowercase / leading digit, not `^[A-Z_][A-Z0-9_]*$`) → `400`; cron without `cron` AND without `run_at` → `400`; cron with non-ISO `run_at` → `400`; explicit invalid slug (uppercase / leading dash, not `^[a-z0-9][a-z0-9_-]{0,127}$`) → `400`.
`TRG-13` `PATCH`/`DELETE`/`activation` edge cases — PATCH unknown slug → `404`; PATCH no-op body `{}` → `200` (no manifest keys, no git commit); DELETE unknown slug → `404`; DELETE invalid slug format (uppercase / leading dash) → `400` (regex gate before manifest lookup); activation pause→resume round-trip persisted on readback (`triggers_paused`); activation non-boolean / missing `paused` → `400`.
...
The worker MCP is not the agent-facing `kortix_voice` connector. The worker MCP gives the LiveKit worker three tools: `ask_kortix`, `run_command`, and `post_turn`. `ask_kortix` sends the recognized human speech to the Kortix agent and waits for its final answer. `run_command` has a short server timeout and reports `timed_out`. `post_turn` persists one transcript line. The MCP has no follow, tail, wait, or stream tool.

`VOICE-1` `PUT /workspaces/:id/channels/meet/name {name}` → `manage` → sets the bot's display name in the call (default "Kortix"); NONMEMBER → 403/404; ANON → 401.
`VOICE-2` `POST /workspaces/:id/sessions/:sid/mcp/voice` — the worker MCP's auth boundary. The per-call `kortix_api_token` is an HMAC over the call id. The call id equals the session id. `startCall` mints the token and sends it to the worker in LiveKit room metadata. A user session, account PAT, missing token, or forged 64-hex bearer returns 401. The route checks authentication before parsing the body. An unauthenticated malformed JSON body returns 401.
`VOICE-3` `GET /workspaces/:id/sessions/:sid/voice-transcript[?cursor&limit]` — a Kortix-authenticated read of the durable `voice_call_turns` record. Workspace `read` and session visibility are required. A non-UUID session id or invalid cursor returns 400. An unknown session returns 404. NONMEMBER returns 403/404. ANON returns 401.
`VOICE-4` The same route returns 200 for a session without a call. The body is `{session_id,call_id,live:false,cursor,count:0,turns:[]}`. `call_id` equals `session_id`. Ordering uses the monotonic `voice_call_turns.cursor`.
`VOICE-5` `GET /public/voice-join/:token` — public and unauthenticated by design. The join link is a 256-bit capability. The token has a `vjl_` prefix and a hash at rest. The route returns a freshly minted LiveKit access token. An unknown token, workspace id, or bare UUID returns 404. A revoked or expired token returns 410.
`VOICE-6` `GET /public/voice-join/:token/transcript[?cursor&limit]` — returns transcript text for the one call identified by the join token. The route takes no workspace or session id. Unknown tokens return 404. Invalid cursor and limit values are clamped. Both public routes use IP rate limits.
...
`GH-1` `GET /workspaces/github/installation?account_id=` → `ACCOUNT_WRITE` → 200; if none → returns `install_url` (`github.com/apps/<slug>/installations/new?state=<hmac>`), state row TTL 30min.
`GH-2` user installs on GitHub → redirect → `$WEB/github/setup?installation_id=&state=&setup_action=install` → `POST /workspaces/github/installation {state,installation_id}` → verify HMAC + iat window + one-time nonce consume → fetch real owner via `GET api.github.com/app/installations/{id}` → upsert `account_github_installations`.
`GH-3` `DELETE /workspaces/github/installation?account_id=` → `ACCOUNT_WRITE` → disconnect. `setup_action=uninstall` → frontend "removed".
`GH-4` Supabase GitHub OAuth popup (user token, distinct from App) — `signInWithOAuth(github, scopes 'read:user read:org')`, `provider_token` posted back to opener; `POST /workspaces/github/installations/linkable {account_id,github_user_token}` lists this App's installations with the App JWT and returns only the authorized personal owner or active organization-admin installations; `POST /workspaces/github/installations/link {account_id,installation_id,github_user_token}` re-verifies App ownership plus GitHub authority before upsert.
...
`GH-6` `PUT /workspaces/:id/git-credential` (BYO) → `manage` → set git auth secret; already server-managed → 409.
`GH-7` `POST /workspaces/:id/git-token` → mint a fresh managed-GitHub installation token; **409 for BYO**; 503 if managed Git is unavailable.
`GH-8` `GET/POST/DELETE /workspaces/:id/cli-token[/:tokenId]` → workspace-scoped CLI tokens.

### `kortix ship` (alias `deploy`)

`SHIP-1` first ship, no `origin` → managed: `POST /workspaces/provision` → set `origin` to the managed GitHub URL, commit, header-injected token push, write `link.json`. Requires `WORKSPACE_CREATE`.
`SHIP-2` first ship, existing `origin` → **BYO** (single-writable-origin rule): `POST /workspaces {repo_url,name}`, **origin never modified**, push with user's own creds. **NB: the API's BYO `POST /workspaces` only accepts a GitHub repo_url** (`normalizeRepoUrl`→`resolveGitHubImport`); a non-GitHub origin is rejected 400 before `saveLink`, so ship exits non-zero, writes no link.json, and (proven) never clobbers the origin. The real happy path needs a live GitHub repo + App install.
...
`SHIP-6` subsequent ship (linked) → `GET /workspaces/:id` (403→access guidance, 404→gone guidance); managed → `POST /workspaces/:id/git-token` (fresh token per ship) → commit + push; BYO → `ensureOrigin` only if missing.
...
`CLI-WORKSPACE` `kortix workspaces ls|info|link|unlink|open|rm` → `GET /workspaces`, `GET /workspaces/:id`, `DELETE /workspaces/:id[?purge=true]` (`--purge` deletes the managed repo; BYO untouched).
...
DB `project_secrets` (AES-256-GCM, key bound to `workspaceId`, unique `(project_id,name)`). **Write-only API — values never returned.**

`SEC-1` `GET /workspaces/:id/secrets` → `manage` → names only + manifest required/optional keys + virtual git-auth row.
`SEC-2` `POST /workspaces/:id/secrets {name,value}` → `manage` → upsert (encrypt); name upper-cased; invalid name format → 400; `KORTIX_*` reserved → 400. M_EDITOR/M_VIEWER → 403.
`SEC-3` `DELETE /workspaces/:id/secrets/:name` → `manage`; invalid name → 400; system secret (git-auth) → 403.
`SEC-4` injection — `buildSessionSandboxEnvVars` decrypts **all** workspace secrets into the session env (workspace-global, no per-member scoping) + minted `KORTIX_TOKEN`/`KORTIX_CLI_TOKEN`, `KORTIX_LLM_*`, `KORTIX_GIT_AUTH_TOKEN`, etc.
`SEC-6` `POST /workspaces/:id/secrets {identifier,name,value}` → two identifiers may share one env-var `name` (e.g. `GMAPS-primary`/`GMAPS-backup` both `GOOGLE_MAPS_API_KEY`); re-submitting an existing `identifier` with a different `name` → 409.
...
`GW-2` standalone gateway `GET /v1/llm/models` plus `/v1/models` and `/v1/openai/models` aliases — account PAT → 200 with a non-empty `{models:{...}}` catalog; ANON → 401/403 (`GW-2b`).
`GW-3` standalone OpenAI-compatible chat aliases reject ANON → 401/403.
`GW-4` workspace LLM gateway routing policy — `GET /workspaces/:id/gateway/routing-policy` returns the persisted workspace document plus effective account/platform inheritance; `PUT` atomically saves workspace/vision defaults, a bounded ordered default chain, and exact-model overrides; `GET /workspaces/:id/model-picker` returns the compact connection-aware selector catalog while the full runtime catalog remains sandbox-only; `POST …/preview {requestedModel,imageInput}` resolves the finite route and model availability without consuming tokens; `DELETE` resets every workspace override. Duplicate/self-loop/`auto` fallback routes → 400 `invalid_routing_policy`; workspace nonmember → 403/404; ANON → 401.
`GW-5` `GET /workspaces/:id/llm-catalog/providers` returns the LIVE provider-level catalog (id/name/env/doc/models per provider) from the same 24h-refreshed runtimeModelCatalog `/llm-catalog` and `/model-picker` read — the source the web connect modal now consumes instead of a baked snapshot. Unlike `/llm-catalog`/`/model-picker`, NOT gated by the workspace's `llm_gateway` flag (the BYOK connect modal applies to native-mode workspaces too) → 200 for both gateway and native workspaces; workspace nonmember → 403/404; ANON → 401.
...
`SEC-C` `NONMEMBER` on `GET/PATCH/DELETE /accounts/:id`, `/workspaces/:id` → 403/404.
`SEC-D` workspace-scoped PAT: allowed only on its bound workspace + `/accounts/me`; **every other surface → 403** (cross-workspace, `/accounts/*`, workspace-list, router/billing/channels/etc.).
`SEC-E` 404 shape — `GET /v1/nonexistent` → `{error:true,message:"Not found",status:404}`.
`SEC-F` webhook sig bypass — Stripe/RevenueCat/Slack/Telegram/workspace-webhook with missing/wrong sig → 400/401.
...
### Role × workspace-action grid (assert per row)

| Action level                                                   | OWNER | ADMIN | M_MANAGER | M_EDITOR | M_VIEWER | MEMBER (no grant) | NONMEMBER |
| -------------------------------------------------------------- | ----- | ----- | --------- | -------- | -------- | ----------------- | --------- |
| `read` (GET workspace/files/sessions)                            | ✓     | ✓     | ✓         | ✓        | ✓        | ✗ 403             | ✗ 403     |
| `session` (create/PATCH/DELETE/restart session — use the chat) | ✓     | ✓     | ✓         | ✓        | ✓        | ✗ 403             | ✗ 403     |
| `write` (PATCH workspace, CR merge, deploy, triggers)            | ✓     | ✓     | ✓         | ✓        | ✗ 403    | ✗ 403             | ✗ 403     |
| `manage` (DELETE workspace, secrets, members/access)             | ✓     | ✓     | ✓         | ✗ 403    | ✗ 403    | ✗ 403             | ✗ 403     |
...
- No account-level vault — secrets are workspace-scoped, all-or-nothing per workspace (the `vault_items`/per-member-scope design was reversed).
- Granular IAM actions `workspace.trigger.*`, `channel.*`, `trigger.*` exist in the catalog but those workspace routes only enforce coarse `read|write|manage` — test the coarse gate, not the fine actions. **Exception:** session lifecycle routes (create/PATCH/DELETE/restart) now enforce `workspace.session.start` via the `session` access tier, which every workspace role (viewer included) holds — so a viewer CAN run sessions but still can't `write`/`manage`.
- No inbound GitHub event webhook (no push/PR receiver) — see `TRG-9`.
- CLI `providers`, `doctor`, `proxy`, `sessions-chat` source files exist but are **not wired** into the dispatcher and not in the reserved list — so `kortix providers …` is **treated as a new-workspace name** (`runCreate`), not an "unknown command" error. Don't test for an error here.
- Cron scheduler scans only first 200 active workspaces/tick.
...
`CONN-2` `GET /executor/workspaces/:id/connectors` → workspace admin → 200; `NONMEMBER` → 403.
`CONN-3` `POST /executor/call {connector,action,args}` → executor-principal route; user JWT + `ANON` → 401.
`CONN-4` `POST /executor/workspaces/:id/connectors/sync` → admin → 200 (re-materialize from kortix.yaml).
`CONN-5` `GET /executor/workspaces/:id/policies` → admin → 200; `PUT …/policies {policies[]}` → admin → 200.
`CONN-7` `PUT /executor/workspaces/:id/connectors/:slug/credential` → accepts a static value or native OAuth2 client-credentials configuration; missing value or a non-HTTPS OAuth2 token URL → 400.
`CONN-8` `POST /executor/workspaces/:id/connectors` → admin; invalid json → 400. `DELETE …/:slug` → admin → ok/404.
`CONN-9` `GET /executor/workspaces/:id/pipedream/apps` → admin → 200 or 501 (pipedream not configured).
`CONN-13` `PUT /executor/workspaces/:id/connectors/:slug/credential-mode|name|policies` → admin (`workspace.connector.write`); body validated before the connector lookup (bad mode/empty name/invalid policy action → 400 even against an unknown slug); well-formed body + unknown connector → 404; NONMEMBER → 403.
`CONN-14` `POST /executor/workspaces/:id/connectors/auth-discovery {provider,spec|url|endpoint|baseUrl}` → admin (`workspace.connector.write`) loads the guarded direct source and returns normalized authentication candidates plus a supported recommendation; omitted auth on `POST …/connectors` applies that recommendation, while explicit `{auth:{type:"none"}}` skips discovery and remains a durable opt-out. Source credential literals are never returned.
`CONN-15` `GET /executor/workspaces/:id/discover/integrations[?q&cursor]` → workspace admin browses the direct integrations.sh catalogue; `GET …/discover/integrations/detail?id=…` → resolves the trusted record's API/MCP/Postman/GraphQL/docs/CLI variants; upstream outage → 502; `NONMEMBER` → 403 before any upstream fetch.
...
**Connector authorization is centralized on the AGENT (2026-07-06).** `PUT /executor/workspaces/:id/connectors/:slug/sharing` and `PUT …/agent-scope` are both RETIRED (route removed — `CONN-6`'s id is intentionally not reused). A connector is now unconditionally workspace-wide visible to every workspace member; the only gate on which agents may call it is the agent's own `connectors` grant (`[[agents]].connectors` in kortix.yaml, enforced by `iam/agent-scope.ts` — see `WS-agents` flows), not anything configured per-connector.

---

## 25. Parallel-authored domains (git/platform/iam/channels/queue/audit/scim)

`GH-9` `GET /git/:workspace/info/refs` · `POST …/git-upload-pack` · `POST …/git-receive-pack` → smart-HTTP proxy, git token auth (not JWT); bad/no token → 401/502.
`GH-10` `GET /git/:workspace/info/refs` → user JWT is not a git token → 401/403; NONMEMBER → 401/403/404.
`GH-11` `GET /workspaces/:id/git/clone-credential` → runtime tokens only; ANON → 401, user JWT/account-PAT → 403.
`GH-12` `POST /workspaces/:id/git/collaborators` → missing username → 400; non-managed → 409; no install → 502.
`GH-13` `GET /workspaces/github/repositories` → WORKSPACE_CREATE; no App install → 409 install_url.
`GH-14` `POST /workspaces/create-repo` → WORKSPACE_CREATE; missing name → 400; no install → 409/503.
`GH-15` `POST /workspaces/link-repository` → WORKSPACE_CREATE; missing repo → 400; no install → 400/409/502; bad token → 400.
`GH-16` `GET /workspaces/github/repository-branches` → WORKSPACE_CREATE; returns the repository default plus every existing branch; missing installation → 409; wrong installation owner → 400.
`PLT-3` `GET /platform/sandbox/version` · `…/latest` · `…/all` · `…/changelog` → 200 (public).
`IAM-14` `GET …/iam/groups/:gid/workspace-grants` → 200; unknown → 404; NONMEMBER → 403.
`IAM-15` `POST …/iam/members/:userId/effective:batch` → 200; non-array → 400.
`IAM-16` `GET …/iam/members/:userId/workspace-access` → 200; NONMEMBER → 403.
...
`CHN-10` `GET /workspaces/:id/channels/slack/mode` → read → 200; non-member 403/404.
...
`CHN-13` `POST /workspaces/:id/channels/email/connect` → manage; requires workspace experimental `agentmail_email`; optional `connector_slug` scopes the inbox to one Email connector profile; optional existing `inbox_id` + `email` attaches an already-created AgentMail inbox; disabled → 403, invalid AgentMail key → 502 or no configured key → 503; non-member 403/404.
`CHN-14` `GET /workspaces/:id/channels/email/installation` → read → 200 null/summary for default or requested `connector_slug`; non-member 403/404.
`CHN-15` `DELETE /workspaces/:id/channels/email/installation` → manage → 200 for default or requested `connector_slug`; non-member 403/404.
...
`CHN-17` `GET /workspaces/:id/channels/email/mode` → read → 200 mode with enabled flag; non-member 403/404.
`CHN-18` `GET /workspaces/:id/channels/bindings` → read → 200 `{projectDefaultAgent,bindings[]}`; non-member 403/404; ANON 401.
`CHN-19` `PATCH /workspaces/:id/channels/bindings/:bindingId` → `workspace.connector.write`; unknown bindingId → 404 before body validation; empty body on an existing binding → 400; non-member 403/404; ANON 401.
...
## 26. Parallel-authored wave 2 (CR/files/apps/sandboxes/billing/access/router/auth/workspaces-misc)

`CR-11` `GET/POST /workspaces/:id/change-requests` → NONMEMBER → 403/404.
`CR-12` `GET /workspaces/:id/change-requests` → ANON → 401.
`WS-9` `POST /workspaces/:id/manifest/validate {raw,format?}` → 200 {valid,issues}; missing raw → 400. `raw` is parsed as TOML or YAML — the format is derived from the workspace's configured `manifestPath` first, falls back to an explicit `format:"toml"|"yaml"` in the body, and defaults to `toml` for back-compat. A `kortix.yaml`-configured workspace's `raw` YAML validates correctly instead of silently mis-parsing as TOML.
`WS-10` `POST /workspaces/:id/cli-token` → 201 workspace PAT; `GET` → 200; `DELETE /:tokenId` → 200; unknown → 404.
`WS-11` `PATCH /workspaces/:id/onboarding {completed}` → 200; NONMEMBER → 403/404.
`WS-12` `GET /workspaces/:id/version-diff?from&into` → 200; missing → 400; same ref → is_same_ref.
`WS-13` `POST /workspaces/:id/oauth/:provider/start|poll` + `GET|DELETE /workspaces/:id/oauth[/:provider]` → poll-based device flow saving CODEX_AUTH_JSON; start unknown provider/invalid sharing → 400, poll missing flow_id → 400, poll bogus → expired, list → 200, delete unknown → 404, NONMEMBER → 404, ANON → 401.
`WS-16` `POST /workspaces/:id/turn-question {session_id,questions[]}` → missing session id → 400; unknown session id → 404 before questions validation.
`WS-17` `POST /workspaces/:id/turn-stream {session_id,text}` → missing session id → 400; unknown session id → 404 before event-payload validation; `kind:end|turn_end` needs only a valid `session_id` (`status: idle|error`) → 200 `ok:false` when no live stream.
`WS-34` `POST /workspaces/:id/execution-lease {action:acquire|renew|release,session_id,lease_ttl_seconds?}` — a sandbox reports active OpenCode work. `acquire` returns the provider keep-alive endpoint once. `renew` and `release` each run one conditional update. A sandbox token is required. ANON returns 401. A user session or account PAT returns 403. An unknown action returns 400.
`WS-18` Workspace cap by plan: a FREE account may own exactly 3 workspaces — `POST /workspaces/provision` for the 4th → 403 `{code:project_limit_reached,limit:3}` (checked before any repo is provisioned); paid/team plans get `MAX_PROJECTS_PER_ACCOUNT`. Requires `managedGit`+`stripe` (billing enforced).
`WS-19` Full v2 agent-config editor (agent-first spec §2.2): `GET /workspaces/:id/agents/:agentName/config` (`read`) → 200 `{agent,schema_version,editable,default_agent,block}` — `editable:false` + `block:null` for a v1/empty manifest (the UI's degrade signal), the agent's full `AgentBlockV2` for a declared v2 agent; `PUT /workspaces/:id/agents/:agentName/config {…AgentBlockV2}` (`manage`, gated `workspace.customize.write`) validates the block through the manifest-schema validator (bad permission tree/enum/ungrantable `kortix_cli` → 400 `invalid_config`) then writes it into the `agents:` map in `kortix.yaml`; a v1 workspace is refused with a 400 upgrade pointer (v2-only); malformed body → 400 `invalid_body`; caller with no workspace grant → 403.
...
`MKTP-3` `POST /workspaces/:workspaceId/marketplace/install {id}` → `write` → 201 `{commit_sha,branch,file_count,installed[],capabilities}` (resolves the catalog item + transitive bundle deps, commits its files + `registry-lock.json` to the default branch). Missing/unknown id → 400; missing workspace / `NONMEMBER` → 404/403. Legacy alias: `/registry/install`.
`MKTP-4` `GET /workspaces/:workspaceId/marketplace` → `read` → 200 `{installed:[{name,type,source,installed_at,file_count}]}` (from `registry-lock.json`; migrates legacy `skills-lock.json`); missing workspace → 404. Legacy alias: `/registry`.
`MKTP-5` `DELETE /workspaces/:workspaceId/marketplace/:name` → `write` → 200 `{ok,removed,commit_sha,branch,file_count}` (removes the item's files + lock entry in one commit to the default branch); item not installed → 404; missing workspace / `NONMEMBER` → 404/403. Legacy alias: `/registry/:name`.
`EXP-1` `PATCH /workspaces/:id/experimental {feature,enabled}` → 200 with `experimental`/`experimental_features` in body; unknown feature → 400; non-bool enabled → 400; `enabled:null` clears the override → 200.
`SNAP-3` `POST /workspaces/:id/snapshots/fix-with-agent` → no failed build → 409; else 201.
`SBX-3` `GET /workspaces/:id/sandboxes` · `/sandbox-health` · `/sandbox-templates` → 200.
`SBX-4` `POST /sandbox-templates` → 201; bad → 400; reserved/dup → 409; `PATCH/DELETE/build /:templateId`; unknown → 404.
`WACC-5` `POST /workspaces/:id/access/invite` → 201 pending; `GET/POST resend/DELETE pending-invites[/:id]` → manage; missing email → 400; unknown → 404.
`WACC-6` `GET/POST /workspaces/:id/group-grants` · `PATCH/DELETE /:groupId` → manage; missing group_id → 400; unknown → 404.
...
`SEC-5` `PUT/DELETE /workspaces/:id/secrets/:name/personal` → per-user secret override set/clear.
`CONN-10` `POST /executor/workspaces/:id/connectors/:slug/connect[/finalize]` → pipedream; unknown connector → 404/501.
`CONN-11` `POST /executor/webhook/pipedream` → public; bad/unsigned payload → rejected.
`CONN-12` `GET /executor/workspaces/:id/connectors/:slug/config` → admin reads a connector's connection def for editing; unknown connector → 404/501; NONMEMBER → 403.
...
`KAAB-1` backend PAT `POST /workspaces/:id/sessions {origin_ref, runtime_context}` → 201; response echoes `origin:"backend"` (derived, not from the body) + `origin_ref`. Overrides omitted = byte-identical to a normal session.
...
`KAAB-3` backend `secrets=[unknown]` → **404 `SECRET_IDENTIFIER_NOT_FOUND`**; `secrets=[]` → 201 with `secrets_allowlist:[]` (inject zero workspace secrets). Narrowing only — never widens beyond the agent's grant.
