...
`SESS-18` Server-owned warm session — `POST /projects/:id/sessions/warm {}` creates one private available session for the current user and returns `{session,reused:false,workspace_refresh:{status:"skipped"}}`; a second ensure returns the same `session_id` with `reused:true`. `POST /projects/:id/sessions/warm/claim {session_id,agent_name?,sandbox_slug?}` atomically changes its warm state to `claimed`; a second claim returns 409 `WARM_SESSION_ALREADY_CLAIMED`; the next ensure creates a different available session. A selected agent or sandbox mismatch returns 409 `WARM_SESSION_CONFIGURATION_MISMATCH`.
