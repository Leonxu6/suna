...
// GW-5 — workspace-scoped LLM catalog surfaces read by the connect modal.
...
flow(
  ...
  async (ctx) => {
    ...
    for (const path of [
      '/v1/workspaces/:workspaceId/llm-catalog',
      '/v1/workspaces/:workspaceId/llm-catalog/providers',
    ] as const) {
    ...
    await ctx.step('OWNER → 200 on the model-level catalog', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get('/v1/workspaces/:workspaceId/llm-catalog', { params });
    ...
    await ctx.step('OWNER → 200 with a provider catalog on /providers', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get('/v1/workspaces/:workspaceId/llm-catalog/providers', { params });
      r.status(200);
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('inherited routing policy is readable', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get('/v1/workspaces/:workspaceId/gateway/routing-policy', { params });
      r.status(200)
        .body()
        .has('$.version', 1)
    ...
    await ctx.step(
      ...
      async () => {
        ...
        const picker = await ctx.client
          .as(ctx.P.OWNER)
          .get('/v1/workspaces/:workspaceId/model-picker', { params });
    ...
    await ctx.step('save and read back the complete project policy', async () => {
      const saved = await ctx.client
        .as(ctx.P.OWNER)
        .put('/v1/workspaces/:workspaceId/gateway/routing-policy', policy, { params });
      saved
        .status(200)
        .body()
    ...
    await ctx.step('preview resolves ordered default and exact-model routes', async () => {
      const defaultRoute = await ctx.client
        .as(ctx.P.OWNER)
        .post(
          '/v1/workspaces/:workspaceId/gateway/routing-policy/preview',
    ...
    await ctx.step('invalid self-loop is rejected without replacing the saved policy', async () => {
      const invalid = await ctx.client.as(ctx.P.OWNER).put(
        '/v1/workspaces/:workspaceId/gateway/routing-policy',
        {
          ...policy,
    ...
    await ctx.step('reset removes every project override', async () => {
      const reset = await ctx.client
        .as(ctx.P.OWNER)
        .del('/v1/workspaces/:workspaceId/gateway/routing-policy', { params });
      reset
        .status(200)
        .body()
        .has('$.project.defaultModel', null)
