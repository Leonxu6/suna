 ...
 *    `POST /workspaces/:id/mcp/voice`; that route is GONE.
 *  - The LiveKit worker's side is the voice MCP,
 *    `POST /workspaces/:id/sessions/:sid/mcp/voice` — channels/voice/routes.ts,
 ...
 * honouring a session or workspace id, a transcript read that skips its workspace
  ...
  async (ctx) => {
    ...
    await ctx.step('OWNER sets the name → 200', async () => {
      const r = await ctx.client.as(ctx.P.OWNER).put(
        '/v1/workspaces/:workspaceId/channels/meet/name',
        { name: 'Kortix QA' },
        {
    ...
    await ctx.step('NONMEMBER → 403/404', async () => {
      const r = await ctx.client.as(ctx.P.NONMEMBER).put(
        '/v1/workspaces/:workspaceId/channels/meet/name',
        { name: 'nope' },
        {
          params: { workspaceId: p.id },
    ...
    await ctx.step('ANON → 401', async () => {
      const r = await ctx.client.as(ctx.P.ANON).put(
        '/v1/workspaces/:workspaceId/channels/meet/name',
        { name: 'nope' },
        {
          params: { workspaceId: p.id },
...
//
// It matters because the route is mounted BEFORE workspacesApp specifically so
// it skips `supabaseAuth`. If that ordering is ever undone, or a `.use()` is
// layered back onto this path, a workspace principal starts being accepted
// here — and this flow goes red.
flow(
  'VOICE-2',
  { domain: 'voice', routes: ['POST /v1/workspaces/:workspaceId/sessions/:sessionId/mcp/voice'] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    ...
    await ctx.step('ANON → 401', async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .post('/v1/workspaces/:workspaceId/sessions/:sessionId/mcp/voice', listTools, { params });
      r.status(401);
    });
    ...
    await ctx.step('auth precedes the body → malformed JSON is still 401', async () => {
      ...
      const r = await ctx.client
        .as(ctx.P.ANON)
        .post('/v1/workspaces/:workspaceId/sessions/:sessionId/mcp/voice', '{not json', {
          params,
          raw: true,
        });
      r.status(401);
...
flow(
  ...
  {
    domain: 'voice',
    routes: ['GET /v1/workspaces/:workspaceId/sessions/:sessionId/voice-transcript'],
  },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
...
// nothing id-shaped is honoured, because a workspace or session id resolving to
...
flow('VOICE-5', { domain: 'voice', routes: ['GET /v1/public/voice-join/:token'] }, async (ctx) => {
  const p = await ctx.fixtures.sharedWorkspace();
  ...
  await ctx.step('a workspace id is not a join token → 404', async () => {
    const r = await ctx.client
      .as(ctx.P.ANON)
...
// was said in a call, and it takes NO workspace or session id —
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('no id-shaped query parameter is honoured → still 404', async () => {
      // If any of these were read, an unknown token plus a known workspace or
      // session id would hand back somebody else's call.
      const r = await ctx.client.as(ctx.P.ANON).get('/v1/public/voice-join/:token/transcript', {
        params: { token: ABSENT_UUID },
        query: { call_id: ABSENT_UUID, session_id: ABSENT_UUID, workspace_id: p.id },
      });
      r.status(404);
    });
