...
flow(
  ...
  async (ctx) => {
    const shared = await ctx.fixtures.sharedWorkspace();

    await ctx.step("list envelope → 200 `{ change_requests: [...] }`", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/change-requests", { params: { workspaceId: shared.id } });
    ...
    await ctx.step("project DELETE → 200 `{ ok: true }` (soft archive, not an echoed status)", async () => {
      const disposable = await ctx.fixtures.workspace();
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .del("/v1/workspaces/:workspaceId", { params: { workspaceId: disposable.id } });
