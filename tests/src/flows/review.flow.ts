...
flow(
  'RV-1',
  { domain: 'review', tags: ['smoke'], routes: ['GET /v1/workspaces/:workspaceId/review/items'] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step('OWNER lists review items → 200 with envelope', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get('/v1/workspaces/:workspaceId/review/items', { params: { workspaceId: p.id } });
      r.status(200).body().exists('$.review_items');
    });
    await ctx.step('invalid segment → 400', async () => {
      const r = await ctx.client.as(ctx.P.OWNER).get('/v1/workspaces/:workspaceId/review/items', {
...
flow(
  'RV-2',
  { domain: 'review', routes: ['GET /v1/workspaces/:workspaceId/review/items/:reviewItemId'] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
...
flow(
  'RV-3',
  { domain: 'review', routes: ['POST /v1/workspaces/:workspaceId/review/items'] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step('missing title → 400', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post(
          '/v1/workspaces/:workspaceId/review/items',
          { kind: 'output' },
          { params: { workspaceId: p.id } },
        );
      r.status(400);
...
flow(
  'RV-4',
  { domain: 'review', routes: ['POST /v1/workspaces/:workspaceId/review/items/:reviewItemId/act'] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step('invalid verdict → 400', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post(
          '/v1/workspaces/:workspaceId/review/items/:reviewItemId/act',
          { verdict: 'merge' },
          { params: { workspaceId: p.id, reviewItemId: RANDOM_UUID } },
        );
      r.status(400);
...
flow(
  'RV-5',
  { domain: 'review', routes: ['POST /v1/workspaces/:workspaceId/review/bulk'] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step('missing ids → 400', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post(
          '/v1/workspaces/:workspaceId/review/bulk',
          { verdict: 'approve' },
          { params: { workspaceId: p.id } },
        );
      r.status(400);
...
flow(
  'RV-6',
  { domain: 'review', routes: ['GET /v1/workspaces/:workspaceId/review/items'] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
