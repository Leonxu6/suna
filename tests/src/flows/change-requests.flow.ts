...
flow(
  "CR-1",
  { domain: "change-requests", tags: ["smoke"], routes: ["GET /v1/workspaces/:workspaceId/change-requests"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("OWNER lists change requests → 200 with envelope", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/change-requests", { params: { workspaceId: p.id } });
      r.status(200).body().exists("$.change_requests");
    });
    await ctx.step("invalid status filter → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/change-requests", { params: { workspaceId: p.id }, query: { status: "wizard" } });
      r.status(400);
    });
  },
);
...
flow(
  "CR-2",
  { domain: "change-requests", routes: ["POST /v1/workspaces/:workspaceId/change-requests"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("missing title → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/:workspaceId/change-requests", { head_ref: "feature" }, { params: { workspaceId: p.id } });
      r.status(400);
    });
    await ctx.step("missing head_ref → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/:workspaceId/change-requests", { title: ctx.fixtures.name("cr") }, { params: { workspaceId: p.id } });
      r.status(400);
    });
    await ctx.step("base_ref == head_ref → 400", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).post(
        "/v1/workspaces/:workspaceId/change-requests",
...
flow(
  "CR-3",
  { domain: "change-requests", routes: ["GET /v1/workspaces/:workspaceId/change-requests/:crId"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("unknown crId → 404", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/change-requests/:crId", { params: { workspaceId: p.id, crId: RANDOM_UUID } });
      r.status(404);
    });
  },
);
...
flow(
  "CR-4",
  { domain: "change-requests", routes: ["PATCH /v1/workspaces/:workspaceId/change-requests/:crId"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("patch unknown crId → 404", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).patch(
        "/v1/workspaces/:workspaceId/change-requests/:crId",
        { title: "renamed" },
...
flow(
  "CR-5",
  { domain: "change-requests", routes: ["GET /v1/workspaces/:workspaceId/change-requests/:crId/diff"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("diff for unknown crId → 404", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/change-requests/:crId/diff", { params: { workspaceId: p.id, crId: RANDOM_UUID } });
      r.status(404);
    });
  },
);
...
flow(
  "CR-6",
  { domain: "change-requests", routes: ["GET /v1/workspaces/:workspaceId/change-requests/:crId/merge-preview"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("merge-preview for unknown crId → 404", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).get("/v1/workspaces/:workspaceId/change-requests/:crId/merge-preview", {
        params: { workspaceId: p.id, crId: RANDOM_UUID },
      });
      r.status(404);
    });
...
flow(
  "CR-7",
  { domain: "change-requests", routes: ["POST /v1/workspaces/:workspaceId/change-requests/:crId/merge"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("merge unknown crId → 404", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).post(
        "/v1/workspaces/:workspaceId/change-requests/:crId/merge",
        {},
        { params: { workspaceId: p.id, crId: RANDOM_UUID } },
      );
      r.status(404);
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step("close unknown crId → 404", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).post(
        "/v1/workspaces/:workspaceId/change-requests/:crId/close",
        {},
        { params: { workspaceId: p.id, crId: RANDOM_UUID } },
      );
      r.status(404);
    ...
    await ctx.step("reopen unknown crId → 404", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).post(
        "/v1/workspaces/:workspaceId/change-requests/:crId/reopen",
        {},
        { params: { workspaceId: p.id, crId: RANDOM_UUID } },
      );
      r.status(404);
...
flow(
  "CR-11",
  { domain: "change-requests", routes: ["GET /v1/workspaces/:workspaceId/change-requests"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
...
flow(
  ...
  {
    domain: "change-requests",
    routes: ["POST /v1/workspaces/:workspaceId/change-requests/:crId/request-changes"],
  },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("missing feedback → 400", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).post(
        "/v1/workspaces/:workspaceId/change-requests/:crId/request-changes",
        {},
        { params: { workspaceId: p.id, crId: RANDOM_UUID } },
      );
      r.status(400);
