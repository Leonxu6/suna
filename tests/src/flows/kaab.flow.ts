...
const CREATE = 'POST /v1/workspaces/:workspaceId/sessions';
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('non-backend (user) setting origin_ref → 403 origin_override_forbidden', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post('/v1/workspaces/:workspaceId/sessions', { origin_ref: 'spoofed' }, { params: { workspaceId: p.id } });
      r.status(403);
    ...
    await ctx.step('non-backend (user) setting secrets → 403 origin_override_forbidden', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post('/v1/workspaces/:workspaceId/sessions', { secrets: ['ANYTHING'] }, { params: { workspaceId: p.id } });
      r.status(403);
      r.body().has('$.code', 'origin_override_forbidden');
    });
  },
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('backend secrets=[] (inject zero) → 201, secrets_allowlist=[]', async () => {
      const r = await ctx.client
        .as(ctx.P.PAT_ACCT)
        .post('/v1/workspaces/:workspaceId/sessions', { secrets: [] }, { params: { workspaceId: p.id } });
      r.status(201);
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('backend runtime_context over the 64-entry cap → 400', async () => {
      ...
      const r = await ctx.client
        .as(ctx.P.PAT_ACCT)
        .post('/v1/workspaces/:workspaceId/sessions', { runtime_context: ctx64 }, { params: { workspaceId: p.id } });
      r.status(400);
    });
  },
);
    ...
    await ctx.step('backend create with Idempotency-Key → 201', async () => {
      const r = await ctx.client
        ...
        .post(
          ...
          { params: { workspaceId: p.id }, headers: { 'Idempotency-Key': key } },
        );
      r.status(201);
      first = r.json<any>()?.session_id ?? r.json<any>()?.id;
      if (first) ctx.track('session', first, { workspaceId: p.id });
    });
    ...
    await ctx.step('oversized Idempotency-Key header → 400 INVALID_IDEMPOTENCY_KEY (not a 500)', async () => {
      const r = await ctx.client
        ...
        .post(
          '/v1/workspaces/:workspaceId/sessions',
          {},
          { params: { workspaceId: p.id }, headers: { 'Idempotency-Key': 'x'.repeat(300) } },
        );
      r.status(400);
