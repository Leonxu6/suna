/**
 * GitHub / git-transport backlog flows: WS-4, GH-4, GH-5, GH-8.
 *
 * Behaviour DERIVED from apps/api/src/projects/index.ts (spec text is treated as
 * a hint, not the contract):
 *
 *  - WS-4  POST /v1/workspaces/create-repo — PROJECT_CREATE-gated repo creation.
 ...
 *    a union so the dual declaration is intentional — this flow pins WS-4's
 ...
 *    POST /v1/workspaces/:workspaceId/git-token (the "mint scoped push token"
 ...
 *    (loadWorkspaceForUser returns null ⇒ 404, never 403). Same route as GH-7;
 *    GH-5 pins the resolution OUTCOMES rather than the auth matrix.
 *
 *  - GH-8  GET/POST/DELETE /v1/workspaces/:workspaceId/cli-token[/:tokenId] —
 *    workspace-scoped CLI tokens. GET = loadWorkspaceForUser('read'); POST/DELETE =
 *    loadWorkspaceForUser('manage'). POST → 201 with a one-time `secret_key` +
 *    `token_id`; GET lists `items` (no secret); DELETE → 200 {ok:true}, unknown
 *    token → 404. ANON → 401; non-member / no-access → 404 (never 403).
 *
 *  - GH-4  POST /v1/workspaces/github/installations/linkable and
 *    POST /v1/workspaces/github/installations/link — the authenticated account
...
// ── WS-4 — create a new GitHub repo (no install ⇒ 409 install_url) ────────
...
flow(
  ...
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client.as(ctx.P.ANON).post("/v1/workspaces/create-repo", { name: "ke2e-repo" });
      r.status(401);
    });
    ...
    await ctx.step("fresh account with no GitHub App install → 409 + install_url (or 503 if GitHub unconfigured)", async () => {
      ...
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/create-repo", { name, private: true, account_id: team.id });
      r.status([409, 503]);
...
// ── GH-8 — workspace-scoped CLI tokens (list / mint / revoke) ────────────────
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step("ANON cannot list → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get("/v1/workspaces/:workspaceId/cli-token", { params: { workspaceId: p.id } });
      r.status(401);
    });
    await ctx.step("ANON cannot mint → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .post("/v1/workspaces/:workspaceId/cli-token", { name: "x" }, { params: { workspaceId: p.id } });
      r.status(401);
    });
    await ctx.step("OWNER mints a workspace-scoped CLI token → 201 with one-time secret", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post(
