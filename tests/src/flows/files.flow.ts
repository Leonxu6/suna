 ...
 * project (ctx.fixtures.sharedWorkspace()) which has an initial commit on its default
...
flow(
  "FILE-1",
  { domain: "files", tags: ["smoke"], routes: ["GET /v1/workspaces/:workspaceId/files"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("OWNER lists repo tree → 200 array", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).get("/v1/workspaces/:workspaceId/files", { params: { workspaceId: p.id } });
      r.status(200).body().exists("$");
    });
    await ctx.step("NONMEMBER → 403/404", async () => {
      const r = await ctx.client
        .as(ctx.P.NONMEMBER)
        .get("/v1/workspaces/:workspaceId/files", { params: { workspaceId: p.id } });
      r.status([403, 404]);
    });
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client.as(ctx.P.ANON).get("/v1/workspaces/:workspaceId/files", { params: { workspaceId: p.id } });
      r.status(401);
    });
  },
);
...
flow(
  "FILE-2",
  { domain: "files", routes: ["GET /v1/workspaces/:workspaceId/files/content"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    ...
    await ctx.step("absent path param → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/files/content", { params: { workspaceId: p.id } });
      r.status(400);
    });
    if (firstFile) {
...
flow(
  "FILE-3",
  { domain: "files", routes: ["GET /v1/workspaces/:workspaceId/files/search"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("absent q param → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/files/search", { params: { workspaceId: p.id } });
      r.status(400);
    });
...
flow(
  "FILE-4",
  { domain: "files", routes: ["GET /v1/workspaces/:workspaceId/files/history"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("absent path param → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/files/history", { params: { workspaceId: p.id } });
      r.status(400);
    });
...
flow(
  "FILE-5",
  { domain: "files", routes: ["GET /v1/workspaces/:workspaceId/files/archive"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("repo archive (no path) → 200 zip stream", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/files/archive", { params: { workspaceId: p.id } });
      r.status([200, 400]);
    });
    await ctx.step("NONMEMBER → 403/404", async () => {
      const r = await ctx.client
...
flow(
  "FILE-6",
  { domain: "files", tags: ["smoke"], routes: ["GET /v1/workspaces/:workspaceId/branches"] },
  async (ctx) => {
    const p = await ctx.fixtures.sharedWorkspace();
    await ctx.step("OWNER lists branches → 200 with default_branch", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/branches", { params: { workspaceId: p.id } });
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step("OWNER lists commits → 200 with commits[] (or 400 if the managed mirror has no readable history yet)", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/workspaces/:workspaceId/commits", { params: { workspaceId: p.id } });
      r.status([200, 400]);
      if (r.statusCode === 200) {
