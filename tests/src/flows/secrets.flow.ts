...
flow(
  "SEC-1",
  { domain: "secrets", routes: ["GET /v1/workspaces/:workspaceId/secrets"] },
  async (ctx) => {
    const p = await ctx.fixtures.workspace();
    await ctx.step("list secret names", async () => {
      const r = await ctx.client.as(ctx.P.OWNER).get("/v1/workspaces/:workspaceId/secrets", { params: { workspaceId: p.id } });
      r.status(200);
    });
  },
);
...
flow(
  "SEC-2",
  { domain: "secrets", routes: ["POST /v1/workspaces/:workspaceId/secrets"] },
  async (ctx) => {
    const p = await ctx.fixtures.workspace();
    await ctx.step("upsert a secret → 200", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/:workspaceId/secrets", { name: "MY_SECRET", value: "v1" }, { params: { workspaceId: p.id } });
      r.status([200, 201]);
    });
    await ctx.step("KORTIX_* reserved → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/:workspaceId/secrets", { name: "KORTIX_HACK", value: "x" }, { params: { workspaceId: p.id } });
      r.status(400);
    });
    await ctx.step("invalid name format → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/:workspaceId/secrets", { name: "not a name!", value: "x" }, { params: { workspaceId: p.id } });
      r.status(400);
    });
  },
);
...
flow(
  "SEC-3",
  { domain: "secrets", routes: ["DELETE /v1/workspaces/:workspaceId/secrets/:name"] },
  async (ctx) => {
    const p = await ctx.fixtures.workspace();
    await ctx.step("create then delete a secret", async () => {
      await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/:workspaceId/secrets", { name: "TO_DELETE", value: "x" }, { params: { workspaceId: p.id } });
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .del("/v1/workspaces/:workspaceId/secrets/:name", { params: { workspaceId: p.id, name: "TO_DELETE" } });
      r.status(200);
    });
    ...
    await ctx.step("system KORTIX_* secret cannot be deleted → 403", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .del("/v1/workspaces/:workspaceId/secrets/:name", { params: { workspaceId: p.id, name: "KORTIX_TOKEN" } });
      r.status(403);
    });
  },
);
  ...
  async (ctx) => {
    ...
    await ctx.step("mint a secret-entry link → 200 with a token url", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post(
          "/v1/workspaces/:workspaceId/secret-requests",
          { names: ["SEC7_TEST_KEY"] },
          { params: { workspaceId: p.id } },
        );
    ...
    await ctx.step("mint with no names → 400", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post("/v1/workspaces/:workspaceId/secret-requests", {}, { params: { workspaceId: p.id } });
      r.status(400);
    });
