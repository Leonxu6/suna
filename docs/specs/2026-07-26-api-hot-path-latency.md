...
| --- | ---: | ---: | ---: | ---: |
| `POST /v1/workspaces/:id/turn-stream` | 24,095 | 1,294 ms | 4,192 ms | 5,419 ms |
| `GET /v1/workspaces/:id/sessions` | 1,195 | 703 ms | 2,500 ms | 3,487 ms |
| `GET /v1/workspaces/:id/secrets` | 895 | 1,237 ms | 5,601 ms | 7,112 ms |
| `GET /v1/workspaces/:id/detail` | 238 | 1,449 ms | 5,176 ms | 6,999 ms |
| `GET /v1/workspaces/:id/model-picker` | 213 | 448 ms | 1,888 ms | 2,896 ms |

The new `us-east-2` production sample reports ordinary workspace reads mainly
...
Generic workspace authorization also triggers speculative session resumes.

- `loadWorkspaceForUser()` runs on every workspace route.
...
- `useOpenCodeProviders()` waits for workspace detail before it starts the
  workspace model-picker request.
- The workspace model-picker can answer before any sandbox runtime is ready.
...
8. Start the workspace model-picker request without waiting for workspace detail.
...
- Remove speculative resume from `loadWorkspaceForUser()`.
- Keep runtime allocation and resume inside explicit session-open paths.
- Do not select a stopped historical session from ordinary workspace activity.

### Model readiness

- Start `GET /model-picker` in parallel with `GET /detail`.
- Use the model-picker response immediately for gateway workspaces.
- Preserve the native runtime-provider fallback for non-gateway workspaces.
- Do not report BYOK provider state as loading after workspace detail and secrets
  have resolved.
