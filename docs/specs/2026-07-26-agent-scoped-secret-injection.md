...
**Date:** 2026-07-26
**Status:** Implemented
**Area:** `apps/api` — workspace secrets → sandbox env

## Problem

A workspace's secrets are injected into a session's sandbox as plain env vars. Which
...
Both call sites wrapped `loadWorkspaceAgents` in `.catch(() => null)`. A `null`
result produced `agentGrantEnv = undefined`, which
`listWorkspaceSecretsSnapshotForUser` interprets as **all secrets**. Any throw from
the loader — not the manifest-parse errors it handles internally, but a genuine
failure of the loader itself — silently widened the session to every workspace
...
```
create session with agent A (secrets: all)   → sandbox env = ALL workspace secrets
...
`apps/api/src/workspaces/lib/secret-grant.ts` is now the single place a session's
...
This required opening up a layer below. `loadWorkspaceAgents` **never throws** by
design: `readManifest` (`workspaces/triggers.ts`) wraps the git I/O in a blanket
`catch { return null; }`, and `loadWorkspaceAgents` answers `null` with
...
**indistinguishable from "blank workspace"** and silently granted every secret,
...
`loadWorkspaceAgents → readManifest → readManifestFromRepo` chain; without the flag
...
- **Background fan-out** (`propagateWorkspaceSecretsToActiveSandboxes`) — the
...
(`new-session-create.ts`, `use-new-workspace-session.ts`), but **that lock is not
...
So this does change behavior for one population: **workspaces that declare
...
workspaces declared, so enforcing it is the point — but the UI currently surfaces
it as a generic error rather than a designed "start a new session" flow.

Everyone else is unaffected: a workspace with no `agents:` map, or with uniform
...
workspace and for the non-binding `default` sentinel, but `'all'` for an agent that
declares `secrets: all` or omits the key. Downstream they are identical —
`resolveGrantedSecretEnv` (`workspaces/secrets.ts`) computes
...
and the workspace's secret set can change under it.
...
| File | Change |
|---|---|
| `workspaces/lib/secret-grant.ts` | New — the single resolver + pure policy |
| `workspaces/lib/secret-grant.test.ts` | New — pure policy |
| `workspaces/lib/secret-grant.fail-closed.test.ts` | New — fail-closed through the real loader chain |
| `workspaces/triggers.ts` | `readManifest` opt-in `rethrowReadErrors` |
| `workspaces/agents.ts` | `loadWorkspaceAgents` threads it through |
| `sandbox-proxy/routes/preview.test.ts` | Covers the 409 / 503 mapping |
| `workspaces/lib/sessions.ts` | Boot uses the shared resolver; no `.catch(() => null)` |
| `workspaces/lib/sandbox-env-sync.ts` | Hot push resolves from the running agent |
...
`getWorkspaceSecretValue` server-side while being excluded by `sanitizeSandboxEnv`.
