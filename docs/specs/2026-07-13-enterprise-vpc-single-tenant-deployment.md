...
| **External site delivery** | Customer-connected site provider | Disable site delivery for the tenant, or connect a customer-approved provider inside its security boundary. |
...
2. **Bedrock-only gateway profile** + disable or replace Daytona, OpenRouter, and external site delivery behind an enterprise config profile.
