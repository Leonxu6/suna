...
The platform starts OpenCode in ACP mode for every normal workspace session.

The workspace experiment does not control the OpenCode process. It controls the
...
normal workspace experiment path.
...
`POST /workspaces/:workspaceId/sessions/:sessionId/start` returns the selected value
as `runtime_transport`.

### Workspace experiment

`acp_runtime` is a server-owned workspace experiment.
...
The workspace setting otherwise controls the selection.
...
1. The API resolves the workspace, session, sandbox, model, agent, and compiled
   agent configuration.
...
15. `useSession(workspaceId, sessionId)` polls `/start` until the runtime is ready.
...
`apps/api/src/workspaces/lib/sessions.ts`.

`apps/api/src/workspaces/lib/session-runtime-env.ts` serializes that value as
...
2. Otherwise, an enabled `acp_runtime` workspace experiment selects ACP.
...
The frontend does not calculate the selection from workspace metadata.
...
`useSession(workspaceId, sessionId)` owns:
...
The session page calls one `useSession(workspaceId, sessionId)` hook.

The workspace settings page renders the server-provided experimental feature
...
### Per-workspace client rollback

Disable the server-provided ACP Runtime experiment for the workspace.
...
This override selects ACP for every workspace while the feature remains
available.

Remove the override to restore workspace-specific selection.

### Operator-wide emergency client rollback

The current configuration has no `rest` operator override.
Remove the ACP override and disable `acp_runtime` on affected workspaces.
...
Retain the per-workspace REST rollback during the soak period.
...
- `apps/api/src/workspaces/lib/session-runtime-env.ts`
...
- `apps/api/src/workspaces/routes/r8.ts`
  returns `runtime_transport` from `/start`.
- `apps/api/src/workspaces/opencode-mapping.ts`
...
- `apps/whitelabel-demo/src/app/workspaces/[id]/sessions/[sessionId]/page.tsx`
...
15. For a full parity check, run one ACP workspace and one REST compatibility
    workspace with the same prompt.
